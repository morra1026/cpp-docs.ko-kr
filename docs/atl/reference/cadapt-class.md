...
`CAdapt`기본 역할은 클래스에 의해 정의 된 주소 연산자를 숨기려면 *T*적용할된 클래스의 특징을 유지 합니다. `CAdapt` 공용 멤버를 보유 하 여이 역할을 수행 [m_T](#m_t), 형식의 *T*, 및 변환 연산자, 비교 연산자 및 복사 생성자의 특수화를 허용 하도록 정의 하 여 `CAdapt` 되도록 형식의 개체 처럼 처리 *T*합니다.
