...
2. 저장 형식 배열이 됩니다 다각형 점 계산 되 면 `POINT`, 따라서 배열 정의 뒤에 추가할 `m_nSides` PolyCtl.h에:
...
2. 이 구현을 추가 합니다 `CalcPoints` PolyCtl.cpp 끝 함수:
...
1. 빌드 및 ActiveX Control Test Container를 시작 합니다. 자세한 내용은 [TSTCON 샘플: ActiveX Control Test Container](../visual-cpp-samples.md)합니다.

2. 테스트 컨테이너에서에 **편집** 메뉴에서 클릭 **새 컨트롤 삽입**합니다.

3. 호출할 수 있는 컨트롤을 찾습니다 `PolyCtl Class`, 클릭 **확인**합니다. 원 안에 녹색 삼각형이 표시 됩니다.

다음 절차에 따라 면의 수를 변경해 보세요. 테스트 컨테이너 내에서 이중 인터페이스의 속성을 수정 하려면 사용 하 여 **메서드 호출**합니다.

#### <a name="to-modify-a-controls-property-from-within-the-test-container"></a>테스트 컨테이너 내에서 컨트롤의 속성을 수정 하려면

1. 테스트 컨테이너를 클릭 **메서드 호출** 에 **제어** 메뉴.

     합니다 **메서드 호출** 대화 상자가 표시 됩니다.

2. 선택는 **PropPut** 버전의는 **양쪽** 속성을 **메서드 이름** 드롭다운 목록 상자.

3. 형식 `5` 에 **매개 변수 값** 상자를 클릭 합니다 **값 설정**를 클릭 하 고 **Invoke**합니다.

컨트롤을 변경 하지 않는 참고 합니다. 설정 하 여 내부적으로 면의 수를 변경 하 여 `m_nSides` 변수에이 시 키 지 않는 그릴 컨트롤입니다. 다른 응용 프로그램으로 전환 하 고 테스트 컨테이너로 다시 전환 하는 경우에 컨트롤 그려져 하에 올바른 개수의 측면을 찾을 수 있습니다.
