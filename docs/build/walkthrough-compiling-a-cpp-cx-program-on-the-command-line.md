...
2.  명령 프롬프트에서 입력 **notepad basiccx.cpp**합니다.  
...
4.  메뉴 모음에서 선택 **파일**하십시오 **저장**합니다.  
  
     Windows 런타임을 사용 하는 Visual c + + 소스 파일을 만들었습니다 [Platform 네임 스페이스](../cppcx/platform-namespace-c-cx.md) 네임 스페이스입니다.  
  
5.  명령 프롬프트에서 입력 **cl /EHsc /ZW basiccx.cpp /link /subsystem: console**합니다. cl.exe 컴파일러는 이 소스 파일을 .obj 파일로 컴파일한 다음 링커를 실행하여 실행 프로그램인 basiccx.exe를 생성합니다. (합니다 [/EHsc](../build/reference/eh-exception-handling-model.md) 컴파일러 옵션에는 c + + 예외 처리 모델을 지정 하며 [/l i n](../build/reference/link-pass-options-to-linker.md) 콘솔 응용 프로그램을 지정 하는 플래그입니다.)  
  
6.  Basiccx.exe 프로그램을 명령 프롬프트를 실행 하려면 입력 **basiccx**합니다.  
