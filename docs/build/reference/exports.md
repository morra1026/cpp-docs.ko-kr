...
C + + 함수에 대 한 이름 데코레이션을 사용 하는 Visual c + + 컴파일러, 때문에 데코레이팅된 이름을 internal_name 사용 하거나 소스 코드에서 extern "C"를 사용 하 여 내보낸된 함수를 정의 합니다. 컴파일러에는 또한 C 함수를 사용 하 여 데코 레이트 합니다 [__stdcall](../../cpp/stdcall.md) 밑줄을 사용 하 여 규칙을 호출 (\_) 접두사 및 접미사 구성는 at 기호 (\@) 뒤에 바이트 수 (10 진수)가 붙은 인수 목록입니다.
