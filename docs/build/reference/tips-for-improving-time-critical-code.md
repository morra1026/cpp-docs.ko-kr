---
title: 시간이 중요 한 코드를 개선 하기 위한 팁 | Microsoft Docs
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-tools
...
- `IsKindOf` 이 함수는 여러 호출을 생성 하 고 많은 참조의 집약성 다른 데이터 영역에서 메모리에 액세스할 수 있습니다. 이 함수는 디버그 빌드(예: ASSERT 호출)에 유용하지만 릴리스 빌드에는 사용하지 않도록 하세요.
...
- `OnIdle` 유휴 이벤트가 발생할 수 시간에 예상 되지 않는, 예: 사이 `WM_KEYDOWN` 고 `WM_KEYUP` 이벤트입니다. 타이머가 코드를 트리거할 보다 효율적인 방법일 수 있습니다. false 메시지를 생성하거나 `OnIdle`의 재정의에서 항상 `TRUE`를 반환하여 `OnIdle`을 강제로 반복적으로 호출하지 마세요. 그러면 스레드 대기를 허용할 수 없습니다. 다시 한 번 말하지만 타이머 또는 개별 스레드가 더욱 적절할 수 있습니다.
...
힙을 측정하면 힙에서의 모든 할당을 처리할 수 있습니다. C 런타임 [디버그 힙 루틴](/visualstudio/debugger/crt-debug-heap-details) 검사점으로 하 고 힙을 덤프 합니다. Microsoft Excel과 같은 스프레드시트 프로그램으로 출력을 읽고 피벗 테이블을 사용하여 결과를 볼 수 있습니다. 할당의 총 수, 크기 및 분포를 확인합니다. 이러한 정보를 작업 집합의 크기와 비교합니다. 또한 관련 크기의 개체 클러스터링을 살펴봅니다.
...
자세한 내용은 [유휴 루프 처리](../../mfc/idle-loop-processing.md) 하 고 [다중 스레딩](../../parallel/multithreading-support-for-older-code-visual-cpp.md)합니다.
...
- 작업 집합의 상한 및 하 한 제한을 설정 하려면 [SetProcessWorkingSetSize](/windows/desktop/api/winbase/nf-winbase-getprocessworkingsetsize)합니다.

- 작업 집합의 상한 및 하 한 한도 가져오려면 [GetProcessWorkingSetSize](/windows/desktop/api/winbase/nf-winbase-setprocessworkingsetsize)합니다.
