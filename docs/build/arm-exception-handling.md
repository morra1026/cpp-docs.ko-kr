...
Windows ARM에서 사용 하 여 *해제 코드* 중의 스택 해제를 제어 하 [구조적 예외 처리](/windows/desktop/debug/structured-exception-handling) (SEH). 해제 코드는 실행 가능 이미지의 .xdata 섹션에 저장되는 바이트 시퀀스로, 함수 프롤로그 및 에필로그 코드의 작동을 요약하여 설명하므로 호출자 스택 프레임 해제를 준비하기 위해 적용된 함수 프롤로그를 실행 취소할 수 있습니다.
...
1. 예외 데이터 다음 (경우는 *E* 헤더의 비트가 0으로 설정 된) 단어에 하나로 압축 되어 작은 시작 오프셋부터 순서 대로 저장 하는 에필로그 범위에 대 한 정보 목록이 있습니다. 각 범위에 포함되는 필드는 다음과 같습니다.
...
1. 경우는 *X* 필드 헤더에이 1 이면 해제 코드 바이트 예외 처리기 정보가 옵니다. 하나의 구성 됩니다 *예외 처리기 RVA* (가변 길이) 양을 예외 처리기에 필요한 데이터가 바로 뒤에 예외 처리기의 주소를 포함 하는 합니다.
