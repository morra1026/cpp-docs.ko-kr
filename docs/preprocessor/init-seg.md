...
*section-name*  
초기화 섹션의 명시적 지정을 허용합니다. 그러나 개체에는 사용자 지정 *섹션 이름* 는 암시적으로 생성 되지 않으면 해당 주소에서 명명 된 섹션에 배치 됩니다 *섹션 이름*합니다.  
...
*func-name*  
프로그램이 종료되면 `atexit` 대신 호출될 함수를 지정합니다. 이 도우미 함수 호출 [atexit](../c-runtime-library/reference/atexit.md) 전역 개체 소멸자에 대 한 포인터를 사용 하 여 합니다. 다음 형식의 pragma에서 함수 식별자를 지정하면  
