...
`#define` *식별자* *토큰 문자열*최적화  
  
`#define` *식별자* `(` *식별자*opt`,`*...*  `,` *식별자*opt`)`*토큰 문자열*최적화  
...
두 번째 구문 형식에서는 매개 변수를 사용하여 함수 형식 매크로를 정의합니다. 이 형식은 괄호 안에 표시되어야 하는 매개 변수의 선택 목록을 허용합니다. 매크로 정의 되 고 각 후속 발생 되 면 *식별자*( *식별자*최적화,..., *식별자*최적화)의 버전으로 바뀝니다는  *토큰 문자열* 정식 매개 변수를 대체 하는 실제 인수를 갖는 인수입니다.  
