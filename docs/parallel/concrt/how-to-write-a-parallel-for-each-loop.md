...
다음 예제에서는 두 번 배열에서 소수의 수를 계산합니다. 이 예제에서는 먼저 사용 합니다 [std:: for_each](../../standard-library/algorithm-functions.md#for_each) 순차적으로 수를 계산 하는 알고리즘입니다. 이 예제에서는 다음 사용을 `parallel_for_each` 동시에 동일한 작업을 수행 하는 알고리즘입니다. 또한 이 예제에서는 두 계산을 모두 수행하는 데 필요한 시간을 콘솔에 출력합니다.
