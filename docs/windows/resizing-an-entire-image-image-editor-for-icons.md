---
title: (아이콘에 대 한 이미지 편집기)는 전체 이미지 크기 조정 | Microsoft Docs
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-windows
...
2. 에 **너비** 및 **높이** 상자에 [속성 창](/visualstudio/ide/reference/properties-window)를 원하는 크기를 입력 합니다.

   이미지의 크기를 증가 하는 경우는 **이미지** 편집기 오른쪽 이나 아래쪽 이미지 또는 둘 모두를 확장 하 고 새 영역을 현재 배경색으로 채웁니다. 이미지를 채우도록 확장 되지 않습니다.

   이미지의 크기를 줄이면 합니다 **이미지** 편집기의 오른쪽 가장자리나 아래쪽 가장자리 또는 둘 다에서 이미지를 자릅니다.
