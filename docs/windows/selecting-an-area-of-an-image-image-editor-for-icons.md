...
1. 선택 영역에 현재 위치에 있는 이미지를 복사 하려면 선택 커서 바깥쪽을 클릭 합니다.
...
- 이미지를 가로로 대칭 이동 (미러 이미지)에서 **이미지** 메뉴 선택 **좌우 대칭**합니다.

- 이미지를 세로로 대칭 이동 (뒤집어)에서 **이미지** 메뉴 선택 **상하 대칭**합니다.

- 이미지를 90도 회전 하는 **이미지** 메뉴 선택 **90도 회전**합니다.
...
### <a name="to-open-a-bitmap-for-stand-alone-editing"></a>독립 실행형 편집에 대 한 비트맵을 열려면
...
1. 에 **속성** 창 이미지에 대 한 모든 속성을 변경 합니다.
   ...
   |**ID**|리소스의 식별자를 설정합니다. 이미지에 대 한 Microsoft Visual Studio에서 기본적으로 할당 일련의 사용 가능한 다음 식별자: IDB_BITMAP1, IDB_BITMAP2, 및 등입니다. 아이콘 및 커서에 대 한 유사한 이름이 사용 됩니다.|
