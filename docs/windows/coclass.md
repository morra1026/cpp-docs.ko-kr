...
Coclass를 정의할 때는 지정할 수도 있습니다는 [uuid](../windows/uuid-cpp-attributes.md)를 [버전](../windows/version-cpp.md)를 [스레딩](../windows/threading-cpp.md)를 [vi_progid](../windows/vi-progid.md), 및 [progid ](../windows/progid.md) 특성입니다. 그 중 하나를 지정 하지 않은 경우 생성 됩니다.
...
- [CComObjectRootEx 클래스](../atl/reference/ccomobjectrootex-class.md) 에서 지정한 스레딩 모델 클래스를 기반으로 템플릿을는 [스레딩](../windows/threading-cpp.md) 특성입니다. 경우는 `threading` 특성을 지정 하지 않으면 스레딩 모델 기본값인 아파트 합니다.

- [IProvideClassInfo2Impl](../atl/reference/iprovideclassinfo2impl-class.md) 를 추가 하면 합니다 [noncreatable](../windows/noncreatable.md) 대상 개체에 대 한 특성을 지정 하지 않으면.
...
- `GetProgID`를 등록, 관련 된 지정 된 문자열을 반환 합니다는 [progid](../windows/progid.md) 특성입니다.

- `GetVersionIndependentProgID` 동일한 기능이 `GetProgID`를 사용 하 여 지정 된 문자열을 반환 하지만 [vi_progid](../windows/vi-progid.md)합니다.

대상 클래스를 COM 맵 관련이 다음 변경 내용이:

- 대상 클래스에서 파생 되는 모든 인터페이스에 대 한 항목 및 지정 된 모든 항목을 사용 하 여 COM 맵에 추가 되는 [COM 인터페이스 진입점](../mfc/com-interface-entry-points.md) 특성 또는에 필요한 것을 [집계](../windows/aggregates.md) 특성입니다.
