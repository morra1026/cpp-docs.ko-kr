...
미리 정의 된 액셀러레이터 키의 목록을 참조 하세요 [액셀러레이터 키](../windows/predefined-accelerator-keys.md)합니다.
...
합니다 **ID** 프로그램 코드에서 각 액셀러레이터 키 테이블 항목을 참조 하는 속성입니다. 이 항목에는 프로그램의 액셀러레이터 키 또는 키 조합을 누를 때 수신 하는 명령 값입니다. 액셀러레이터 메뉴 항목 동일 확인 하십시오 해당 **ID** 동일 아웃할를 **ID** 액셀러레이터 키 테이블은 동일 합니다 **ID** 메뉴 리소스에 대 한 합니다.

속성은 세 가지 각 accelerator **ID**: **한정자**하십시오 **키**, 및 **형식**

#### <a name="modifier-property"></a>한정자 속성
...
다음은 사용할 수 있는 항목에 대 한 합니다 **한정자** 액셀러레이터 키 테이블의 속성:
   ...
   |**Ctrl**|눌러야 **Ctrl** 하기 전에 **키** 값입니다. ASCII 형식으로 유효 하지 않습니다.|
   |**Shift**|눌러야 **Shift** 하기 전에 **키** 값입니다.|
   |**Ctrl+Alt**|눌러야 **Ctrl** 하 고 **Alt** 하기 전에 **키** 값입니다. ASCII 형식으로 유효 하지 않습니다.|
   |**Ctrl+Shift**|눌러야 **Ctrl** 하 고 **Shift** 하기 전에 **키** 값입니다. ASCII 형식으로 유효 하지 않습니다.|
   |**Alt+Shift**|눌러야 **Alt** 하 고 **Shift** 하기 전에 **키** 값입니다. ASCII 형식으로 유효 하지 않습니다.|
...
다음은 사용할 수 있는 항목에 대 한 합니다 **키** 액셀러레이터 키 테이블의 속성:
   ...
   |A-Z 범위의 단일 키보드 문자 (대문자)를 앞에 캐럿 (^).<br/><br/>예를 들어 ^3.|이 옵션으로 눌린 키의 ASCII 값을 입력 합니다 **Ctrl** 키를 누르고 있습니다.|
...
> [!TIP]
> 액셀러레이터 키를 정의 하는 바로 가기 또는 액셀러레이터 키 테이블에서 여러 항목을 마우스 오른쪽 단추로 클릭 하는 것입니다. 선택할 **다음 입력 된 키** 한 다음 키보드의 키 또는 키 조합 중 하나를 누릅니다.
>
> 합니다 **다음 키 입력** 명령에서 제공 됩니다. 합니다 **편집** 메뉴.

#### <a name="type-property"></a>형식 속성

합니다 **형식** 속성 바로 가기 키 조합을 액셀러레이터 키를 사용 하 여 연결 여부를 결정 **ID** ASCII/ANSI 키 값을 또는 가상 키 (VIRTKEY) 조합으로 해석 됩니다.

- 경우는 **형식** 속성은 **ASCII**의 **한정자** 속성 수만 있습니다 `None` 또는 `Alt`, 합니다 를사용하는액셀러레이터키를가질수있는**Ctrl** 키 (키 앞에 지정 된 된 `^`).

- 경우는 **형식** 속성은 **VIRTKEY**, 조합을 **한정자** 및 **키** 값이 잘못 되었습니다.

> [!NOTE]
> 액셀러레이터 키 테이블에 값을 입력 하 고 값이 ASCII/ANSI 선택으로 처리 하려는 경우는 **형식** 테이블에서 선택 항목에 대 한 **ASCII** 드롭다운 목록에서에서. 그러나 사용 하는 경우는 **다음 입력 된 키** 명령을 **편집** 지정 하려면 메뉴를 **키**를 변경 해야 합니다는 **형식** 속성 **VIRTKEY** 하 **ASCII** *전에* 입력 합니다 **키** 코드입니다.
...
1. 형식 합니다 *키* 액셀러레이터 키로 사용 하 여 또는 마우스 오른쪽 단추로 클릭 하 고 선택 하려는 **다음 입력 된 키** 키 조합을 설정 하거나 메뉴로 이동 하려면 바로 가기 메뉴에서 **편집**  >  **입력 한 다음 키**합니다.
...
> [!TIP]
> 삭제 하는 바로 가기 키를 누릅니다 하는 것은 **삭제** 키입니다.

이동 하거나 다른 리소스 스크립트 파일에 액셀러레이터 키 테이블 항목을 복사 합니다.
