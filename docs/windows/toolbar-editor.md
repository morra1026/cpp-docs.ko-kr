...
1. **자원** 보기.rc 파일을 마우스 오른쪽 단추로 클릭 하 고 선택 **리소스 추가**합니다. 단추로 기존 도구 모음.rc 파일에 있는 경우는 **도구 모음** 선택한 폴더 **도구 모음 삽입**합니다.
...
1. 기존 비트맵 리소스를 엽니다는 [이미지 편집기](../windows/image-editor-for-icons.md)합니다. 비트맵.rc 파일에 있지 않은 경우.rc 파일을 마우스 오른쪽 단추로 클릭 하 고 선택 **가져오기**,.rc 파일에 추가 하 고 선택 하려는 비트맵에 다음 탐색 **오픈**합니다.
