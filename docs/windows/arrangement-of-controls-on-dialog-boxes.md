---
title: '방법: 정렬 컨트롤 (c + +) | Microsoft Docs'
ms.date: 02/15/2019
f1_keywords:
...
ms.locfileid: "56336633"
---
# <a name="how-to-arrange-controls-c"></a>방법: 정렬 컨트롤 (c + +)
...
## <a name="guides-and-grids"></a>안내선과 모눈
...
### <a name="to-create-edit-and-delete-guides-and-margins"></a>만들기, 편집 및 삭제 안내선과 여백
...
대화 상자를 만들 때 네 가지 여백 중 제공 됩니다. 여백은 수정 된 가이드, 파란색 점선으로 표시 합니다. 다음 작업을 참조 하세요.

- 눈금자를 내에서 지침을 만드는 지침을 만들려면 한 번 선택 합니다. (한 번의 클릭 새로 만들고 가이드; 시작을 두 번 클릭 합니다 **안내선 설정** 가이드 설정을 지정할 수 있는 대화 상자.)

- 대화 상자에서 지침을을 설정 하려면이 가이드를 선택 하 고 새 위치로 끌어다 놓습니다. (선택할 수 있습니다도 화살표를 끌어 관련된 가이드 눈금자의.) 이 가이드의 좌표는 눈금자 창의 아래쪽 상태 표시줄에 표시 됩니다. 이 가이드의 정확한 위치를 표시 하려면 눈금자 화살표 위로 포인터를 이동 합니다.

- 여백 이동할 여백을 새 위치로 끕니다. 사라집니다 여백을 있도록 여백을 0의 위치로 이동 합니다. 가져오고 다시 여백, 0 위치는 여백 위에 포인터를 놓고 여백을 위치로 이동 합니다.

- 지침을 삭제, 가이드 대화 상자에서 끌거나 눈금자 해제 대응 하는 화살표를 끌어 옵니다.

### <a name="to-align-controls-on-a-guide"></a>안내선에 컨트롤을 맞추려면
...
**형식** 메뉴 선택 **안내선 설정**, 그런 다음 합니다 **모눈 간격** 필드 Dlu의 새 너비와 높이 지정 합니다.

### <a name="to-disable-guides"></a>안내선을 사용 하지 않도록 설정
...
### <a name="to-modify-the-layout-grid"></a>레이아웃 모눈 수정
...
- 켜기 / 끄기 레이아웃 모눈을 설정 하려면에서 **형식** 메뉴에서 선택 **안내선 설정**을 선택 하거나 선택을 취소 합니다 **표** 단추.
...
- 레이아웃 모눈의 크기를 변경 하는 **형식** 메뉴 선택 **안내선 설정**, 다음 높이 너비에에서 입력 Dlu 표의 셀에 대 한 합니다. 최소 높이 또는 너비는 4 개의 Dlu입니다.
...
### <a name="to-select-controls"></a>컨트롤을 선택 하려면
...
1. 누른를 추가 하거나 선택한 컨트롤의 그룹에서 컨트롤을 제거 하는 **Shift** 키를 추가 하거나 제거 하려는 컨트롤을 선택 합니다.

> [!NOTE]
> 누른 합니다 **Ctrl** 키 및 선택 영역 내에 있는 컨트롤을 선택 하면 선택에서 기준 컨트롤을 제어 하는 확인 됩니다.

### <a name="to-select-a-dominant-control"></a>기준 컨트롤을 선택 하려면

- 기준 컨트롤을 지정 하려면 누른 합니다 **Ctrl** 키 및 크기와 다른 컨트롤의 위치를 정하는 데 사용 하려는 컨트롤을 선택 *첫 번째*입니다.

- 기준 컨트롤을 변경 하려면 현재 선택된 된 컨트롤이 모두 외부를 선택 하 여 현재 선택을 취소 하 고 다른 컨트롤을 먼저 선택 하면 이전 절차를 반복 합니다.
...
> [!TIP]
> 그 텍스트에 맞게 컨트롤 크기를 자동으로 엽니다는 **형식** 메뉴 또는 컨트롤을 마우스 오른쪽 단추로 클릭 하 고 선택 **콘텐츠 크기를 조정**합니다.

### <a name="to-make-controls-the-same-size"></a>수 있도록 동일한 크기를 제어 합니다.
...
1. **형식** 메뉴에서 선택 **같은 크기로**, 선택 **둘 다**를 **높이**, 또는 **너비**.
...
1. 정렬 하려는 컨트롤을 선택 합니다. 먼저 기준 되도록 하려는 컨트롤을 선택 해야 하거나 맞춤을 실행 하거나 명령 크기를 조정 하기 전에 주요 컨트롤 수를 설정 합니다.

   컨트롤 그룹의 최종 위치 기준 컨트롤의 위치에 따라 달라 집니다.
...
### <a name="to-even-spacing-between-controls"></a>컨트롤 사이의 간격을 일정
...
1. **형식** 메뉴 선택 **균등 하 게 공간**, 다음 간격 맞춤 중 하나를 선택 하 고:
   ...
   |`Down`|맨 위 및 맨 아래 선택 된 컨트롤 간에 균등 하 게 공간 제어 합니다.|

### <a name="to-center-controls"></a>컨트롤 가운데
...
1. **형식** 메뉴 선택 **센터에서 대화 상자**, 다음 배치 방법 중 하나를 선택 하 고:
   ...
   |정렬|설명|
   |---|---|
   |`Vertical`|대화 상자에서 세로로 가운데 제어 합니다.|
   |`Horizontal`|대화 상자에서 가로 방향으로 가운데 제어 합니다.|

### <a name="to-arrange-push-buttons"></a>누름 단추 정렬 하려면
