...
명령 문자열 같은 하나 이상의 매개 변수를 포함 하는 경우 \[db_column] 또는 \[bindto], **db_command** 행 집합 및 이러한 바인딩된 변수를 서비스 하기 위해 접근자 맵을 생성 합니다. 자세한 내용은 [db_accessor](../windows/db-accessor.md) 를 참조하세요.
...
*name* (선택 사항)  
행 집합 작업에 사용하는 핸들의 이름입니다. *name*을 지정하는 경우, **db_command** 는 지정된 *name*의 클래스를 생성하며, 이 클래스는 행 집합을 이동하거나 여러 작업 쿼리를 실행하는 데 사용할 수 있습니다. *name*을 지정하지 않으면 사용자에게 둘 이상의 결과 행을 반환할 수 없습니다.
