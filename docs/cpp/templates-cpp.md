...
C# 및 Java와 같은 다른 언어로 제네릭 형식과 달리 C++ 템플릿은 값 매개변수라고도 하는 비형식 매개변수를 지원합니다. 예를 들어, std:: array 클래스는 표준 라이브러리에 이 예제에서와 마찬가지로 배열의 길이를 지정하는 상수 정수 값을 제공할 수 있습니다.
