---
title: 'Rvalue 참조 선언 자: &amp; &amp; | Microsoft Docs'
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-language
...
## <a name="remarks"></a>설명  
 rvalue 참조를 사용하면 lvalue와 rvalue를 구별할 수 있습니다. lvalue 참조와 rvalue 참조는 구문 및 의미 체계가 비슷하지만 다소 다른 규칙을 따릅니다. Lvalue 및 rvalue에 대 한 자세한 내용은 참조 하세요. [Lvalue 및 Rvalue](../cpp/lvalues-and-rvalues-visual-cpp.md)합니다. Lvalue 참조에 대 한 자세한 내용은 참조 하십시오 [Lvalue 참조 선언 자: &](../cpp/lvalue-reference-declarator-amp.md)합니다.  
  
 다음 단원에서는 rvalue 참조의 구현을 지원 하는 방법 *이동 의미 체계* 및 *전달을 완벽*합니다.  
...
```  
 ...
 의미 체계 이동은 컴파일러에서 RVO(반환 값 최적화) 또는 NRVO(명명된 반환 값 최적화)를 사용할 수 없는 경우에도 도움이 됩니다. 이러한 경우에 컴파일러는 형식에 정의된 이동 생성자를 호출합니다. 명명 된 반환 값 최적화에 대 한 자세한 내용은 참조 하세요. [명명 된 Return Value Optimization in Visual c + + 2005](https://msdn.microsoft.com/en-us/library/ms364057.aspx)합니다.  
 ...
 Visual c + + 2010에서 c + + 표준 라이브러리에 의미 체계 이동 도입한 것에 대 한 자세한 내용은 참조 하세요. [c + + 표준 라이브러리](../standard-library/cpp-standard-library-reference.md)합니다.  
  
## <a name="perfect-forwarding"></a>완벽한 전달  
 완벽한 전달은 오버로드된 함수의 필요성을 줄이고 전달 문제를 방지하는 데 도움이 됩니다. 합니다 *전달 문제* 참조를 매개 변수로 사용 하는 제네릭 함수를 작성 하 고 전달 하는 경우 발생할 수 있습니다 (또는 *전달*) 이러한 매개 변수를 다른 함수입니다. 예를 들어 제네릭 함수가 `const T&` 형식의 매개 변수를 사용하는 경우 호출된 함수는 해당 매개 변수의 값을 수정할 수 없습니다. 제네릭 함수가 `T&` 형식의 매개 변수를 사용하는 경우에는 rvalue(예: 임시 개체 또는 정수 리터럴)를 사용하여 해당 함수를 호출할 수 없습니다.  
 ...
 `W`, `X`, `Y` 및 `Z`의 4가지 형식을 선언하는 다음 예제를 살펴보십시오. 각 형식에 대 한 생성자의 다른 조합을 **상수** 및 비-**const** lvalue 참조를 매개 변수로 합니다.  
...
```  
 ...
 이 예제에서는 rvalue 참조를 매개 변수로 사용 하 여는 `factory` 함수입니다. 용도 [std::forward](../standard-library/utility-functions.md#forward) 기능은 템플릿 클래스의 생성자에 factory 함수의 매개 변수를 전달 하는 것입니다.  
 ...
 **Lvalue 참조 및 rvalue 참조를 수행 하는 함수를 오버 로드할 수 있습니다.**  
  
 함수를 오버 로드는 **const** lvalue 참조 또는 rvalue 참조를 수정할 수 없는 개체 (lvalue)를 구분 하는 코드를 작성할 수 있습니다 하 고 수정할 수 있는 임시 값 (rvalue). 개체로 표시 되어 있지 않으면 rvalue 참조를 사용 하는 함수 개체를 전달할 수 있습니다 **const**합니다. 다음 예제에서는 lvalue 참조 및 rvalue 참조를 사용하도록 오버로드되는 `f` 함수를 보여 줍니다. `main` 함수는 lvalue와 rvalue를 둘 다 사용하여 `f`를 호출합니다.  
 ...
 **컴파일러는 명명된 된 rvalue 참조를 lvalue로 및 명명 되지 않은 rvalue 참조를 rvalue로 처리 합니다.**  
...
-   **Lvalue를 rvalue 참조로 캐스팅할 수 있습니다.**  
 ...
 C + + 표준 라이브러리 [std:: move](../standard-library/utility-functions.md#move) 함수 개체와 해당 개체에 대 한 rvalue 참조를 변환할 수 있습니다. 사용할 수 있습니다는 **static_cast** 키워드 다음 예제에서와 같이 lvalue를 rvalue 참조로 캐스팅 합니다.  
 ...
 **함수 템플릿은 해당 템플릿 인수 형식을 추론 하 고 참조 축소 규칙을 사용 합니다.**  
  
 일반적으로 전달 하는 함수 템플릿을 작성 (또는 *전달*) 매개 변수는 다른 함수입니다. 템플릿 형식 추론이 rvalue 참조를 사용하는 함수 템플릿에 대해 어떻게 작용하는지를 이해해야 합니다.  
  
 함수 인수가 rvalue이면 컴파일러는 인수를 rvalue 참조로 추론합니다. 예를 들어 `X` 형식을 매개 변수로 사용하는 템플릿 함수에 `T&&` 형식의 개체에 대한 rvalue 참조를 전달하는 경우, 템플릿 인수 추론에서는 `T`가 `X`인 것으로 추론합니다. 따라서 매개 변수의 형식은 `X&&`합니다. 함수 인수가 lvalue 또는 **상수** lvalue 인 컴파일러 추론 해당 형식의 lvalue 참조 또는 **const** 해당 형식의 lvalue 참조 합니다.  
