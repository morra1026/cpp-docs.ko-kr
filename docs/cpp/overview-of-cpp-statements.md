...
대부분의 경우 C++ 문 구문은 ANSI C의 문 구문과 동일합니다. 둘 사이의 주요 차이점은 C에서의 선언은 블록의 시작에서만 허용되는 반면, C++에서는 *declaration-statement*가 추가되었으므로 효율적으로 이 제약사항이 제거되었다는 점입니다. 이에 따라 미리 계산된 초기화 값이 계산될 수 있는 프로그램의 지점에 변수를 도입할 수 있습니다.
