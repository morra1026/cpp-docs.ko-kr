...
- **sort**, **lower_bound** 및 그밖에 기본 정렬 및 검색 알고리즘.
...
가능한 경우 수작업으로 만든 루프를 사용하는것 대신 범위 기반 **for**나 알고리즘을 호출하거나 혹은 두가지를 모두 사용하세요. **copy**, **transform**, **count_if**, **remove_if**나 이런류의 것은 의도가 분명하고 보다 버그 없는 코드를 작성하기가 쉽기 때문에 수작업으로 작성한 루프보다 훨씬 좋습니다. 또한 많은 C++ 라이브러리 알고리즘에는 보다 효율적으로 구현 최적화된 기능이 있습니다.

과거의 C++ 대신 다음과 같은 코드를 사용하세요.
...
오래된 CRT `rand()` 함수에 많은 문제가 있다는 사실은 이제 널리 알려져 있습니다. 최신 C++에서는 이러한 단점을 고민할 필요가 없고, 더이상 균일하게 분산된 난수 생성기를 직접 만들지 않아도 됩니다. 품질 좋은 난수를 빠르고 쉽게 생성할 수 있는 도구는 C++ 표준 라이브러리에 포함된 [ \<random>](../standard-library/random.md)을 참조합니다.
