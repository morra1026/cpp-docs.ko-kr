...
오래된 CRT `rand()` 함수에 많은 문제가 있다는 사실은 이제 널리 알려져 있습니다. 최신 C++에서는 이러한 단점을 고민할 필요가 없고, 더 이상 균일하게 분산된 난수 생성기를 직접 만들지 않아도 됩니다. 품질 좋은 난수를 빠르고 쉽게 생성할 수 있는 도구는 C++ 표준 라이브러리에 포함된 [ \<random>](../standard-library/random.md)을 참조합니다.
