...
- 연산자에 제공된 피연산자의 형식이 일치하는 피연산자 매개 변수의 형식과 다릅니다. 기본 제공 연산자의 경우 양쪽 피연산자는 같은 형식이어야 하며 양쪽 피연산자를 나타낼 수 있는 공용 형식으로 변환됩니다. 자세한 내용은 [표준 변환](standard-conversions.md)합니다. 사용자 정의 연산자의 경우 각 피연산자는 일치하는 피연산자 매개 변수와 같은 형식이어야 합니다.
...
- 모호한 함수 호출. 변환이 대상 형식의 변환 생성자 및 소스 형식의 변환 함수로 정의됩니다. 자세한 내용은 [변환 함수](#ConvFunc)합니다.

일반적으로, 포함된 형식의 이름을 더욱 완벽하게 한정하거나 명시적 캐스트를 수행하여 의도를 분명하게 밝힘으로써 모호성을 해결할 수 있습니다.

변환 생성자와 변환 함수는 멤버 액세스 제어 규칙을 따르지만 변환 액세스 가능성은 모호하지 않은 변환을 확인할 수 있는 경우에만 고려됩니다. 즉, 변환은 경쟁 변환의 액세스 수준으로 인해 사용되지 못하게 되더라도 모호해질 수 있습니다. 멤버 액세스 가능성에 대 한 자세한 내용은 참조 하세요. [멤버 Access Control](../cpp/member-access-control-cpp.md)합니다.
...
문제가 발생할 수 있는 암시적 변환의 잘 알려진 예로로 변환입니다 **bool**합니다. 부울 컨텍스트에서 사용할 수 있는 클래스 형식을 만들려고 할 수 있는 여러 가지가 있습니다-예를 들어, 따라서 한다는 수 제어 하는 **경우** 문이나 루프-컴파일러에 사용자 정의 변환을 수행 하는 경우 있지만 기본 제공 형식, 컴파일러는 표준 변환을 추가적으로 나중에 적용할 수 있습니다. 이 표준 변환 추가적의 목적은의 확장 등을 허용 하는 것 **짧은** 하 **int**, 또한 하면로 인해 모호한 변환 될 수 있지만-예를 들어  **bool** 하 **int**, 클래스 형식이 의도 하지 않은 정수 컨텍스트에서 사용할 수 있습니다. 이 특정 문제 라고 합니다 *안전 부울 문제의*합니다. 이러한 종류의 문제는 합니다 **명시적** 키워드로 해결할 수 있습니다.

합니다 **명시적** 키워드는 컴파일러가 암시적 변환을 수행 하기 위해 지정 된 변환을 사용할 수 있습니다. 전에 암시적 변환의 구문 편의 필요한 경우 사용자는 **명시적** 키워드 도입, 해당 암시적 변환이 의도 하지 않은 결과 그대로 적용 하거나 덜 편리한 사용 해야 해결 방법으로 명명 된 변환 함수입니다. 사용 하 여 이제는 **명시적** 키워드를 사용할 수 있는 명시적 캐스트 또는 직접 초기화를 수행 하 고 안전 부울 문제의 예가 문제를 발생 시킬지 않습니다 하는 편리한 변환을 만들 수 있습니다.
...
`display_balance` 형식의 인수를 사용하는 `Money` 함수에 대한 첫 번째 호출의 경우 인수가 올바른 형식이므로 변환이 필요하지 않습니다. 그러나 두 번째 호출에서 `display_balance`, 때문에 변환이 필요 인수의 형식을 **double** 값을 사용 하 여 `49.95`, 없습니다 함수에 필요한 것은입니다. 함수는이 값을 직접 사용할 수 없습니다 인수의 형식으로 변환 되어 있지만-**이중**-일치 하는 매개 변수의 형식에-`Money`-형식의 임시 값 `Money` 에서 생성 됩니다 인수 함수 호출을 완료 하는 데 사용 합니다. 세 번째 호출에서 `display_balance`, 인수가 아닙니다는 **이중**,이 아니라는 **float** 값을 사용 하 여 `9.99`-하는 아직 함수 호출을 여전히 수행할 때문에 컴파일러는 표준 변환을 수행할 수 있습니다-에서 예제의 경우 **float** 하 **double**-고에서 사용자 정의 변환이 수행 **double** 를`Money` 필요한 변환을 완료 합니다.
...
변환 생성자를 선언 하 여 **명시적**를 사용할 수 있습니다만 개체에 대 한 직접 초기화를 수행 하거나 명시적 캐스트를 수행 합니다. 이를 통해, 클래스 형식 인수를 사용하는 함수가 변환 생성자의 소스 형식 인수를 암시적으로 사용할 수 없도록 하고 클래스 형식이 소스 형식 값을 통해 복사 초기화되지 않도록 할 수 있습니다. 다음 예제에서는 명시적 변환 생성자를 정의하는 방법과 올바른 형식의 코드에서의 그 효과를 보여줍니다.
...
이 예제에서는 명시적 변환 생성자를 사용하여 `payable`에 대한 직접 초기화를 수행할 수도 있습니다. 대신, `Money payable = 79.99;`을 복사 초기화하려는 경우에는 오류가 됩니다. `display_balance`에 대한 첫 번째 호출의 경우 인수의 형식이 올바르므로 오류가 아닙니다. `display_balance`에 대한 두 번째 호출의 경우에는 암시적 변환을 수행하는 데 변환 생성자를 사용할 수 없으므로 오류입니다. 세 번째 호출 `display_balance` 의 명시적 캐스트로 인해 올바릅니다 `Money`, 컴파일러는 여전히 데 도움이 되는 통지의 암시적 캐스트를 삽입 하 여 캐스팅 완료할 **float** 에 **double**.
...
변환 함수는 파생 클래스에 의해 상속됩니다. 파생 클래스의 변환 함수는 완전히 같은 형식으로 변환될 경우에만 상속된 변환 함수를 재정의합니다. 파생된 클래스의 예를 들어, 사용자 정의 변환 함수 **operator int** 덮어쓰지 않습니다-또는 아무런 영향을 주지-기본 클래스의 사용자 정의 변환 함수 **짧은 연산자**도, 표준 변환 간의 변환 관계를 정의 하지만 **int** 하 고 **짧은**합니다.
