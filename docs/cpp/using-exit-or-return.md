...
위의 예제에서 `main`에 진입하기 전에 정적 개체 `sd1` 및 `sd2`가 만들어지고 초기화됩니다. **return** 문을 사용하여 이 프로그램이 종료된 후, 먼저 `sd2`가 소멸된 다음 `sd1`이 소멸됩니다. `ShowData` 클래스에 대한 소멸자가 이 정적 개체와 연결된 파일을 닫습니다. 초기화, 생성자 및 소멸자에 대한 자세한 내용은 특수 멤버 함수를 참조하십시오.
