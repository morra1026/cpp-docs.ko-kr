...
C5028|'*name*': 이전 선언에 지정 된 맞춤 (*수*) 정의에 지정 되지 않은
...
C4380|'*type*': 기본 생성자를 사용 되지 않을 수 없습니다.
...
C4435|'*type*': 가상 기본으로 인해/vd2의 개체 레이아웃이 변경 됩니다 '*형식*'
...
C4359|'*type*': 맞춤 지정 자가 실제 맞춤 보다 작습니다 (*수*), 무시 됩니다.
...
C4431|형식 지정자가 없습니다. int로 가정합니다. 참고: C 기본 int를 더 이상 지원
