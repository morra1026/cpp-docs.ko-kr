...
|경고|메시지|
|-------------|-------------|
|[컴파일러 경고(수준 3) C4800](compiler-warning-level-3-c4800.md)|'*형식*': bool 'true' 또는 'false' (성능 경고)로 값을 강제 적용|
...
|컴파일러 경고 (수준 1) C4806|'*작업이*': 안전 하지 않은 작업: 값 형식 '*type1*'형식으로 승격'*type2*' 주어진된 상수와 같을 수|
|컴파일러 경고 (수준 1) C4807|'*작업이*': 안전 하지 않은 다양 한 형식 '*type1*'및' 형식의 비트 필드 서명*type2*'|
|컴파일러 경고 (수준 1) C4808|대/소문자 '*값*' 형식의 switch 조건에 대 한 유효한 값이 아닌 'bool'는|
|컴파일러 경고 (수준 1) C4809|switch 문에 중복 'default' 레이블이 가능한 모든 'case' 레이블이 제공 됩니다.|
...
|컴파일러 경고 (수준 4) C4816|'*param*': 매개 변수는 크기가 0 인 배열이 있으면 잘리게 됩니다 수 (하지 않는 한 개체 참조로 전달 됨)|
...
|컴파일러 경고 (수준 1) C4822|'member f': 지역 클래스 멤버 함수에 본문이 없습니다.|
...
|컴파일러 경고 C4900|Il 불일치 '*tool1*'version'*version1*'및'*tool2*'version'*version2*'|
...
|컴파일러 경고 (수준 1) C4912|'*특성*': 특성의 중첩된 UDT에 대 한 동작으로 정의 되지 않은.|
...
|컴파일러 경고 (수준 1) C4920|enum enum 멤버 member_1 이미 enum enum member_2로 나온 value_1 = value_2 =|
|컴파일러 경고 (수준 3) C4921|'*설명을*': 특성 값 '*특성*' 여러 번 지정할 수 없습니다|
|컴파일러 경고 (수준 1) C4925|'*메서드*': 스크립트에서 dispinterface 메서드를 호출할 수 없습니다.|
|컴파일러 경고 (수준 1) C4926|'*식별자*': 기호가 이미 정의 되어: 무시 된 특성|
...
|컴파일러 경고 (수준 4) C4932|__identifier (*식별자*) 및 \_(_i) (*식별자*)를 구분할 수 없습니다|
|컴파일러 경고 (수준 1) C4934|'__delegate(multicast)'는 사용 되지 않습니다.를 사용 하 여 '\__delegate' 대신|
|컴파일러 경고 (수준 1) C4935|어셈블리 액세스 지정 자가 수정에서 '*액세스*'|
|컴파일러 경고 (수준 1, 오류) C4936|/clr 또는 /clr:pure를 지정하여 컴파일한 경우에만 이 __declspec를 사용할 수 있습니다.|
|컴파일러 경고 (수준 4) C4937|'*text1*'및'*text2*'에 대 한 인수로 구분할 수 없습니다 는'*지시문*'|
|컴파일러 경고 (수준 4) C4938|'*var*': 부동 지점 환산 변수 /fp 결과가 일치 하지 않을 수 있습니다: strict 또는 #pragma fenv_access|
...
|컴파일러 경고 (수준 1) C4947|'*type_or_member*': 되지 않음으로 표시|
...
|컴파일러 경고 (수준 1, 오류) C4950|'*type_or_member*': 되지 않음으로 표시|
...
|컴파일러 경고 C4954|'*함수*': 프로 파일링 되지 ((__int64 switch 식 포함)|
|컴파일러 경고 C4955|'*import2*': 가져오기 무시;에서 이미 가져온 '*import1*'|
|컴파일러 경고 (수준 1, 오류) C4956|'*형식*':이 형식은 확인할 수 없습니다|
...
|컴파일러 경고 (수준 4) C4960|'*함수*' 너무 커서 프로 파일링 할 수 없습니다.|
|컴파일러 경고 (수준 1) C4961|프로필 데이터가 '.pgd file'에 병합되지 않았습니다. 프로필 기반 최적화를 사용하지 않습니다.|
|컴파일러 경고 (수준 4) C4962|'*함수*': 최적화 프로필 데이터가 일관성이 없는 상태가 발생 하므로 사용 하지 않도록 설정 하는 프로필 기반 최적화|
...
|컴파일러 경고 (수준 3) C4981|Warbird: 함수 '*함수*' 아니라 __forceinline로 표시 인라인 예외 의미 체계를 포함 하기 때문에|
...
|컴파일러 경고 (수준 4) C4989|'*형식*': 형식은 정의가 충돌 합니다.|
...
|컴파일러 경고 C5034|내장 함수의 사용 하 여 '*내장*' 함수 *함수* 게스트 코드로 컴파일됩니다|
|컴파일러 경고 C5035|기능을 사용 하 여 '*기능*' 함수 *함수* 게스트 코드로 컴파일됩니다|
|컴파일러 경고 (수준 1) C5036|/ hybrid:x86arm64를 사용 하 여 컴파일할 때의 varargs 함수 포인터 변환이 있습니다. '*type1*'to'*type2*'|
|컴파일러 경고 (error) C5037|'*멤버 함수*': 클래스 템플릿의 멤버는 아웃오브 라인 정의 기본 인수를 사용할 수 없습니다|
|[컴파일러 경고 (수준 4) C5038](c5038.md)|데이터 멤버 '*member1*'다음에 초기화 됩니다 데이터 멤버'*member2*'|
