...
- 멀티바이트 문자 집합(MBCS) 기반 문자열에 포함된 문자 단위를 바이트 단위로 인덱싱으로 접근하면 포인터 조작 했을 때와 유사한 문제가 발생합니다. 문자열에서 백슬래시 문자를 검색하는 당므 예제를 살펴보세요.
   ...
   이렇게 인덱싱 한다면 선행 바이트가 아닌 후행 바이트를 인덱싱 할 수 있으므로  올바른 `문자`를 얻지 못할 수 있습니다.
...
- 이런 문제를 해결하려면 [_mbclen](../c-runtime-library/reference/mbclen-mblen-mblen-l.md) 함수를 사용하세요.
   ...
   이렇게 하면 선행 바이트를 인덱싱하여 올바른 `문자`를 얻을 수 있습니다. `_mbclen` 함수는 1바이트인지 2 바이트인지에 대한 크기를 판단하여 처리합니다.
