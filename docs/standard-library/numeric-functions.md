...
*첫 번째* 합을 계산 하거나 지정된 된 이진 연산에 따라 결합 된 범위에서 첫 번째 요소를 지정 하는 입력된 반복기입니다.

*마지막* 합을 계산 하거나 반복된 된 누적에 실제로 포함 된 마지막 요소 하나 다음 위치의 된 지정된 된 이진 연산에 따라 결합 된 범위에서 마지막 요소의 주소를 지정 하는 입력된 반복기입니다.

*val* 초기 값을 각 요소를 다시 추가 하거나 지정된 된 이진 연산에 따라 사용 하 여 결합 합니다.

*binary_op* 지정된 된 범위 및 해당 이전 응용 프로그램의 결과의 각 요소에 적용 되는 이진 연산입니다.
...
*첫 번째* 선행을 사용 하 여 차별화 요소가 또는 작업을 수행할 다른 값 쌍을 인 입력된 범위에서 첫 번째 요소를 지정 하는 입력된 반복기를 지정 된 이진 연산입니다.

*마지막* 선행을 사용 하 여 차별화 요소가 또는 작업을 수행할 다른 값 쌍을 인 입력된 범위에서 마지막 요소의 주소를 지정 하는 입력된 반복기를 지정 된 이진 연산입니다.

*결과* 일련의 차이 또는 지정된 된 작업의 결과를 저장할 수 있는 대상 범위의 첫 번째 요소를 지정 하는 출력 반복기입니다.

*binary_op* 차이 절차에서 차감 연산을 대체 하는 일반화 된 연산에 적용 되는 이진 연산입니다.
...
*first1* 내부 곱 또는 일반화 된 내부 곱을 두 번째 범위를 사용 하 여 계산 되는 첫 번째 범위의 첫 번째 요소를 지정 하는 입력된 반복기입니다.

*last1* 내부 곱 또는 일반화 된 내부 곱을 두 번째 범위를 사용 하 여 계산 되는 첫 번째 범위에서 마지막 요소의 주소를 지정 하는 입력된 반복기입니다.

*first2* 1 초 동안에서 첫 번째 요소를 지정 하는 입력된 반복기 범위 내부 제품 또는 첫 번째 범위를 사용 하 여 일반화 된 내부 곱을 계산 합니다.

*val* 내부 곱 또는 일반화 된 내부 곱 범위 사이 추가 하는 초기 값입니다.

*binary_op1* 내부 곱 일반화에서 요소별 곱에 적용 되는 합계의 내부 곱 연산을 대체 하는 이항 연산입니다.

*binary_op2* 내부 곱 일반화에서 곱하기의 내부 곱 요소별 연산을 대체 하는 이항 연산입니다.
...
*첫 번째* 채울 범위의 첫 번째 요소를 해결 하는 입력된 반복기입니다.

*마지막* 채울 범위의 마지막 요소를 해결 하는 입력된 반복기입니다.

*값* 연속적으로 증분할 후속 요소에 대 한 첫 번째 요소에 저장 된 시작 값입니다.
...
*첫 번째* 부분적으로 합을 계산 하거나 지정된 된 이진 연산에 따라 결합 된 범위에서 첫 번째 요소를 지정 하는 입력된 반복기입니다.

*마지막* 부분적으로 합을 계산 하거나 반복된 된 누적에 실제로 포함 된 마지막 요소 하나 다음 위치의 된 지정된 된 이진 연산에 따라 결합 된 범위에서 마지막 요소의 주소를 지정 하는 입력된 반복기입니다.

*결과* 일련의 부분적 합 또는 지정된 된 작업의 결과를 저장할 수 있는 대상 범위의 첫 번째 요소를 지정 하는 출력 반복기입니다.

*binary_op* 부분적 합 절차에서 합 연산을 대체 하는 일반화 된 연산에 적용 되는 이진 연산입니다.
