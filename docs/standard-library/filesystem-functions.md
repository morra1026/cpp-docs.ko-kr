...
1. pval.has_root_name() && !pval.has_root_directory()인 경우 함수는 pval.root_name() / absolute(base).root_directory() / absolute(base).relative_path() / pval.relative_path()를 반환합니다.

1. !pval.has_root_name() && pval.has_root_directory()인 경우 함수는 absolute(base).root_name() / pval을 반환합니다.

1. !pval.has_root_name() && !pval.has_root_directory()인 경우 함수는 absolute(base) / pval을 반환합니다.
...
1. 모든 경로 구성 요소 . (점은 이전 경로 구성 요소에서 설정한 현재 디렉터리임)이 제거됩니다.

1. 모든 경로 구성 요소 X/.. 쌍 (점-점은 이전 경로 구성 요소에서 설정한 부모 디렉터리임)이 제거됩니다.
