...
C++ 표준 라이브러리를 통해 정의되는 컨테이너 클래스의 개체는 `Alloc` 형식의 저장된 개체(대개 템플릿 매개 변수)를 통해 제어하는 시퀀스용 저장소를 할당하고 확보합니다. 그러한 할당자 개체는 클래스의 개체와 같은 외부 인터페이스가 있어야 합니다. `allocator<Ty>`합니다. 특히 `Alloc` 와 동일한 형식 이어야 합니다 `Alloc::rebind<value_type>::other`
