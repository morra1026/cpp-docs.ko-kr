...
다양한 `printf` 및 `wprintf` 함수는 형식 문자열 및 선택적 인수를 사용하고 출력에 대해 형식이 지정된 시퀀스를 생성합니다. 형식 문자열에는 출력용 리터럴 문자이거나 출력에서 인수에 형식을 지정하는 방법을 설명하는 인코드된 *변환 사양*인 *지시문*이 없거나 하나 이상 포함되어 있습니다. 이 항목에서는 형식 문자열에서 변환 사양을 인코드하는 데 사용되는 구문을 설명합니다. 이러한 함수의 목록을 보려면 [스트림 I/O](../c-runtime-library/stream-i-o.md)를 참조하세요.
