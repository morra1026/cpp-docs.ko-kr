...
**strtoumax** 있으면 변환 된 값을 반환 하거나 **UINTMAX_MAX** 오버플로에서 합니다. **strtoumax** 변환 작업 없이 수행할 수 있으면 0을 반환 합니다. **wcstoumax** 와 동일한 값을 반환 **strtoumax**합니다. 두 함수 모두에 대 한 **errno** 로 설정 된 **ERANGE** 오버플로 또는 언더플로가 발생 하는 경우.
...
이러한 함수의 각 입력된 문자열을 변환 *strSource* 에 **uintmax_t** 정수 값입니다.

**strtoumax** 문자열 읽기를 중지 *strSource* 숫자의 일부분으로 인식할 수 없는 첫 문자에서 합니다. 종결 null 문자를 수 있습니다 또는 보다 크거나 같은 경우에 첫 번째 숫자 문자일 수도 *기본*입니다. 합니다 **LC_NUMERIC** 로캘 범주 설정의 기 수 문자 인식이 결정 *strSource*합니다. 자세한 내용은 [setlocale, _wsetlocale](setlocale-wsetlocale.md)을 참조하세요. **strtoumax** 하 고 **wcstoumax** 현재 로캘을 사용 **_strtoumax_l** 하 고 **_wcstoumax_l** 에서 전달 된 로캘을 대신 사용 한다는 점을 제외 하면 동일 합니다. 자세한 내용은 [Locale](../../c-runtime-library/locale.md)을 참조하세요.
...
와이드 문자 버전의 **strtoumax** 됩니다 **wcstoumax**; 해당 *strSource* 인수는 와이드 문자 문자열입니다. 그 외의 경우에는 이들 함수가 동일하게 동작합니다.
