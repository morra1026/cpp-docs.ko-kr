...
> [!NOTE]
> **SIGINT** Win32 응용 프로그램에 대 한 지원 되지 않습니다. Ctrl+C 인터럽트가 발생할 때 Win32 운영 체제는 특히 해당 인터럽트를 처리하기 위해 새 스레드를 생성합니다. 이렇게 하면 UNIX에서와 같은 단일 스레드 응용 프로그램이 다중 스레드가 되며 예기치 않은 동작을 발생시킵니다.
