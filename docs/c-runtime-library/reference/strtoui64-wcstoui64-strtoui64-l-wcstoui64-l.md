...
**_strtoui64** 문자열에서 나타내는 값을 반환 *strSource*표현, 인해 오버플로가 발생 하는 경우 반환 제외 하 고 **_UI64_MAX**합니다. **_strtoui64** 변환 작업 없이 수행할 수 있으면 0을 반환 합니다.

**_UI64_MAX** 제한에서 정의 됩니다. 8.
...
합니다 **_strtoui64** 변환 함수 *strSource* 하는 **부호 없는** **__int64**합니다. **_wcstoui64** 의 와이드 문자 버전이 **_strtoui64**; 해당 *strSource* 인수는 와이드 문자 문자열입니다. 그 외의 경우에는 이들 함수가 동일하게 동작합니다.

두 함수 모두 문자열 읽기를 중지 *strSource* 숫자의 일부분으로 인식할 수 없는 첫 문자에서 합니다. 종결 null 문자를 수 있습니다 또는 보다 크거나 같은 첫 번째 숫자 문자일 수도 *기본*입니다.
...
현재 로캘의 **LC_NUMERIC** 범주 설정의 기 수 문자 인식이 결정 *strSource*; 자세한 내용은 참조 하십시오 [setlocale](setlocale-wsetlocale.md)합니다. _L 접미사가 없는 함수는 현재 로캘을 사용합니다 **_strtoui64_l** 하 고 **_wcstoui64_l** 없는 해당 함수와 동일 합니다 **_l** 대신 전달 된 로캘을 사용 한다는 점을 제외 하면 접미사가 있습니다. 자세한 내용은 [Locale](../../c-runtime-library/locale.md)을 참조하세요.
