...
C 런타임 라이브러리의 디버그 힙 유효성 검사를 수행 하는 빈도 지정할 수 있습니다 (**_CrtCheckMemory**)에 대 한 호출의 수에 따라 **malloc**하십시오 **realloc**를 **무료**, 및 **_msize**합니다.

**_CrtSetDbgFlag** 다음의 상위 16 비트를 검사 합니다 *newFlag* 값에 대 한 매개 변수입니다. 지정 된 값입니다 **malloc**, **realloc**하십시오 **무료**, 및 **_msize** 호출 사이의 **_CrtCheckMemory**  호출 합니다. 이러한 용도로 미리 정의된 매크로 4개가 제공됩니다.
...
기본적으로 **_CrtCheckMemory** 1,024 번 호출할 때마다 한 번 호출 됩니다 **malloc**를 **realloc**하십시오 **무료**, 및 **_ msize**합니다.

힙의 모든 16 확인을 지정할 수는 예를 들어 **malloc**, **realloc**합니다 **무료**, 및 **_msize** 다음 코드를 사용 하 여 작업:
...
상위 16 비트를 *newFlag* _CRTDBG_CHECK_ALWAYS_DF가 지정 하는 경우 매개 변수가 무시 됩니다. 이 예에서 **_CrtCheckMemory** 호출 될 때마다 호출 됩니다 **malloc**를 **realloc**를 **무료**, 및 **_msize**.

*newFlag* 에 적용할 새 상태는 **_crtDbgFlag** 비트 필드의 각 값의 조합입니다.
...
1. 호출 **_CrtSetDbgFlag** 사용 하 여 *newFlag* 같음 **_CRTDBG_REPORT_FLAG** 현재 가져올 **_crtDbgFlag** 상태 및 저장 합니다 임시 변수에서 반환 된 값입니다.

1. 비트 하 여 모든 비트를 설정 **또는** 의 해당 비트 마스크 (응용 프로그램 코드에서 상수로 표시)를 사용 하 여 임시 변수입니다.

1. 적절한 비트 마스크의 비트 **NOT** 연산자를 사용하여 변수를 **AND** 연산하여 다른 비트를 해제합니다.

1. 호출 **_CrtSetDbgFlag** 사용 하 여 *newFlag* 에 대 한 새 상태를 설정 하려면 임시 변수에 저장 된 값과 같으면 **_crtDbgFlag**합니다.

다음 코드에서는 메모리 부족을 시뮬레이션 하는 방법을 유지 하 여 해제 된 메모리 블록을 힙의 연결된 리스트에서 조건과 방지 **_CrtCheckMemory** 할당 요청 시 마다 호출 되 고:
...
사용 하 여 플래그를 사용 하지 않도록 설정 합니다 **_CrtSetDbgFlag** 함수를 수행 해야 합니다 **AND** 비트를 사용 하 여 변수 **하지** 비트 마스크의 비트.

하는 경우 *newFlag* 잘못 된 값이 함수에 설명 된 대로 잘못 된 매개 변수 처리기를 호출 [매개 변수 유효성 검사](../../c-runtime-library/parameter-validation.md)합니다. 이 함수를 설정 하는 경우는 계속 실행 하도록 허용 합니다 **errno** 하 **EINVAL** 의 이전 상태를 반환 하 고 **_crtDbgFlag**합니다.
