...
합니다 **_pipe** 함수를 만듭니다를 *파이프*, 프로그램을 사용 하 여 다른 프로그램에 정보를 전달 하는 인위적인 I/O 채널인은 합니다. 파이프는 파일 포인터나 파일 설명자 또는 두 항목을 모두 포함하며, 표준 라이브러리 입력 및 출력 함수를 사용하여 읽거나 쓸 수 있으므로 파일과 비슷합니다. 그러나 파이프는 특정 파일 또는 디바이스를 나타내지 않습니다. 대신 전적으로 운영 체제에 의해 제어되며 프로그램의 자체 메모리와는 독립적인 임시 저장소를 나타냅니다.
