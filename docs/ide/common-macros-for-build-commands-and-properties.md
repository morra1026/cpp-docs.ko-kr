...
현재 사용 가능한 매크로를 표시하려면 **속성 페이지** 대화 상자의 속성 페이지에서 속성 행 끝에 있는 드롭다운 화살표를 선택합니다. **편집**을 사용할 수 있는 경우 해당 항목을 선택한 다음, 편집 대화 상자에서 **매크로** 단추를 선택합니다. Visual Studio에 표시되는 현재 속성 및 매크로 집합은 각각에 대한 현재 값과 함께 나열됩니다. 자세한 내용은 **Property Pages (C++)** 의 [속성 페이지](../ide/property-pages-visual-cpp.md)를 클릭합니다.
