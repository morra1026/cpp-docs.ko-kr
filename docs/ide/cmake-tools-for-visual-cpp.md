...
1. **variables**: **-D**_이름_**=**_값_으로 CMake에 전달되는 CMake 변수의 이름-값 쌍이 포함됩니다. CMake 프로젝트 빌드 지침에서 CMake 캐시 파일에 변수를 직접 추가하도록 지정하는 경우 여기에 대신 추가하는 것이 좋습니다. 다음 예제에서는 이름-값 쌍을 지정하는 방법을 보여 줍니다.
