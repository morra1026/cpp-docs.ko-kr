...
1. 개체의 값을 설정 [m_strFilter](../../mfc/reference/crecordset-class.md#m_strfilter) 데이터 멤버입니다.
...
1. 정렬 순서, 잠금 모드 또는 매개 변수 등 필요한 다른 옵션을 설정 합니다. 매개 변수를 지정 하는 것은 특히 유용 합니다. 필터를 매개 변수화 하는 방법에 대 한 정보를 참조 하세요 [레코드 집합: 레코드 집합 (ODBC)를 매개 변수화](../../data/odbc/recordset-parameterizing-a-recordset-odbc.md)합니다.

1. 호출 `Open` 새 개체에 대 한 (또는 `Requery` 이전에 열린된 개체에 대 한).
