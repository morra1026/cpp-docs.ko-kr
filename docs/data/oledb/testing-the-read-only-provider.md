...
열려는 클래스의 각 줄은 공급자의 각 COM 개체를 만듭니다. 사용 하 여 공급자를 찾으려면는 `ProgID` 공급자입니다. 가져올 수 있습니다는 `ProgID` MyProvider.rgs 파일을 확인 하거나 시스템 레지스트리에서 (공급자의 디렉터리를 열고 검색할는 `ProgID` 키).  
