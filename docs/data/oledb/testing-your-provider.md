...
1. OLE DB 공급자에 대 한 공급자 수준 0 표준을 충족 하도록 하려면 (ADO 적합성 테스트 포함) OLE DB 적합성 테스트를 실행 합니다. (수준 0의 설명에 대 한 테스트에 대 한"OLE DB 수준 0 규칙"에서 검색 [OLE DB Programmer's Guide](/previous-versions/windows/desktop/ms713643)합니다. 이러한 테스트와 관련된 설명서는 Visual c + + Data Access SDK에 포함 됩니다. 이러한 테스트 다른 집계 하는 경우에 공급자 실행 되도록 도움이 [서비스 공급자](../../data/oledb/ole-db-resource-pooling-and-services.md) 속성을 추가 하거나 수정할 경우 특히 유용 합니다. 규칙 테스트에 대 한 자세한 내용은 Visual Studio Cd 중 하나에 있는 Data Access SDK에 대 한 추가 정보 파일을 참조 하세요.  
