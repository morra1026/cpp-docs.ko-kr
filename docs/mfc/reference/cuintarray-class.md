...
멤버 함수 `CUIntArray` 클래스의 멤버 함수와 비슷합니다 [CObArray](../../mfc/reference/cobarray-class.md)합니다. 이처럼 두 함수가 비슷하므로 `CObArray` 참조 설명서에서 멤버 함수 관련 사항을 확인할 수 있습니다. 표시 될 때마다는 `CObject` 함수 매개 변수 또는 반환 값으로 포인터를 UINT를 대체 합니다.
...
부호 없는 정수 또는 UINT, 단어 및 2 배 워드 점에서 다릅니다 UINT의 실제 크기는 대상 운영 환경에 따라 변경할 수 있습니다. UINT에는 워드도 동일한 크기입니다.

`CUIntArray` 통합 된 [IMPLEMENT_DYNAMIC](run-time-object-model-services.md#implement_dynamic) 런타임 형식 액세스 및 대 한 덤핑을 지원 하기 위해 매크로 [CDumpContext](../../mfc/reference/cdumpcontext-class.md) 개체입니다. 부호 없는 정수를 개별 요소의 덤프가 필요한 경우 덤프 컨텍스트 깊이 1 이상으로 설정 해야 합니다. 부호 없는 정수 배열은 직렬화 할 수 없습니다.
