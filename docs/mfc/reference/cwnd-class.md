...
|[CWnd::get_accHelp](#get_acchelp)|개체를 검색 하기 위해 프레임 워크를 호출한 **도움말** 속성 문자열입니다.|
...
|[CWnd::GetNextDlgTabItem](#getnextdlgtabitem)|첫 번째 컨트롤을 검색 합니다 [WS_TABSTOP](styles-used-by-mfc.md#window-styles) 스타일 지정된 된 컨트롤을 따릅니다 (또는 앞).|
...
|[CWnd::GetSafeHwnd](#getsafehwnd)|반환 `m_hWnd`, 경우에 NULL 또는 합니다 **이** 포인터가 NULL입니다.|
...
|[CWnd::IsD2DSupportEnabled](#isd2dsupportenabled)|D2D 지원을 사용 되는지 여부를 결정 합니다.|
...
|[CWnd::OnHelpFinder](#onhelpfinder)|ID_HELP_FINDER 및 ID_DEFAULT_HELP 명령 처리합니다.|
...
|[CWnd::OpenClipboard](#openclipboard)|클립보드를 엽니다. 다른 응용 프로그램을 Windows까지 클립보드를 수정할 수 없습니다 [CloseClipboard](http://msdn.microsoft.com/library/windows/desktop/ms649035) 함수를 호출 합니다.|
...
|[CWnd::PreSubclassWindow](#presubclasswindow)|필요한 다른 하위 클래스 앞에 오는 [SubclassWindow](#subclasswindow) 라고 합니다.|
...
|[CWnd::SetTimer](#settimer)|보내는 시스템 타이머를 설치를 [WM_TIMER](#ontimer) 트리거되면 메시지입니다.|
...
|[CWnd::OnCharToItem](#onchartoitem)|자식 목록 상자에서 호출 합니다 [LBS_WANTKEYBOARDINPUT](../../mfc/reference/styles-used-by-mfc.md#list-box-styles) 스타일에 대 한 응답에서을 [WM_CHAR](#onchar) 메시지.|
...
|[CWnd::OnMouseHover](#onmousehover)|에 대 한 이전 호출에 지정 된 기간 동안 커서로 창의 클라이언트 영역 가리킬 때 호출 [TrackMouseEvent](http://msdn.microsoft.com/library/windows/desktop/ms646265)합니다.|
|[CWnd::OnMouseHWheel](#onmousehwheel)|현재 창이 DWM(바탕 화면 창 관리자)에 의해 구성되고 해당 창이 최대화되면 호출됩니다.|
|[CWnd::OnMouseLeave](#onmouseleave)|커서에 대 한 이전 호출에서 지정 된 창의 클라이언트 영역을 벗어날 때 호출 됩니다 [TrackMouseEvent](http://msdn.microsoft.com/library/windows/desktop/ms646265)합니다.|
...
|[CWnd::OnNcCreate](#onnccreate)|이전에 호출 [OnCreate](#oncreate) 비클라이언트 영역이 만들어질 때.|
...
|[CWnd::OnNcMouseHover](#onncmousehover)|에 대 한 이전 호출에 지정 된 기간 동안 커서로 창의 비클라이언트 영역 가리킬 때 호출 [TrackMouseEvent](http://msdn.microsoft.com/library/windows/desktop/ms646265)합니다.|
|[CWnd::OnNcMouseLeave](#onncmouseleave)|커서에 대 한 이전 호출에서 지정 된 창의 비클라이언트 영역을 벗어나면 프레임 워크가이 멤버 함수를 호출 [TrackMouseEvent](http://msdn.microsoft.com/library/windows/desktop/ms646265)합니다.|
...
|[CWnd::OnStyleChanged](#onstylechanged)|나타내는 합니다 [SetWindowLong](http://msdn.microsoft.com/library/windows/desktop/ms633591) Windows 함수는 하나 이상의 창 스타일 변경 되었습니다.|
|[CWnd::OnStyleChanging](#onstylechanging)|나타내는 합니다 [SetWindowLong](http://msdn.microsoft.com/library/windows/desktop/ms633591) Windows 함수를 하나 이상의 창 스타일 변경 하려고 합니다.|
...
|[CWnd::OnVKeyToItem](#onvkeytoitem)|소유한 목록 상자를 호출한 `CWnd` 에 대 한 응답을 [WM_KEYDOWN](#onkeydown) 메시지입니다.|
...
|[CWnd::OnWindowPosChanging](#onwindowposchanging)|크기, 위치 또는 Z 순서에 대 한 호출의 결과로 변경 되 면 호출 [SetWindowPos](#setwindowpos) 또는 다른 창 관리 함수입니다.|
...
|[CWnd::PostNcDestroy](#postncdestroy)|이 가상 함수는 기본적으로 호출 됩니다 [OnNcDestroy](#onncdestroy) 창이 소멸 된 후에 작동 합니다.|
...
|[CWnd::operator! =](#operator_neq)|창이 없으면 해당 핸들이 창과 동일한 지를 결정 [m_hWnd](#m_hwnd)합니다.|
|[CWnd::operator ==](#operator_eq_eq)|창 핸들이 있는 창으로 같은지 여부를 확인 [m_hWnd](#m_hwnd)합니다.|

### <a name="public-data-members"></a>공용 데이터 멤버

|이름|설명|
|----------|-----------------|
|[CWnd::m_hWnd](#m_hwnd)|이 연결 된 HWND 나타냅니다 `CWnd`합니다.|

## <a name="remarks"></a>설명

`CWnd` 개체는 Windows 창과 다르지만 둘이 밀접하게 연결되어 있습니다. `CWnd` 개체는 `CWnd` 생성자 및 소멸자로 만들거나 제거합니다. Windows 창 다른 한편으로 Windows에서 만든 내부 데이터 구조를 `Create` 멤버 함수에서 파괴 하는 `CWnd` 가상 소멸자입니다. 합니다 [DestroyWindow](#destroywindow) 함수 개체를 제거 하지 않고 Windows 창을 제거 합니다.

합니다 `CWnd` 클래스 및 숨기기 메시지 맵 메커니즘은 `WndProc` 함수입니다. 들어오는 Windows 알림 메시지에 자동으로 적절 한 메시지 맵을 통해 라우팅되 **에서 * * * 메시지* `CWnd` 멤버 함수입니다. 재정의 하는 **에서 * * * 메시지* 파생된 클래스에서 멤버의 특정 메시지를 처리 하는 멤버 함수입니다.
...
자식 창은 두 단계로 만듭니다. 먼저 생성자를 호출 `CWnd` 생성 하는 `CWnd` 개체를 만든 다음 호출 합니다 [만들기](#create) 자식 창을 만들고에 연결 하는 멤버 함수는 `CWnd` 개체입니다.

사용자가 자식 창을 종료하면 `CWnd` 개체를 제거하거나 `DestroyWindow` 멤버 함수를 호출하여 창을 제거하고 해당 데이터 구조를 제거합니다.

Microsoft Foundation Class 라이브러리 내에서 추가 클래스를 `CWnd`에서 파생시켜 특정 창 형식을 제공합니다. 이러한 클래스를 포함 하 여 많은 [CFrameWnd](../../mfc/reference/cframewnd-class.md), [CMDIFrameWnd](../../mfc/reference/cmdiframewnd-class.md)합니다 [CMDIChildWnd](../../mfc/reference/cmdichildwnd-class.md)를 [CView](../../mfc/reference/cview-class.md), 및 [CDialog](../../mfc/reference/cdialog-class.md)에 추가 파생을 위해 설계 되었습니다. 컨트롤 클래스에서 파생 `CWnd`와 같은 [CButton](../../mfc/reference/cbutton-class.md), 직접 사용할 수 있습니다 또는 추가 클래스 파생에 사용할 수 있습니다.

사용 하 여 대 한 자세한 내용은 `CWnd`를 참조 하세요 [프레임 Windows](../../mfc/frame-windows.md) 하 고 [창 개체](../../mfc/window-objects.md)합니다.
...
성공, 실패 시 COM 오류 코드는 S_OK를 반환 합니다. 참조 **값을 반환** 에 [의 상호](http://msdn.microsoft.com/library/windows/desktop/dd318470) Windows SDK에에서 있습니다.

### <a name="remarks"></a>설명

이 함수는 mfc의 일부가 [Active Accessibility](http://msdn.microsoft.com/library/windows/desktop/dd373592) 지원 합니다.

이 함수를 재정의 하 `CWnd`-개체의 기본 동작을 수행 하는 클래스를 파생 합니다. 자세한 내용은 [의 상호](http://msdn.microsoft.com/library/windows/desktop/dd318470) Windows SDK에 있습니다.
...
*pvarChild*  
지정 된 지점에 개체를 식별 하는 정보를 받는 *xLeft* 하 고 *yTop*합니다. 참조 *pvarID* 에 [IAccessible::accHitTest](http://msdn.microsoft.com/library/windows/desktop/dd318471) Windows SDK에 있습니다.

### <a name="return-value"></a>반환 값

성공, 실패 시 COM 오류 코드는 S_OK를 반환 합니다. 참조 **값을 반환** 에서 `IAccessible::accHitTest` Windows SDK에에서 있습니다.

### <a name="remarks"></a>설명

이 함수는 mfc의 일부가 [Active Accessibility](http://msdn.microsoft.com/library/windows/desktop/dd373592) 지원 합니다.

이 함수를 재정의 하 `CWnd`-nonwindowed 사용자 인터페이스 요소 (MFC를 처리 하는 창 없는 ActiveX 컨트롤) 이외의 경우 클래스를 파생 합니다.

자세한 내용은 [IAccessible::accHitTest](http://msdn.microsoft.com/library/windows/desktop/dd318471) Windows SDK에 있습니다.
...
성공, 실패 시 COM 오류 코드는 S_OK를 반환 합니다. 참조 **값을 반환** 에서 `IAccessible::accLocation` Windows SDK에에서 있습니다.

### <a name="remarks"></a>설명

이 함수를 재정의 하 `CWnd`-nonwindowed 사용자 인터페이스 요소 (MFC를 처리 하는 창 없는 ActiveX 컨트롤) 이외의 경우 클래스를 파생 합니다.

자세한 내용은 참조 하세요. `IAccessible::accLocation` Windows SDK에 있습니다.
...
*navDir*  
이동할 방향을 지정 합니다. 참조 *navDir* 에 [IAccessible::accNavigate](http://msdn.microsoft.com/library/windows/desktop/dd318473) Windows SDK에 있습니다.
...
성공, 실패 시 COM 오류 코드는 S_OK를 반환 합니다. 참조 **값을 반환** 에서 `IAccessible::accNavigate` Windows SDK에에서 있습니다.
...
자세한 내용은 [IAccessible::accNavigate](http://msdn.microsoft.com/library/windows/desktop/dd318473) Windows SDK에 있습니다.
...
*flagsSelect*  
현재 선택 영역이 나 포커스를 변경 하는 방법을 지정 합니다. 참조 *flagsSelect* 에 [IAccessible::accSelect](http://msdn.microsoft.com/library/windows/desktop/dd318474) Windows SDK에 있습니다.
...
성공, 실패 시 COM 오류 코드는 S_OK를 반환 합니다. 참조 **값을 반환** 에서 `IAccessible::accSelect` Windows SDK에에서 있습니다.
...
자세한 내용은 [IAccessible::accSelect](http://msdn.microsoft.com/library/windows/desktop/dd318474) Windows SDK에 있습니다.

##  <a name="animatewindow"></a>  CWnd::AnimateWindow

표시 하거나 windows를 숨길 때 특수 효과 생성 합니다.
...
*dwFlags*  
애니메이션의 종류를 지정합니다. 가능한 값의 전체 목록을 참조 하세요 [AnimateWindow](http://msdn.microsoft.com/library/windows/desktop/ms632669)합니다.
...
이 멤버 함수는 함수의 기능을 에뮬레이트합니다 [AnimateWindow](http://msdn.microsoft.com/library/windows/desktop/ms632669)Windows SDK에 설명 된 대로 합니다.
...
함수가 성공할 경우 아이콘의 한 행의 높이 그렇지 않으면 0입니다.

### <a name="remarks"></a>설명

이 멤버 함수는 또한 전체 화면에 걸쳐 있는 데스크톱 창에서 아이콘을 정렬 합니다. 합니다 [GetDesktopWindow](#getdesktopwindow) 멤버 함수는 데스크톱 창 개체에 대 한 포인터를 검색 합니다.

MDI 클라이언트 창에서 MDI 자식 창 아이콘을 정렬 하려면 호출 [CMDIFrameWnd::MDIIconArrange](../../mfc/reference/cmdiframewnd-class.md#mdiiconarrange)합니다.
...
이 예제에서는 MDI 클라이언트 창을 매핑할 Attach 및 Detach를 사용 하는 방법을 보여 줍니다.
...
준비 `CWnd` 그리기 및 입력 한 후에는 `PAINTSTRUCT` 그리기에 대 한 정보를 사용 하 여 데이터 구조입니다.
...
에 대 한 장치 컨텍스트를 식별 `CWnd`합니다. 포인터는 임시적 이며의 범위를 벗어나 저장 해서는 안 [EndPaint](#endpaint)합니다.

### <a name="remarks"></a>설명

그리기 구조를 완전히 업데이트 지역 및 백그라운드의 내용이 지워졌습니다 적이 있는지 여부를 지정 하는 플래그를 둘러싸는 가장 작은 사각형에는 RECT 데이터 구조를 포함 합니다.

업데이트 영역에 의해 설정 됩니다는 [Invalidate](#invalidate), [InvalidateRect](#invalidaterect), 또는 [InvalidateRgn](#invalidatergn) 멤버 함수 및 시스템에서 크기를 조절, 이동, 만들고, 스크롤, 또는 클라이언트 영역에 영향을 주는 다른 작업을 수행 합니다. 업데이트 영역 지우기, 표시 되 면 `BeginPaint` 보냅니다는 [WM_ONERASEBKGND](#onerasebkgnd) 메시지입니다.

호출 하지 마십시오는 `BeginPaint` 대 한 응답으로 제외 하 고 멤버 함수는 [WM_PAINT](#onpaint) 메시지입니다. 각 호출에는 `BeginPaint` 멤버 함수에 대 한 일치 하는 호출 있어야 합니다.는 [EndPaint](#endpaint) 멤버 함수입니다. 캐럿을 그릴 영역에 있는 경우는 `BeginPaint` 멤버 함수는 자동으로를 방지 하기 위해 캐럿을 숨깁니다.
...
데이터 소스 컨트롤의 데이터 원본, 사용자 이름, 암호 및 SQL 속성으로 정의 된 기본 커서를 호출 하는 개체의 바인딩된 기본 단순 속성을 (예: 편집 컨트롤)은 형식 라이브러리에 표시 된 대로 바인딩합니다.
...
데이터 소스 컨트롤에 커서 바인딩된 속성 (예: grid 컨트롤) 데이터 바인딩된 컨트롤에 바인딩하고 MFC 바인딩 관리자를 사용 하 여 해당 관계를 등록 합니다.
...
계산된 창 사각형의 크기는 메뉴 모음에 대 한 공간이 포함 되지 않습니다.
...
> [!NOTE]
> 이 멤버 함수를 사용 하 여 해도 코드에서 관리 되는 도구 설명에는 영향이 없습니다. 관리 하는 도구 설명 컨트롤에만 적용 [CWnd::EnableToolTips](#enabletooltips)합니다.
...
일반적으로 호출할 [CDialog::OnInitDialog](../../mfc/reference/cdialog-class.md#oninitdialog) 센터 대화 상자 응용 프로그램의 주 창 기준으로 합니다. 기본적으로 함수에는 해당 부모 창 및 소유자를 기준으로 팝업 창을 기준으로 자식 창을 가운데로 맞춥니다. 팝업 창 소유 하지 않은 경우 화면을 기준으로 가운데에 배치 합니다. 소유자 또는 부모 되지 않는 특정 창 기준으로 창을 가운데에 *pAlternateOwner* 유효한 창에 매개 변수를 설정할 수 있습니다. 화면을 기준으로 가운데 맞춤을 강제로 반환한 값 전달 [CWnd::GetDesktopWindow](#getdesktopwindow) 으로 *pAlternateOwner*합니다.
...
제거 `CWnd` 창에서 지정한 클립보드 뷰어 및는 체인에서 *hWndNext* 의 하위 항목을 `CWnd` 체인에서 상위 합니다.
...
(위치 옆에 확인 표시가 a)를 선택 하거나 취소 (제거에서 확인 표시) 단추 또는 세 가지 상태 단추의 상태를 변경 합니다.
...
`CheckDlgButton` 보냅니다 함수를 [BM_SETCHECK](http://msdn.microsoft.com/library/windows/desktop/bb775989) 메시지에서 지정 된 단추입니다.
...
선택 (확인 표시를 추가) 그룹에 지정 된 라디오 단추 및 지웁니다 (확인 표시에서 제거) 다른 모든 라디오 단추 그룹에.
...
`CheckRadioButton` 보냅니다 함수를 [BM_SETCHECK](http://msdn.microsoft.com/library/windows/desktop/bb775989) 지정된 라디오 단추에는 메시지입니다.
...
결정에 속하는 자식 창에 있는 경우 `CWnd` 지정된 위치를 포함 합니다.
...
|CWP_SKIPINVISIBLE|보이지 않는 자식 창을 건너뜁니다.|
|CWP_SKIPDISABLED|비활성화 된 자식 창을 건너뛰기|
|CWP_SKIPTRANSPARENT|투명 한 자식 창을 건너뜁니다.|

### <a name="return-value"></a>반환 값

지점을 포함 하는 자식 창을 식별 합니다. 클라이언트 영역 외부에 있는 지정 된 경우이 값은 NULL입니다. 지점 클라이언트 영역 내 모든 자식 창에 포함 되어 있지 경우 `CWnd` 반환 됩니다.

이 멤버 함수는 지정 된 지점을 포함 하는 숨김 또는 비활성화 자식 창을 반환 합니다.

둘 이상의 창이 지정된 된 점을 포함할 수 있습니다. 그러나이 함수를만 반환 합니다 `CWnd`* 지점을 포함 하는 첫 번째 창 발생 합니다.

`CWnd`* 임시 수 있으며 나중에 사용할 저장 해서는 안 되는 합니다.
...
`ClientToScreen` 멤버 함수에서 클라이언트 좌표를 사용 하는 `POINT` 또는 `RECT` 구조 또는 `CPoint` 하거나 `CRect` 가리키는 개체 *lpPoint* 또는 *lpRect* 새 화면 좌표가;를 계산 합니다. 다음 구조의 좌표를 새 좌표로 대체합니다. 새 화면 좌표가 시스템 디스플레이의 왼쪽 위 모퉁이 기준으로 합니다.

`ClientToScreen` 멤버 함수는 지정 된 점 또는 사각형이 클라이언트 좌표로 나타낸에서 것으로 가정 합니다.
...
이 멤버 함수는 함수의 기능을 에뮬레이트합니다 [CloseWindow](http://msdn.microsoft.com/library/windows/desktop/ms632678)Windows SDK에 설명 된 대로 합니다.

##  <a name="continuemodal"></a>  CWnd::ContinueModal

이 멤버 함수는 호출한 [RunModalLoop](#runmodalloop) 모달 상태를 종료 해야 하는 경우를 확인 하려면.
...
모달 루프; 계속 될 경우 0이 아닌 값 0 [EndModalLoop](#endmodalloop) 라고 합니다.
...
지정 된 자식 창을 만들고에 연결 합니다 [CWnd](../../mfc/reference/cwnd-class.md) 개체입니다.
...
> [!WARNING]
> `CWnd::PreCreateWindow` 이제의 hMenu 멤버를 할당 해당 `CREATESTRUCT` 매개 변수를 **이** 포인터 메뉴가 NULL이 고 스타일 WS_CHILD를 포함 하는 경우. 적절 한 기능에 대 한 대화 상자 컨트롤에 NULL이 아닌 ID를 확인 합니다.
>
> 이 변경 관리/네이티브 interop 시나리오에서 충돌을 해결합니다. 추적 문을 `CWnd::Create` 개발자 문제를 경고 합니다.

사용 된 [AfxRegisterWndClass](../../mfc/reference/application-information-and-management.md#afxregisterwndclass) 창 클래스를 등록 하는 함수입니다. 사용자는 등록 된 창 클래스 모듈에서 사용할 수 있습니다.

합니다 [CWnd::OnCreate](#oncreate) 전에 메서드가 호출 되는 `Create` 메서드가 반환 창이 표시 되기 전에 및 합니다.
...
`CreateCaret` 캐럿을 소유 하는 기간에 관계 없이 모든 경우에 자동으로 이전 캐럿 셰이프를 제거 합니다. 를 만든 후 캐럿 처음에 숨겨져 있습니다. 캐럿을 표시 합니다 [ShowCaret](#showcaret) 멤버 함수를 호출 해야 합니다.

시스템 캐럿에는 공유 리소스입니다. `CWnd` 입력 포커스가 또는 활성화 된 경우에 캐럿을 만들어야 합니다. 입력된 포커스를 잃이 되거나 비활성화 될 전에 캐럿을 파괴 해야 해당 합니다.
...
MFC 프로그램에서의 표현 될 ActiveX 컨트롤을 만들려면이 멤버 함수를 사용 하 여를 `CWnd` 개체입니다.
...
`CreateControl` 직접적인 아날로그가 합니다 [CWnd::Create](#create) 함수에 대 한 창을 만듭니다.는 `CWnd`합니다. `CreateControl` 일반 창 대신 ActiveX 컨트롤을 만듭니다.
...
> `CWnd::PreCreateWindow` 이제의 hMenu 멤버를 할당 해당 `CREATESTRUCT` 매개 변수를 **이** 포인터 메뉴가 NULL이 고 스타일 WS_CHILD를 포함 하는 경우. 적절 한 기능에 대 한 대화 상자 컨트롤에 NULL이 아닌 ID를 확인 합니다.
>
> 이 변경 관리/네이티브 interop 시나리오에서 충돌을 해결합니다. A `TRACE` 문에서 `CWnd::Create` 개발자 문제를 경고 합니다.

확장 창 스타일은 WS_EX_LEFT 기본 창 스타일은 WS_OVERLAPPED 합니다.

사용 된 [AfxRegisterWndClass](../../mfc/reference/application-information-and-management.md#afxregisterwndclass) 창 클래스를 등록 하는 함수입니다. 사용자는 등록 된 창 클래스 모듈에서 사용할 수 있습니다.

자식 창에 대 한 차원은 부모 창의 클라이언트 영역의 왼쪽 위 모퉁이 기준으로 합니다. 최상위 창에 대 한 차원은 화면의 왼쪽 위 모퉁이 기준으로 합니다.

합니다 [CWnd::OnCreate](#oncreate) 전에 메서드가 호출 되는 `CreateEx` 메서드가 반환 창이 표시 되기 전에 및 합니다.
...
시스템 캐럿에 대 한 회색 사각형을 만들고 캐럿의 소유권을 요구 합니다.
...
캐럿 셰이프 줄 또는 블록을 수 있습니다.

매개 변수 *nWidth* 하 고 *nHeight* 캐럿의 너비와 높이 (논리 단위)를 지정 하며 매핑 모드에 따라 달라 집니다 정확한 너비와 높이 (픽셀)입니다.

시스템의 창 테두리 너비 또는 높이에서 검색할 수는 [GetSystemMetrics](http://msdn.microsoft.com/library/windows/desktop/ms724385) SM_CXBORDER 및 SM_CYBORDER 인덱스를 사용 하 여 Windows 함수입니다. 창 테두리 너비 또는 높이 사용 하 여 캐럿 고해상도 디스플레이에 표시 되도록 보장 합니다.
...
시스템 캐럿에 대 한 견고한 사각형 만들고 캐럿의 소유권을 요구 합니다.
...
Windows 창을 생성 및 될 때까지 연결 되지 않는 합니다 [CreateEx](#createex) 또는 [만들기](#create) 멤버 함수를 호출 합니다.
...
기본 창 프로시저는 기본 응용 프로그램 처리 하지 않는 모든 창 메시지에 대 한 처리를 제공 합니다. 이 멤버 함수는 모든 메시지 처리 되지 않는 확인 합니다.
...
응용 프로그램 처리 하지 않는 모든 창 메시지에 대 한 기본 처리를 제공 하는 기본 창 프로시저를 호출 합니다.
...
이 멤버 함수는 모든 메시지 처리 되지 않는 확인 합니다. 창 프로시저를 수신한 것과 동일한 매개 변수를 사용 하 여 호출 해야 합니다.
...
임시 삭제 `CWnd` 개체에 의해 생성 된 `FromHandle` 멤버 함수입니다.
...
에 연결 된 Windows 창을 소멸 된 `CWnd` 개체입니다.
...
창이 소멸 되; 경우 0이 아닌 값 그렇지 않으면 0입니다.

### <a name="remarks"></a>설명

`DestroyWindow` 멤버 함수를 비활성화 하 고 입력된 포커스가 제거 창에 적절 한 메시지를 보냅니다. 또한 창 메뉴를 제거, 응용 프로그램 큐를 플러시, 처리 중인 타이머를 제거, 클립보드 소유권을 제거 하 고 경우 클립보드 뷰어 체인을 중단 `CWnd` 뷰어 체인의 맨 위에 있는 됩니다. 보냅니다 [WM_DESTROY](#ondestroy) 하 고 [WM_NCDESTROY](#onncdestroy) 창에는 메시지입니다. 제거 하지 않습니다는 `CWnd` 개체입니다.

`DestroyWindow` 정리를 수행 하기 위한 자리 표시자가입니다. 때문에 `DestroyWindow` 가상 함수에 표시 됩니다 `CWnd`-클래스 뷰에서 클래스를 파생 합니다. 이 함수를 재정의 하는 경우에 있지만 하 `CWnd`-파생 클래스인 `DestroyWindow` 반드시 호출 되지 않습니다. 경우 `DestroyWindow` 명시적으로 호출할 수 하려는 경우 사용자 고유의 코드에서 호출 하 여 MFC 코드에서 호출 되지 않습니다.

예를 들어: 재정의 한 것으로 가정 `DestroyWindow` 에 `CView`-파생 클래스입니다. MFC 소스 코드를 호출 하지 않습니다 때문 `DestroyWindow` 중 하나에서 해당 `CFrameWnd`-파생 클래스에서 재정의 된 `DestroyWindow` 명시적으로 호출 하지 않으면 호출 되지 것입니다.

창의 모든 windows의 부모 이면 부모 창을 소멸 될 때 이러한 자식 창은 자동으로 제거 됩니다. `DestroyWindow` 멤버 함수는 자식 창을 제거와 창 자체입니다.
...
Windows 개체에는 HWND입니다.

### <a name="example"></a>예

  예를 참조 하세요 [CWnd::Attach](#attach)합니다.
...
- DDL_POSTMSGS LB_DIR 플래그입니다. Windows에서 생성 하는 메시지를 배치 LB_DIR 플래그가 설정 된 경우 `DlgDirList` 응용 프로그램의 큐에이 고, 그렇지 직접로 보내지는 대화 상자 프로시저입니다.

- DDL_DRIVES 드라이브입니다. DDL_DRIVES 플래그가 설정 되 면 DDL_EXCLUSIVE 플래그를 자동으로 설정 됩니다. 따라서 드라이브 및 파일을 포함 하는 디렉터리 목록 만들기를 호출 해야 합니다 `DlgDirList` 두 번: 한 번의 DDL_DRIVES를 사용 하 여 플래그 집합 및 한 번은 나머지 부분에 대 한 플래그를 사용 하 여 합니다.

- DDL_EXCLUSIVE 배타적 비트입니다. 단독 비트가 설정 된 경우 지정 된 형식의 파일에만 나열 됩니다. 그렇지 않으면 일반 파일 및 지정 된 형식의 파일 나열 됩니다.
...
`DlgDirList` 보냅니다 [LB_RESETCONTENT](http://msdn.microsoft.com/library/windows/desktop/bb761325) 하 고 [LB_DIR](http://msdn.microsoft.com/library/windows/desktop/bb775185) 목록 상자에는 메시지입니다. 지정 된 목록 상자를 채우도록 *nIDListBox* 하 여 지정 된 경로 일치 하는 모든 파일의 이름으로 *lpPathSpec*합니다.

합니다 *lpPathSpec* 매개 변수는 다음과 같은 형식을 갖습니다.

`[drive:] [ [\u]directory[\idirectory]...\u] [filename]`

이 예제에서는 *드라이브* 드라이브 문자입니다 *디렉터리* 은 올바른 디렉터리 이름, 및 *filename* 가 하나 이상의 와일드 카드를 포함 해야 하는 올바른 파일입니다. 와일드 카드는 모든 문자를 나타내고 별표와 일치 함을 의미 하는 물음표 (*) ( **\***), 즉 문자의 번호와 일치 합니다.

에 대 한 길이가 0 인 문자열을 지정 하는 경우 *lpPathSpec*, 또는 문자열을 변경할 수는 디렉터리 이름만 지정 해도 모든 파일 사양이 없는 경우 "*.\*"입니다.

하는 경우 *lpPathSpec* 는 드라이브 및/또는 디렉터리 이름, 목록 상자 채워집니다 전에 현재 드라이브 및 디렉터리에 지정 된 드라이브 및 디렉터리로 변경 됩니다. 으로 식별 되는 텍스트 컨트롤이 *nIDStaticPath* 새 드라이브 및/또는 디렉터리 이름으로 업데이트 됩니다.

목록 상자를 입력 한 후 *lpPathSpec* 경로의 드라이브 및/또는 디렉터리 부분을 제거 하 여 업데이트 됩니다.
...
- DDL_POSTMSGS CB_DIR 플래그입니다. Windows에서 생성 하는 메시지를 배치 CB_DIR 플래그가 설정 된 경우 `DlgDirListComboBox` 응용 프로그램의 큐에이 고, 그렇지 직접로 보내지는 대화 상자 프로시저입니다.

- DDL_DRIVES 드라이브입니다. DDL_DRIVES 플래그가 설정 되 면 DDL_EXCLUSIVE 플래그를 자동으로 설정 됩니다. 따라서 드라이브 및 파일을 포함 하는 디렉터리 목록 만들기를 호출 해야 합니다 `DlgDirListComboBox` 두 번: 한 번의 DDL_DRIVES를 사용 하 여 플래그 집합 및 한 번은 나머지 부분에 대 한 플래그를 사용 하 여 합니다.

- DDL_EXCLUSIVE 배타적 비트입니다. 단독 비트가 설정 된 경우 지정 된 형식의 파일에만 나열 됩니다. 그렇지 않으면 일반 파일 및 지정 된 형식의 파일 나열 됩니다.

### <a name="return-value"></a>반환 값

함수의 결과 지정 합니다. 목록을 수행한는 빈 목록도 하는 경우 0이 아닌 값입니다. 0 반환 값을 입력된 문자열에 유효한 검색 경로 포함 하지 않은 것을 의미 합니다.

### <a name="remarks"></a>설명

`DlgDirListComboBox` 보냅니다 [CB_RESETCONTENT](http://msdn.microsoft.com/library/windows/desktop/bb775878) 하 고 [CB_DIR](http://msdn.microsoft.com/library/windows/desktop/bb775832) 콤보 상자에는 메시지입니다. 지정 된 콤보 상자의 목록 상자를 채우도록 *nIDComboBox* 하 여 지정 된 경로 일치 하는 모든 파일의 이름으로 *lpPathSpec*합니다.
...
콤보 상자 목록 상자를 입력 한 후 *lpPathSpec* 경로의 드라이브 및/또는 디렉터리 부분을 제거 하 여 업데이트 됩니다.
...
목록 상자를 여 채워진 것으로 가정 합니다 [DlgDirList](#dlgdirlist) 멤버 함수 및 선택한 않음을 드라이브 문자, 파일 또는 디렉터리 이름입니다.

합니다 `DlgDirSelect` 제공한 버퍼에 선택 영역을 복사 하는 멤버 함수 *lpString*합니다. 선택 영역이 없는 경우 *lpString* 변경 되지 않습니다.

`DlgDirSelect` 보냅니다 [LB_GETCURSEL](http://msdn.microsoft.com/library/windows/desktop/bb775197) 하 고 [LB_GETTEXT](http://msdn.microsoft.com/library/windows/desktop/bb761313) 목록 상자에는 메시지입니다.

목록 상자에서 반환 될 둘 이상의 파일 이름을 허용 하지 않습니다. 목록 상자를 다중 선택 목록 상자 아니어야 합니다.
...
목록 상자를 여 채워진 것으로 가정 합니다 [DlgDirListComboBox](#dlgdirlistcombobox) 멤버 함수 및 선택한 않음을 드라이브 문자, 파일 또는 디렉터리 이름입니다.

`DlgDirSelectComboBox` 멤버 함수는 지정 된 버퍼를 선택 영역을 복사 합니다. 선택 영역이 없는 경우에 버퍼의 내용은 변경 되지 않습니다.

`DlgDirSelectComboBox` 보냅니다 [CB_GETCURSEL](http://msdn.microsoft.com/library/windows/desktop/bb775845) 하 고 [CB_GETLBTEXT](http://msdn.microsoft.com/library/windows/desktop/bb775862) 콤보 상자에는 메시지입니다.

콤보 상자에서 반환 될 둘 이상의 파일 이름을 허용 하지 않습니다.
...
이 함수를 직접 호출 하지 않습니다. 에 의해 호출 되는 [UpdateData](#updatedata) 멤버 함수입니다. 호출 `UpdateData` 대화 상자의 컨트롤을 초기화 하거나 대화 상자에서 데이터를 검색 합니다.

응용 프로그램 관련 대화 상자 클래스에서 파생 하는 경우 [CDialog](../../mfc/reference/cdialog-class.md), 프레임 워크의 자동 데이터 교환 및 유효성 검사를 활용 하려는 경우이 멤버 함수를 재정의 해야 합니다. 변수 추가 마법사를 원하는 포함 된이 멤버 함수 재정의 된 버전을 작성 합니다 (DDX) 대화 상자 데이터 교환 및 유효성 검사 (DDV) 전역 함수 호출의 "데이터 맵"입니다.

이 멤버 함수 재정의 된 버전을 자동으로 생성 하려면 대화 상자 편집기를 사용 하 여 대화 상자 리소스 만들기를 응용 프로그램 관련 대화 상자 클래스를 파생 합니다. 새 대화 상자에서 다양 한 컨트롤을 사용 하 여 변수, 데이터 및 유효성 검사 범위를 연결 하려면 다음 변수 추가 마법사를 사용 합니다. 마법사는 다음 재정의 된 기록 `DoDataExchange`, 데이터 맵을 포함 하는 합니다. 다음은 변수 추가 마법사에서 생성 하는 예제에서 DDX/DDV 코드 블록입니다.

[!code-cpp[NVC_MFCWindowing#90](../../mfc/reference/codesnippet/cpp/cwnd-class_29.cpp)]

`DoDataExchange` 재정의 된 멤버 함수 소스 파일에 매크로 문 앞에 야 합니다.

대화 상자 데이터 교환 및 유효성 검사에 대 한 자세한 내용은 참조 하세요. [표시 및 양식에서 데이터 조작](../../data/odbc/displaying-and-manipulating-data-in-a-form.md) 하 고 [대화 상자 데이터 교환 및 유효성 검사](../../mfc/dialog-data-exchange-and-validation.md)합니다. 변수 추가 마법사에서 생성 된 DDX_ 및 DDV_ 매크로 대 한 참조 [기술 참고 26](../../mfc/tn026-ddx-and-ddv-routines.md)합니다.

##  <a name="dragacceptfiles"></a>  CWnd::DragAcceptFiles

창 내에서이 멤버 함수를 호출를 사용 하는 `CWnd` 응용 프로그램에 대 한 포인터 [CWinApp::InitInstance](../../mfc/reference/cwinapp-class.md#initinstance) Windows 파일 관리자 또는 파일 탐색기에서 끌어 놓은 파일 창에 수락 함을 나타내려면 함수 .
...
호출 하는 창만 `DragAcceptFiles` 사용 하 여 합니다 *bAccept* 매개 변수를 TRUE로 설정 WM_DROPFILES Windows 메시지를 처리 하는 일을 할 것으로 확인 했습니다. MDI 응용 프로그램의 예를 들어 경우는 `CMDIFrameWnd` 에서 창 포인터를 사용 합니다 `DragAcceptFiles` 함수 호출에만 `CMDIFrameWnd` 창 WM_DROPFILES 메시지를 가져옵니다. 이 메시지를 모든 보내지 오픈 `CMDIChildWnd` windows. 에 대 한는 `CMDIChildWnd` 이 메시지를 받는 창 호출 해야 합니다 `DragAcceptFiles` 사용 하 여는 `CMDIChildWnd` 창 포인터입니다.

끌어온된 파일 수신을 중단을 사용 하 여 멤버 함수 호출 *bAccept* FALSE로 설정 합니다.
...
왼쪽된 단추를 누른 채 끌기 사각형을 외부에서 마우스를 이동 하는 사용자, 반환 값은 0이 아닌 값입니다.

사용자는 왼쪽된 단추를 누른 채 끌기 사각형을 외부에서 마우스를 이동 하지 않은, 반환 값은 0입니다.

### <a name="remarks"></a>설명

이 멤버 함수는 함수의 기능을 에뮬레이트합니다 [DragDetect](http://msdn.microsoft.com/library/windows/desktop/ms646256)Windows SDK에 설명 된 대로 합니다.
...
*lprcFrom*  
에 대 한 포인터를 [RECT](http://msdn.microsoft.com/library/windows/desktop/dd162897) 구조 아이콘 또는 최소화 된 창의 크기와 위치를 지정 합니다.

*lprcTo*  
에 대 한 포인터를 [RECT](http://msdn.microsoft.com/library/windows/desktop/dd162897) 복원 된 창의 크기와 위치를 지정 하는 구조체
...
이 멤버 함수는 함수의 기능을 에뮬레이트합니다 [DrawAnimatedRects](http://msdn.microsoft.com/library/windows/desktop/dd162475)Windows SDK에 설명 된 대로 합니다.
...
*uFlags*  
그리기 옵션을 지정합니다. 값의 전체 목록은 참조 하세요 [DrawCaption](http://msdn.microsoft.com/library/windows/desktop/dd162476)합니다.
...
이 멤버 함수는 함수의 기능을 에뮬레이트합니다 [DrawCaption](http://msdn.microsoft.com/library/windows/desktop/dd162476)Windows SDK에 설명 된 대로 합니다.
...
메뉴 모음에는 Windows에서 창을 만든 후 변경 되 면 변경된 메뉴 모음에 그릴이 함수를 호출 합니다.

### <a name="example"></a>예

  예를 참조 하세요 [CWnd::GetMenu](#getmenu)합니다.

##  <a name="enableactiveaccessibility"></a>  CWnd::EnableActiveAccessibility

수 있도록 사용자 정의 Active Accessibility 함수입니다.
...
표준 windows 및 ActiveX 컨트롤;를 비롯 하 여 컨트롤에 대 한 MFC의 기본 Active Accessibility 지원이 부족 그러나 경우에 `CWnd`-nonwindowed 사용자 인터페이스 요소를 포함 하는 파생된 클래스에 MFC에 대 한 알 수 없습니다. 이 경우 해당 재정의 해야 합니다 [Active Accessibility 멤버 함수](http://msdn.microsoft.com/68af04ac-4eb9-4b7d-b33f-c45512097a74) 클래스에서 호출 해야 하 고 `EnableActiveAccessibility` 클래스의 생성자에서.
...
- SB_HORZ 사용 하거나 사용 하지 않도록 설정 창을 사용 하 여 가로 스크롤 막대의 화살표를 연결 합니다.
...
화살표를 지정 된 대로 사용 하는 경우에 0이 아닙니다. 그렇지 않은 화살표는 이미 요청 된 상태 또는 오류가 발생 했음을 나타내는 0입니다.

##  <a name="enablescrollbarctrl"></a>  CWnd::EnableScrollBarCtrl

사용 하거나이 창에 대 한 스크롤 막대를 사용 하지 않도록 설정 합니다.
...
창 형제 스크롤 막대 컨트롤에는 스크롤 막대 사용 됩니다. 그렇지 않으면 창의 스크롤 막대 사용 됩니다.
...
도구 팁을 사용 하면 TRUE입니다. 그렇지 않으면 FALSE입니다.
...
> [!NOTE]
> 일부 windows와 같은 [CToolBar](../../mfc/reference/ctoolbar-class.md)에 구현 된 기본 제공 [OnToolHitTest](#ontoolhittest)합니다.

참조 [TOOLINFO](http://msdn.microsoft.com/library/windows/desktop/bb760256) 이 구조에 대 한 자세한 내용은 Windows SDK에 있습니다.

호출 `EnableToolTips` 부모 창에서 파생 된 경우가 아니면에 자식 컨트롤에 대 한 도구 설명을 표시 하는 데 충분 하지 않습니다 `CFrameWnd`합니다. 왜냐하면 `CFrameWnd` TTN_NEEDTEXT 알림에 대 한 기본 처리기를 제공 합니다. 부모 창에서 파생 되지 않은 경우 `CFrameWnd`, 즉, 대화 상자나 폼 보기 인 경우 도구 설명에 대 한 귀하의 자녀 TTN_NEEDTEXT 도구 팁 알림에 대 한 처리기를 제공 하지 않으면 컨트롤이 제대로 표시 되지 것입니다. 참조 [도구 설명](../../mfc/tool-tips-in-windows-not-derived-from-cframewnd.md)합니다.

제공 하 여 windows에 대 한 기본 도구 설명을 `EnableToolTips` 연결 된 텍스트에 있지 않습니다. 표시할 도구 설명에 대 한 텍스트를 검색 하려면 TTN_NEEDTEXT 알림 도구 설명 창이 표시 되기 직전에 도구 설명 컨트롤의 부모 창으로 전송 됩니다. 일부 값을 할당할이 메시지에 대 한 처리기가 합니다 *pszText* 의 멤버는 `TOOLTIPTEXT` 구조에 없는 텍스트 도구 설명에 표시 됩니다.
...
사용 하거나 추적 도구를 사용 하지 않도록 설정 합니다.
...
이전 상태를 나타냅니다는 `EnableWindow` 멤버 함수를 호출 했습니다. 반환 값은 창의 이전에 해제 한 경우 0이 아닙니다. 창에서 이전에 설정 된 경우 오류가 발생 했습니다. 반환 값은 0입니다.

### <a name="remarks"></a>설명

추적 도구 팁을 동적으로 화면에 배치할 수 있는 도구 설명 창이 있습니다. 도구 설명 창이 표시 위치를 신속 하 게 업데이트 하 여 원활 하 게 이동 하거나 "추적 합니다." 이 기능은 도구 설명 텍스트 포인터의 위치를 이동할 때 수행 해야 하는 경우에 유용할 수 있습니다.
...
입력을 사용 하지 않도록 설정 하는 경우에 마우스 클릭 및 키 입력은 무시 됩니다. 같은 입력 합니다. 입력을 사용 하는 경우 창의 모든 입력을 처리 합니다.

활성화 상태가 변경 되 면 합니다 [WM_ENABLE](#onenable) 이 함수가 반환 하기 전에 전송 됩니다.

사용 하지 않으면 모든 자식 창은 암시적으로 WM_ENABLE 메시지 전송 되지 않습니다 하지만 비활성화 됩니다.

창이 활성화 될 수 전에 활성화 되어야 합니다. 예를 들어, 응용 프로그램 모덜리스 대화 상자를 표시 하 고 해당 기본 창이 비활성화, 대화 상자를 제거 하기 전에 주 창을 사용할 수 있어야 합니다. 이 고, 그렇지 다른 창 입력된 포커스를 받 및 활성화할 수 있습니다. 자식 창에는 비활성화 된 경우 Windows는 창에 마우스 메시지가 해야 확인 하려고 할 때 무시 됩니다.

기본적으로 창에는 만들어질 때 사용 됩니다. 응용 프로그램에서 WS_DISABLED 스타일을 지정할 수는 [Create](#create) 또는 [CreateEx](#createex) 멤버 함수를 처음부터 사용할 수 있는 창을 만듭니다. 창을 만든 후 응용 프로그램 이용할 수 있습니다는 `EnableWindow` 멤버 함수를 사용 하는 창 사용 안 함.

응용 프로그램 대화 상자에서 컨트롤을 사용할지 여부를이 함수를 사용할 수 있습니다. 비활성화 된 컨트롤에 입력된 포커스를 받을 수 없습니다. 사용자에 액세스할 수 없습니다.
...
합니다 *n 결과* 매개 변수에서 반환 값으로 전파 됩니다 `RunModalLoop`합니다.
...
합니다 `EndPaint` 멤버 함수는 각 호출에 필요한는 `BeginPaint` 그리기를 완료 한 후에만 멤버 함수입니다.

캐럿에서 숨겨진 된 경우는 `BeginPaint` 멤버 함수 `EndPaint` 화면에 캐럿을 복원 합니다.

### <a name="example"></a>예

  예를 참조 하세요 [cwnd:: Beginpaint](#beginpaint)합니다.
...
대화 상자 리소스를 실행 하는 경우 TRUE입니다. 그렇지 않으면 FALSE입니다.

### <a name="remarks"></a>설명

`ExecuteDlgInit` 다른 원본에서 실행 중인 모듈에 바인딩된 리소스 또는 리소스를 사용 합니다. 이를 위해 `ExecuteDlgInit` 리소스 핸들을 호출 하 여 찾은 `AfxFindResourceHandle`합니다. MFC 응용 프로그램 공유 DLL 사용 하지 않습니다 (MFCx0 [U] [D]. DLL), `AfxFindResourceHandle` 호출 [AfxGetResourceHandle](application-information-and-management.md#afxgetresourcehandle), 실행 파일에 대 한 현재 리소스 핸들을 반환 하는 합니다. 경우 MFCx0를 사용 하는 MFC 응용 프로그램 [U] [D]. DLL `AfxFindResourceHandle` 트래버스하는 `CDynLinkLibrary` 개체 목록을 공유 하 고 MFC 확장명 Dll 올바른 리소스를 찾으시나요 처리 합니다.
...
대부분의 MFC 응용 프로그램에서이 메서드는 프레임 워크 [PreTranslateMessage](#pretranslatemessage) 하 고 [EnableToolTips](#enabletooltips), 직접 호출할 필요가 없습니다.

그러나 특정 응용 프로그램에서 일부 ActiveX 컨트롤 예를 들어 framework에서 이러한 메서드를 호출할 수 있습니다 및 FilterToolTipMessage를 직접 호출 해야 합니다. 자세한 내용은 [방법의 만들기 도구 팁](../../mfc/methods-of-creating-tool-tips.md)합니다.

##  <a name="findwindow"></a>  CWnd::FindWindow

최상위 반환 `CWnd` 해당 창 클래스를 지정 하 여 *lpszClassName* 인 창 이름 또는 제목을 지정 하 여 및 *lpszWindowName*합니다.
...
지정 된 클래스 이름 및 창 이름이 있는 창을 식별 합니다. 이러한 창이 없으면이 값은 NULL입니다.

`CWnd`* 임시 수 있으며 나중에 사용할 저장 되어야 합니다.

### <a name="remarks"></a>설명

이 함수는 자식 windows를 검색 하지 않습니다.
...
해당 창 이름과 클래스 이름에 지정한 문자열과 일치 하는 창 개체를 검색 합니다.
...
*lpszClass*  
에 대 한 이전 호출에서 생성 하는 클래스 atom 또는 클래스 이름을 지정 하는 null로 끝나는 문자열에 대 한 포인터를 [RegisterClass](http://msdn.microsoft.com/library/windows/desktop/ms633586) 하거나 [RegisterClassEx](http://msdn.microsoft.com/library/windows/desktop/ms633587)합니다.
...
함수가 성공할 경우 반환 값은 창 이름과 지정 된 클래스를 소유 하는 창 개체에 대 한 포인터입니다. 함수가 실패 한 경우 반환 값은 NULL입니다.

### <a name="remarks"></a>설명

이 멤버 함수는 함수의 기능을 에뮬레이트합니다 [FindWindowEx](http://msdn.microsoft.com/library/windows/desktop/ms633500)Windows SDK에 설명 된 대로 합니다.
...
연속 깜박임에 대 한 시스템 타이머를 만들고 반복 해 서 호출 `FlashWindow`합니다. 깜박이 `CWnd` 제목 표시줄의 모양을 변경 처럼는 `CWnd` 비활성에서 활성 상태에서 또는 그 반대로 변경 되었습니다. (비활성 제목 표시줄을 활성 제목 표시줄을 변경 합니다; 활성 제목 표시줄을 비활성 제목 표시줄을로 변경)

일반적으로 창은 사용자에 게 알려 주의가 필요 없는이 현재 입력된 포커스를 업데이트 했습니다.

합니다 *bInvert* 매개 변수 창이 입력된 포커스를 시작 하는 시점과 더 이상 깜박일 수만 FALSE 여야 합니다; TRUE 여야 합니다 입력된 포커스를 얻기 위해 기다리는 동안 연속 호출에서.

이 함수는 항상 최소화 된 창에 대해 0을 반환합니다. 창을 최소화 하면 `FlashWindow` 창의 아이콘을 간단히 깜박입니다 *bInvert* 최소화 된 창을 무시 됩니다.
...
*dwFlags*  
플래시 상태를 지정합니다. 값의 전체 목록은 참조 하세요. 합니다 [FLASHWINFO](http://msdn.microsoft.com/library/windows/desktop/ms679348) 구조입니다.
...
반환 값을 호출 하기 전의 창의 상태를 지정 된 `FlashWindowEx` 함수입니다. 창 캡션에 호출 하기 전에 활성으로 그릴 경우 반환 값은 0이 아닌 값입니다. 그렇지 않은 경우 반환 값은 0입니다.

### <a name="remarks"></a>설명

이 메서드는 함수의 기능을 에뮬레이트합니다 [FlashWindowEx](http://msdn.microsoft.com/library/windows/desktop/ms679347)Windows SDK에 설명 된 대로 합니다.
...
경우는 `CWnd` 개체가 핸들에 연결 되지 않은, NULL이 반환 됩니다.

이 함수에서와 다르게 [FromHandle](#fromhandle), 임시 개체를 만들지 않습니다.
...
성공, 실패 시 COM 오류 코드는 S_OK를 반환 합니다. 참조 **값을 반환** 에 [IAccessible::get_accChild](http://msdn.microsoft.com/library/windows/desktop/dd318475) Windows SDK에에서 있습니다.
...
자세한 내용은 [IAccessible::get_accChild](http://msdn.microsoft.com/library/windows/desktop/dd318475) Windows SDK에 있습니다.
...
성공, 실패 시 COM 오류 코드는 S_OK를 반환 합니다. 참조 **값을 반환** 에 [IAccessible::get_accChildCount](http://msdn.microsoft.com/library/windows/desktop/dd318476) Windows SDK에에서 있습니다.

### <a name="remarks"></a>설명

이 함수는 mfc의 일부가 [Active Accessibility](http://msdn.microsoft.com/library/windows/desktop/dd373592) 지원 합니다.

이 함수를 재정의 하 `CWnd`-nonwindowed 사용자 인터페이스 요소 (MFC를 처리 하는 창 없는 ActiveX 컨트롤) 이외의 경우 클래스를 파생 합니다. 기본 클래스 버전을 호출 하 고 nonwindowed 자식 요소를 추가 합니다.

자세한 내용은 [IAccessible::get_accChildCount](http://msdn.microsoft.com/library/windows/desktop/dd318476) Windows SDK에 있습니다.
...
성공, 실패 시 COM 오류 코드는 S_OK를 반환 합니다. 참조 **값을 반환** 에 [IAccessible::get_accDefaultAction](http://msdn.microsoft.com/library/windows/desktop/dd318477) Windows SDK에에서 있습니다.
...
자세한 내용은 [IAccessible::get_accDefaultAction](http://msdn.microsoft.com/library/windows/desktop/dd318477) Windows SDK에 있습니다.
...
성공, 실패 시 COM 오류 코드는 S_OK를 반환 합니다. 참조 **값을 반환** 에 [IAccessible::get_accDescription](http://msdn.microsoft.com/library/windows/desktop/dd318478) Windows SDK에에서 있습니다.

### <a name="remarks"></a>설명

이 함수는 mfc의 일부가 [Active Accessibility](http://msdn.microsoft.com/library/windows/desktop/dd373592) 지원 합니다.

이 함수를 재정의 하면 `CWnd`-개체를 설명 하는 클래스를 파생 합니다. 기본 클래스 버전을 호출 하 고 설명을 추가 합니다.

자세한 내용은 [IAccessible::get_accDescription](http://msdn.microsoft.com/library/windows/desktop/dd318478) Windows SDK에 있습니다.
...
*pvarChild*  
포커스가 있는 개체에 대 한 정보를 받습니다. 참조 *pvarID* 에 [IAccessible::get_accFocus](http://msdn.microsoft.com/library/windows/desktop/dd318479) Windows SDK에 있습니다.

### <a name="return-value"></a>반환 값

성공, 실패 시 COM 오류 코드는 S_OK를 반환 합니다. 참조 **값을 반환** 에서 `IAccessible::get_accFocus` Windows SDK에에서 있습니다.
...
자세한 내용은 [IAccessible::get_accFocus](http://msdn.microsoft.com/library/windows/desktop/dd318479) Windows SDK에 있습니다.

##  <a name="get_acchelp"></a>  CWnd::get_accHelp

개체를 검색 하기 위해 프레임 워크를 호출한 **도움말** 속성 문자열입니다.
...
성공, 실패 시 COM 오류 코드는 S_OK를 반환 합니다. 참조 **값을 반환** 에 [IAccessible::get_accHelp](http://msdn.microsoft.com/library/windows/desktop/dd318480) Windows SDK에에서 있습니다.
...
자세한 내용은 [IAccessible::get_accHelp](http://msdn.microsoft.com/library/windows/desktop/dd318480) Windows SDK에 있습니다.

##  <a name="get_acchelptopic"></a>  CWnd::get_accHelpTopic

전체 경로 검색 하기 위해 프레임 워크에서 호출 된 **WinHelp** 지정한 개체와 해당 파일 내의 해당 항목의 식별자를 사용 하 여 연결 된 파일입니다.
...
*pidTopic*  
지정된 된 개체와 연결 된 도움말 파일 항목을 식별 합니다. 참조 *pidTopic* 에 [IAccessible::get_accHelpTopic](http://msdn.microsoft.com/library/windows/desktop/dd318481) Windows SDK에 있습니다.

### <a name="return-value"></a>반환 값

성공, 실패 시 COM 오류 코드는 S_OK를 반환 합니다. 참조 **값을 반환** 에서 `IAccessible::get_accHelpTopic` Windows SDK에에서 있습니다.

### <a name="remarks"></a>설명

이 함수는 mfc의 일부가 [Active Accessibility](http://msdn.microsoft.com/library/windows/desktop/dd373592) 지원 합니다.

이 함수를 재정의 하 `CWnd`-개체에 대 한 도움말 정보를 제공 하는 클래스를 파생 합니다.

자세한 내용은 [IAccessible::get_accHelpTopic](http://msdn.microsoft.com/library/windows/desktop/dd318481) Windows SDK에 있습니다.
...
성공, 실패 시 COM 오류 코드는 S_OK를 반환 합니다. 참조 **값을 반환** 에 [IAccessible::get_accKeyboardShortcut](http://msdn.microsoft.com/library/windows/desktop/dd318482) Windows SDK에에서 있습니다.
...
자세한 내용은 [IAccessible::get_accKeyboardShortcut](http://msdn.microsoft.com/library/windows/desktop/dd318482) Windows SDK에 있습니다.
...
성공, 실패 시 COM 오류 코드는 S_OK를 반환 합니다. 참조 **값을 반환** 에 [IAccessible::get_accName](http://msdn.microsoft.com/library/windows/desktop/dd318483) Windows SDK에에서 있습니다.

### <a name="remarks"></a>설명

이 함수는 mfc의 일부가 [Active Accessibility](http://msdn.microsoft.com/library/windows/desktop/dd373592) 지원 합니다.

이 함수를 재정의 하 `CWnd`-개체의 이름을 반환 하는 클래스를 파생 합니다.

자세한 내용은 [IAccessible::get_accName](http://msdn.microsoft.com/library/windows/desktop/dd318483) Windows SDK에 있습니다.
...
성공, 실패 시 COM 오류 코드는 S_OK를 반환 합니다. 참조 **값을 반환** 에 [IAccessible::get_accParent](http://msdn.microsoft.com/library/windows/desktop/dd318484) Windows SDK에에서 있습니다.
...
자세한 내용은 [IAccessible::get_accParent](http://msdn.microsoft.com/library/windows/desktop/dd318484) Windows SDK에 있습니다.
...
*pvarRole*  
역할 정보를 받습니다. 참조 *pvarRole* 에 [IAccessible::get_accRole](http://msdn.microsoft.com/library/windows/desktop/dd318485) Windows SDK에 있습니다.

### <a name="return-value"></a>반환 값

성공, 실패 시 COM 오류 코드는 S_OK를 반환 합니다. 참조 **값을 반환** 에서 `IAccessible::get_accRole` Windows SDK에에서 있습니다.
...
자세한 내용은 [IAccessible::get_accRole](http://msdn.microsoft.com/library/windows/desktop/dd318485) Windows SDK에 있습니다.
...
*pvarChildren*  
선택 된 자식에 대 한 정보를 받습니다. 참조 *pvarChildren* 에 [IAccessible::get_accSelection](http://msdn.microsoft.com/library/windows/desktop/dd318486) Windows SDK에 있습니다.

### <a name="return-value"></a>반환 값

성공, 실패 시 COM 오류 코드는 S_OK를 반환 합니다. 참조 **값을 반환** 에서 `IAccessible::get_accSelection` Windows SDK에에서 있습니다.
...
자세한 내용은 [IAccessible::get_accSelection](http://msdn.microsoft.com/library/windows/desktop/dd318486) Windows SDK에 있습니다.
...
*pvarState*  
개체의 상태에 대 한 정보를 받습니다. 참조 *pvarState* 에 [IAccessible::get_accState](http://msdn.microsoft.com/library/windows/desktop/dd318487) Windows SDK에 있습니다.

### <a name="return-value"></a>반환 값

성공, 실패 시 COM 오류 코드는 S_OK를 반환 합니다. 참조 **값을 반환** 에서 `IAccessible::get_accState` Windows SDK에에서 있습니다.
...
자세한 내용은 [IAccessible::get_accState](http://msdn.microsoft.com/library/windows/desktop/dd318487) Windows SDK에 있습니다.
...
성공, 실패 시 COM 오류 코드는 S_OK를 반환 합니다. 참조 **값을 반환** 에 [IAccessible::get_accValue](http://msdn.microsoft.com/library/windows/desktop/dd318488) Windows SDK에에서 있습니다.
...
자세한 내용은 [IAccessible::get_accValue](http://msdn.microsoft.com/library/windows/desktop/dd318488) Windows SDK에 있습니다.
...
활성 창 또는 호출 시 창이 활성 상태 였던 경우 NULL입니다. 해당 포인터는 임시적이며, 나중에 사용하려고 저장하면 안됩니다.

### <a name="remarks"></a>설명

활성 창에는 현재 입력 포커스가 있는 창이 나 명시적으로 활성화를 수행 하는 창 합니다 [SetActiveWindow](#setactivewindow) 멤버 함수입니다.
...
*gaFlags*  
검색할 상위 항목을 지정 합니다. 가능한 값의 전체 목록은 참조 하세요 [GetAncestor](http://msdn.microsoft.com/library/windows/desktop/ms633502)합니다.

### <a name="return-value"></a>반환 값

함수가 성공할 경우 반환 값은 상위 창 개체에 대 한 포인터입니다. 함수가 실패 한 경우 반환 값은 NULL입니다.

### <a name="remarks"></a>설명

이 멤버 함수는 함수의 기능을 에뮬레이트합니다 [GetAncestor](http://msdn.microsoft.com/library/windows/desktop/ms633502)Windows SDK에 설명 된 대로 합니다.
...
마우스 캡처가 있는 창을 식별 합니다. 창이 마우스 캡처가 포함 된 경우 NULL입니다.

반환 값을 임시 수 있으며 나중에 사용할 저장 되어야 합니다.

### <a name="remarks"></a>설명

하나의 창에 마우스 캡처가 언제 든 지 있습니다. 창이 마우스를 받을 때 캡처 합니다 [SetCapture](#setcapture) 멤버 함수를 호출 합니다. 이 창 테두리 내에서 커서를 여부 마우스 입력을 받습니다.

##  <a name="getcaretpos"></a>  CWnd::GetCaretPos

캐럿 현재 위치의 클라이언트 좌표를 검색 하 고 그대로 반환을 `CPoint`입니다.
...
캐럿 위치의 클라이언트 좌표에 지정 되는 `CWnd` 창입니다.

##  <a name="getcheckedradiobutton"></a>  CWnd::GetCheckedRadioButton

지정된 된 그룹의 현재 선택 된 라디오 단추의 ID를 검색합니다.
...
선택 된 라디오 단추 또는 하나도 선택 하지 않은 경우 0의 ID입니다.

##  <a name="getclientrect"></a>  CWnd::GetClientRect

클라이언트 좌표에 복사 합니다 `CWnd` 가리키는 클라이언트 영역 구조로 *lpRect*합니다.
...
클라이언트 좌표를 클라이언트 영역의 왼쪽 및 오른쪽 아래 모퉁이 지정합니다. 클라이언트 좌표는 왼쪽 위 모퉁이 기준으로 하므로 `CWnd` 클라이언트 영역의 왼쪽 위 모퉁이의 좌표는 (0, 0).

### <a name="example"></a>예

  예를 참조 하세요 [CWnd::IsIconic](#isiconic)합니다.
...
함수에 성공 하는 경우 클립보드를 소유 하는 창을 식별 합니다. 그렇지 않을 경우 NULL입니다.

반환된 된 포인터 임시 수 있으며 나중에 사용할 저장 되어야 합니다.
...
성공할 경우 클립보드 표시를 현재 담당 창을 식별합니다 그렇지 않은 경우 (예를 들어 경우 뷰어가) NULL입니다.
...
에 대 한 포인터를 [IUnknown](http://msdn.microsoft.com/library/windows/desktop/ms680509) 이 표시 되는 OLE 컨트롤의 인터페이스 `CWnd` 개체입니다. 이 개체가 OLE 컨트롤을 나타내지 않는 경우 반환 값은 NULL입니다.

### <a name="remarks"></a>설명

이 해제 하지 해야 `IUnknown` 포인터입니다. 일반적으로 컨트롤의 특정 인터페이스를 가져오려면 사용할 수 있습니다.

반환 된 인터페이스 포인터를 `GetControlUnknown` 참조 개수가 계산 되지 않습니다. 호출 하지 마세요 [iunknown:: Release](http://msdn.microsoft.com/library/windows/desktop/ms682317) 포인터에 대해 이전에 호출한 경우가 아니면 [iunknown:: Addref](http://msdn.microsoft.com/library/windows/desktop/ms691379) 에 합니다.
...
이 창에서 현재 처리 중인 메시지에 대한 포인터를 반환합니다. 만 호출 해야 경우는 **에서 * * * 메시지* 메시지 처리기 멤버 함수입니다.
...
에 대 한 포인터를 반환 합니다 [메시지](../../mfc/reference/msg-structure1.md) 창 메시지를 포함 하는 구조는 현재 처리 하는 합니다. 만 호출 해야 경우는 **에서 * * * 메시지* 처리기입니다.

### <a name="example"></a>예

  예를 참조 하세요 [CMDIFrameWnd::MDICascade](../../mfc/reference/cmdiframewnd-class.md#mdicascade)합니다.

##  <a name="getdc"></a>  CWnd::GetDC

일반적인에 대 한 포인터, 클래스 또는 개인 장치에 대 한 지정 된 클래스 스타일에 따라 클라이언트 영역에 대 한 상황에 맞는 검색 된 `CWnd`합니다.
...
에 대 한 장치 컨텍스트를 식별 하는 `CWnd` 클라이언트 영역 경우이 고, 그렇지 않으면, 반환 값은 NULL입니다. 해당 포인터는 임시적이며, 나중에 사용하려고 저장하면 안됩니다.

### <a name="remarks"></a>설명

일반적인 장치 컨텍스트에 대 한 `GetDC` 검색 될 때마다 컨텍스트에 기본 특성을 할당 합니다. 클래스 및 개인 컨텍스트 `GetDC` 변경 하지 않고 이전에 할당 된 특성을 유지 합니다. 장치 컨텍스트를 클라이언트 영역에 그릴 후속 그래픽 장치 인터페이스 (GDI) 함수에서 사용할 수 있습니다.

장치 컨텍스트 창 클래스에 속하는 경우가 아니면 합니다 [ReleaseDC](#releasedc) 그리기 후 컨텍스트를 해제 하려면 멤버 함수를 호출 해야 합니다.

에 게 속한 장치 컨텍스트로 합니다 `CWnd` 클래스에서 반환 되는 `GetDC` 이 있는, CS_OWNDC, 또는 CS_PARENTDC에 스타일으로 지정 된 경우 멤버 함수는 `WNDCLASS` 클래스 등록 되었을 때 구조체.

##  <a name="getdcex"></a>  CWnd::GetDCEx

에 대 한 장치 컨텍스트의 핸들을 검색 합니다 `CWnd` 창입니다.
...
- DCX_CLIPCHILDREN 아래의 모든 자식 창의 표시 영역을 제외 합니다 `CWnd` 창입니다.

- DCX_CLIPSIBLINGS 위의 모든 형제 창이의 표시 영역을 제외 합니다 `CWnd` 창입니다.

- 로 식별 되는 클리핑 영역 DCX_EXCLUDERGN 제외 *prgnClip* 반환 된 장치 컨텍스트의 표시 되는 영역에서 합니다.

- DCX_INTERSECTRGN로 식별 되는 클리핑 영역을 교차 *prgnClip* 반환 된 장치 컨텍스트의 표시 영역 내에서.

- DCX_LOCKWINDOWUPDATE 수 없는 경우에 그리기를 `LockWindowUpdate` 그렇지 않은 경우이 창의 제외 하는 효과 호출 합니다. 이 값은 추적 하는 동안 그리기에 사용 됩니다.

- DCX_PARENTCLIP 부모 창의 표시 영역을 사용 하 고 부모 창의 WS_CLIPCHILDREN 및 WS_PARENTDC 스타일 비트를 무시 합니다. 왼쪽 위 모퉁이에 있는 장치 컨텍스트의 원본을 설정 하는이 값은 `CWnd` 창입니다.

- DCX_WINDOW는 클라이언트 사각형이 아닌 창 사각형을 해당 하는 장치 컨텍스트를 반환 합니다.

### <a name="return-value"></a>반환 값

함수가 성공할 경우 지정된 된 기간에 대 한 장치 컨텍스트 그렇지 않으면 NULL입니다.

### <a name="remarks"></a>설명

장치 컨텍스트를 클라이언트 영역에 그릴 이후의 GDI 함수에서 사용할 수 있습니다.

이 함수를 확장 하는 [GetDC](http://msdn.microsoft.com/library/windows/desktop/dd144871) 함수를 창에 대 한 장치 컨텍스트 잘립니다 하는 방법을 보다 자세히 제어 응용 프로그램에 제공 합니다.

장치 컨텍스트 창 클래스에 속하는 경우가 아니면 합니다 [ReleaseDC](http://msdn.microsoft.com/library/windows/desktop/dd162920) 그리기 후 컨텍스트를 해제 하려면 함수를 호출 해야 합니다. 일반적인 5 개의 장치 컨텍스트를 사용할 수 있는 특정된 시점에 있으므로 장치 컨텍스트를 해제 하지 못하면 다른 응용 프로그램을 장치 컨텍스트에 액세스 하지 못하도록 방지할 수 있습니다.

캐시 된 장치 컨텍스트를 가져오려면 응용 프로그램 지정 해야 합니다 [DCX_CACHE](http://msdn.microsoft.com/library/windows/desktop/dd144873)합니다. 기간은 모두 CS_OWNDC DCX_CACHE 지정 되지 않은 경우 나 [이 있는](http://msdn.microsoft.com/library/windows/desktop/ms633576),이 함수는 NULL을 반환 합니다.

반환 하는 특수 한 특성을 사용 하 여 장치 컨텍스트에 [GetDCEx](http://msdn.microsoft.com/library/windows/desktop/dd144873) 경우 함수는 있는 [CS_OWNDC](http://msdn.microsoft.com/library/windows/desktop/ms633576), 또는 [CS_PARENTDC](http://msdn.microsoft.com/library/windows/desktop/ms633576) 스타일 합니다 에지정된[ WNDCLASS](http://msdn.microsoft.com/library/windows/desktop/ms633576) 클래스 등록 되었을 때 구조체입니다.

이러한 특성에 대 한 자세한 내용은 참조에 대 한 설명을 `WNDCLASS` Windows SDK에는 구조입니다.
...
함수가 성공할 경우 디바이스 컨텍스트 렌더링 대상이 지정된 된 기간 그렇지 않으면 NULL입니다.

### <a name="remarks"></a>설명

##  <a name="getdescendantwindow"></a>  CWnd::GetDescendantWindow

지정된 된 ID에서 지정한 하위 창을 찾을 수이 멤버 함수 호출
...
에 대 한 포인터를 `CWnd` 개체 또는 자식 창이 없는 경우 NULL입니다.

### <a name="remarks"></a>설명

이 멤버 함수는 자식 창, 직접 자식인 windows 뿐만 아니라 전체 트리를 검색 합니다.
...
Windows 바탕 화면 창을 식별합니다. 이 포인터 임시 수 있으며 나중에 사용할 저장 되어야 합니다.

### <a name="remarks"></a>설명

바탕 화면 창 화면 전체를 채울 이며 영역 위에 모든 아이콘 및 기타 windows 칠해집니다.

##  <a name="getdlgctrlid"></a>  CWnd::GetDlgCtrlID

모든 자식 창에 대 한 창 또는 컨트롤 ID 값을 반환 하지는 대화 상자에서 컨트롤의 합니다.
...
숫자 식별자를 `CWnd` 자식 창에 0이 고, 그렇지 않으면 함수는 경우.

### <a name="remarks"></a>설명

최상위 창에는 ID 값이 없으므로이 함수의 반환 값 올바르지 않습니다 경우는 `CWnd` 최상위 창입니다.

### <a name="example"></a>예

  예를 참조 하세요 [CWnd::OnCtlColor](#onctlcolor)합니다.
...
지정된 된 컨트롤이 나 자식 창에 대 한 포인터입니다. 경우 제공한 정수 ID 사용 하 여 없는 컨트롤을 *nID* 매개 변수가 있는지,이 값은 NULL입니다.

반환된 된 포인터 임시 수 있으며 나중에 사용할 저장 되어야 합니다.

### <a name="remarks"></a>설명

일반적으로 식별 된 컨트롤의 형식으로 캐스팅 된 반환 된 포인터 *nID*합니다.
...
대화 상자에 있는 항목 텍스트의 번역 된 값을 지정 합니다. 0은 유효한 반환 값 이므로 *lpTrans* 오류를 검색 하려면 사용 해야 합니다. 서명 된 반환 값을 원하는 경우로 캐스트를 **int** 형식입니다.

함수 반환 값에 번역 된 수 (부호 없음) 용 INT_MAX (예: 부호 있는 숫자) 또는 UINT_MAX 보다 큰 경우 0입니다.

숫자가 아닌 문자를 발견 하 고 위의 최대값을 초과 같은 오류가 발생 하면 `GetDlgItemInt` 0에서 가리키는 위치에 복사 *lpTrans*합니다. 오류가 없는 경우 *lpTrans* 0이 아닌 값을 받습니다. 하는 경우 *lpTrans* 가 null 인 경우 `GetDlgItemInt` 오류에 대 한 경고를 표시 하지 않습니다.

### <a name="remarks"></a>설명

텍스트의 시작 부분에 추가 공백을 제거 하 고 10 진수 변환 하 여 지정된 된 대화 상자에서 지정된 된 컨트롤의 텍스트를 정수 값으로 변환 합니다. 텍스트의 끝에 도달한 또는 숫자가 아닌 문자를 발견 하는 경우에 번역을 중지 합니다.

하는 경우 *bSigned* 가 TRUE 인 `GetDlgItemInt` 텍스트의 시작 부분에 빼기 기호 (-)에 대해 확인 하 고 텍스트를 부호 있는 숫자를 변환 합니다. 그렇지 않으면 부호 없는 값을 만듭니다.

보내는 [WM_GETTEXT](http://msdn.microsoft.com/library/windows/desktop/ms632627) 컨트롤에는 메시지입니다.

##  <a name="getdlgitemtext"></a>  CWnd::GetDlgItemText

제목 또는 대화 상자에서 컨트롤과 연결 된 텍스트를 검색 하려면이 멤버 함수를 호출 합니다.
...
실제 종료 null 문자를 포함 하지 않습니다 버퍼에 복사 되는 문자 수를 지정 합니다. 텍스트가 복사 되는 경우 값은 0입니다.

### <a name="remarks"></a>설명

합니다 `GetDlgItemText` 에서 가리키는 위치에 텍스트를 복사 하는 멤버 함수 *lpStr* 복사 된 바이트 수의 수를 반환 합니다.
...
데이터 소스 컨트롤에서 정의한 커서에 대 한 포인터입니다. 호출 하는 MFC 담당 `AddRef` 포인터에 대 한 합니다.

### <a name="remarks"></a>설명

반환 된 포인터를 사용 하 여 데이터 바인딩된 표 컨트롤과 같은 복잡 한 데이터 바인딩된 컨트롤의 ICursor 속성을 설정 합니다. 데이터 소스 컨트롤을 첫 번째 바인딩된 컨트롤이 해당 커서를 요청할 때까지 활성화 되지 않습니다. 이 중 하나를 호출 하 여 명시적으로 발생할 수 있습니다 `GetDSCCursor` 또는 MFC 바인딩 관리자에 의해 암시적으로 합니다. 두 경우 모두 데이터 소스 컨트롤을 호출 하 여 활성화를 강제할 수 있습니다 `GetDSCCursor` 호출한 `Release` 반환 된 포인터의 `IUnknown`합니다. 활성화에는 데이터 소스 컨트롤의 데이터 원본에 연결 하려고 하면 합니다. 반환된 된 포인터는 다음 컨텍스트에서 사용할 수 있습니다.
...
창의 확장 스타일의 합니다. MFC에서 사용 된 확장된 창 스타일에 대 한 자세한 내용은 참조 하세요. [확장 창 스타일](../../mfc/reference/styles-used-by-mfc.md#extended-window-styles)합니다.

##  <a name="getfocus"></a>  CWnd::GetFocus

에 대 한 포인터를 검색 합니다 `CWnd` 현재 입력된 포커스를가지고 있는 합니다.
...
포커스 창 없는 경우 NULL을 현재 포커스가 있는 창에 대 한 포인터입니다.

해당 포인터는 임시적이며, 나중에 사용하려고 저장하면 안됩니다.

##  <a name="getfont"></a>  CWnd::GetFont

현재 글꼴을 검색할 창 WM_GETFONT 메시지를 보냅니다.
...
에 대 한 포인터를 [CFont](../../mfc/reference/cfont-class.md) 창의 현재 글꼴에 연결 된 개체입니다.

### <a name="remarks"></a>설명

창 WM_GETFONT 메시지를 처리 하지 않는 한 효과가 없습니다. 파생 되는 여러 MFC 클래스 `CWnd` WM_GETFONT 메시지에 대 한 메시지 처리기를 포함 하는 미리 정의 된 창 클래스에 연결 하기 때문에이 메시지를 처리 합니다. 클래스에서 파생 되는이 방법을 사용 하려면 `CWnd` WM_GETFONT 메시지에 대 한 메서드 처리기를 정의 해야 합니다.
...
전경 창에 대 한 포인터입니다. 이 임시 않을 `CWnd` 개체입니다.

### <a name="remarks"></a>설명

전경 창이 최상위 창 (프레임 창 또는 대화 상자)에 적용 됩니다.

##  <a name="geticon"></a>  CWnd::GetIcon

표시 된 대로 중 하나는 큰 (32 x 32)에 대 한 핸들 또는 작은 (16 x 16) 아이콘에 대 한 핸들을 가져오려면이 함수를 호출 *bBigIcon*합니다.
...
### <a name="example"></a>예
  ...
  예를 참조 하세요 [CWnd::FindWindow](#findwindow)합니다.
...
*pdwFlags*  
레이어 플래그를 받는 DWORD에 대 한 포인터입니다. 이 인수가 필요 하지 않은 경우 NULL을 수 있습니다. 가능한 값의 전체 목록은 참조 하세요 [GetLayeredWindowAttributes](http://msdn.microsoft.com/library/windows/desktop/ms633508)합니다.
...
이 멤버 함수는 함수의 기능을 에뮬레이트합니다 [GetLayeredWindowAttributes](http://msdn.microsoft.com/library/windows/desktop/ms633508)Windows SDK에 설명 된 대로 합니다.
...
메뉴를 식별합니다. 이 경우 값은 NULL `CWnd` 메뉴가 없습니다. 경우에 반환 값이 정의 되지 `CWnd` 자식 창입니다.
...
*idObject*  
메뉴 개체를 지정합니다. 가능한 값 목록을 참조 하세요 [의 GetMenuBarInfo](http://msdn.microsoft.com/library/windows/desktop/ms647833)합니다.

*idItem*  
정보를 검색 하는 항목을 지정 합니다. 이 매개 변수가 0 이면 함수는 메뉴 자체에 대 한 정보를 검색 합니다. 이 매개 변수가 1 이면 함수는 고 메뉴에서 첫 번째 항목에 대 한 정보를 검색 합니다.

*pmbi*  
에 대 한 포인터를 [MENUBARINFO](http://msdn.microsoft.com/library/windows/desktop/ms647564) 정보를 수신 하는 구조입니다.
...
이 멤버 함수는 함수의 기능을 에뮬레이트합니다 [의 GetMenuBarInfo](http://msdn.microsoft.com/library/windows/desktop/ms647833)Windows SDK에 설명 된 대로 합니다.
...
컨트롤 그룹을 사용 하 여 만든 컨트롤을 사용 하 여 시작 합니다 [WS_GROUP](styles-used-by-mfc.md#window-styles) 스타일 및 WS_GROUP 스타일을 사용 하 여 생성 되지 않은 마지막 컨트롤에 끝납니다.

기본적으로 `GetNextDlgGroupItem` 멤버 함수는 그룹의 다음 컨트롤에 대 한 포인터를 반환 합니다. 하는 경우 *pWndCtl* 그룹의 첫 번째 컨트롤을 식별 하 고 *이전* 가 TRUE 인 `GetNextDlgGroupItem` 그룹의 마지막 컨트롤에 대 한 포인터를 반환 합니다.

> [!NOTE]
> MFC 창 없는 ActiveX 컨트롤, 표준 ActiveX 컨트롤 및 windows를 지원 하므로 충분 HWND만 하 여 컨트롤에 더 이상 참조 합니다. `COleControlSiteOrWnd` 개체에는 다음과 같이 창 있는 ActiveX 컨트롤, 창 없는 ActiveX 컨트롤 또는 창으로 개체를 식별 하는 정보가 포함 됩니다.

|컨트롤이 나 창 유형|식별 정보|
|----------------------------|-----------------------------|
|창 있는 ActiveX 컨트롤|HWND를 포함 하 고 연결 된 [COleControlSite](../../mfc/reference/colecontrolsite-class.md) 개체를 합니다. 합니다 `m_hWnd` 소속 `COleControlSiteOrWnd` 컨트롤의 HWND로 설정 되어 및 `m_pSite` 멤버 컨트롤을 가리키는 `COleControlSite`합니다.|
|창 없는 ActiveX 컨트롤|없는 HWND를 포함합니다. 합니다 `m_pSite` 소속 `COleControlSiteOrWnd` 컨트롤을 가리키는 `COleControlSite`, 및 `m_hWnd` 멤버는 NULL입니다.|
|표준 창|HWND를 포함 되어 있습니다. 합니다 `m_hWnd` 소속 `COleControlSiteOrWnd` 창의 HWND로 설정 되어 및 `m_pSite` 멤버는 NULL입니다.|

##  <a name="getnextdlgtabitem"></a>  CWnd::GetNextDlgTabItem

만든 첫 번째 컨트롤에 대 한 포인터를 검색 합니다 [WS_TABSTOP](styles-used-by-mfc.md#window-styles) 스타일 및는 이전 또는 지정된 된 컨트롤을 따릅니다.
...
멤버 함수는 성공 하는 경우 WS_TABSTOP 스타일이 적용 된 이전 또는 다음 컨트롤에 대 한 포인터입니다.

반환된 된 포인터 임시 수 있으며 나중에 사용할 저장 되어야 합니다.

에 대 한 자세한 내용은 `COleControlSiteOrWnd`를 참조 하세요 [CWnd::GetNextDlgGroupItem](#getnextdlggroupitem)합니다.
...
다음 (또는 이전) 식별 창 관리자의 목록 멤버 함수는 성공 하는 경우에 창입니다.

반환된 된 포인터 임시 수 있으며 나중에 사용할 저장 되어야 합니다.

### <a name="remarks"></a>설명

창 관리자의 목록에는 모든 최상위 창, 해당 연결 된 자식 창과 모든 자식 창의 자식 창에 대 한 항목이 들어 있습니다.

하는 경우 `CWnd` 이면 최상위 창에 다음 (또는 이전) 최상위 창;에 대 한 함수 검색 `CWnd` 함수 검색 된 다음 (또는 이전)에 대 한 자식 창의 자식 창에는 합니다.
...
함수가 성공할 경우 현재 클립보드가 창의 핸들을 열려면 그렇지 않으면 NULL입니다.
...
창 소유자가 없는 경우 부모 창 개체에 대 한 포인터는 기본적으로 반환 됩니다. 소유자 및 소유 된 간의 관계는 부모-자식 요소는 몇 가지 중요 한 측면에서 다른 것을 참고 합니다. 예를 들어, 부모를 사용 하 여 창은 해당 부모 창의 클라이언트 영역으로 제한 됩니다. 소유 windows 바탕 화면에서 모든 위치에서 그릴 수 있습니다.

이 함수의 소유권 개념 소유권 개념이 다릅니다 [GetWindow](http://msdn.microsoft.com/library/windows/desktop/ms633515)합니다.
...
반환 값 섹션을 참조 하세요 [GetParent](http://msdn.microsoft.com/library/windows/desktop/ms633510) Windows SDK에 있습니다.

### <a name="remarks"></a>설명

`GetParent` 함수 (있는 경우)의 바로 위 부모에 대 한 포인터를 반환 합니다. 반면에 [GetParentOwner](#getparentowner) 함수 (없습니다 WS_CHILD 스타일) 자식 창 없는 즉각적인 부모 또는 소유자 창에 대 한 포인터를 반환 합니다. 자식 창 내에서 자식 창이 있는지 `GetParent` 고 `GetParentOwner` 다른 결과 반환 합니다.
...
될 때까지 부모 체인을 검색 하는 멤버 함수는 [CFrameWnd](../../mfc/reference/cframewnd-class.md) (또는 파생 클래스) 개체를 찾을 수 있습니다.

##  <a name="getparentowner"></a>  CWnd::GetParentOwner

자식 창의 부모 창 또는 소유자 창에 대 한 포인터를 가져오려면이 함수를 호출 합니다.
...
`GetParentOwner` (없습니다 WS_CHILD 스타일) 자식 창 없는 즉각적인 부모 또는 소유자 창에 대 한 포인터를 반환 합니다. 현재 소유자 창을 사용 하 여 설정할 수 있습니다 [SetOwner](#setowner)합니다. 기본적으로 부모 창에 해당 소유자입니다.

반면에 [GetParent](#getparent) 함수 인지 자식 창을 여부 직계 부모에 대 한 포인터를 반환 합니다. 자식 창 내에서 자식 창이 있는지 `GetParent` 고 `GetParentOwner` 다른 결과 반환 합니다.

##  <a name="getproperty"></a>  CWnd::GetProperty

지정 된 ActiveX 컨트롤 속성을 가져오는이 멤버 함수 호출 *dwDispID*합니다.
...
`GetProperty` 통해 값을 반환 합니다 *pvProp*합니다.
...
반환 `m_hWnd`, 경우에 NULL 또는 합니다 **이** 포인터가 NULL입니다.
...
창의 창 핸들을 반환합니다. 없으면 NULL을 반환 합니다 `CWnd` 창에 연결 되지 NULL을 사용 하는 경우 또는 `CWnd` 포인터입니다.

### <a name="example"></a>예

  예를 참조 하세요 [CWnd::SubclassWindow](#subclasswindow)합니다.

##  <a name="getsafeowner"></a>  CWnd::GetSafeOwner

대화 상자를 사용 해야 하는 소유자 창 또는 기타 모달 창을 검색 하려면이 멤버 함수를 호출 합니다.
...
안전한 소유자가의 첫 번째 자식 아닌 부모 창을 *pParent*합니다. 하는 경우 *pParent* 가 null 인 경우 주 스레드의 창 (통해 검색 [AfxGetMainWnd](../../mfc/reference/application-information-and-management.md#afxgetmainwnd)) 소유자를 찾는 데 사용 됩니다.

> [!NOTE]
> 프레임 워크 자체는 소유자 지정 하지 않으면 대화 상자 및 속성 시트에 대 한 올바른 소유자 창 확인 하려면이 함수를 사용 합니다.

##  <a name="getscrollbarctrl"></a>  CWnd::GetScrollBarCtrl

지정 된 형제 스크롤 막대나 분할기에 대 한 포인터를 가져오려면이 함수를 호출 창입니다.
...
이 멤버 함수 WS_HSCROLL 또는 WS_VSCROLL 비트 창 만드는 동안 설정 된 경우 생성 하는 스크롤 막대에서 작동 하지 않습니다. `CWnd` 이 함수의 구현은 단순히 NULL을 반환 합니다. 파생 클래스와 같은 `CView`, 설명한 기능을 구현 합니다.
...
*idObject*  
메뉴 개체를 지정합니다. 가능한 값 목록을 참조 하세요 [GetScrollBarInfo](http://msdn.microsoft.com/library/windows/desktop/bb787581)합니다.

*psbi*  
에 대 한 포인터를 [SCROLLBARINFO](http://msdn.microsoft.com/library/windows/desktop/bb787535) 정보를 수신 하는 구조입니다.
...
이 멤버 함수는 함수의 기능을 에뮬레이트합니다 [GetScrollBarInfo](http://msdn.microsoft.com/library/windows/desktop/bb787581)Windows SDK에 설명 된 대로 합니다.

##  <a name="getscrollinfo"></a>  CWnd::GetScrollInfo

정보를 검색 하려면이 멤버 함수를 호출 하는 `SCROLLINFO` 스크롤 막대에 대 한 구조를 유지 관리 합니다.
...
MFC Windows 메시지 처리기 스크롤 막대 위치를 나타내는 [CWnd::OnHScroll](#onhscroll) 하 고 [CWnd::OnVScroll](#onvscroll)만 16 비트 위치 데이터를 제공 합니다. `GetScrollInfo` 및 `SetScrollInfo` 스크롤 막대 위치 데이터의 32 비트를 제공 합니다. 응용 프로그램을 호출할 수 있습니다 따라서 `GetScrollInfo` 중 하나를 처리 하는 동안 `CWnd::OnHScroll` 또는 `CWnd::OnVScroll` 32 비트 스크롤 막대 위치 데이터를 가져오려고 합니다.

##  <a name="getscrolllimit"></a>  CWnd::GetScrollLimit

최대 스크롤 막대의 스크롤 위치를 검색 하려면이 멤버 함수를 호출 합니다.
...
성공할 경우 스크롤 막대의 스크롤 상자의 현재 위치를 지정 합니다. 그렇지 않으면 0입니다.

### <a name="remarks"></a>설명

현재 위치가 현재 스크롤 범위에 의존 하는 상대 값을 보여 줍니다. 예를 들어, 스크롤 범위가 50에서 100 이며 스크롤 상자 막대를 중간에 현재 위치는 75입니다.
...
경우 `CWnd` 스크롤 막대 없는 경우 `GetScrollRange` 멤버 함수는 0에서 복사 *lpMinPos* 하 고 *lpMaxPos*.
...
창 스타일입니다. MFC에서 사용 된 창 스타일에 대 한 자세한 내용은 참조 하세요. [창 스타일](styles-used-by-mfc.md#window-styles)합니다.
...
경우에 컨트롤 메뉴의 복사본을 식별 *bRevert* 은 FALSE입니다. 하는 경우 *bRevert* 가 TRUE 이면 반환 값은 정의 되지 않습니다.

반환된 된 포인터 임시 수 있으며 나중에 사용할 저장 되어야 합니다.

### <a name="remarks"></a>설명

사용 하지 않는 모든 창 `GetSystemMenu` 메뉴 컨트롤의 자체 복사본을 확인 하기 위해 표준 컨트롤 메뉴를 받습니다.

반환 된 포인터를 `GetSystemMenu` 멤버 함수를 사용 하 여 사용할 수 있습니다는 [CMenu::AppendMenu](../../mfc/reference/cmenu-class.md#appendmenu), [CMenu::InsertMenu](../../mfc/reference/cmenu-class.md#insertmenu), 또는 [CMenu::ModifyMenu](../../mfc/reference/cmenu-class.md#modifymenu) 함수 컨트롤 메뉴를 변경 합니다.

처음에 컨트롤 메뉴 SC_CLOSE, SC_MOVE, SC_SIZE 등 다양 한 ID 값으로 식별 하는 항목을 포함 합니다. 제어 메뉴 항목 생성 [WM_SYSCOMMAND](#onsyscommand) 메시지입니다. 미리 정의 된 모든 컨트롤 메뉴 항목 id 0xF000 보다 큰 경우 응용 프로그램 컨트롤 메뉴 항목을 추가, F000 미만의 ID 번호가 사용 해야 합니다.

Windows 자동으로 수 없도록 할 수 항목 표준 제어 메뉴. `CWnd` 선택 영역 또는 사용할 수 없는 자체에 응답 하 여 수행할 수는 [WM_INITMENU](#oninitmenu) 모든 메뉴가 표시 되기 전에 전송 되는 메시지입니다.
...
*pti*  
에 대 한 포인터를 [TITLEBARINFO](http://msdn.microsoft.com/library/windows/desktop/ms632608) 정보를 수신 하는 구조입니다.

### <a name="remarks"></a>설명

이 멤버 함수는 함수의 기능을 에뮬레이트합니다 [GetTitleBarInfo](http://msdn.microsoft.com/library/windows/desktop/ms633513)Windows SDK에 설명 된 대로 합니다.
...
하는 경우 `CWnd` 에 연결 된 창이 없습니다 없거나 최상위 부모는 [CFrameWnd](../../mfc/reference/cframewnd-class.md)-파생 개체를이 함수는 NULL을 반환 합니다.
...
최상위 창을 식별합니다. 반환된 된 포인터 임시 수 있으며 나중에 사용할 저장 되어야 합니다.

### <a name="remarks"></a>설명

최상위 데스크톱의 자식 창이입니다. 경우 `CWnd` 에 연결 된 창이 없습니다이 함수는 NULL을 반환 합니다.
...
`GetTopLevelParent` 비슷합니다 [GetTopLevelFrame](#gettoplevelframe) 하 고 [GetTopLevelOwner](#gettoplevelowner)하지만 현재 소유자 창으로 설정 된 값을 무시 합니다.

##  <a name="gettopwindow"></a>  CWnd::GetTopWindow

속한 최상위 자식 창에 대 한 검색 `CWnd`합니다.
...
최상위 자식 창을 식별을 `CWnd` 자식 창의 연결된 목록입니다. 자식 창이 없으면 값은 NULL입니다.

반환된 된 포인터 임시 수 있으며 나중에 사용할 저장 되어야 합니다.

### <a name="remarks"></a>설명

경우 `CWnd` 자식이 없으면,이 함수는 NULL을 반환 합니다.
...
업데이트 영역의 상태를 지정합니다. 값 0이 아닌 업데이트 영역의; 비어 있지 않습니다. 그렇지 않으면 0입니다.

경우는 *lpRect* 매개 변수가 NULL로 설정 된, 반환 값은 업데이트 영역을 존재 하면 0이 아니고 그렇지 않으면 0입니다.

### <a name="remarks"></a>설명

하는 경우 `CWnd` 만들어진 CS_OWNDC 스타일 및 매핑을 사용 하 여 모드가 MM_TEXT는 `GetUpdateRect` 멤버 함수는 논리적 좌표에서 사각형을 제공 합니다. 그렇지 않으면 `GetUpdateRect` 클라이언트 사각형 좌표를 제공 합니다. 업데이트 영역 없음 있으면 `GetUpdateRect` 비워 (모든 좌표를 0으로 설정) 사각형을 설정 합니다.

합니다 *bErase* 매개 변수를 지정 하는지 여부를 `GetUpdateRect` 업데이트 영역 배경의 지우기 해야 합니다. 하는 경우 *bErase* 가 TRUE이 고 업데이트 영역에 비어 있지, 백그라운드 지워집니다. 백그라운드 지울 `GetUpdateRect` 보냅니다 합니다 [WM_ERASEBKGND](#onerasebkgnd) 메시지입니다.

업데이트 사각형을 검색 하 여는 [BeginPaint](#beginpaint) 멤버 함수는 동일 하 여 검색 된 `GetUpdateRect` 멤버 함수입니다.

합니다 `BeginPaint` 멤버 함수 자동으로 유효성을 검사, 업데이트 영역을 호출 하므로 `GetUpdateRect` 에 대 한 호출 바로 다음 `BeginPaint` 빈 업데이트 영역을 검색 합니다.

##  <a name="getupdatergn"></a>  CWnd::GetUpdateRgn

로 식별 되는 지역으로 업데이트 영역을 가져옵니다 *pRgn*합니다.
...
결과 영역의 유형을 나타내는 short 정수 플래그를 지정 합니다. 값 중 하나를 사용할 수 있습니다.
...
합니다 [BeginPaint](#beginpaint) 멤버 함수 자동으로 유효성을 검사, 업데이트 영역을 호출 하므로 `GetUpdateRgn` 에 대 한 호출 바로 다음 `BeginPaint` 빈 업데이트 영역을 검색 합니다.

##  <a name="getwindow"></a>  CWnd::GetWindow

요청 창에 없는 경우 NULL 포인터를 반환 합니다.
...
- GW_HWNDFIRST 경우 `CWnd` 자식 창, 첫 번째 형제 창을 반환 합니다. 이 고, 그렇지는 목록의 첫 번째 최상위 창을 반환합니다.

- GW_HWNDLAST 경우 `CWnd` 자식 창, 마지막 형제 창을 반환 합니다. 그렇지 않으면 목록에서 마지막 최상위 창을 반환합니다.

- GW_HWNDNEXT 창 관리자의 목록에서 다음을 반환합니다.

- GW_HWNDPREV 창 관리자의 목록에서 이전 창을 반환합니다.

- GW_OWNER 하 게 식별 하는 `CWnd` 소유자입니다.
...
창과 사용 하 여 연결 된 경우 도움말 컨텍스트 식별자를 검색 하려면이 멤버 함수를 호출 합니다.
...
함수가 성공할 경우 지정된 된 창에 대 한 디스플레이 컨텍스트를 식별 합니다. 그렇지 않으면 NULL입니다.

반환된 된 포인터 임시 수 있으며 나중에 사용할 저장 되어야 합니다. [ReleaseDC](#releasedc) 성공한 각 호출에 대 한 한 번 호출 해야 `GetWindowDC`합니다.

### <a name="remarks"></a>설명

디스플레이 컨텍스트 창에서 아무 곳 이나 그리기 허용 `CWnd`컨텍스트의 원점은 왼쪽 위 모퉁이의 때문 `CWnd` 클라이언트 영역 대신 합니다.

기본 특성 컨텍스트를 검색할 때마다 디스플레이 컨텍스트에 할당 됩니다. 이전의 특성이 손실 됩니다.

`GetWindowDC` 내에서 특별 한 그리기 효과에 사용 되는 것은 `CWnd` 비클라이언트 영역입니다. 모든 창의 비클라이언트 영역에서 그리기 권장 되지 않습니다.

합니다 [GetSystemMetrics](http://msdn.microsoft.com/library/windows/desktop/ms724385) 비클라이언트 영역의 캡션 표시줄, 메뉴 및 스크롤 막대와 같은 다양 한 파트의 크기를 검색할 Windows 함수를 사용할 수 있습니다.
...
*pwi*  
에 대 한 포인터를 [WINDOWINFO](http://msdn.microsoft.com/library/windows/desktop/ms632610) 구조입니다.

### <a name="remarks"></a>설명

이 멤버 함수는 함수의 기능을 에뮬레이트합니다 [GetWindowInfo](http://msdn.microsoft.com/library/windows/desktop/ms633516)Windows SDK에 설명 된 대로 합니다.
...
경계 사각형의 크기를 복사 합니다 `CWnd` 가리키는 구조에는 개체가 *lpRect*합니다.
...
크기를 화면 좌표로 표시 화면의 왼쪽 위 모퉁이 기준으로 제공 됩니다. 캡션, 테두리 및 스크롤 막대의 차원이 있는 경우 포함 됩니다.

##  <a name="getwindowrgn"></a>  CWnd::GetWindowRgn

창의 창 영역을 가져오려면이 함수를 호출 합니다.
...
반환 값 함수를 가져오는 영역의 형식을 지정 합니다. 다음 값 중 하나일 수 있습니다.

- 지역 NULLREGION 비어 있습니다.

- SIMPLEREGION 영역에는 단일 사각형입니다.

- COMPLEXREGION 지역에 둘 이상의 사각형입니다.

- 오류는 오류가 발생 했습니다. 영역을 받지 않습니다.

### <a name="remarks"></a>설명

창 영역을 운영 체제에서 그리기를 허용 하는 위치 창 내에서 결정 합니다. 운영 체제 창의 창 영역 외부에 있는 일부가 표시 되지 않습니다.

창의 창 영역 좌표가 되지 않은 창의 클라이언트 영역 창의 왼쪽 위 모퉁이 기준으로 합니다.

창의 창 영역을 설정 하려면 호출 [CWnd::SetWindowRgn](#setwindowrgn)합니다.

##  <a name="getwindowtext"></a>  CWnd::GetWindowText

복사 합니다 `CWnd` 에서 가리키는 버퍼에 제목 (있으면) 캡션 *lpszStringBuf* 또는 대상 문자열에 *rString*합니다.
...
Null 종결 문자를 포함 하지 않습니다 복사한 문자열의 문자에서 길이 지정 합니다. 경우에 0을입니다 `CWnd` 캡션을 포함 하지 캡션이 비어 있는 경우 또는 합니다.

### <a name="remarks"></a>설명

경우는 `CWnd` 개체가 컨트롤은 `GetWindowText` 캡션을 복사 하는 대신 컨트롤 내에서 텍스트를 복사 하는 멤버 함수입니다.

이 멤버 함수는 [WM_GETTEXT](http://msdn.microsoft.com/library/windows/desktop/ms632627) 보낼 메시지를 `CWnd` 개체입니다.

### <a name="example"></a>예

  예를 참조 하세요 [CWnd::SetWindowText](#setwindowtext)합니다.

##  <a name="getwindowtextlength"></a>  CWnd::GetWindowTextLength

길이 반환 합니다 `CWnd` 캡션 제목 개체입니다.
...
모든 null 종료 문자를 포함 하지 않고 문자를 텍스트 길이 지정 합니다. 이러한 텍스트가 없으면 값은 0입니다.

### <a name="remarks"></a>설명

경우 `CWnd` 컨트롤을 `GetWindowTextLength` 캡션 대신 컨트롤 내에서 텍스트의 길이 반환 하는 멤버 함수입니다.

이 멤버 함수는 [WM_GETTEXTLENGTH](http://msdn.microsoft.com/library/windows/desktop/ms632628) 보낼 메시지를 `CWnd` 개체입니다.
...
사용 하 여 다시 표시할 수 없습니다 캐럿을 더 이상 볼 수 있지만 합니다 [ShowCaret](#showcaret) 멤버 함수입니다. 캐럿을 숨기기 현재 모양을 제거 하지 않습니다.

숨기기 누적 됩니다. 하는 경우 `HideCaret` 행에서 다섯 번 호출 했습니다는 `ShowCaret` 멤버 함수가 먼저 호출 해야 5 번 캐럿이 표시 됩니다.
...
- MF_BYPOSITION 해석 *nIDHiliteItem* 메뉴 항목의 0부터 시작 오프셋입니다.

- MF_HILITE 항목을 강조 표시 합니다. 이 값은 지정 하지 않으면 강조 표시가 항목에서 제거 됩니다.

- MF_UNHILITE 항목에서 강조 표시를 제거합니다.

### <a name="return-value"></a>반환 값

메뉴 항목을 강조 표시 된 여부를 지정 합니다. 0이 아닌 항목을 다음과 같이 강조 했습니다. 그렇지 않으면 0입니다.

### <a name="remarks"></a>설명

이 멤버 함수를 사용 하 여만 MF_HILITE 및 MF_UNHILITE 플래그를 사용할 수 있습니다. 와 함께 사용할 수는 [CMenu::ModifyMenu](../../mfc/reference/cmenu-class.md#modifymenu) 멤버 함수입니다.
...
참조 [CWinApp::HtmlHelp](../../mfc/reference/cwinapp-class.md#htmlhelp) 자세한 내용은 합니다.
...
클라이언트 영역 그리기 때 표시 된 다음 [WM_PAINT](#onpaint) 메시지가 발생 합니다. 영역도 유효성을 검사할 수 WM_PAINT 메시지에서 발생 하기 전에 합니다 [ValidateRect](#validaterect) 하거나 [ValidateRgn](#validatergn) 멤버 함수입니다.

합니다 *bErase* 매개 변수 업데이트 영역 내에서 백그라운드 업데이트 영역에서 처리 될 때 지울 인지 여부를 지정 합니다. 경우 *bErase* 가 TRUE 인 백그라운드 때 지워집니다 합니다 [BeginPaint](#beginpaint) 경우 멤버 함수를 호출할 *bErase* 은 백그라운드 그대로 FALSE입니다. 하는 경우 *bErase* 특정된 부분에서 뿐만 아니라 전체 지역의 백그라운드 업데이트 영역의 일부가 지워집니다 TRUE입니다.

Windows 보냅니다를 [WM_PAINT](#onpaint) 때마다 메시지를 `CWnd` 업데이트 영역을 비어 있지 않고 해당 창에 대 한 응용 프로그램 큐에 다른 메시지가 없습니다.

### <a name="example"></a>예

  예를 참조 하세요 [CWnd::UpdateWindow](#updatewindow)합니다.

##  <a name="invalidaterect"></a>  CWnd::InvalidateRect

해당 사각형을 추가 하 여 지정된 된 사각형 내에서 클라이언트 영역을 무효화 합니다 `CWnd` 업데이트 영역입니다.
...
업데이트 영역에서 다른 모든 영역 함께 무효화 사각형을 그리는 경우 표시 되어 다음 [WM_PAINT](#onpaint) 전송 됩니다. 지역, 다음 WM_PAINT 호출이 발생 하는 경우 처리 될 때까지 또는 지역에서 유효성을 검사 될 때까지 업데이트 영역에서 무효화 된 영역을 누적 합니다 [ValidateRect](#validaterect) 하거나 [ValidateRgn](#validatergn) 멤버 함수입니다.

합니다 *bErase* 매개 변수 업데이트 영역 내에서 백그라운드 업데이트 영역에서 처리 될 때 지울 인지 여부를 지정 합니다. 경우 *bErase* 가 TRUE 인 백그라운드 때 지워집니다 합니다 [BeginPaint](#beginpaint) 경우 멤버 함수를 호출할 *bErase* 은 백그라운드 그대로 FALSE입니다. 하는 경우 *bErase* 특정된 부분에서 뿐 아니라 전체 지역의 백그라운드 업데이트 영역의 일부가 지워진는 TRUE입니다.
...
업데이트 지역의 다른 모든 영역에 함께 무효화 된 영역을 그리는 경우 표시 됩니다는 [WM_PAINT](#onpaint) 메시지를 다음으로 보내집니다. 지역 다음 WM_PAINT 메시지를 보낼 때 처리 될 때까지 또는 지역에서 유효성을 검사 될 때까지 업데이트 영역에서 무효화 된 영역을 누적 합니다 [ValidateRect](#validaterect) 하거나 [ValidateRgn](#validatergn) 멤버 함수입니다.

합니다 *bErase* 매개 변수 업데이트 영역 내에서 백그라운드 업데이트 영역에서 처리 될 때 지울 인지 여부를 지정 합니다. 경우 *bErase* 가 TRUE 인 백그라운드 때 지워집니다 합니다 [BeginPaint](#beginpaint) 경우 멤버 함수를 호출할 *bErase* 은 백그라운드 그대로 FALSE입니다. 하는 경우 *bErase* 특정된 부분에서 뿐만 아니라 전체 지역의 백그라운드 업데이트 영역의 일부가 지워집니다 TRUE입니다.

Windows 보냅니다를 [WM_PAINT](#onpaint) 때마다 메시지를 `CWnd` 업데이트 영역을 비어 있지 않고 해당 창에 대 한 응용 프로그램 큐에 다른 메시지가 없습니다.

지정된 된 영역 해야 이전에 만든 지역 함수 중 하나가 합니다.

##  <a name="invokehelper"></a>  CWnd::InvokeHelper

이 ActiveX 컨트롤 메서드나 속성으로 지정 된 호출 함수를 호출 *dwDispID*, 지정 된 컨텍스트에서 *wFlags*합니다.
...
창으로 지정 되었는지 여부를 나타냅니다 *pWnd* 자식 창 또는 다른 직계 하위 `CWnd`합니다.
...
함수의 결과 지정 합니다. 값이 창으로 식별 하는 경우 0이 아닌 *pWnd* 의 자식 창이 `CWnd`그렇지 않으면 0입니다.

### <a name="remarks"></a>설명

자식 창에는 직접 하위 항목에 대 `CWnd` 경우는 `CWnd` 개체가 부모 창의 자식 창에는 원래 팝업 창에서 이어지는 체인에 있습니다.
...
지정된 된 메시지는 모덜리스 대화 상자에 대 한 할지 결정 하는이 멤버 함수를 호출 합니다. 이 경우이 함수는 메시지를 처리 합니다.
...
멤버 함수는 지정된 된 메시지를 처리 하는지 여부를 지정 합니다. 0이 아닌 메시지에 처리 된 경우 그렇지 않으면 0입니다. 반환이 0 이면 호출 된 [cwnd:: Pretranslatemessage](#pretranslatemessage) 메시지를 처리 하는 기본 클래스의 멤버 함수입니다. 재정의 `CWnd::PreTranslateMessage` 멤버 함수 코드를 다음과 같이 표시 됩니다.
...
처리 하는 메시지 전달 하면 안 `IsDialogMessage` 에 [TranslateMessage](http://msdn.microsoft.com/library/windows/desktop/ms644955) 또는 [DispatchMessage](http://msdn.microsoft.com/library/windows/desktop/ms644934) Windows 함수를 이미 처리 되었습니다.
...
지정된 된 컨트롤을 선택한 경우에 0이 아닌 선택 하지 않은 경우 0입니다. 만 라디오 단추 및 확인란을 확인할 수 있습니다. 세 가지 상태 단추에 대 한 단추 확정적이 지 없는 경우 반환 값 2를 수 있습니다. 이 멤버 함수는 누름 단추에 대 한 0을 반환합니다.
...
TRUE 이면 `CWnd` 가 터치 지원, 그렇지 않으면 FALSE입니다.

### <a name="remarks"></a>설명

##  <a name="iswindowenabled"></a>  CWnd::IsWindowEnabled

지정 여부를 `CWnd` 마우스 및 키보드 입력에 대 한 사용 가능 합니다.
...
0이 아닌 경우 `CWnd` 표시 됩니다 (에 [WS_VISIBLE](styles-used-by-mfc.md#window-styles) 스타일 비트가 설정 되며 부모 창 표시). WS_VISIBLE 스타일 비트의 상태를 반영 하는 반환 값을 반환 값 수 있습니다 0이 아닌 경우에 `CWnd` 를 다른 windows 완전히 가립니다.

### <a name="remarks"></a>설명

창은 WS_VISIBLE 스타일 비트를 나타내는 표시 여부 상태를가지고 있습니다. 호출 하 여이 스타일 비트가 설정 된 경우는 [ShowWindow](#showwindow) 멤버 함수는 창 표시 되 고 창에 스타일 비트 집합으로 후속 그리기 창에 표시 됩니다.

창을 다른 창에서 검사 하는 또는 해당 부모 창에서 잘린 경우 모든 그리기 WS_VISIBLE 스타일이 적용 된 창에 표시 되지 않습니다.
...
으로 식별 되는 타이머 이벤트를 종료 *nIDEvent* 대 한 이전 호출에서 `SetTimer`합니다.
...
함수의 결과 지정 합니다. 값이 이벤트가 종료 된 경우 0이 아닌 경우 경우에 0을입니다는 `KillTimer` 멤버 함수는 지정 된 타이머 이벤트를 찾지 못했습니다.

### <a name="remarks"></a>설명

보류 중인 [WM_TIMER](#ontimer) 타이머와 연결 된 메시지는 메시지 큐에서 제거 되지 않습니다.

### <a name="example"></a>예

  예를 참조 하세요 [CWnd::SetTimer](#settimer)합니다.
...
지정된 된 창에서 그리기는 사용 하지 않도록 설정 합니다.
...
함수가 성공하는 경우 0이 아닙니다. 오류가 발생 하거나 경우 0입니다는 `LockWindowUpdate` 잠금 다른 창에 함수가 사용 되었습니다.
...
경우는 잠긴된 창 (또는 모든 잠긴된 자식 창)를 사용 하 여 응용 프로그램 호출을 [GetDC,](http://msdn.microsoft.com/library/windows/desktop/dd144871) [GetDCEx를](http://msdn.microsoft.com/library/windows/desktop/dd144873) 또는 [BeginPaint](http://msdn.microsoft.com/library/windows/desktop/dd183362) Windows 함수 호출된 된 함수가 반환 장치 표시 된 지역은 빈 컨텍스트. 이 응용 프로그램 창을 호출 하 여 계정의 잠금을 해제할 때까지 진행 된 `UnlockWindowUpdate` 멤버 함수입니다.

창이 업데이트 잠겨 하는 동안 시스템 잠긴된 창과 연결 된 장치 컨텍스트와 모든 그리기 작업의 경계 사각형을를 추적 합니다. 이 경계 사각형 잠긴된 창 및 최종를 강제로 해당 자식 창에서 무효화 되기 그리기를 다시 사용 하도록 설정 하는 경우 [WM_PAINT](http://msdn.microsoft.com/library/windows/desktop/dd145213) 화면을 업데이트 하는 메시지입니다. 없는 그리기 창이 업데이트 된 잠겨 있는 동안 수행 되 면 영역이 무효화 됩니다.

`LockWindowUpdate` 멤버 함수는 하지 않도록 지정된 된 창 보이지 않는 하 고를 지우지 않습니다 합니다 [WS_VISIBLE](styles-used-by-mfc.md#window-styles) 스타일 비트입니다.

##  <a name="m_hwnd"></a>  CWnd::m_hWnd

이 연결 된 Windows 창의 핸들이 `CWnd`합니다.
...
만들고 응용 프로그램에서 제공 하는 메시지는 캡션 및 미리 정의 된 아이콘 및 누름 단추에 설명 된 조합을 포함 하는 창을 표시 합니다 [메시지 상자 스타일](../../mfc/reference/styles-used-by-mfc.md#message-box-styles) 목록입니다.
...
이 메서드는 [MessageBox](http://msdn.microsoft.com/library/windows/desktop/ms645505) Windows SDK에 정의 된 대로 작동 합니다. 이 메서드는이 함수를 호출한 결과 반환 합니다.

### <a name="remarks"></a>설명

전역 함수를 사용 하 여 [AfxMessageBox](../../mfc/reference/cstring-formatting-and-message-box-display.md#afxmessagebox) 응용 프로그램에서 메시지 상자를 구현 하려면이 멤버 함수를 대신 합니다.

다음 메시지 상자에 사용할 수 있는 다양 한 시스템 아이콘을 보여 줍니다.
...
스타일; 성공적으로 수정 하는 경우 0이 아닌 값 그렇지 않을 경우 0입니다.

### <a name="remarks"></a>설명

비트 OR를 사용 하 여 추가 하거나 제거 하는 스타일을 결합할 수 있습니다 (&#124;) 연산자. 항목을 참조 하세요 [창 스타일](http://msdn.microsoft.com/library/windows/desktop/ms632600) 하 고 [CreateWindow](http://msdn.microsoft.com/library/windows/desktop/ms632679) 사용할 창 스타일에 대 한 내용은 Windows SDK의 합니다.

하는 경우 *nFlags* 이 값은 0 `ModifyStyle` Windows API 함수를 호출 [SetWindowPos](http://msdn.microsoft.com/library/windows/desktop/ms633545) 결합 하 여 창을 다시 그립니다 *nFlags* 다음 4 개를 사용 하 여 미리 설정 플래그:
...
창을 수정의 확장 스타일을 참조 하십시오 [ModifyStyleEx](#modifystyleex)합니다.

> [!NOTE]
> 특정 컨트롤 (ES_READONLY 스타일 예를 들어 편집 컨트롤에서)의 일부 스타일 `ModifyStyle` 변경할 수 없습니다 제대로 스타일 컨트롤 내부 특수 한 처리를 수행 해야 할 수도 있으므로. 이러한 경우에 스타일을 변경 하려면 해당 메시지를 사용할 수 있습니다 (언급 한 예에서 EM_SETREADONLY) 됩니다.
...
비트 OR를 사용 하 여 추가 하거나 제거 하는 스타일을 결합할 수 있습니다 (&#124;) 연산자. 항목을 참조 하세요 [확장 창 스타일](../../mfc/reference/styles-used-by-mfc.md#extended-window-styles) 이 가이드의 및 [CreateWindowEx](http://msdn.microsoft.com/library/windows/desktop/ms632680) 사용 가능한 항목에 대 한 내용은 Windows SDK의 확장 스타일

하는 경우 *nFlags* 이 값은 0 `ModifyStyleEx` Windows API 함수를 호출 [SetWindowPos](http://msdn.microsoft.com/library/windows/desktop/ms633545) 결합 하 여 창을 다시 그립니다 *nFlags* 다음 4 개를 사용 하 여 미리 설정 플래그:
...
최상위 수준에 대 한 `CWnd` 개체를 *x* 하 고 *y* 화면의 왼쪽 위 모퉁이 기준으로 매개 변수는 합니다. 자식에 대해 `CWnd` 개체에서는 부모 창의 클라이언트 영역의 왼쪽 위 모퉁이 기준으로 합니다.

합니다 `MoveWindow` 보냅니다 함수는 [WM_GETMINMAXINFO](#ongetminmaxinfo) 메시지입니다. 이 메시지를 처리 하면 `CWnd` 가장 크고 가장 작은 windows에 대 한 기본값을 수정할 수 있습니다. 경우에 매개 변수는 `MoveWindow` 이러한 값을 초과 하는 멤버 함수, 최소값 또는 최대값 WM_GETMINMAXINFO 처리기 값을 기준으로 값을 바꿀 수 있습니다.

### <a name="example"></a>예

  예를 참조 하세요 [CWnd::ClientToScreen](#clienttoscreen)합니다.

##  <a name="notifywinevent"></a>  CWnd::NotifyWinEvent

미리 정의된 이벤트가 발생한 시스템에 신호를 보냅니다. 모든 클라이언트 응용 프로그램 이벤트에 대 한 후크 함수를 등록 하는 경우 시스템이 클라이언트의 후크 함수를 호출 합니다.
...
*event*  
발생 한 이벤트를 지정 합니다. 이 값 중 하나 여야 합니다 [이벤트 상수](http://msdn.microsoft.com/library/windows/desktop/dd318066)합니다.

*idObjectType*  
이벤트를 생성 하는 개체의 종류를 식별 합니다. 이 값을 사용 하면 미리 정의 된 중 하나인 [식별자 개체](http://msdn.microsoft.com/library/windows/desktop/dd373606) 또는 사용자 지정 개체 ID 값입니다.
...
이 멤버 함수는 함수의 기능을 에뮬레이트합니다 [NotifyWinEvent](http://msdn.microsoft.com/library/windows/desktop/dd373603)Windows SDK에 설명 된 대로 합니다.

##  <a name="onactivate"></a>  CWnd::OnActivate

프레임 워크가이 멤버 함수를 호출 하면를 `CWnd` 개체가 활성화 또는 비활성화 합니다.
...
- WA_ACTIVE 창을 마우스 이외의 다른 몇 가지 메서드를 통해 활성화 되 고 (예: 창을 선택 하는 키보드 인터페이스 사용)를 클릭 합니다.
...
경우는 `CWnd` 마우스 클릭을 사용 하 여 개체 활성화를 받게 됩니다는 [OnMouseActivate](#onmouseactivate) 멤버 함수 호출 합니다.
...
비활성화 하 고 작업의 모든 최상위 창 및 활성화 되 고 작업의 모든 최상위 창에이 멤버 함수를 호출 하는 프레임 워크입니다.
...
프레임 워크가 앰비언트 속성 값을 가져오도록 OLE 컨트롤을 포함 하는 창에서이 멤버 함수를 호출 합니다.
...
앰비언트 속성 지원 되 면 TRUE입니다. 그렇지 않은 경우 FALSE입니다.

### <a name="remarks"></a>설명

해당 컨트롤을 OLE 컨트롤 컨테이너에 의해 반환 된 앰비언트 속성 값의 기본 변경 하려면이 함수를 재정의 합니다. 재정의 함수에서 처리 되지 않은 모든 앰비언트 속성 요청이 기본 클래스 구현에 전달 해야 합니다.

##  <a name="onappcommand"></a>  CWnd::OnAppCommand

프레임 워크는 사용자가 응용 프로그램 명령 이벤트를 생성 하는 경우이 멤버 함수를 호출 합니다. 이러한 이벤트에는 사용자가 응용 프로그램 명령 단추 또는 형식 응용 프로그램 명령 키를 때 발생 합니다.
...
|[in] *nCmd*|응용 프로그램 명령을 나타냅니다. 에서 가능한 값 목록은 아래에 있는 명령의 참조 하세요.는 *cmd* 섹션을 *lParam* 의 매개 변수 [WM_APPCOMMAND](http://msdn.microsoft.com/library/windows/desktop/ms646275).|
|[in] *nDevice*|입력된 이벤트를 생성 하는 입력된 장치입니다. 가능한 값 목록을 아래에서 장치를 참조 하세요.는 *uDevice* 섹션을 *lParam* 의 매개 변수 [WM_APPCOMMAND](http://msdn.microsoft.com/library/windows/desktop/ms646275).|
|[in] *nKey*|CTRL 키 또는 마우스 왼쪽된 단추와 같은 축소 된 모든 가상 키를 나타냅니다. 에서 가능한 값 목록은 아래에 키를 참조 하세요.를 *dwKeys* 섹션을 *lParam* 의 매개 변수 [WM_APPCOMMAND](http://msdn.microsoft.com/library/windows/desktop/ms646275)합니다. 자세한 내용은 참조에서 "메시지 매개 변수" 부제목 [마우스 입력에 대 한](http://msdn.microsoft.com/library/windows/desktop/ms645601)합니다.|

### <a name="remarks"></a>설명

이 메서드를 수신 합니다 [WM_APPCOMMAND](http://msdn.microsoft.com/library/windows/desktop/ms646275) Windows SDK에 설명 된 알림.
...
클립보드 데이터를 포함 하는 경우이 멤버 함수는 CF_OWNERDISPLAY 처리 프레임 워크 호출 (즉, 표시 하는 경우 클립보드 소유자가 클립보드 내용을) 서식을 지정 합니다.
...
이 멤버 함수를 재정의 하 고 CF_OWNERDISPLAY 형식의 이름을 지정 하는 바이트의 최대 수를 초과 하지 않는 지정된 된 버퍼에 복사 합니다.
...
경우는 `CWnd` 개체에 포커스가 있으면 해당 `OnCancelMode` 대화 상자나 메시지 상자를 표시할 때 멤버 함수를 호출 합니다. 이렇게 하면는 `CWnd` 를 같은 마우스 캡처 모드를 취소할 수 있습니다.

기본 구현을 호출 하 여 응답 합니다 [ReleaseCapture](http://msdn.microsoft.com/library/windows/desktop/ms646261) Windows 함수입니다. 다른 모드를 처리 하려면 파생된 클래스에서이 멤버 함수를 재정의 합니다.

##  <a name="oncapturechanged"></a>  CWnd::OnCaptureChanged

프레임 워크가 마우스 캡처가 손실 되는 창에 알리기 위해이 멤버 함수를 호출 합니다.
...
창을 호출 하는 경우에이 메시지를 받으면 [ReleaseCapture](http://msdn.microsoft.com/library/windows/desktop/ms646261) 자체입니다. 응용 프로그램에 대 한 응답이 메시지에 마우스 캡처를 설정 하지 않아야 합니다. 이 메시지를 받으면 창 다시 그릴지를 새 마우스 캡처 상태를 반영 하기 위해 필요한 경우.

정보에 대 한 Windows SDK에서 참조 된 `ReleaseCapture` Windows 함수.
...
프레임 워크는 창이 체인에서 제거 됨을 알려주는 클립보드 뷰어 체인의 각 창에 대 한이 멤버 함수를 호출 합니다.
...
각 `CWnd` 수신 하는 개체를 `OnChangeCbChain` 호출을 사용 해야 합니다 [SendMessage](http://msdn.microsoft.com/library/windows/desktop/ms644950) 보내도록 Windows 함수를 [WM_CHANGECBCHAIN](http://msdn.microsoft.com/library/windows/desktop/ms649019) 클립보드 뷰어에서 다음 창에 메시지를 체인 (에서 반환한 핸들 `SetClipboardViewer`). 하는 경우 *hWndRemove* 에서 지정한 창과 체인의 다음 창이 *hWndAfter* 다음 창 되며 클립보드 메시지에 전달 됩니다.
...
- UIS_INITIALIZE UI 상태 요소 (지정 된 *nUIElement*) 마지막 입력된 이벤트에 따라 변경 해야 합니다. 자세한 내용은 참조는 **주의** 부분 [WM_CHANGEUISTATE](http://msdn.microsoft.com/library/windows/desktop/ms646342).
...
이 멤버 함수는의 기능을 에뮬레이션 합니다 [WM_CHANGEUISTATE](http://msdn.microsoft.com/library/windows/desktop/ms646342) Windows SDK에 설명 된 대로 메시지입니다.

##  <a name="onchar"></a>  CWnd::OnChar

프레임 워크는 키가 아닌 문자로 변환 하는 경우이 멤버 함수를 호출 합니다.
...
|값|의미|
|-----------|-------------|
|0-15|반복 횟수를 지정합니다. 값을 사용 하면 사용자 키를 누른 키 입력을 반복 하는 횟수입니다.|
|16-23|검색 코드를 지정합니다. 원래 장비 제조업체 (OEM) 되는 값|
|24|키를 향상 된 101 또는 102 키 키보드에 표시 되는 오른쪽 ALT 및 CTRL 키와 같은 확장 된 키를 지정 합니다. 확장 된 키를; 이면 값이 1 그렇지 않으면 0입니다.|
|25-28|Windows에서 내부적으로 사용 합니다.|
|29|상황에 맞는 코드를 지정합니다. 값이 1 ALT 키를 누르고 키가 눌러져 있습니다. 그렇지 않으면 값은 0입니다.|
|30|이전 키 상태를 지정합니다. 키 메시지를 보낼지 또는 0은 해당 키가 놓여 전에 다운 된 경우 값은 1입니다.|
|31|전환 상태를 지정합니다. 값 1 경우 해제 되는 키 또는 키를 누른 경우 0입니다.|

### <a name="remarks"></a>설명

하기 전에이 함수를 호출 합니다 [OnKeyUp](#onkeyup) 멤버 함수 후 합니다 [OnKeyDown](#onkeydown) 구성원 함수가 호출 됩니다. `OnChar` 키보드 키를 누르거나 놓을의 값을 포함 합니다.

없기 때문에 반드시 일대일로 대응 누른 키 사이 및 `OnChar` 호출에서 정보를 생성 *nFlags* 아닙니다 일반적으로 응용 프로그램에 유용 합니다. 에 있는 정보 *nFlags* 에 대 한 최근 호출에만 적용 됩니다는 `OnKeyUp` 멤버 함수 또는 `OnKeyDown` 멤버 함수에 대 한 호출 앞에 오는 `OnChar`합니다.

IBM 고급 101 및 102 키 키보드에 대 한 향상 된 키가 오른쪽 alt 키와 키보드;의 주요 섹션에 있는 오른쪽 CTRL 키 기능, DEL, 홈, END, PAGE UP, PAGE DOWN 및 왼쪽의 숫자 키패드로;의 클러스터에서 화살표 키 슬래시 (/) 및 숫자 키패드에서 ENTER 키입니다. 다른 일부 키보드에 있는 확장 된 키 비트를 지원할 수 있습니다 *nFlags*합니다.
...
때 사용 하 여 목록 상자를 호출 합니다 [LBS_WANTKEYBOARDINPUT](../../mfc/reference/styles-used-by-mfc.md#list-box-styles) 스타일 보냅니다 소유자를 [WM_CHARTOITEM](http://msdn.microsoft.com/library/windows/desktop/bb761358) 대 한 응답으로 메시지를 [WM_CHAR](#onchar) 메시지.
...
프레임 워크는 응용 프로그램 호출에 대 한 응답으로 수행 하는 동작을 지정 하려면이 멤버 함수를 호출 합니다. -2의 반환 값에는 응용 프로그램 항목을 선택 하는 모든 측면을 처리 하 고 목록 상자에 추가 작업이 없으므로 나타냅니다. 반환 값이-1 목록 상자 키 입력에 대 한 응답의 기본 작업을 수행 해야 나타냅니다. 반환 값 0 이상인 목록 상자에서 항목의 0부터 시작 인덱스를 지정 하 고 목록 상자에서 지정된 된 항목 키에 대 한 기본 작업을 수행 해야 나타냅니다.
...
경우는 `CWnd` 개체는 여러 문서 MDI (인터페이스) 자식 창 `OnChildActivate` 창의 제목 표시줄을 마우스 오른쪽 단추로 클릭할 때 프레임 워크에서 호출 되거나 창이 활성화 되 면 이동 또는 크기 조정 합니다.
...
이 멤버 함수는이 창에 적용 되는 알림 메시지를 받으면이 창의 부모 창에서 호출 됩니다.
...
이 기간은; 부모에 전송 된 메시지를 처리 하는 일을 담당 하는 경우 0이 아닌 값 그렇지 않으면 0입니다.

### <a name="remarks"></a>설명

이 멤버 함수를 직접 호출 하지 않습니다.

이 멤버 함수를 기본 구현 즉, 부모의 메시지를 처리 하는 0을 반환 합니다.
...
프레임 워크를이 멤버 함수는 신호로 호출 합니다 `CWnd` 또는 응용 프로그램 종료 하는 것입니다.
...
비클라이언트 영역 렌더링 정책이 변경 된 경우이 멤버를 호출 하는 프레임 워크입니다.
...
이 메서드를 수신 합니다 [WM_DWMNCRENDERINGCHANGED](http://msdn.microsoft.com/library/windows/desktop/dd388198) Windows SDK에 설명 된 알림 메시지입니다.
...
프레임 워크는 액셀러레이터 키 입력 번역 된 경우 또는 자식 컨트롤 알림 메시지를 보내면 사용자 메뉴에서 항목을 선택 하는 경우이 멤버 함수를 호출 합니다.
...
응용 프로그램을이 메시지를 처리 하는 경우 0이 아닌 반환 그렇지 않으면 0입니다.

### <a name="remarks"></a>설명

`OnCommand` 컨트롤 알림과 ON_COMMAND 항목에 대 한 메시지 맵을 처리 하 고 적절 한 멤버 함수를 호출 합니다.

이 멤버 함수를 처리 하려면 파생된 클래스에서 재정의 된 [WM_COMMAND](http://msdn.microsoft.com/library/windows/desktop/ms647591) 메시지입니다. 재정의 메시지 맵을 처리할 하지 않는 한 기본 클래스 `OnCommand` 라고 합니다.
...
프레임 워크는 Windows 검색 12.5% 이상이 30 비트를 60 초 간격에 대해 시스템 시간 메모리 압축 소요 되는 경우 모든 최상위 창에 대 한이 멤버 함수를 호출 합니다.
...
경우는 `CWnd` 이 통화를 수신 하는 개체, 응용 프로그램 및 Windows에서 실행 중인 응용 프로그램의 총 활동의 현재 수준을 고려 최대한 많은 메모리를 해제 해야 해당 합니다. 응용 프로그램이 얼마나 많은 응용 프로그램을 실행 하는 결정 하는 Windows 함수를 호출할 수 있습니다.
...
목록 또는 콤보 상자를 사용 하 여 만들어지는 경우 합니다 [CBS_SORT](../../mfc/reference/styles-used-by-mfc.md#combo-box-styles) 또는 [LBS_SORT](../../mfc/reference/styles-used-by-mfc.md#list-box-styles) 스타일, Windows 메시지를 보냅니다 콤보 상자 또는 목록 상자의 소유자 WM_COMPAREITEM 때마다 응용 프로그램에 새 항목을 추가 합니다.

목록 또는 콤보 상자에서 두 항목의 과정이 생략 되는 `COMPAREITEMSTRUCT` 가리키는 구조 *lpCompareItemStruct*합니다. `OnCompareItem` 다른 하기 전에 표시 됩니다는 항목을 나타내는 값을 반환 해야 합니다. 일반적으로 Windows 호출이 여러 번까지 새 항목에 대 한 정확한 위치를 결정 합니다.

경우는 `hwndItem` 의 멤버는 `COMPAREITEMSTRUCT` 구조에 속하는 [CListBox](../../mfc/reference/clistbox-class.md) 또는 [CComboBox](../../mfc/reference/ccombobox-class.md) 개체는 `CompareItem` 해당 클래스의 가상 함수를 호출 합니다. 재정의 `CComboBox::CompareItem` 나 `CListBox::CompareItem` 에서 파생 된 `CListBox` 또는 `CComboBox` 항목 비교를 수행 하는 클래스입니다.
...
프레임 워크는 데스크톱 창 관리자 (DWM) 컴퍼지션을 사용 되었거나 사용 하지 않도록 설정 하는 경우 모든 최상위 창에 대 한이 멤버 함수를 호출 합니다.
...
이 메서드를 수신 합니다 [WM_DWMCOMPOSITIONCHANGED](http://msdn.microsoft.com/library/windows/desktop/dd388199) Windows SDK에 설명 된 알림.
...
사용자가 창의 (마우스 오른쪽 단추로 클릭)를 마우스 오른쪽 단추를 클릭 하는 프레임 워크에서 호출 합니다.
...
사용 하 여 상황에 맞는 메뉴를 표시 하 여이 메시지를 처리할 수 있습니다 합니다 [TrackPopupMenu](../../mfc/reference/cmenu-class.md#trackpopupmenu)합니다.

상황에 맞는 메뉴를 표시 하지 않으므로이 메시지를 전달 해야 합니다 [DefWindowProc](#defwindowproc) 함수입니다. 창에 자식 창에 있으면 `DefWindowProc` 부모에 대 한 메시지를 보냅니다. 그렇지 않으면 `DefWindowProc` 창 캡션에 지정 된 위치에 있으면 기본 상황에 맞는 메뉴를 표시 합니다.

##  <a name="oncopydata"></a>  CWnd::OnCopyData

이 멤버 함수는 다른 응용 프로그램에서 데이터를 복사 하기 위해 프레임 워크에서 호출 됩니다.
...
*pCopyDataStruct*  
에 대 한 포인터를 [COPYDATASTRUCT](http://msdn.microsoft.com/library/windows/desktop/ms649010) 전송 되는 데이터를 포함 하는 구조입니다.

### <a name="return-value"></a>반환 값

수신 응용 프로그램 데이터를 성공적으로 수락 하는 경우 TRUE를 반환 합니다. 그렇지 않으면 FALSE를 반환합니다.

### <a name="remarks"></a>설명

전달 되는 데이터 포인터 또는 개체 데이터를 받는 응용 프로그램에 액세스할 수 없음에 대 한 다른 참조를 포함할 수 없습니다.

데이터를 복사 되는 동안 보내는 프로세스의 다른 스레드에 의해 변경 하지 않아야 합니다.

읽기 전용 수신 응용 프로그램에서 데이터를 고려해 야 합니다. 하지만 매개 변수에서 가리키는 구조 *pCopyDataStruct* 데이터를 전송 하는 동안에 유효 수신 응용 프로그램을 해제 하면 안 구조와 연결 된 메모리입니다.

이 함수가 반환 된 후의 데이터에 액세스 해야 하는 수신 응용 프로그램에서이 로컬 버퍼에 받은 데이터를 복사 해야 합니다.
...
프레임 워크는 Windows 창을 호출 하 여 만들 수는 응용 프로그램이 요청 하는 경우이 멤버 함수를 호출 합니다 [Create](#create) 또는 [CreateEx](#createex) 멤버 함수입니다.
...
`OnCreate` 만들기를 계속 하려면 0을 반환 해야 합니다는 `CWnd` 개체입니다. -1을 반환 하는 응용 프로그램 창을 소멸 됩니다.

### <a name="remarks"></a>설명

`CWnd` 개체 창을 만든 후에 있지만 표시 되기 전에이 호출을 수신 합니다. `OnCreate` 전에 호출 되는 `Create` 또는 `CreateEx` 멤버 함수를 반환 합니다.

파생된 클래스의 모든 필요한 초기화를 수행 하려면이 멤버 함수를 재정의 합니다.

`CREATESTRUCT` 구조 창을 만들기 위해 사용 된 매개 변수 복사본을 포함 합니다.
...
자식 컨트롤을 그리려고 할 때 프레임 워크에서이 멤버 함수를 호출 합니다.
...
`OnCtlColor` 컨트롤의 배경을 그리는 데 사용할 브러시에 대 한 핸들을 반환 해야 합니다.

### <a name="remarks"></a>설명

대부분의 컨트롤이이 메시지를 준비 하는 부모 (일반적으로 대화 상자)를 보낼 합니다 *pDC* 올바른 색을 사용 하 고 컨트롤 그리기에 대 한 합니다.

텍스트 색을 변경 하려면 호출을 `SetTextColor` 원하는 빨강, 녹색 및 파란색 (RGB) 값을 사용 하 여 멤버 함수입니다.

단일 줄 편집 컨트롤의 배경색을 변경 하려면 CTLCOLOR_EDIT와 CTLCOLOR_MSGBOX 메시지 코드와 호출에서 브러시 핸들을 설정 합니다 [CDC::SetBkColor](../../mfc/reference/cdc-class.md#setbkcolor) CTLCOLOR_EDIT 코드에 대 한 응답에는 함수입니다.

`OnCtlColor` 호출 되지 않습니다 드롭다운 콤보 상자의 목록 상자에 대 한 드롭다운 목록 상자는 실제로 콤보 상자의 자식 창의 자식이 아닙니다. 드롭다운 목록 상자의 색을 변경 하려면 만들기를 `CComboBox` 재정의 사용 하 여 `OnCtlColor` CTLCOLOR_LISTBOX에서 확인 하는 `nCtlColor` 매개 변수입니다. 이 처리기는 `SetBkColor` 멤버 함수에서 텍스트의 배경색을 설정 하는 데 사용 해야 합니다.

> [!NOTE]
> 이 멤버 함수는 응용프로그램에서 Windows 메시지를 처리할 수 있도록 프레임워크에서 호출됩니다. 함수에 전달되는 매개 변수는 해당 메시지가 수신되었을 때 프레임워크에서 받았던 매개 변수를 반영합니다. 이 함수의 기본 클래스 구현을 호출하는 경우, 해당 구현은 해당 함수에 공급한 매개변수가 아닌, 메시지와 함께 전달 받았던 원래의 매개 변수를 사용할 것입니다. 대화 상자 클래스에 다음 메서드를 추가 하려면 Visual Studio 속성 창을 사용 하 여 WM_CTLCOLOR에 대 한 메시지 처리기를 추가 합니다. 또는 메시지 맵에 ON_WM_CTLCOLOR() 진입점을 수동으로 추가할 수 있습니다.
...
이 멤버 함수를 호출 하는 프레임 워크 때를 [OnKeyUp](#onkeyup) 멤버 함수 및 [OnKeyDown](#onkeydown) 멤버 함수가 호출 됩니다.
...
|값|설명|
|-----------|-----------------|
|0-7|코드 (OEM 종속 값)을 검색 합니다. 상위 단어의 낮은 바이트입니다.|
|8|기능 키 (확장 된 키가 있으면 1 그렇지 않으면 0) 숫자 키패드의 키 등 확장 된 키입니다.|
|9-10|사용되지 않습니다.|
|11-12|Windows에서 내부적으로 사용 합니다.|
|13|상황에 맞는 코드 (1 키를 누르고 있는 동안 ALT 키를 누르고 그렇지 않으면 0)입니다.|
|14|이전 키 상태 (등록 키가 있으면 0 호출 전에 키가 눌러져 있으면 1)입니다.|
|15|전환 상태 (키를 해제 하는 경우 1, 0 키를 누른 경우)입니다.|

### <a name="remarks"></a>설명

데드 키의 문자 값을 지정 하려면이 멤버 함수를 사용할 수 있습니다. 데드 키가 복합 문자를 구성 하려면 다른 문자로 결합 되는 움라우트 (이중 점) 문자를 같은 키입니다. 예를 들어 움라우트-O 문자 데드 키, 움라우트와 O 키 이루어져 있습니다.

응용 프로그램에서는 보통 `OnDeadChar` 각 키를 누른 상태에 대 한 사용자 의견을 보내주세요. 예를 들어, 응용 프로그램 캐럿을 이동 하지 않고 현재 문자 위치에서 악센트를 표시할 수 있습니다.

하지 않으므로 반드시 일대일로 대응 누른 키 사이 및 `OnDeadChar` 호출을 정보 *nFlags* 아닙니다 일반적으로 응용 프로그램에 유용 합니다. 에 있는 정보 *nFlags* 에 대 한 최근 호출에만 적용 됩니다는 [OnKeyUp](#onkeyup) 멤버 함수 또는 [OnKeyDown](#onkeydown) 앞에 있는 멤버 함수는 `OnDeadChar` 호출 합니다.
...
목록 상자 또는 콤보 상자는 제거 또는 항목에서 제거 되었습니다는 소유자 그리기 목록 상자 또는 콤보 상자의 소유자를 알리기 위해이 멤버 함수를 호출 하는 프레임 워크 [CComboBox::DeleteString](../../mfc/reference/ccombobox-class.md#deletestring), [CListBox::D eleteString](../../mfc/reference/clistbox-class.md#deletestring)하십시오 [CComboBox::ResetContent](../../mfc/reference/ccombobox-class.md#resetcontent), 또는 [CListBox::ResetContent](../../mfc/reference/clistbox-class.md#resetcontent)합니다.
...
경우는 `hwndItem` 의 멤버를 `DELETEITEMSTRUCT` 구조 콤보 상자 또는 목록 상자에 속하는 해당 `DeleteItem` 해당 클래스의 가상 함수를 호출 합니다. 재정의 `DeleteItem` 항목-관련 데이터를 삭제 하는 적절 한 컨트롤 클래스의 멤버 함수입니다.
...
프레임 워크에 알리기 위해이 멤버 함수 호출을 `CWnd` 소멸 되는 개체입니다.
...
`OnDestroy` 처음에 대 한 호출을 `CWnd` 소멸 되 고, 다음의 자식 창에 대 한 `CWnd` 으로 소멸 됩니다. 모든 자식 창은 여전히 있는지 간주 될 수 있습니다 하는 동안 `OnDestroy` 실행 합니다.

경우는 `CWnd` 클립보드 뷰어 체인의 일부인 소멸 될 개체 (호출 하 여 설정 합니다 [SetClipboardViewer](#setclipboardviewer) 멤버 함수), `CWnd` 제거 해야 자체 클립보드 뷰어 체인에서 호출 하 여는 [ChangeClipboardChain](#changeclipboardchain) 멤버 함수에서 반환 하기 전에 `OnDestroy` 함수입니다.

##  <a name="ondestroyclipboard"></a>  CWnd::OnDestroyClipboard

프레임 워크를 호출 하 여 클립보드를 비울 때 클립보드 소유자에 대 한이 멤버 함수를 호출 합니다 [EmptyClipboard](http://msdn.microsoft.com/library/windows/desktop/ms649037) Windows 함수입니다.
...
프레임 워크는 응용 프로그램 또는 장치 또는 컴퓨터의 하드웨어 구성에 대 한 변경의 장치 드라이버에 알리기 위해이 멤버 함수를 호출 합니다.
...
배출 및 잠금 같은 소프트웨어를 제어할 수 있는 기능을 제공 하는 장치에 대 한 운영 체제 일반적으로 응용 프로그램에 있도록 DBT_DEVICEREMOVEPENDING 메시지를 보내고 장치 드라이버 장치의 용도 정상적으로 종료 합니다.

장치의 운영 체제에서 강제로 제거를 하는 경우 그 전에 DBT_DEVICEQUERYREMOVE 메시지 하지 보낼 수 있습니다.

합니다 *n 이벤트* 매개 변수는 다음이 값 중 하나일 수 있습니다.

- [DBT_DEVICEARRIVAL](http://msdn.microsoft.com/library/windows/desktop/aa363205) 삽입 된 장치와 출시 되었습니다.

- [DBT_DEVICEQUERYREMOVE](http://msdn.microsoft.com/library/windows/desktop/aa363206) 장치를 제거 하는 사용 권한을 요청 합니다. 모든 응용 프로그램이이 요청을 거부 하 고 제거를 취소할 수 있습니다.

- [DBT_DEVICEQUERYREMOVEFAILED](http://msdn.microsoft.com/library/windows/desktop/aa363207) 장치를 제거 하는 요청이 취소 되었습니다.

- [DBT_DEVICEREMOVEPENDING](http://msdn.microsoft.com/library/windows/desktop/aa363209) 장치를 제거 하려고 합니다. 거부 될 수 없습니다.

- [DBT_DEVICEREMOVECOMPLETE](http://msdn.microsoft.com/library/windows/desktop/aa363208) 장치가 제거 되었습니다.

- [DBT_DEVICETYPESPECIFIC](http://msdn.microsoft.com/library/windows/desktop/aa363210) 장치별 이벤트입니다.
...
프레임 워크에 대 한이 멤버 함수를 모든 최상위 호출 `CWnd` 사용자 장치 모드 설정을 변경할 때 개체입니다.
...
WM_DEVMODECHANGE 메시지를 처리 하는 응용 프로그램 장치 모드 설정을 다시 초기화할 수 있습니다. Windows를 사용 하는 응용 프로그램 `ExtDeviceMode` 함수를 저장 하 고 장치 설정을 복원이이 함수를 일반적으로 처리 하지 않습니다.

사용자는 제어판에서 기본 프린터를 변경 하는 경우에이 함수가 호출 되지 않습니다. 이 경우에 `OnWinIniChange` 함수를 호출 합니다.
...
클립보드의 내용이 변경 될 때 클립보드 뷰어 체인의 각 창에 대 한이 멤버 함수를 호출 하는 프레임 워크입니다.
...
응용 프로그램에만 호출 하 여 클립보드 뷰어 체인 가입 합니다 [SetClipboardViewer](#setclipboardviewer) 멤버 함수에서이 호출에 응답 해야 합니다.

수신 하는 각 창을 `OnDrawClipboard` 호출 호출 해야 합니다 [SendMessage](http://msdn.microsoft.com/library/windows/desktop/ms644950) 전달할 Windows 함수를 [WM_DRAWCLIPBOARD](http://msdn.microsoft.com/library/windows/desktop/ms649025) 클립보드 뷰어 체인에서 다음 창에 메시지. 다음 창 핸들에서 반환 되는 [SetClipboardViewer](#setclipboardviewer) 멤버 함수에 대 한 응답에서 수정할 수 있습니다는 [OnChangeCbChain](#onchangecbchain) 멤버 함수 호출.
...
파생된 클래스에서이 메서드를 재정의 하 고 미리 보기 및 보기를 사용자 지정 하기 위해 지정된 된 장치 컨텍스트에 그립니다. 하는 경우 *bThumbnail* 가 TRUE 인 *szRequiredThumbnailSize* 무시 될 수 있습니다. 이 경우 (즉, 전체 클라이언트 영역을 포함 하는 비트맵) 전체 크기의 비트맵을 그릴 수 인식 해야 합니다. 장치 컨텍스트 ( *dc*) 선택한 32 비트 비트맵을 사용 하 여 제공 됩니다. 기본 구현은 PRF_CLIENT, PRF_CHILDREN, 및 PRF_NONCLIENT 플래그를 사용 하 여이 창을 WM_PRINT 보냅니다.

##  <a name="ondrawitem"></a>  CWnd::OnDrawItem

소유자 그리기 단추 컨트롤, 콤보 상자 컨트롤, 목록 상자 컨트롤 또는 컨트롤의 시각적 측면이 경우 메뉴의 소유자에 대 한이 멤버 함수를 호출 하는 프레임 워크 또는 메뉴 변경 되었습니다.
...
경우는 `hwndItem` 멤버가 속한를 [CButton](../../mfc/reference/cbutton-class.md)를 [CMenu](../../mfc/reference/cmenu-class.md)를 [CListBox](../../mfc/reference/clistbox-class.md), 또는 [CComboBox](../../mfc/reference/ccombobox-class.md) 개체는 `DrawItem` 해당 클래스의 가상 함수가 호출 됩니다. 재정의 `DrawItem` 항목을 그리는 적절 한 컨트롤을 클래스의 멤버 함수입니다.
...
끌어 놓은 파일의 수신자로 등록 되어 있는 창을 통해 마우스 왼쪽된 단추를 놓을 때 프레임 워크에서이 멤버 함수를 호출 합니다.
...
일반적으로 끌어 놓은 파일을 지원 하기 위해 파생된 클래스 디자인 됩니다 하 고 창 생성 하는 동안 자체 등록 됩니다.
...
프레임 워크는 응용 프로그램의 활성화 상태를 변경 하는 경우이 멤버 함수를 호출 합니다 `CWnd` 개체입니다.
...
`OnEnable` 전에 호출 됩니다는 [EnableWindow](#enablewindow) 멤버 함수는 반환 하지만 사용 하도록 설정 하는 창 상태 후 ( [WS_DISABLED](styles-used-by-mfc.md#window-styles) 스타일 비트) 변경 되었습니다.
...
프레임 워크가 후이 멤버 함수를 호출 합니다 `CWnd` 개체의 0이 아닌 값을 반환 했습니다를 [OnQueryEndSession](#onqueryendsession) 멤버 함수 호출 합니다.
...
합니다 `OnEndSession` 호출 알립니다는 `CWnd` 세션이 실제로 종료 되 고 있는지 여부를 개체입니다.

하는 경우 *구부러진* 가 TRUE 이면 Windows 응용 프로그램을 모두이 호출을 처리에서 반환 된 후 언제 든 지 종료할 수 있습니다. 따라서 응용 프로그램 내에서 종료 하는 데 필요한 모든 작업을 수행할가 `OnEndSession`입니다.

호출 해야 합니다 [DestroyWindow](#destroywindow) 멤버 함수 또는 [PostQuitMessage](http://msdn.microsoft.com/library/windows/desktop/ms644945) 세션이 종료 되는 경우에 Windows 작동.
...
프레임 워크는 모달 대화 상자 또는 메뉴가 유휴 상태로 전환 되는 응용 프로그램의 주 창 프로시저에 알리기 위해이 멤버 함수를 호출 합니다.
...
모달 대화 상자 또는 메뉴에 하나 이상의 이전 메시지 처리 후에 메시지가 없는 큐에 대기 중인 때 유휴 상태로 전환 합니다.
...
메뉴 모달 루프가 시작 된 경우이 멤버 함수를 호출 하는 프레임 워크입니다.
...
이 메서드를 수신 합니다 [WM_ENTERSIZEMOVE](http://msdn.microsoft.com/library/windows/desktop/ms632622) Windows SDK에 설명 된 알림.

창의 입력을 이동 하거나 창의 제목 표시줄 또는 크기 조정 테두리를 두 번 클릭할 때 또는 창에 전달 하는 경우 모달 루프의 크기를 조정 합니다 [WM_SYSCOMMAND](http://msdn.microsoft.com/library/windows/desktop/ms646360) 메시지를 [CWnd::DefWindowProc](#defwindowproc) 함수 및 합니다 *wParam* SC_MOVE 또는 SC_SIZE 해당 메시지의 매개 변수를 지정 합니다.
...
이 멤버 함수를 호출 하는 프레임 워크는 경우를 `CWnd` 개체 배경을 지워야 (예를 들어, 크기를 조정할 때).
...
백그라운드; 지우는 경우 0이 아닌 값 그렇지 않으면 0입니다.

### <a name="remarks"></a>설명

무효화 된 영역을 그리기 위해 준비 하는 것이 라고 합니다.

기본 구현은 지우고 지정 된 창 클래스 배경 브러시를 사용 하 여 백그라운드를 `hbrBackground` 창 클래스 구조체의 멤버입니다.

경우는 `hbrBackground` 멤버가 null 인 경우 재정의 된 버전의 `OnEraseBkgnd` 배경색을 삭제 해야 합니다. 버전 의도 된 브러시의 원점을 정렬 해야 합니다 `CWnd` 첫 번째 호출 하 여 좌표 [UnrealizeObject](http://msdn.microsoft.com/library/windows/desktop/dd145164) 브러시를 선택한 다음 브러시에 대 한 합니다.

재정의 된 `OnEraseBkgnd` 메시지를 처리 하 고 백그라운드 지웁니다 WM_ERASEBKGND에 대 한 응답에 0이 아닌 반환 해야 하 고 더 이상 지우기 과정 나타냅니다. 0을 반환 하는 경우 창을 지울 수 하는 것으로 표시 된 상태로 유지 됩니다. (즉 일반적으로 `fErase` 의 멤버는 `PAINTSTRUCT` 구조를 true로 설정 됩니다.)

Windows는 백그라운드 MM_TEXT 매핑 모드를 사용 하 여 계산을 가정 합니다. 장치 컨텍스트를 다른 매핑 모드를 사용 하는 경우에 표시 부분 클라이언트 영역 내에서 지울 영역 아닐 수 있습니다.
...
메뉴 모달 루프가 종료 된 경우이 멤버 함수를 호출 하는 프레임 워크입니다.
...
이 메서드를 수신 합니다 [WM_EXITSIZEMOVE](http://msdn.microsoft.com/library/windows/desktop/ms632623) Windows SDK에 설명 된 알림.
...
시스템의 모든 최상위 창 수신는 `OnFontChange` 응용 프로그램 글꼴 리소스 풀이 변경 후 프레임 워크에서 호출 합니다.
...
추가 또는 글꼴을 시스템에서 제거 하는 응용 프로그램 (예는 [AddFontResource](http://msdn.microsoft.com/library/windows/desktop/dd183326) 또는 [RemoveFontResource](http://msdn.microsoft.com/library/windows/desktop/dd162922) Windows 함수) 보내야 하는 [WM_FONTCHANGE](http://msdn.microsoft.com/library/windows/desktop/dd145211) 모든 최상위 창에는 메시지입니다.

이 메시지를 보내려면 다음을 사용 합니다 [SendMessage](http://msdn.microsoft.com/library/windows/desktop/ms644950) Windows 함수를 *hWnd* HWND_BROADCAST 매개 변수 설정.
...
하나 이상의 응용 프로그램 프로세스 입력의 형식을 나타내는 다음 값 중:

- DLGC_BUTTON 단추 (제네릭)입니다.

- DLGC_DEFPUSHBUTTON 기본 누름 단추입니다.

- DLGC_HASSETSEL EM_SETSEL 메시지입니다.

- DLGC_UNDEFPUSHBUTTON 아니요 기본 누름 단추 처리 합니다. (응용 프로그램 수 사용이 플래그 DLGC_BUTTON 단추 입력 처리는 있지만 기본 누름 단추 처리에 대 한 시스템에 의존 합니다.)
...
- DLGC_WANTMESSAGE 모든 키보드 입력 합니다. 응용 프로그램 컨트롤에이 메시지를 전달합니다.

- DLGC_WANTTAB TAB 키입니다.

### <a name="remarks"></a>설명

Windows에 모든 화살표 키 및 TAB 키 입력을 처리 하는 일반적으로 `CWnd` 제어 합니다. 재정의 하 여 `OnGetDlgCode`, `CWnd` 컨트롤 자체를 처리 하는 특정 유형의 입력을 선택할 수 있습니다.

기본 `OnGetDlgCode` 함수 미리 정의 된 컨트롤 클래스는 각 클래스에 대 한 적절 한 코드를 반환 합니다.

##  <a name="ongetminmaxinfo"></a>  CWnd::OnGetMinMaxInfo

프레임 워크는 Windows에서 최대화 된 위치 또는 차원 또는 최소 또는 최대 추적 크기를 알고 해야 할 때마다이 멤버 함수를 호출 합니다.
...
최대화 된 상태로 크기 창의 크기 경우 테두리 완전히 확장 됩니다. 추적 창의 크기는 최대에 테두리를 사용 하 여 창의 크기를 조정 하 여 얻을 수 있는 가장 큰 창 크기입니다. 최소 창 크기를 추적에 테두리 창의 크기를 조정 하 여 얻을 수 있는 가장 작은 창 크기입니다.

Windows는 다양 한 위치 및 차원에 대 한 기본값을 지정 하는 요소의 배열을 채웁니다. 응용 프로그램에서 이러한 값을 변경할 수 있습니다 `OnGetMinMaxInfo`합니다.
...
참조 [CWinApp::OnHelp](../../mfc/reference/cwinapp-class.md#onhelp) 자세한 내용은 합니다.
...
참조 [CWinApp::OnHelpFinder](../../mfc/reference/cwinapp-class.md#onhelpfinder) 자세한 내용은 합니다.
...
참조 [CWinApp::OnHelpIndex](../../mfc/reference/cwinapp-class.md#onhelpindex) 자세한 내용은 합니다.
...
*lpHelpInfo*  
에 대 한 포인터를 [HELPINFO](http://msdn.microsoft.com/library/windows/desktop/bb773313) 메뉴 항목, 컨트롤, 대화 상자 또는 창 도움말을 요청 하는 방법에 대 한 정보가 포함 된 구조체입니다.

### <a name="return-value"></a>반환 값

창이 키보드 포커스를 갖는 또는 메뉴 창 내에서 활성화 된 경우 TRUE를 반환 합니다. 창이 키보드 포커스에 FALSE를 반환 합니다.

### <a name="remarks"></a>설명

WM_HELP 메뉴;와 연결 된 창으로 전송 됩니다 f1 키를 누르면 메뉴가 활성화 되 면 그렇지 않으면 WM_HELP 키보드 포커스가 있는 창으로 전송 됩니다. 창이 키보드 포커스에 WM_HELP 현재 활성 창으로 전송 됩니다.
...
참조 [CWinApp::OnHelpUsing](../../mfc/reference/cwinapp-class.md#onhelpusing) 자세한 내용은 합니다.

##  <a name="onhotkey"></a>  CWnd::OnHotKey

사용자가 시스템 차원 바로 가기 키를 누를 때 프레임 워크에서이 멤버 함수를 호출 합니다.
...
이 메서드를 수신 합니다 [WM_HOTKEY](http://msdn.microsoft.com/library/windows/desktop/ms646279) Windows SDK에 설명 된 알림. 이 메시지는 바로 가기 키를 등록 하는 스레드와 연결 된 메시지 큐의 맨 위에 있는 배치 됩니다. 사용 된 [RegisterHotKey](http://msdn.microsoft.com/library/windows/desktop/ms646309) 시스템 차원 바로 가기 키를 등록 하는 함수입니다.
...
프레임 워크는 창의 가로 스크롤 막대를 클릭할 때이 멤버 함수를 호출 합니다.
...
- SB_ENDSCROLL 최종 스크롤입니다.

- SB_LINELEFT 스크롤 왼쪽입니다.

- SB_LINERIGHT 스크롤 권한입니다.

- SB_PAGELEFT 스크롤 하나에 왼쪽 페이지입니다.

- SB_PAGERIGHT 스크롤 적절 한 페이지입니다.

- 맨 오른쪽으로 SB_RIGHT 스크롤입니다.

- 절대 위치로 SB_THUMBPOSITION 스크롤입니다. 현재 위치에서 지정 된 *nPos* 매개 변수입니다.
...
일반적으로 SB_THUMBTRACK 스크롤 막대 코드 스크롤 상자를 끄는 동안 몇 가지 피드백을 제공 하는 응용 프로그램에서 사용 됩니다.

스크롤 막대에 의해 제어 내용을 스크롤 하는 응용 프로그램, 고도 다시 설정 해야 사용 하 여 스크롤 상자 위치를 [SetScrollPos](#setscrollpos) 멤버 함수입니다.
...
클립보드 소유자 `OnHScrollClipboard` 클립보드 데이터에 있는 경우 클립보드 뷰어에서 구성원 함수가 호출 되는 `CF_OWNERDISPLAY` 서식을 지정 하 고 클립보드 뷰어의 가로 스크롤 막대에는 이벤트입니다.
...
- 한 줄 아래로 SB_LINEDOWN 스크롤입니다.

- 한 줄 위로 SB_LINEUP 스크롤입니다.

- 한 페이지 아래로 SB_PAGEDOWN 스크롤입니다.

- 한 페이지 위로 SB_PAGEUP 스크롤입니다.

- 절대 위치로 SB_THUMBPOSITION 스크롤입니다. 현재 위치에서 제공 됩니다 *nPos*합니다.
...
소유자는 클립보드 이미지를 스크롤 하 고 적절 한 섹션을 무효화 하 고 스크롤 막대 값을 업데이트 해야 합니다.
...
프레임 워크가이 멤버 함수가 최소화 (아이콘)를 호출 `CWnd` 아이콘을 그리기 전에 아이콘의 배경을 채워야 하는 경우 개체입니다.
...
`CWnd` 클래스 아이콘을 창의 기본 구현은;에 대해 정의 된 경우에이 호출을 수신 그렇지 않으면 [OnEraseBkgnd](#onerasebkgnd) 라고 합니다.

합니다 [DefWindowProc](#defwindowproc) 멤버 함수는 부모 창의 배경 브러시를 사용 하 여 아이콘 배경을 채웁니다.
...
`OnInitMenu` 사용자가 메뉴 모음에서 항목을 클릭 하거나 메뉴 키를 누를 때 호출 됩니다. 표시 되기 전에 메뉴를 수정 하려면이 멤버 함수를 재정의 합니다.

`OnInitMenu` 경우 메뉴 먼저 액세스 됩니다 (예를 들어, 사용자가 메뉴 모음에서 항목을 클릭) 한 번만 호출 됩니다. 이 메서드는 메뉴 항목에 대 한 정보를 제공 하지 않습니다. 사용자가 메뉴 내의 항목 (예를 들어 몇 가지 메뉴 항목에서 마우스를 이동)로으로 함수가 다시 호출 되지 않습니다. 사용자 (예를 들어, 클릭 하 여 응용 프로그램의 클라이언트 영역에서) 메뉴에서 종료 하 고 나중에 메뉴 모음에서 항목을 클릭할 함수가 다시 호출 됩니다.
...
이 응용 프로그램을을 전체 메뉴를 변경 하지 않고 표시 하기 전에 팝업 메뉴를 수정할 수 있습니다.
...
프레임 워크는 I/O 장치를 추가 하거나 시스템에서 제거 하는 경우이 멤버 함수를 호출 합니다.
...
이 메서드를 수신 합니다 [WM_INPUT_DEVICE_CHANGE](http://msdn.microsoft.com/library/windows/desktop/ms645591) Windows SDK에 설명 된 알림. 일반 입력된 장치 메시지입니다.
...
프레임 워크는 응용 프로그램의 입력된 언어가 변경 된 후 최상위 영향을 받는 창에 대 한이 멤버를 호출 합니다.
...
|매개 변수|설명|
|---------------|-----------------|
|[in] *nCharSet*|새 로캘의 문자 집합입니다. 자세한 내용은 참조는 *lfCharSet* 의 매개 변수를 [LOGFONT](http://msdn.microsoft.com/library/windows/desktop/dd145037) 구조입니다.|
|[in] *nLocaleId*|입력된 로캘 식별자입니다. 자세한 내용은 [문자열과 언어 식별자 상수](http://msdn.microsoft.com/library/windows/desktop/dd318693)합니다.|

### <a name="remarks"></a>설명

이 메서드를 수신 합니다 [WM_INPUTLANGCHANGE](http://msdn.microsoft.com/library/windows/desktop/ms632629) Windows SDK에 설명 된 알림 메시지입니다.
...
새 입력된 언어를 선택 하면 포커스가 있는 창에 대 한이 멤버를 호출 하는 프레임 워크입니다.
...
이 메서드를 수신 합니다 [WM_INPUTLANGCHANGEREQUEST](http://msdn.microsoft.com/library/windows/desktop/ms632630) Windows SDK에 설명 된 알림 메시지입니다. 이 메시지는 사용자가 키보드 컨트롤 제어판 응용 프로그램 또는 시스템 작업 표시줄에서 표시기에서 지정 된 두 핫키를 사용 하 여 새 입력된 언어를 선택할 때 게시 됩니다.
...
|값|설명|
|-----------|-----------------|
|0-7|코드 (OEM 종속 값)을 검색 합니다.|
|9|Function 키 또는 숫자 키패드 (확장 된 키가 있으면 1)의 키와 같은 확장 된 키입니다.|
|9-10|사용되지 않습니다.|
|11-12|Windows에서 내부적으로 사용 합니다.|
|13|상황에 맞는 코드 (1 키를 누르고 있는 동안 ALT 키를 누르고 그렇지 않으면 0)입니다.|
|14|이전 키 상태 (등록 키가 있으면 0 호출 전에 키가 눌러져 있으면 1)입니다.|
...
WM_KEYDOWN 메시지에 대해 키 전환 비트 (비트 15) 0이 고 상황에 맞는 코드 비트 (비트 13)은 0입니다.

### <a name="remarks"></a>설명

시스템 키 이외의 키를 누를 때 ALT 키를 누르지는 키보드 키 인지는 키보드 키를 누를 때를 `CWnd` 입력된 포커스를가지고 있습니다.

자동-반복을 하나 이상으로 인해 `OnKeyDown` 호출 하기 전에 발생할 수 있습니다는 [OnKeyUp](#onkeyup) 멤버 함수 호출 됩니다. 이전 키 상태를 나타내는 비트를 확인 하기 위해 사용할 수 있는지 여부를 `OnKeyDown` 호출이 첫 번째 하위 전환 되었거나 아래로 반복된 전환 합니다.
...
WM_KEYUP 메시지에 대해 키 전환 비트 (비트 15) 1 이며 상황에 맞는 코드 비트 (비트 13)은 0입니다.
...
입력된 포커스를 잃을 직전이 멤버 함수를 호출 하는 프레임 워크입니다.
...
프레임 워크가 마우스 왼쪽된 단추를 두 번 클릭할 때이 멤버 함수를 호출 합니다.
...
- 왼쪽 마우스 단추 MK_LBUTTON 설정 중단 되었습니다.

- 마우스 가운데 단추를 눌렀음을 MK_MBUTTON 설정 합니다.

- 오른쪽 마우스 단추 MK_RBUTTON 설정 중단 되었습니다.
...
CS_DBLCLKS 있는 창만 [WNDCLASS](http://msdn.microsoft.com/library/windows/desktop/ms633576) 스타일 받을 `OnLButtonDblClk` 호출 합니다. 이것이 Microsoft Foundation Class windows에 대 한 기본값입니다. Windows 호출 `OnLButtonDblClk` , 릴리스를 클릭 하 고 다음 시스템 내에 다시 마우스 왼쪽된 단추를 누를 때 제한 시간을 두 번 클릭 합니다. 4 개의 이벤트를 생성 실제로 마우스 왼쪽된 단추를 두 번 클릭 합니다. [WM_LBUTTONDOWN](#onlbuttondown)를 [WM_LBUTTONUP](#onlbuttonup) 메시지, WM_LBUTTONDBLCLK 호출 및 단추 때 다른 WM_LBUTTONUP 메시지 해제 합니다.
...
사용자가 마우스 왼쪽된 단추를 누를 때 프레임 워크에서이 멤버 함수를 호출 합니다.
...
마우스 왼쪽된 단추를 놓을 때 프레임 워크에서이 멤버 함수를 호출 합니다.
...
프레임 워크가 마우스 가운데 단추를 두 번 클릭할 때이 멤버 함수를 호출 합니다.
...
프레임 워크는 사용자가 마우스 가운데 단추를 놓으면이 멤버 함수를 호출 합니다.
...
프레임 워크를 비활성화 하 고 자식 창을 활성화 되 고 자식 창에이 멤버 함수를 호출 합니다.
...
MDI 자식 창에는 MDI 프레임 창을 독립적으로 활성화 됩니다. 프레임 활성화를 사용 하 여 마지막으로 활성화 된 자식 창을 `OnMDIActivate` 호출 수신를 [WM_NCACTIVATE](#onncactivate) 메시지를 활성 창 프레임 및 캡션 표시줄 하지만 그리기를 다른 수신 하지 않습니다 `OnMDIActivate` 호출 합니다.
...
컨트롤을 만들 때 프레임 워크를이 멤버 함수는 프레임 워크에서 소유자 그리기 단추, 콤보 상자, 목록 상자 또는 메뉴 항목의 소유자에 대 한 호출 합니다.
...
이 멤버 함수를 재정의 하 고를 입력 합니다 `MEASUREITEMSTRUCT` 가리키는 데이터 구조 *lpMeasureItemStruct* 반환; 및 컨트롤의 크기의 Windows를 알리고 사용자 상호 작용을 처리 하는 Windows를 사용 하면이 올바르게를 제어 합니다.

목록 상자 또는 콤보 상자를 사용 하 여 만들어지는 경우 합니다 [LBS_OWNERDRAWVARIABLE](../../mfc/reference/styles-used-by-mfc.md#list-box-styles) 하거나 [CBS_OWNERDRAWVARIABLE](../../mfc/reference/styles-used-by-mfc.md#combo-box-styles) 스타일 프레임 워크를이 함수를 호출 컨트롤의 각 항목에 대 한 소유자이 고, 그렇지이 함수는 한 번 호출 됩니다.

Windows 시작에 대 한 호출 `OnMeasureItem` 콤보 상자와 목록 상자를 보내기 전에 OWNERDRAWFIXED 스타일을 사용 하 여 만든의 소유자는 [WM_INITDIALOG](http://msdn.microsoft.com/library/windows/desktop/ms645428) 메시지입니다. 결과적으로, 소유자가이 호출을 받으면 Windows에서 아직 확인 하지 컨트롤에 사용 된 글꼴의 너비와 높이 함수 호출 및 이러한 값을 필요로 하는 계산 응용 프로그램 또는 라이브러리의 main 함수에서 발생 해야 합니다.

측정할 항목이 `CMenu`, `CListBox` 또는 `CComboBox` 개체는 `MeasureItem` 해당 클래스의 가상 함수를 호출 합니다. 재정의 `MeasureItem` 계산 하 고 각 항목의 크기를 설정 하려면 적절 한 컨트롤을 클래스의 멤버 함수입니다.

`OnMeasureItem` 런타임 시 컨트롤의 클래스를 만들거나 LBS_OWNERDRAWVARIABLE 또는 CBS_OWNERDRAWVARIABLE 스타일을 사용 하 여 만든 경우에 호출 됩니다. 대화 상자 편집기에서 컨트롤을 만드는 경우 `OnMeasureItem` 호출 되지 것입니다. 왜냐하면 합니다 [WM_MEASUREITEM](http://msdn.microsoft.com/library/windows/desktop/bb775925) 컨트롤의 생성 프로세스 초기에 전송 됩니다. 경우 서브 클래스를 사용 하 여 `DDX_Control`, `SubclassDlgItem`, 또는 `SubclassWindow`, 하위 클래스를 작성 만들기 프로세스가 완료 된 후 일반적으로 발생 합니다. 따라서 없기를 처리 하는 [WM_MEASUREITEM](http://msdn.microsoft.com/library/windows/desktop/bb775925) 컨트롤의 메시지 `OnChildNotify` 함수는 MFC ON_WM_MEASUREITEM_REFLECT를 구현 하기 위해 사용 하는 메커니즘입니다.
...
현재 메뉴의 미리 정의 된 니모닉과 일치 하지 않는 메뉴 니모닉 문자를 누를 때 프레임 워크에서이 멤버 함수를 호출 합니다.
...
반환 값의 상위 단어를 다음 명령 코드 중 하나가 있어야 합니다.

|값|설명|
|-----------|-----------------|
|0|Windows는 사용자 누를 만들고 짧은 경고음 시스템 스피커의 문자를 삭제 하도록 지시 합니다.|
|1|현재 메뉴를 닫으려면는 Windows를 알려 줍니다.|
|2|반환 값의 하위 단어는 특정 항목에 대 한 항목 수를 포함 한 Windows를 알립니다. 이 항목은 Windows로 선택 됩니다.|

0 또는 1 상위 단어를 포함 하는 경우에 하위 단어는 무시 됩니다. 액셀러레이터 키는 메뉴에 배치 하는 비트맵을 선택 하는 데 사용 됩니다 하는 경우 응용 프로그램에서이 메시지를 처리 해야 합니다.

### <a name="remarks"></a>설명

보낼는 `CWnd` 메뉴를 소유 하는 합니다. `OnMenuChar` 키 니모닉 문자에 해당 하지 않는 경우에 사용자가 alt 키와 다른 키를 누를 때을 호출도 됩니다. 이 경우 *pMenu* 소유 하 고 메뉴를 가리키는 합니다 `CWnd`, 및 *nFlags* 0.
...
사용자 메뉴 항목을 끌기 시작할 때 현재 끌어서 놓기 메뉴의이 멤버 함수를 호출 하는 프레임 워크입니다.
...
|반환 값|의미|
|------------------|-------------|
|MND_CONTINUE|메뉴는 활성 상태로 유지 해야 합니다. 마우스를 해제 하는 경우 무시 됩니다.|
|MND_ENDMENU|메뉴를 종료 해야 합니다.|

### <a name="remarks"></a>설명

이 메서드를 수신 합니다 [WM_MENUDRAG](http://msdn.microsoft.com/library/windows/desktop/ms647606) Windows SDK에 설명 된 알림.
...
프레임 워크가 마우스 커서를 메뉴 항목으로 가져가거나 위쪽 이나 아래쪽 항목의 항목의 가운데에서 이동 하는 경우 현재 끌어서 놓기 메뉴의이 멤버 함수를 호출 합니다.
...
|매개 변수|설명|
|---------------|-----------------|
|[in] *pMenu*|에 대 한 포인터를 [MENUGETOBJECTINFO](http://msdn.microsoft.com/library/windows/desktop/ms647572) 끌어서 놓기 메뉴 마우스 커서에 대 한 정보를 포함 하는 구조 켜져 있습니다.|

### <a name="return-value"></a>반환 값

|반환 값|의미|
|------------------|-------------|
|MNGO_NOERROR|Drop 끌기 작업을 지 원하는 인터페이스 포인터를 반환 됩니다 합니다 `pvObj` 의 멤버는 [MENUGETOBJECTINFO](http://msdn.microsoft.com/library/windows/desktop/ms647572) 구조입니다. 현재만 합니다 [IDropTarget](http://msdn.microsoft.com/library/windows/desktop/ms679679) 인터페이스를 지원 합니다.|
|MNGO_NOINTERFACE|Drop 끌어서 인터페이스 없이 사용할 수 있습니다.|

### <a name="remarks"></a>설명

이 메서드를 수신 합니다 [WM_MENUGETOBJECT](http://msdn.microsoft.com/library/windows/desktop/ms647607) Windows SDK에 설명 된 알림.
...
커서가 메뉴 항목에 있는 동안 마우스 오른쪽 단추를 놓을 때 프레임 워크에서이 멤버 함수를 호출 합니다.
...
이 메서드를 수신 합니다 [WM_MENURBUTTONUP](http://msdn.microsoft.com/library/windows/desktop/ms647610) Windows SDK에 설명 된 알림. 합니다 [WM_MENURBUTTONUP](http://msdn.microsoft.com/library/windows/desktop/ms647610) 메시지 통해 응용 프로그램 항목 메시지에 지정 된 메뉴에 대 한 상황에 맞는 메뉴를 제공할 수 있습니다.
...
- MF_DISABLED 항목이 비활성화 됩니다.

- MF_GRAYED 항목이 흐리게 표시 됩니다.

- 마우스를 사용 하 여 MF_MOUSESELECT 항목을 선택 합니다.

- MF_OWNERDRAW는 소유자 그리기 항목입니다.
...
경우 *nFlags* 0xFFFF를 포함 하 고 *hSysMenu* , 0이 포함 된 사용자 ESC 키를 누르거나 메뉴 밖을 클릭 하기 때문에 Windows에서 메뉴를 닫았습니다.
...
활성화 여부를 지정 합니다 `CWnd` 및 마우스 이벤트 취소 여부. 다음 값 중 하나 여야 합니다.

- MA_ACTIVATE 활성화 `CWnd` 개체입니다.

- 활성화 하지 않는 MA_NOACTIVATE `CWnd` 개체입니다.

- MA_ACTIVATEANDEAT 활성화 `CWnd` 개체 및 마우스 이벤트를 삭제 합니다.

- 활성화 하지 않는 MA_NOACTIVATEANDEAT `CWnd` 개체 및 마우스 이벤트를 삭제 합니다.

### <a name="remarks"></a>설명

기본 구현 처리 되기 전에 부모 창에이 메시지를 전달 합니다. 부모 창에서 TRUE를 반환 하는 경우 처리가 중지 됩니다.

개별 적중 테스트 영역 코드에 대 한 참조를 [OnNcHitTest](#onnchittest) 멤버 함수
...
프레임 워크에 대 한 이전 호출에 지정 된 기간 동안 커서로 창의 클라이언트 영역 가리킬 때이 멤버 함수를 호출 [TrackMouseEvent](http://msdn.microsoft.com/library/windows/desktop/ms646265)합니다.
...
이 메서드를 수신 합니다 [WM_MOUSEHOVER](http://msdn.microsoft.com/library/windows/desktop/ms645613) Windows SDK에 설명 된 알림.

합니다 *nFlags* 매개 변수는 다음 표에 나열 된 한정자 키 조합 될 수 있습니다. 자세한 내용은 [마우스 입력에 대 한](http://msdn.microsoft.com/library/windows/desktop/ms645601)합니다.
...
현재 창에서을 관리자 DWM (데스크톱 창)에서 구성 됩니다 하 고 해당 창이 최대화 될 때이 멤버를 호출 하는 프레임 워크입니다.
...
|매개 변수|설명|
|---------------|-----------------|
|[in] *nFlags*|한정자 키를 누르면 여부를 나타내는 플래그의 비트 조합 (OR)입니다. 예를 들어 MK_CONTROL 플래그 CTRL 키를 눌렀음을 나타냅니다.<br /><br /> 플래그 목록은 참조에서 "메시지 매개 변수" 부제목 [마우스 입력에 대 한](http://msdn.microsoft.com/library/windows/desktop/ms645601)합니다.|
...
이 메서드를 수신 합니다 [WM_MOUSEHWHEEL](http://msdn.microsoft.com/library/windows/desktop/ms645614) Windows SDK에 설명 된 알림 메시지입니다. 이 메시지는 가로 스크롤 휠을 기울이거나 회전할 때 포커스를가지고 있는 창으로 전송 됩니다.
...
커서에 대 한 이전 호출에서 지정 된 창의 클라이언트 영역을 벗어날 때 프레임 워크를이 멤버 함수 호출 [TrackMouseEvent](http://msdn.microsoft.com/library/windows/desktop/ms646265)합니다.
...
이 메서드를 수신 합니다 [WM_MOUSELEAVE](http://msdn.microsoft.com/library/windows/desktop/ms645615) Windows SDK에 설명 된 알림.
...
프레임 워크가 마우스 커서를 이동 하는 경우이 멤버 함수를 호출 합니다.
...
WM_MOUSEMOVE 메시지를 수신한 마우스를 캡처하지 않으면 경우는 `CWnd` 마우스 커서; 아래에 있는 개체 메시지 창에 마우스를 캡처로 이동 하는 고, 그렇지 합니다.
...
프레임 워크는 사용자가 마우스 휠을 굴 리 및 휠을 다음 번 발생 하는 대로이 멤버 함수를 호출 합니다.
...
마우스 휠 스크롤을 사용 하면 0이 아닌 값 그렇지 않으면 0입니다.

### <a name="remarks"></a>설명

재정의 되지 않으면 `OnMouseWheel` 기본값인 호출 [WM_MOUSEWHEEL](http://msdn.microsoft.com/library/windows/desktop/ms645617)합니다. Windows는 자동으로 포커스가 있는 컨트롤이 나 자식 창에 메시지를 라우팅합니다. Win32 함수 [DefWindowProc](http://msdn.microsoft.com/library/windows/desktop/ms633572) 처리 하는 창에 부모 체인은 메시지를 전파 합니다.
...
WHEEL_DELTA 자유롭게 회전 휠이 없는 조건으로 같은 더 좋은 해상도 바퀴 있도록 120으로 설정 되었습니다. 더 좋은 해상도 휠 회전 당 더 많은 메시지 보내지만 각 메시지에 더 작은 델타 값입니다. 들어오는 추가 하거나 그러한 바퀴를 사용 하려면 *zDelta* 값까지 WHEEL_DELTA (받을 수 있도록 동일한 응답 델타 회전을 지정된에 대 한), 또는 자주 메시지에 대 한 응답에서 부분 줄 스크롤. 또한 스크롤 세분성을 선택할 수 있으며 WHEEL_DELTA에 도달할 때까지 델타를 누적 합니다.

고유한 마우스 휠 스크롤 동작을 제공 하려면이 멤버 함수를 재정의 합니다.
...
프레임 워크가 호출 후에이 멤버 함수는 `CWnd` 개체 이동 되었습니다.
...
비클라이언트 영역을 활성 또는 비활성 상태를 나타내기 위해 변경 해야 하는 경우이 멤버 함수를 호출 하는 프레임 워크입니다.
...
Windows 기본 처리를 계속 해야 하는 경우 0이 아닌 값 비활성화 하 고에서 아이콘을 캡션 표시줄을 방지 하기 위해 0입니다.

### <a name="remarks"></a>설명

기본 구현은 그립니다 제목 표시줄 및 제목 표시줄 텍스트를 현재 색 경우 *bActive* 는 TRUE 이며 해당 비활성 색 경우 *bActive* 은 FALSE.
...
클라이언트 영역의 위치와 크기를 계산 해야 하는 경우이 멤버 함수를 호출 하는 프레임 워크입니다.
...
이 메시지를 처리 하 여 크기 또는 창의 위치를 변경 하는 경우 응용 프로그램 창의 클라이언트 영역의 내용을 제어할 수 있습니다.

값에 관계 없이 *bCalcValidRects*, 지정 된 배열의 첫 번째 사각형을 `rgrc` 구조체 멤버의는 `NCCALCSIZE_PARAMS` 구조 창의 좌표를 포함 합니다. 자식 창의 부모 창의 클라이언트 영역을 기준으로 좌표 있습니다. 최상위 창에 대 한 좌표는 화면 좌표입니다. 응용 프로그램을 수정 해야 합니다 `rgrc[0]` 사각형 크기와 클라이언트 영역의 위치를 반영 하도록 합니다.

`rgrc[1]` 하 고 `rgrc[2]` 사각형은 유효한 경우에만 *bCalcValidRects* 가 TRUE입니다. 이 경우에 `rgrc[1]` 사각형을 이동 하거나 크기를 조정 하기 전에 창의 좌표를 포함 합니다. `rgrc[2]` 사각형 창 옮겨졌습니다 전에 창의 클라이언트 영역의 좌표를 포함 합니다. 모든 좌표가 부모 창이 나 화면을 기준으로 하는 것입니다.

기본 구현 (스크롤 막대, 메뉴 및 등의 현재 상태), 창 특징을 기반으로 클라이언트 영역의 크기를 계산 하 고 고 결과가 *lpncsp*합니다.
...
이전에이 멤버 함수를 호출 하는 프레임 워크를 [WM_CREATE](#oncreate) 메시지는 `CWnd` 개체가 먼저 만들어진 합니다.
...
비클라이언트 영역을 만드는 경우에 0이 아닙니다. 오류가 발생 하는 경우 0입니다. 합니다 `Create` 함수는 반환 **오류** 이 경우.
...
비클라이언트 영역이 소멸 되 고, 및 Windows 창을 소멸 될 때 마지막 멤버 함수가 호출 되는 경우 프레임 워크에서 호출 됩니다.
...
기본 구현은 일부 정리를 수행한 다음 가상 멤버 함수를 호출 [PostNcDestroy](#postncdestroy)합니다.

재정의 `PostNcDestroy` 와 같은 고유한 정리를 수행 하려는 경우는 **삭제** 작업 합니다. 재정의 하는 경우 `OnNcDestroy`를 호출 해야 `OnNcDestroy` 창이 해제에 대해 내부적으로 할당 된 메모리 되도록 기본 클래스에서.

##  <a name="onnchittest"></a>  CWnd::OnNcHitTest

프레임 워크에 대 한이 멤버 함수를 호출 합니다 `CWnd` 커서가 포함 된 개체 (또는 `CWnd` 사용 하는 개체에 [SetCapture](#setcapture) 멤버 함수는 마우스 입력 캡처를) 마우스를 이동할 때마다.
...
프레임 워크의 비클라이언트 영역 내 커서가 있는 동안 마우스 왼쪽된 단추를 클릭할 때이 멤버 함수를 호출 `CWnd`합니다.
...
해당 하는 경우는 [WM_SYSCOMMAND](#onsyscommand) 전송 됩니다.
...
프레임 워크의 비클라이언트 영역 내에 커서를 하는 동안 사용자가 왼쪽된 마우스 단추를 누르면이 멤버 함수를 호출 합니다 `CWnd` 개체입니다.
...
해당 하는 경우는 [WM_SYSCOMMAND](#onsyscommand) 보내집니다.
...
커서가 비클라이언트 영역 내에서 있는 동안 마우스 왼쪽된 단추를 놓을 때 프레임 워크에서이 멤버 함수를 호출 합니다.
...
해당 하는 경우 [WM_SYSCOMMAND](#onsyscommand) 보내집니다.
...
커서가 비클라이언트 영역 내에서 있는 동안 마우스 가운데 단추를 클릭할 때 프레임 워크에서이 멤버 함수를 호출 합니다.
...
비클라이언트 영역 내에서 커서를 사용자가 마우스 가운데 단추를 누를 때 프레임 워크에서이 멤버 함수를 호출 합니다.
...
비클라이언트 영역 내에서 커서를 사용자가 가운데 마우스 단추를 놓으면이 멤버 함수를 호출 하는 프레임 워크입니다.
...
프레임 워크에 대 한 이전 호출에 지정 된 기간 동안 커서로 창의 비클라이언트 영역 가리킬 때이 멤버 함수를 호출 [TrackMouseEvent](http://msdn.microsoft.com/library/windows/desktop/ms646265)합니다.
...
|매개 변수|설명|
|---------------|-----------------|
|[in] *nHitTest*|적중 테스트 값을 반환 합니다 [CWnd::DefWindowProc](#defwindowproc) 처리의 결과로 함수는 [WM_NCHITTEST](http://msdn.microsoft.com/library/windows/desktop/ms645618) 메시지입니다.|
|[in] *지점*|A [CPoint](../../atl-mfc-shared/reference/cpoint-class.md) 지정 하는 개체를 *x* 하 고 *y* 화면의 왼쪽 위 모퉁이 기준으로 커서의 좌표가 합니다.|

### <a name="remarks"></a>설명

이 메서드를 수신 합니다 [WM_NCMOUSEHOVER](http://msdn.microsoft.com/library/windows/desktop/ms645625) Windows SDK에 설명 된 알림.
...
커서에 대 한 이전 호출에서 지정 된 창의 비클라이언트 영역을 벗어나면 프레임 워크가이 멤버 함수를 호출 [TrackMouseEvent](http://msdn.microsoft.com/library/windows/desktop/ms646265)합니다.
...
이 메서드를 수신 합니다 [WM_NCMOUSELEAVE](http://msdn.microsoft.com/library/windows/desktop/ms645626) Windows SDK에 설명 된 알림.
...
비클라이언트 영역 내에서 커서를 이동 하면이 멤버 함수를 호출 하는 프레임 워크입니다.
...
비클라이언트 영역을 그릴 해야 할 때 프레임 워크를이 멤버 함수를 호출 합니다.
...
응용 프로그램이이 호출을 재정의 하 고 자체 사용자 지정 창 프레임을 그릴 수 있습니다. 프레임의 셰이프를 변경할 경우에 클리핑 영역을 사각형에 항상입니다.

##  <a name="onncrbuttondblclk"></a>  CWnd::OnNcRButtonDblClk

프레임 워크의 비클라이언트 영역 내에 커서를 마우스 오른쪽 단추를 클릭할 때이 멤버 함수를 호출 `CWnd`합니다.
...
비클라이언트 영역 내에서 커서를 마우스 오른쪽 단추를 누를 때 프레임 워크에서이 멤버 함수를 호출 합니다.
...
비클라이언트 영역 내에서 커서를 마우스 오른쪽 단추를 놓을 때 프레임 워크에서이 멤버 함수를 호출 합니다.
...
이 메서드를 수신 합니다 [WM_DWMNCRENDERINGCHANGED](http://msdn.microsoft.com/library/windows/desktop/dd388200) Windows SDK에 설명 된 알림.
...
프레임 워크는 사용자가 XBUTTON1 또는 xbutton2를 놓으면 커서가 창의 비클라이언트 영역에 있는 동안이 멤버 함수를 호출 합니다.
...
이 메서드를 수신 합니다 [WM_XBUTTONDBLCLK](http://msdn.microsoft.com/library/windows/desktop/ms646244) Windows SDK에 설명 된 알림. 이 메시지는 커서가 포함 된 창에 게시 됩니다. 창에 마우스를 캡처한 경우이 메시지를 게시 되지 않습니다.
...
커서가 창의 비클라이언트 영역에 있는 동안 마우스의 XBUTTON1 또는 XBUTTON2를 누를 때 프레임 워크에서이 멤버 함수를 호출 합니다.
...
이 메서드를 수신 합니다 [WM_NCXBUTTONDOWN](http://msdn.microsoft.com/library/windows/desktop/ms645632) Windows SDK에 설명 된 알림. 이 메시지는 커서가 포함 된 창에 게시 됩니다. 창에 마우스를 캡처한 경우이 메시지를 게시 되지 않습니다.
...
커서가 창의 비클라이언트 영역에 있는 동안 마우스의 XBUTTON1 또는 XBUTTON2를 해제 하는 경우이 멤버 함수를 호출 하는 프레임 워크입니다.
...
이 메서드를 수신 합니다 [WM_NCXBUTTONUP](http://msdn.microsoft.com/library/windows/desktop/ms646240) Windows SDK에 설명 된 알림. 이 메시지는 커서가 포함 된 창에 게시 됩니다. 창에 마우스를 캡처한 경우이 메시지를 게시 되지 않습니다.
...
|매개 변수|설명|
|---------------|-----------------|
|[in] *nKey*|한정자 키를 누르면 여부를 나타내는 플래그의 비트 조합 (OR)입니다. 예를 들어 MK_CONTROL 플래그 CTRL 키를 눌렀음을 나타냅니다.<br /><br /> 플래그 목록은 참조에서 "메시지 매개 변수" 부제목 [마우스 입력에 대 한](http://msdn.microsoft.com/library/windows/desktop/ms645601)합니다.|
|[in] *lpMdiNextMenu*|에 대 한 포인터를 [MDINEXTMENU](http://msdn.microsoft.com/library/windows/desktop/ms647561) 활성화할 메뉴에 대 한 정보를 포함 하는 구조입니다.|

### <a name="remarks"></a>설명

이 메서드를 수신 합니다 [WM_UNINITMENUPOPUP](http://msdn.microsoft.com/library/windows/desktop/ms647614) Windows SDK에 설명 된 알림. 이 메시지에 응용 프로그램을 설정할 수는 `hmenuNext` 의 멤버는 [MDINEXTMENU](http://msdn.microsoft.com/library/windows/desktop/ms647561) 전환 메뉴를 지정 하는 구조 및 `hwndNext` 창을 메뉴 알림 메시지를 지정 하는 멤버 .
...
프레임 워크는 컨트롤의 이벤트 되었음을 또는 컨트롤 몇 가지 종류의 정보에 필요한 컨트롤의 부모 창에 알리기 위해이 멤버 함수를 호출 합니다.
...
WM_NOTIFY 메시지를 처리 하려면 파생된 클래스에서이 멤버 함수를 재정의 합니다. 재정의 메시지 맵을 처리할 하지 않는 한 기본 클래스 `OnNotify` 라고 합니다.

WM_NOTIFY 메시지에 대 한 자세한 내용은 기술 참고 61 (TN061)을 참조 하세요 [ON_NOTIFY 및 WM_NOTIFY 메시지](../../mfc/tn061-on-notify-and-wm-notify-messages.md)합니다. 에 설명 된 관련된 항목을 참조할 수도 [제어 항목](../../mfc/controls-mfc.md), 및 TN062 합니다 [Windows 컨트롤에 대 한 메시지 리플렉션](../../mfc/tn062-message-reflection-for-windows-controls.md)합니다.

##  <a name="onnotifyformat"></a>  CWnd::OnNotifyFormat

프레임 워크가 현재 창 WM_NOTIFY 알림 메시지의 ANSI 또는 유니코드 구조를 허용 하는 경우를 결정 하는이 멤버 함수를 호출 합니다.
...
|매개 변수|설명|
|---------------|-----------------|
|[in] *pWnd*|에 대 한 포인터를 `CWnd` 전송 창을 나타내는 개체를 [WM_NOTIFY](http://msdn.microsoft.com/library/windows/desktop/bb775583) 메시지입니다.<br /><br /> 경우이 매개 변수는 컨트롤에 대 한 포인터를 *된 명령* 매개 변수는 NF_QUERY, 또는 컨트롤의 부모 창에 대 한 포인터 경우 *된 명령* NF_REQUERY 됩니다.|
...
|반환 값|의미|
|------------------|-------------|
|NFR_ANSI|컨트롤에서 보낸 WM_NOTIFY 메시지의 ANSI 구조체를 사용 해야 합니다.|
|NFR_UNICODE|컨트롤에서 보낸 WM_NOTIFY 메시지에서 유니코드 구조체를 사용 해야 합니다.|
|0|오류가 발생했습니다.|

### <a name="remarks"></a>설명

이 메서드를 수신 합니다 [WM_NOTIFYFORMAT](http://msdn.microsoft.com/library/windows/desktop/bb775584) Windows SDK에 설명 된 알림. 해당 부모 창에 공용 컨트롤 및 공용 컨트롤을 부모 창에서 WM_NOTIFY 메시지 전송 됩니다.
...
프레임 워크는 Windows 또는 응용 프로그램에는 응용 프로그램 창의 일부를 다시 그리기를 요청 하면이 멤버 함수를 호출 합니다.
...
[WM_PAINT](http://msdn.microsoft.com/library/windows/desktop/dd145137) 메시지를 보낼 때 합니다 [UpdateWindow](#updatewindow) 또는 [RedrawWindow](#redrawwindow) 멤버 함수를 호출 합니다.

창을 호출의 결과로 내부 그리기 메시지를 받을 수는 `RedrawWindow` 멤버 함수는 RDW_INTERNALPAINT 사용 하 여 플래그를 설정 합니다. 이 경우 창의 업데이트 영역을 가질 수는 없습니다. 응용 프로그램에서 호출 해야 합니다 [GetUpdateRect](#getupdaterect) 창에는 업데이트 영역에 있는지 여부를 결정 하는 멤버 함수입니다. 하는 경우 `GetUpdateRect` 0을 응용 프로그램 반환을 호출 하지 않아야 합니다 [BeginPaint](#beginpaint) 하 고 [EndPaint](#endpaint) 멤버 함수입니다.

모든 필요한 내부 그려질 또는 잘못 된 영역 및 호출 하 여 WM_PAINT메시지를발생하기때문에각WM_PAINT메시지에대한해당내부데이터구조를확인하여업데이트를확인해야하는응용프로그램의`RedrawWindow`멤버 함수는 RDW_INTERNALPAINT 사용 하 여 플래그를 설정 합니다.

내부 WM_PAINT 메시지를 Windows에서 한 번만 전송 됩니다. 내부 WM_PAINT 후 메시지를 하 여 창을 합니다 `UpdateWindow` 멤버 함수를 더 이상 WM_PAINT 메시지를 보내거나 창을 무효화 될 때까지 또는 될 때까지 게시를 `RedrawWindow` 멤버 함수는 RDW_INTERNALPAINT 다시 호출 됩니다 플래그 집합입니다.

문서/뷰 응용 프로그램에서 이미지 렌더링에 대 한 내용은 참조 하세요 [cview::](../../mfc/reference/cview-class.md#ondraw)합니다.

사용에 대 한 자세한 내용은 `WM_Paint`, Windows SDK의 다음 항목을 참조 하세요.
...
클립보드 소유자의 `OnPaintClipboard` 멤버 함수는 클립보드 소유자 CF_OWNERDISPLAY 형식으로 클립보드에 데이터를 배치 하 고 클립보드 뷰어의 클라이언트 영역을 다시 그려야 할 때 클립보드 뷰어에서 호출 됩니다.
...
전체 클라이언트 영역 또는 그 일부만 그려야 할지 결정할 클립보드 소유자에 지정 된 그리기 영역의 크기를 비교 해야 합니다는 `rcpaint` 의 멤버는 `PAINTSTRUCT` 가장 최근의 에지정된구조체[ OnSizeClipboard](#onsizeclipboard) 멤버 함수 호출 합니다.

`OnPaintClipboard` 사용 해야는 [GlobalLock](http://msdn.microsoft.com/library/windows/desktop/aa366584) Windows 함수를 포함 하는 메모리를 잠그려면를 `PAINTSTRUCT` 데이터 구조 및 사용 하 여 해당 메모리의 잠금을 해제 합니다 [GlobalUnlock](http://msdn.microsoft.com/library/windows/desktop/aa366595) 종료 되기 전에 Windows 함수입니다.
...
이 호출은 해당 논리 팔레트를 실현 하 고 해당 클라이언트 영역을 업데이트 하는 색상표를 사용 하는 입력된 포커스가 없는 창 수 있습니다.

`OnPaletteChanged` 시스템 색상표를 변경 하 고 보낼 WM_PALETTECHANGED 메시지가 발생 하는 것을 포함 하 여 모든 최상위 및 겹쳐진 창, 멤버 함수를 호출 합니다. 모든 자식 창에 색 색상표를 사용 하는 경우이 메시지 전달 하 여 되어야 합니다.

무한 루프를 방지 하려면 결정 하는 경우가 아니면 창 팔레트가 실현 하지 않아야 *pFocusWnd* 자체에 대 한 포인터를 포함 하지 않습니다.
...
프레임 워크는 응용 프로그램은 논리 팔레트가 나타나도록 하려는 응용 프로그램에 알리기 위해이 멤버 함수를 호출 합니다.
...
부모의 `OnParentNotify` 멤버 함수는 해당 자식 창을 만들거나 제거할 때 또는 커서가 자식 창 위에 있는 동안 마우스 단추를 클릭할 때 프레임 워크에서 호출 됩니다.
...
- WM_DESTROY 자식 창을 소멸 되 고 있습니다.

- 사용자 WM_LBUTTONDOWN 자식 창 위에 마우스 커서를 배치 있으며 마우스 왼쪽된 단추를 클릭 합니다.

- WM_MBUTTONDOWN 사용자에 자식 창 위에 마우스 커서를 배치 하 고 마우스 가운데 단추를 클릭 합니다.
...
자식 창에는 만들어질 때 시스템 호출 `OnParentNotify` 직전 합니다 [만들기](#create) 창을 만듭니다. 멤버 함수를 반환 합니다. 자식 창을 소멸 될 때, 시스템 호출 `OnParentNotify` 처리를 수행 하기 전에 창을 소멸 하 합니다.

`OnParentNotify` 최상위 창을 포함 하 여 자식 창의 모든 상위 항목 창에 대해 호출 됩니다.

모든 자식 창은 해당 제외 하 고는 [WS_EX_NOPARENTNOTIFY](../../mfc/reference/styles-used-by-mfc.md#extended-window-styles) 스타일 부모 창에이 메시지를 보냅니다. 기본적으로 대화 상자에서 자식 창에 WS_EX_NOPARENTNOTIFY 스타일 자식 창을 호출 하 여이 스타일 없이 생성 된 경우가 아니면 합니다 [CreateEx](#createex) 멤버 함수입니다.
...
이 이벤트는 요청, TRUE를 반환 하는 경우에 요청을 거부 하도록 요청 또는 BROADCAST_QUERY_DENY 부여 합니다.

### <a name="remarks"></a>설명

이 메서드를 수신 합니다 [WM_POWERBROADCAST](http://msdn.microsoft.com/library/windows/desktop/aa373247) Windows SDK에 설명 된 메시지입니다.

합니다 *nPowerEvent* 배터리 양이 부족 같은 매개 변수 지정 이벤트, 전원 상태가 변경, 권한 작업을 일시 중단 요청 또는 거부, 이벤트 후 작업을 자동으로 다시 시작, 시스템 일시 중단 되었다가 재개 되 고 작업 또는 작업을 일시 중단 합니다. 합니다 *nEventData* 매개 변수는 일반적으로 사용 되지 않습니다. 자세한 내용은 참조는 *wParam* 및 *lParam* 의 매개 변수는 [WM_POWERBROADCAST](http://msdn.microsoft.com/library/windows/desktop/aa373247) 메시지입니다.
...
하위 워드에서 아이콘 또는 커서 핸들을 포함 하는 워드 값입니다. 아이콘 또는 커서를 디스플레이 드라이버의 해상도와 호환 되어야 합니다. 응용 프로그램에서 NULL을 반환 하면 기본 커서를 자동으로 표시 됩니다. 기본 반환 값은 NULL입니다.

### <a name="remarks"></a>설명

시스템에서는이 호출을 최소화 된 창을 끄는 동안 표시할 커서를 가져옵니다. 아이콘 또는 커서의 핸들을 반환 하는 응용 프로그램, 시스템 흑백으로 변환 합니다. 응용 프로그램에는 핸들을 반환 하는 경우 핸들 단색 커서 또는 디스플레이 드라이버의 해상도와 호환 되는 아이콘을 식별 해야 합니다. 응용 프로그램에서 호출할 수는 [CWinApp::LoadCursor](../../mfc/reference/cwinapp-class.md#loadcursor) 또는 [CWinApp::LoadIcon](../../mfc/reference/cwinapp-class.md#loadicon) 실행 파일의 리소스에서 아이콘 또는 커서를 로드 하 고이 핸들을 가져오지 멤버 함수입니다.

##  <a name="onqueryendsession"></a>  CWnd::OnQueryEndSession

사용자가 Windows 세션 또는 응용 프로그램을 호출 하면 종료 하도록 선택 하면 프레임 워크가이 멤버 함수를 호출 합니다 [ExitWindows](http://msdn.microsoft.com/library/windows/desktop/aa376867) Windows 함수입니다.
...
0을 반환 하는 모든 응용 프로그램을 Windows 세션을 종료 하지 않습니다. 호출을 중지 하는 Windows `OnQueryEndSession` 응용 프로그램 0을 반환 하 고 보내는 즉시 합니다 [WM_ENDSESSION](#onendsession) 메시지에서 이미 0이 아닌 반환 된 모든 응용 프로그램에 대 한 매개 변수 값은 FALSE.

##  <a name="onquerynewpalette"></a>  CWnd::OnQueryNewPalette

이 멤버 함수를 호출 하는 프레임 워크 때 합니다 `CWnd` 개체를 입력된 포커스를 받을 제공는 `CWnd` 포커스를 받으면 논리 팔레트가 나타나도록 수.
...
0이 아닌 값을 `CWnd` 해당 논리를 인식 색상표 그렇지 않으면 0입니다.

##  <a name="onqueryopen"></a>  CWnd::OnQueryOpen

이 멤버 함수를 호출 하는 프레임 워크 때를 `CWnd` 개체를 최소화 하 고 사용자가 요청 하는 `CWnd` preminimized 크기와 위치로 복원 합니다.
...
아이콘 열 수를 0이 아닌 또는 열려는에서 아이콘을 방지 하는 0입니다.

### <a name="remarks"></a>설명

에 있는 동안 `OnQueryOpen`, `CWnd` (예: 대화 상자 만들기)를 활성화 또는 포커스 변경 시키는 모든 작업을 수행 하지 않아야 합니다.
...
포커스 표시기 및 키보드 액셀러레이터 키 표시 되는 경우 반환 값은 NULL입니다. 그렇지 않으면 반환 값은 다음 값 중 하나 이상의 수 있습니다.
...
이 멤버 함수는의 기능을 에뮬레이션 합니다 [WM_QUERYUISTATE](http://msdn.microsoft.com/library/windows/desktop/ms646355) Windows SDK에 설명 된 대로 메시지입니다.

##  <a name="onrawinput"></a>  CWnd::OnRawInput

프레임 워크가 현재 창 원시 입력을 가져올 때이 멤버 함수를 호출 합니다.
...
|[in] *hRawInput*|에 대 한 핸들을 [RAWINPUT](http://msdn.microsoft.com/library/windows/desktop/ms645562) 장치의 원시 입력을 포함 하는 구조입니다.|

### <a name="remarks"></a>설명

이 메서드를 수신 합니다 [WM_INPUT](http://msdn.microsoft.com/library/windows/desktop/ms646275) Windows SDK에 설명 된 알림.
...
프레임 워크를 마우스 오른쪽 단추를 두 번 클릭할 때이 멤버 함수를 호출 합니다.
...
- 마우스 단추를 두면 MK_LBUTTON 설정 중단 되었습니다.

- 마우스 가운데 단추를 눌렀음을 MK_MBUTTON 설정 합니다.

- 마우스 오른쪽 단추를 눌렀음을 MK_RBUTTON 설정 합니다.
...
CS_DBLCLKS 있는 창만 [WNDCLASS](http://msdn.microsoft.com/library/windows/desktop/ms633576) 스타일을 받을 수 `OnRButtonDblClk` 호출 합니다. 이것이 Microsoft Foundation Class 라이브러리 내에서 windows에 대 한 기본값입니다. Windows 호출 `OnRButtonDblClk` , 릴리스를 클릭 하 고 다시 시스템 내에서 마우스 오른쪽 단추를 누를 때 제한 시간을 두 번 클릭 합니다. 4 개의 이벤트를 생성 실제로 마우스 오른쪽 단추를 두 번 클릭: [WM_RBUTTONDOWN](#onrbuttondown) 하 고 [WM_RBUTTONUP](#onrbuttonup) 메시지를 `OnRButtonDblClk` 호출과 다른 WM_RBUTTONUP 메시지 단추 때 해제 합니다.
...
프레임 워크를 마우스 오른쪽 단추를 놓을 때이 멤버 함수를 호출 합니다.
...
이 이번에 불필요 한 합니다. 항상 0입니다.

### <a name="remarks"></a>설명

재정의 되지 않으면 `OnRegisteredMouseWheel` 적절 한 window (부모 창 포커스를 사용 하 여) 및 호출 메시지를 라우팅하는 [WM_MOUSEWHEEL](http://msdn.microsoft.com/library/windows/desktop/ms645617) 해당 창에 대 한 처리기입니다.

마우스 휠 스크롤 동작을 변경 하거나 고유한 메시지 라우팅을 제공 하기에이 멤버 함수를 재정의 합니다.

> [!NOTE]
> `OnRegisteredMouseWheel` Windows 95/98 및 Windows NT 3.51에 대 한 메시지를 처리합니다. Windows NT 4.0 메시지 처리를 위해 사용 하 여 [OnMouseWheel](#onmousewheel)합니다.

##  <a name="onrenderallformats"></a>  CWnd::OnRenderAllFormats

클립보드 소유자의 `OnRenderAllFormats` 소유자 응용 프로그램이 소멸 될 때 멤버 함수는 프레임 워크에서 호출 됩니다.
...
클립보드 소유자를 생성 하는 수 고를 호출 하 여 클립보드에 각 형식에 대 한 데이터 핸들을 전달 된 모든 형식에서 데이터를 렌더링 해야 합니다 [SetClipboardData](http://msdn.microsoft.com/library/windows/desktop/ms649051) Windows 함수입니다. 이렇게 하면 클립보드 데이터를 렌더링 하는 응용 프로그램은 제거 하는 경우에 올바른 데이터를 포함 합니다. 응용 프로그램을 호출 해야 합니다 [OpenClipboard](#openclipboard) 멤버 함수 호출 하기 전에 [SetClipboardData](http://msdn.microsoft.com/library/windows/desktop/ms649051) Windows 함수를 호출 합니다 [CloseClipboard](http://msdn.microsoft.com/library/windows/desktop/ms649035) Windows 나중에 함수입니다.

##  <a name="onrenderformat"></a>  CWnd::OnRenderFormat

클립보드 소유자의 `OnRenderFormat` 멤버 함수는 지연 된 렌더링을 사용 하 여 특정 형식으로 렌더링 해야 할 때 프레임 워크에서 호출 됩니다.
...
받는 사람을 해당 형식으로 데이터를 렌더링 하 고 호출 하 여 클립보드에 전달 된 [SetClipboardData](http://msdn.microsoft.com/library/windows/desktop/ms649051) Windows 함수입니다.

호출 하지 마십시오 합니다 `OpenClipboard` 멤버 함수 또는 `CloseClipboard` 내에서 Windows 함수 `OnRenderFormat`합니다.
...
프레임 워크가 세션 상태의 변경 응용 프로그램에 알리기 위해이 멤버 함수를 호출 합니다.
...
프레임 워크가 마우스 커서 움직임 내에서 사용 하면 마우스 입력이 캡처되지 않은 경우이 멤버 함수를 호출 합니다 `CWnd` 개체입니다.
...
계속 하려면 0 또는 추가 처리를 중지 하려면 0이 아닙니다.

### <a name="remarks"></a>설명

기본 구현 호출 부모 창의 `OnSetCursor` 처리 하기 전에 합니다. 부모 창에서 TRUE를 반환 하는 경우 추가 처리가 중지 됩니다. 부모 창에 호출 제어할 부모 창의 자식 창에서 커서의 설정은 있습니다.

기본 구현에서는 커서를 설정 클라이언트 영역에 없는 경우 화살표 또는 등록 클래스 커서 경우.

경우 *nHitTest* HTERROR는 및 *메시지* 마우스 button-down 메시지는 `MessageBeep` 멤버 함수를 호출 합니다.

합니다 *메시지* 매개 변수가 0 인 경우 `CWnd` 메뉴 모드를 시작 합니다.
...
입력된 포커스를 확보 한 후이 멤버 함수를 호출 하는 프레임 워크입니다.
...
프레임 워크 호출 `OnSettingChange` Win32 SystemParametersInfo 함수는 시스템 차원의 설정 변경 될 때 모든 최상위 창에 대 한 합니다.
...
*uFlags*  
시스템의 결과로 메시지를 전송 하는 경우는 `SystemParametersInfo` 호출에서이 매개 변수는 변경 된 system 매개 변수를 나타내는 플래그입니다. 값의 목록을 참조 하세요 [SystemParametersInfo](http://msdn.microsoft.com/library/windows/desktop/ms724947) Windows SDK에 있습니다. 응용 프로그램 메시지를 보내면이 매개 변수는 0 이어야 합니다.
...
응용 프로그램 시스템 매개 변수를 변경 하면 사용자가 제어판을 통해 설정을 변경 하면 Windows에서 메시지를 전송할 때 모든 최상위 창에 메시지를 전송 해야 합니다.
...
프레임 워크가이 멤버 함수를 호출 하면는 `CWnd` 개체를 숨기 거 나 표시 하 려 합니다.
...
- SW_PARENTOPENING 부모 창을 열고 (표시 됨) 또는 팝업 창이 표시 됩니다.

### <a name="remarks"></a>설명

창을 숨기 거 나 때 표시 되는 `ShowWindow` 구성원 함수가 호출 되는 overlapped 또는 팝업 창의 닫을 때 또는 겹친된 창을 최대화 또는 복원 하는 경우 (됨 아이콘) 하거나 (화면에 표시 된) 열입니다. 겹친된 창을 종료 되 면 해당 창과 연결 된 모든 팝업 창은 숨겨져 있습니다.
...
- SIZE_MINIMIZED 창은 최소화 된 합니다.

- SIZE_RESTORED 창의 크기가 조정 된, 있지만 SIZE_MINIMIZED 아니고 SIZE_MAXIMIZED 적용 됩니다.

- 일부 다른 창이 최대화 되어 SIZE_MAXHIDE을 모든 팝업 창으로 전송 됩니다.
...
경우는 [SetScrollPos](#setscrollpos) 또는 [MoveWindow](#movewindow) 멤버 함수에서 자식 창의 라고 `OnSize`, `bRedraw` 의 매개 변수 `SetScrollPos` 또는 `MoveWindow` 에 0이 아닌 값 이어야 합니다 발생 된 `CWnd` 다시 그려져 야 합니다.
...
클립보드 소유자 `OnSizeClipboard` 클립보드 데이터를 포함 하는 경우 클립보드 뷰어에서 구성원 함수가 호출 되는 `CF_OWNERDISPLAY` 특성 및 클립보드 뷰어 창의 클라이언트 영역 크기가 변경 되었습니다.
...
`OnSizeClipboard` 구성원 함수가 호출 되는 null 사각형 (0,0,0,0)를 사용 하 여 새 크기로 클립보드 응용 프로그램이 최소화 되거나 소멸 될 때입니다. 이렇게 하면 클립보드 소유자의 표시 리소스를 해제 합니다.

내 `OnSizeClipboard`, 응용 프로그램을 사용 해야 합니다 [GlobalLock](http://msdn.microsoft.com/library/windows/desktop/aa366584) RECT 데이터 구조를 포함 하는 메모리를 잠그려면 Windows 함수. 응용 프로그램을 사용 하 여 해당 메모리의 잠금을 해제 합니다 [GlobalUnlock](http://msdn.microsoft.com/library/windows/desktop/aa366595) Windows 함수를 생성 하거나 컨트롤을 반환 합니다.
...
프레임 워크는 사용자가 크기를 조정 사각형을 표시 하려면이 멤버 함수를 호출 합니다.
...
이 메시지를 처리 하 여 응용 프로그램 끌기 사각형의 위치와 크기를 모니터링 하 고, 필요한 경우 변경할 수의 크기 또는 위치입니다.
...
프레임 워크는 작업이 추가 되거나 인쇄 관리자 큐에서 제거 될 때마다 인쇄 관리자에서이 멤버 함수를 호출 합니다.
...
프레임 워크가 후이 멤버 함수를 호출 합니다 [SetWindowLong](http://msdn.microsoft.com/library/windows/desktop/ms633591) 함수는 하나 이상의 창 스타일 변경 되었습니다.
...
*lpStyleStruct*  
가리키는 [STYLESTRUCT](http://msdn.microsoft.com/library/windows/desktop/ms632607) 창에 대 한 새 스타일을 포함 하는 구조입니다. 응용 프로그램 스타일을 검사할 수 있지만 변경 되지 않습니다 수 있습니다.
...
이 멤버 함수를 호출 하는 프레임 워크 때 합니다 [SetWindowLong](http://msdn.microsoft.com/library/windows/desktop/ms633591) 함수는 하나 이상의 창 스타일을 변경 하려고 합니다.
...
*lpStyleStruct*  
가리키는 [STYLESTRUCT](http://msdn.microsoft.com/library/windows/desktop/ms632607) 창에 대 한 새 스타일을 포함 하는 구조입니다. 응용 프로그램 스타일을 검사 하 고 변경할 수 있습니다.
...
프레임 워크가이 멤버 함수를 호출 하는 경우 `CWnd` 입력된 포커스를가지고 하며 [WM_SYSKEYUP](#onsyskeyup) 하 고 [WM_SYSKEYDOWN](#onsyskeydown) 메시지 변환 됩니다.
...
|값|의미|
|-----------|-------------|
|0-15|반복 횟수를 지정합니다. 값은 키 입력 키를 누른 사용자로 인해 반복 횟수...|
|16-23|검색 코드를 지정합니다. 원래 장비 제조업체 (OEM) 되는 값|
|24|키를 향상 된 101 또는 102 키 키보드에 표시 되는 오른쪽 ALT 및 CTRL 키와 같은 확장 된 키를 지정 합니다. 확장 된 키를; 이면 값이 1 그렇지 않으면 0입니다.|
|25-28|Windows에서 내부적으로 사용 합니다.|
|29|상황에 맞는 코드를 지정합니다. 값이 1 ALT 키를 누르고 키가 눌러져 있습니다. 그렇지 않으면 값은 0입니다.|
...
프레임 워크에 시스템 색 설정이 변경 될 때 모든 최상위 창에 대 한이 멤버 함수를 호출 합니다.
...
Windows 호출 `OnSysColorChange` 시스템 색 변경이 영향을 받는 모든 창에 대 한 합니다.

기존 시스템 색을 사용 하는 브러시를 없는 응용 프로그램의 해당 브러시를 삭제 하 고 다시 새 시스템 색을 사용 하 여 만듭니다.

##  <a name="onsyscommand"></a>  CWnd::OnSysCommand

프레임 워크는 사용자가 최대화 또는 최소화 단추를 선택 하거나 사용자가 컨트롤 메뉴에서 명령을 선택할 때이 멤버 함수를 호출 합니다.
...
- SC_HOTKEY 활성화는 `CWnd` 응용 프로그램을 지정 하는 바로 가기 키와 연결 된 개체입니다. 하위 워드입니다 *lParam* 활성화 된 창의 HWND를 식별 합니다.
...
- SC_MINIMIZE (또는 SC_ICON) 최소화를 `CWnd` 개체입니다.

- SC_MOUSEMENU 마우스 메뉴 클릭을 검색 합니다.

- SC_MOVE 이동 된 `CWnd` 개체입니다.

- SC_NEXTWINDOW 다음 창으로 이동 합니다.

- SC_PREVWINDOW 이전 창으로 이동 합니다.

- SC_RESTORE 복원 창 표준 위치 및 크기입니다.

- SC_SCREENSAVE 시스템의 [부팅] 섹션에서 지정한 화면 보호기 응용 프로그램을 실행 합니다. INI 파일입니다.
...
- SC_SCREENSAVE 제어판의 데스크톱 섹션에서 지정한 화면 저장 응용 프로그램을 실행 합니다.

### <a name="remarks"></a>설명

기본적으로 `OnSysCommand` 앞의 표에 지정 된 미리 정의 된 동작에 대 한 컨트롤 메뉴 요청을 수행 합니다.

네 가지 하위 비트는 WM_SYSCOMMAND 메시지를 *nID* 매개 변수는 Windows에서 내부적으로 사용 됩니다. 응용 프로그램의 값을 테스트 하는 경우 *nID*, 0xFFF0 값을 결합 해야 합니다 *nID* 비트를 사용 하 여 값-및 연산자 올바른 결과를 가져옵니다.

사용 하 여 컨트롤 메뉴에서 메뉴 항목을 수정할 수는 `GetSystemMenu`, `AppendMenu`를 `InsertMenu`, 및 `ModifyMenu` 멤버 함수입니다. 컨트롤 메뉴를 수정 하는 응용 프로그램 WM_SYSCOMMAND 메시지를 처리 해야 하 고 응용 프로그램에서 처리 되지 않은 WM_SYSCOMMAND 메시지에 전달 해야 합니다 `OnSysCommand`합니다. 응용 프로그램에 의해 추가 된 모든 명령 값 응용 프로그램에서 처리 해야 하 고 전달할 수 없습니다 `OnSysCommand`합니다.

응용 프로그램 명령을 실행할 수 있는 시스템 언제 든 지 WM_SYSCOMMAND 메시지를 전달 하 여 `OnSysCommand`입니다.

컨트롤 메뉴에서 항목을 선택 하도록 정의 된 액셀러레이터 키 입력으로 변환 됩니다 `OnSysCommand` 호출; 키 입력이 변환 되는 다른 모든 가속기 [WM_COMMAND](#oncommand) 메시지입니다.
...
프레임 워크가이 멤버 함수를 호출 하는 경우는 `CWnd` 개체에 입력 포커스를 [OnSysKeyUp](#onsyskeyup) 또는 [OnSysKeyDown](#onsyskeydown) 멤버 함수를 호출 합니다.
...
경우는 `CWnd` 개체에서 입력된 포커스가 `OnSysKeyDown` 멤버 함수는 사용자 ALT 키를 보유 하 고 다음 다른 키를 누를 때 프레임 워크에서 호출 됩니다.
...
|14|이전 키 상태 (메시지가 전송 되기 전에, 0 등록 키가 키가 눌러져 있으면 1)입니다.|
|15|전환 상태 (키를 해제 하는 경우 1, 0 키를 누른 경우)입니다.|

에 대 한 `OnSysKeyDown` 호출 키 전환 비트 (비트 15)은 0입니다. 상황에 맞는 코드 비트 (비트 13)는 1이 ALT 키 키가 눌러져; 동안 다운 된 경우 창이 입력된 포커스를가지고 있으므로 활성 창 메시지 전송 된 경우 0입니다.

### <a name="remarks"></a>설명

창이 현재 입력 포커스가, 활성 창의 경우 `OnSysKeyDown` 멤버 함수를 호출 합니다. 합니다 `CWnd` 메시지를 받는 개체에서 상황에 맞는 코드를 확인 하 여 이러한 두 가지 상황을 구분할 수 `nFlags`입니다.

WM_SYSKEYDOWN 메시지를 수신한 상황에 맞는 코드를이 0 이면 `OnSysKeyDown` 전달할 수는 [TranslateAccelerator](http://msdn.microsoft.com/library/windows/desktop/ms646373) Windows 함수를 시스템 키 메시지 대신 키가 일반 메시지 처럼 처리 됩니다. 이 액셀러레이터 키를 활성 창에 입력된 포커스가 없는 경우에 활성 창에 사용할 수 있습니다.

자동-반복을 하나 이상으로 인해 `OnSysKeyDown` 호출 하기 전에 발생할 수 있습니다 합니다 [WM_SYSKEYUP](#onsyskeyup) 메시지를 수신 합니다. 이전 키 상태 (비트 14)를 확인 하기 위해 사용할 수 있는지 여부를 `OnSysKeyDown` 호출이 첫 번째 하위 전환 또는 아래로 반복된 전환을 나타냅니다.
...
경우는 `CWnd` 개체에 포커스가 있으면는 `OnSysKeyUp` 멤버 함수는 ALT 키를 누른가 누른 키를 놓으면 때 프레임 워크에서 호출 됩니다.
...
에 대 한 `OnSysKeyUp` 호출 키 전환 비트 (비트 15)은 1입니다. 상황에 맞는 코드 비트 (비트 13)는 1이 ALT 키 키가 눌러져; 동안 다운 된 경우 창이 입력된 포커스를가지고 있으므로 활성 창 메시지 전송 된 경우 0입니다.

### <a name="remarks"></a>설명

창이 현재 입력 포커스가, 활성 창의 경우 `OnSysKeyUp` 멤버 함수를 호출 합니다. 합니다 `CWnd` 호출을 받는 개체에서 상황에 맞는 코드를 확인 하 여 이러한 두 가지 상황을 구별할 수 *nFlags*합니다.

WM_SYSKEYUP 메시지를 수신한 상황에 맞는 코드를이 0 이면 `OnSysKeyUp` 전달할 수는 [TranslateAccelerator](http://msdn.microsoft.com/library/windows/desktop/ms646373) Windows 함수를 시스템 키 메시지 대신 키가 일반 메시지 처럼 처리 됩니다. 이렇게 하면 가속기 활성 창에 입력된 포커스가 없는 경우에 활성 창에 사용할 키 (바로 가기).

IBM 고급 101 및 102 키 키보드에 대 한 향상 된 키가 오른쪽 alt 키와 키보드;의 주요 섹션에 있는 오른쪽 CTRL 키 기능, DEL, 홈, END, PAGE UP, PAGE DOWN 및 왼쪽의 숫자 키패드로;의 클러스터에서 화살표 키 슬래시 (/) 및 숫자 키패드에서 ENTER 키입니다. 다른 일부 키보드에 있는 확장 된 키 비트를 지원할 수 있습니다 *nFlags*합니다.

미국 외 지역에 대 한 향상 된 102 키 키보드, 오른쪽 ALT 키를 CTRL + ALT 키 조합으로 처리 됩니다. 다음 메시지와이 키를 놓을 때 발생 하는 호출의 순서를 보여 줍니다.

|Sequence|액세스 하는 함수|전달 된 메시지|
|--------------|-----------------------|--------------------|
...
프레임 워크를 만들 수 있는 단추를 클릭할 때이 멤버 함수를 호출 합니다.
...
- IDCANCEL 사용자를 만들 수 있는 취소 단추를 클릭 합니다.

- IDCLOSE 사용자 교육 카드를 닫힙니다.

- IDHELP 사용자를 만들 수 있는 Windows 도움말 단추를 클릭 합니다.

- IDIGNORE 사용자를 만들 수 있는 무시 단추를 클릭 합니다.

- IDOK 사용자를 만들 수 있는 확인 단추를 클릭 합니다.

- IDNO 사용자 없음 단추를 만들 수 있는 클릭 합니다.

- IDRETRY 사용자를 만들 수 있는 다시 시도 단추를 클릭 합니다.

- HELP_TCARD_DATA 사용자를 만들 수 있는 단추를 클릭 합니다. 합니다 *dwActionData* 매개 변수 도움말 작성자가 지정 된 정수 (long)를 포함 합니다.

- HELP_TCARD_NEXT 사용자를 만들 수 있는 다음 단추를 클릭 합니다.

- 다른 응용 프로그램 HELP_TCARD_OTHER_CALLER 교육 카드를 요청 했습니다.
...
이 함수는 응용 프로그램 Windows 도움말 사용 교육 카드를 시작한 경우에 호출 됩니다. 응용 프로그램에 대 한 호출에서 HELP_TCARD 명령을 지정 하 여 학습 카드를 시작 합니다 [WinHelp](../../mfc/reference/cwinapp-class.md#winhelp) 함수입니다.

##  <a name="ontimechange"></a>  CWnd::OnTimeChange

프레임 워크가 시스템 시간 변경 된 후이 멤버 함수를 호출 합니다.
...
변경 내용이 시스템에이 메시지를 보낼 모든 최상위 창에 모든 응용 프로그램이 생깁니다. 모든 최상위 창으로 WM_TIMECHANGE 메시지를 보내려고 응용 프로그램이 사용할 수는 [SendMessage](http://msdn.microsoft.com/library/windows/desktop/ms644950) Windows 함수와 해당 *hwnd* HWND_BROADCAST로 매개 변수를 설정 합니다.

##  <a name="ontimer"></a>  CWnd::OnTimer

에 지정 된 간격 후이 멤버 함수를 호출 하는 프레임 워크를 [SetTimer](#settimer) 타이머를 설치 하는 데 사용 되는 멤버 함수입니다.
...
[DispatchMessage](http://msdn.microsoft.com/library/windows/desktop/ms644934) Windows 보냅니다 함수는 [WM_TIMER](http://msdn.microsoft.com/library/windows/desktop/ms644902) 메시지가 다른 메시지는 응용 프로그램의 메시지 큐입니다.
...
### <a name="example"></a>예
  ...
  예제를 참조 하세요 [CWnd::SetTimer](#settimer)합니다.

##  <a name="ontoolhittest"></a>  CWnd::OnToolHitTest

프레임 워크가 지정 된 도구의 경계 사각형에 점이 있는지 여부를 웜 하려면이 멤버 함수를 호출 합니다.
...
*PTI*  
에 대 한 포인터를 [TOOLINFO](http://msdn.microsoft.com/library/windows/desktop/bb760256) 구조입니다. 다음 구조 값은 기본적으로 설정 됩니다.

- *hwnd*  =  `m_hWnd` 창에 대 한 핸들

- *uId*  =  `(UINT)hWndChild` 자식 창에 대 한 핸들

- *uFlags* &#124;도구의 TTF_IDISHWND 처리 =

- *lpszText* = LPSTR_TEXTCALLBACK Pointer 지정한 창에 표시 되도록 하는 문자열

### <a name="return-value"></a>반환 값

창 컨트롤 id입니다. 도구 설명 컨트롤을 찾을 경우 도구 설명 컨트롤이 없는 경우,-1입니다.

### <a name="remarks"></a>설명

지점이 사각형에 있으면 도구에 대 한 정보를 검색 합니다.

도구 설명이 연결 된 영역 단추 없으면 `OnToolHitTest` TTF_NOTBUTTON TTF_CENTERTIP 구조 플래그를 설정 합니다.

재정의 `OnToolHitTest` 기본 제공 보다 다양 한 정보를 제공 하도록 합니다.

참조 [TOOLINFO](http://msdn.microsoft.com/library/windows/desktop/bb760256), 구조에 대 한 자세한 내용은 Windows sdk에서 합니다.
...
응용 프로그램 Windows 터치 입력을 처리 하는 경우 TRUE입니다. 그렇지 않으면 FALSE입니다.
...
TRUE 이면 응용 프로그램 프로세스 Windows 터치 입력이 있습니다. 그렇지 않으면 FALSE입니다.
...
이 메서드를 수신 합니다 [WM_UNICHAR](http://msdn.microsoft.com/library/windows/desktop/ms646288) Windows SDK에 설명 된 알림. 합니다 [WM_UNICHAR](http://msdn.microsoft.com/library/windows/desktop/ms646288) 메시지는 보내거나 ANSI windows 유니코드 문자를 게시 하도록 설계 되었습니다. 한 것과 동일 합니다 [WM_CHAR](http://msdn.microsoft.com/library/windows/desktop/ms646276) 반면 유니코드 변환 형식-32 인코딩을 (UTF-32)을 사용 하지만 메시지를 [WM_CHAR](http://msdn.microsoft.com/library/windows/desktop/ms646276) u t F-16을 사용 하 여 메시지.
...
이 멤버 함수는 경우 드롭다운 메뉴를 호출 하는 프레임 워크 또는 하위 메뉴 제거 되었습니다.
...
이 메서드를 수신 합니다 [WM_UNINITMENUPOPUP](http://msdn.microsoft.com/library/windows/desktop/ms647614) Windows SDK에 설명 된 알림.
...
- UIS_INITIALIZE UI 상태 요소 (지정 된 *nUIElement*) 마지막 입력된 이벤트에 따라 변경 해야 합니다. 자세한 내용은 참조는 **주의** 부분 [WM_UPDATEISTATE](http://msdn.microsoft.com/library/windows/desktop/ms646361).
...
이 멤버 함수는의 기능을 에뮬레이션 합니다 [WM_UPDATEUISTATE](http://msdn.microsoft.com/library/windows/desktop/ms646361) Windows SDK에 설명 된 대로 메시지입니다.

##  <a name="onuserchanged"></a>  CWnd::OnUserChanged

프레임 워크를 끄거나 사용자가 로그온 한 후 모든 windows에 대 한이 멤버를 호출 합니다.
...
이 메서드를 수신 합니다 [WM_USERCHANGED](http://msdn.microsoft.com/library/windows/desktop/ms632651) Windows SDK에 설명 된 알림 메시지입니다. 사용자가 켜거나 로그, 운영 체제는 사용자별 설정을 업데이트 합니다. 시스템 설정 업데이트 직후이 메시지를 보냅니다.
...
경우는 `CWnd` 목록 상자를 소유 하는 개체를 [LBS_WANTKEYBOARDINPUT](../../mfc/reference/styles-used-by-mfc.md#list-box-styles) 스타일 목록 상자에서 메시지를 보내지 WM_VKEYTOITEM WM_KEYDOWN 메시지에 대 한 응답에서입니다.
...
응용 프로그램에 대 한 응답 메시지에 수행 하는 동작을 지정 합니다. 반환 값-2의 응용 프로그램 항목을 선택 하는 모든 측면을 처리 하 고 목록 상자에 추가 작업이 없으므로 필요를 나타냅니다. 반환 값이-1 목록 상자 키 입력에 대 한 응답의 기본 작업을 수행 해야 나타냅니다. 반환 값 0 이상인 목록 상자에서 항목의 0부터 시작 인덱스를 지정 하 고 목록 상자에서 지정된 된 항목 키에 대 한 기본 작업을 수행 해야 나타냅니다.

### <a name="remarks"></a>설명

이 구성원 함수가 있는 목록 상자에 대해서만 프레임 워크에서 호출 되는 [LBS_HASSTRINGS](../../mfc/reference/styles-used-by-mfc.md#list-box-styles) 스타일입니다.
...
- 지정 된 위치로 SB_THUMBTRACK 끌어서 스크롤 상자입니다. 현재 위치에서 제공 됩니다 *nPos*합니다.
...
`OnVScroll` 일반적으로 스크롤 상자를 끄는 동안 몇 가지 피드백을 제공 하는 응용 프로그램에서 사용 됩니다.

하는 경우 `OnVScroll` 내용을 스크롤 하는 `CWnd` 개체를 사용 하 여 스크롤 상자 위치도 다시 설정 해야 합니다는 [SetScrollPos](#setscrollpos) 멤버 함수입니다.
...
클립보드 소유자의 `OnVScrollClipboard` 클립보드 데이터 CF_OWNERDISPLAY 형식은 고 이벤트에 클립보드 뷰어의 세로 스크롤 막대 경우 클립보드 뷰어에서 멤버 함수 라고 합니다.
...
현재 창을 최대화 하 고 창에는 데스크톱 창 관리자 (DWM)으로 구성 됩니다 하는 경우이 멤버를 호출 하는 프레임 워크입니다.
...
이 메서드를 수신 합니다 [WM_DWMWINDOWMAXIMIZEDCHANGE](http://msdn.microsoft.com/library/windows/desktop/dd388201) Windows SDK에 설명 된 알림 메시지입니다.
...
프레임 워크에 대 한 호출의 결과로 크기, 위치 또는 Z 순서가 변경 될 때이 멤버 함수를 호출 합니다 [SetWindowPos](http://msdn.microsoft.com/library/windows/desktop/ms633545) 멤버 함수 또는 다른 창 관리 함수입니다.
...
기본 구현은 보냅니다 합니다 [WM_SIZE](http://msdn.microsoft.com/library/windows/desktop/ms632646) 하 고 [WM_MOVE](http://msdn.microsoft.com/library/windows/desktop/ms632631) 메시지를 창. 응용 프로그램에서 처리 하는 경우에 이러한 메시지가 전송 되지 않습니다는 `OnWindowPosChanged` 해당 기본 클래스를 호출 하지 않고 호출 합니다. 것이 더 효율적 모든 이동을 수행 하 여 변경 처리를 호출 하는 동안 크기 `OnWindowPosChanged` 해당 기본 클래스를 호출 하지 않고 있습니다.
...
크기, 위치 또는 Z 순서에 대 한 호출의 결과로 변경 하려고 할 때 프레임 워크에서이 멤버 함수를 호출 합니다 [SetWindowPos](http://msdn.microsoft.com/library/windows/desktop/ms633545) 멤버 함수 또는 다른 창 관리 함수입니다.
...
사용 하 여 창에 대 한 합니다 [WS_OVERLAPPED](styles-used-by-mfc.md#window-styles) 또는 [WS_THICKFRAME](styles-used-by-mfc.md#window-styles) 스타일을 기본 구현은 보냅니다는 [WM_GETMINMAXINFO](http://msdn.microsoft.com/library/windows/desktop/ms632626) 창에 메시지를 합니다. 새 크기와 창의 위치를 확인 하 고 CS_BYTEALIGNCLIENT 및 CS_BYTEALIGN 클라이언트 스타일을 적용할 수행 됩니다. 응용 프로그램 하지 해당 기본 클래스를 호출 하 여이 기능을 재정의할 수 있습니다.
...
프레임 워크를이 멤버 함수를 호출 Windows 초기화 파일, 변경 된 후 WIN 합니다. INI 합니다.
...
합니다 [SystemParametersInfo](http://msdn.microsoft.com/library/windows/desktop/ms724947) Windows 함수 호출 `OnWinIniChange` 후 응용 프로그램에서 승리 설정을 변경 하려면 함수를 사용 합니다. INI 파일입니다.

모든 최상위 창으로 WM_WININICHANGE 메시지를 보내려고 응용 프로그램이 사용할 수는 [SendMessage](http://msdn.microsoft.com/library/windows/desktop/ms644950) Windows 함수와 해당 *hwnd* HWND_BROADCAST로 매개 변수를 설정 합니다.

응용 프로그램의 여러 다른 섹션을 변경 하는 경우 승리 합니다. 동시에 INI, 응용 프로그램 하나 WM_WININICHANGE 메시지를 전송 해야 *lpszSection* NULL로 설정 합니다. 그렇지 않은 경우 응용 프로그램 각 WM_WININICHANGE 때마다 WIN 변경 하면 전송 해야 합니다. INI 합니다.

응용 프로그램에서 수신 하는 경우는 `OnWinIniChange` 사용 하 여 호출할 *lpszSection* NULL로 설정 확인 하는 모든 WIN 섹션입니다. 응용 프로그램에 영향을 주는 INI 합니다.
...
이 멤버 함수는 호출한 `WindowProc`, 또는 메시지 리플렉션 중에 호출 됩니다.
...
메시지가 처리 되었으면 TRUE 그렇지 않으면 FALSE입니다.

### <a name="remarks"></a>설명

`OnWndMsg` 적절 한 프레임 워크 함수는 메시지 유형 및 호출 중 하나를 결정 (예를 들어 [OnCommand](#oncommand) WM_COMMAND에 대 한) 또는 메시지 맵에 적절 한 메시지를 찾습니다.

메시지 리플렉션에 대 한 자세한 내용은 참조 하세요. [리플렉션된 메시지 처리](../../mfc/handling-reflected-messages.md)합니다.

##  <a name="onxbuttondblclk"></a>  CWnd::OnXButtonDblClk

프레임 워크는 사용자가 XBUTTON1 또는 xbutton2를 놓으면 커서가 창의 클라이언트 영역에 있는 동안이 멤버 함수를 호출 합니다.
...
이 메서드를 수신 합니다 [WM_XBUTTONDBLCLK](http://msdn.microsoft.com/library/windows/desktop/ms646244) Windows SDK에 설명 된 알림. 마우스 캡처되지 않은 경우 커서 아래에 있는 창에 메시지가 게시 됩니다. 그렇지 않으면 메시지 창에 마우스를 캡처에 게시 됩니다.
...
커서가 창의 클라이언트 영역에 있는 동안 사용자가 XBUTTON1 또는 xbutton2를 놓으면 누를 때 프레임 워크에서이 멤버 함수를 호출 합니다.
...
이 메서드를 수신 합니다 [WM_XBUTTONDOWN](http://msdn.microsoft.com/library/windows/desktop/ms646245) Windows SDK에 설명 된 알림. 마우스 캡처되지 않은 경우 커서 아래에 있는 창에 메시지가 게시 됩니다. 그렇지 않으면 메시지 창에 마우스를 캡처에 게시 됩니다.
...
커서가 창의 클라이언트 영역에 있는 동안 사용자가 XBUTTON1 또는 xbutton2를 놓으면 놓을 때 프레임 워크에서이 멤버 함수를 호출 합니다.
...
이 메서드를 수신 합니다 [WM_XBUTTONUP](http://msdn.microsoft.com/library/windows/desktop/ms646246) Windows SDK에 설명 된 알림. 마우스 캡처되지 않은 경우 커서 아래에 있는 창에 메시지가 게시 됩니다. 그렇지 않으면 메시지 창에 마우스를 캡처에 게시 됩니다.
...
클립보드를 통해 열리는 경우 0이 아닌 `CWnd`, 하거나 다른 응용 프로그램 또는 창에는 클립보드에 있는 경우에 0에서 엽니다.

### <a name="remarks"></a>설명

다른 응용 프로그램까지 클립보드를 수정할 수 없습니다는 [CloseClipboard](http://msdn.microsoft.com/library/windows/desktop/ms649035) Windows 함수를 호출 합니다.

현재 `CWnd` 개체에 게 될 때까지 클립보드 소유자는 [EmptyClipboard](http://msdn.microsoft.com/library/windows/desktop/ms649037) Windows 함수를 호출 합니다.
...
이 연산자를 사용 하 여에 대 한 핸들을 가져오려면는 `CWnd` 개체입니다.
...
두 `CWnd` 경우 없는 동일한 개체가 [m_hWnd](#m_hwnd)합니다.
...
두 `CWnd` 개체가 동일한 있는지 [m_hWnd](#m_hwnd)합니다.
...
컨트롤 컨테이너와 창 없는 컨트롤은이 있으면 TRUE를 반환은 성공적으로 그린, 그렇지 않으면 FALSE입니다.
...
0이 아닌 메시지 게시 됩니다. 그렇지 않으면 0입니다.

### <a name="remarks"></a>설명

메시지 큐의 메시지를 호출 하 여 검색 되는 [GetMessage](http://msdn.microsoft.com/library/windows/desktop/ms644936) 하거나 [PeekMessage](http://msdn.microsoft.com/library/windows/desktop/ms644943) Windows 함수입니다.

Windows [PostMessage](http://msdn.microsoft.com/library/windows/desktop/ms644944) 함수를 사용 하 여 다른 응용 프로그램에 액세스할 수 있습니다.

### <a name="example"></a>예

  예를 참조 하세요 [AfxGetMainWnd](../../mfc/reference/application-information-and-management.md#afxgetmainwnd)합니다.

##  <a name="postncdestroy"></a>  CWnd::PostNcDestroy

기본값을 호출한 [OnNcDestroy](#onncdestroy) 창이 소멸 된 후에 멤버 함수입니다.
...
파생된 클래스의 삭제와 같은 사용자 지정 정리에 대 한이 함수를 사용할 수 있습니다 합니다 **이** 포인터입니다.

##  <a name="precreatewindow"></a>  CWnd::PreCreateWindow

이 연결 된 Windows 창을 만들기 전에 프레임 워크에서 호출 `CWnd` 개체입니다.
...
> [!WARNING]
> `CWnd::PreCreateWindow` 이제의 hMenu 구성원을 할당 *cs* 에 **이** 포인터 메뉴가 NULL이 고 스타일 WS_CHILD를 포함 하는 경우. 적절 한 기능에 대 한 대화 상자 컨트롤에 NULL이 아닌 ID를 확인 합니다.
>
> 이 변경 관리/네이티브 interop 시나리오에서 충돌을 해결합니다. 추적 문을 `CWnd::Create` 개발자 문제를 경고 합니다.
...
각 클래스에서 파생 `CWnd` 해당 재정의에 고유한 기능을 추가 `PreCreateWindow`합니다. 기본적으로의 이러한 파생 `PreCreateWindow` 설명 하지 않습니다. 각 클래스 및 스타일 간 상호 종속성에 적절 한 스타일을 확인 하려면 응용 프로그램의 기본 클래스에 대 한 MFC 소스 코드를 검사할 수 있습니다. 재정의 하려는 경우 `PreCreateWindow,` 응용 프로그램의 기본 클래스에서 사용 된 스타일 MFC 소스 코드에서 수집 된 정보를 사용 하 여 필요한 기능을 제공 하는지 여부를 확인할 수 있습니다.
...
이 멤버 함수는 서브클래싱된 전에 발생 하는 데 필요한 다른 서브클래싱 할 수 있도록 프레임 워크에서 호출 됩니다.
...
컨트롤의 동적 서브클래싱 허용이 멤버 함수를 재정의 합니다. 고급 재정의할 수 있습니다.

##  <a name="pretranslatemessage"></a>  CWnd::PreTranslateMessage

창 메시지가 [TranslateMessage](../../mfc/reference/cwinapp-class.md) 및 [DispatchMessage](http://msdn.microsoft.com/library/windows/desktop/ms644955) Windows 함수로 디스패치되기 전에 [CWinApp](http://msdn.microsoft.com/library/windows/desktop/ms644934) 클래스가 이 메시지를 해석하는 데 사용됩니다.
...
이 멤버 함수는 가장 일반적으로 프린터 디바이스 컨텍스트에서 지정 된 장치 컨텍스트에서 현재 창을 그리기를 호출 합니다.
...
-   PRF_ERASEBKGND 지정 된 경우 전송 창에 [WM_ERASEBKGND](http://msdn.microsoft.com/library/windows/desktop/ms648055) 메시지입니다.

-   PRF_CLIENT 지정 된 경우 전송 창에 [WM_PRINTCLIENT](http://msdn.microsoft.com/library/windows/desktop/dd145217) 메시지입니다.

-   PRF_CHILDREN을 설정 하는 경우 보낼 각 표시 되는 자식 창에 [WM_PRINT](http://msdn.microsoft.com/library/windows/desktop/dd145216) 메시지입니다.

-   PRF_OWNED 설정 된 경우 표시 되는 소유 창과 각 WM_PRINT 메시지를 보냅니다.

##  <a name="printclient"></a>  CWnd::PrintClient

지정 된 디바이스 컨텍스트 (일반적으로 프린터 장치 컨텍스트)에서 모든 창에 그릴이 멤버 함수를 호출 합니다.
...
*nFlags*  
그리기 옵션을 지정합니다. 가능한 값 목록을 참조 하세요 [PrintWindow](http://msdn.microsoft.com/library/windows/desktop/dd162869)합니다.
...
이 멤버 함수는 함수의 기능을 에뮬레이트합니다 [PrintWindow](http://msdn.microsoft.com/library/windows/desktop/dd162869)Windows SDK에 설명 된 대로 합니다.

##  <a name="redrawwindow"></a>  CWnd::RedrawWindow

지정 된 사각형 또는 지정 된 창의 클라이언트 영역에서 영역을 업데이트합니다.
...
- RDW_ERASE 인해 창에서 수신 하는 [WM_ERASEBKGND](http://msdn.microsoft.com/library/windows/desktop/ms648055) 메시지가 창 다시 그려집니다. RDW_INVALIDATE 플래그를 지정 해야 합니다. 그렇지 않으면 RDW_ERASE 영향을 주지 않습니다.

- RDW_FRAME 하면 받으려면 업데이트 영역을 교차 하는 창의 비클라이언트 영역의 일부를 [WM_NCPAINT](http://msdn.microsoft.com/library/windows/desktop/dd145212) 메시지입니다. RDW_INVALIDATE 플래그를 지정 해야 합니다. 그렇지 않으면 RDW_FRAME 영향을 주지 않습니다.

- RDW_INTERNALPAINT 하면를 [WM_PAINT](http://msdn.microsoft.com/library/windows/desktop/dd145213) 창이 잘못 된 영역이 포함 되는지 여부에 관계 없이 창에 게시할 메시지입니다.

- RDW_INVALIDATE 무효화 *lpRectUpdate* 하거나 *prgnUpdate* (하나 NULL 일 수 없습니다)만 합니다. 둘 다 NULL 인 경우 전체 창을 무효화 됩니다.

다음 플래그는 창의 유효성을 검사 하는 데 사용 됩니다.

- RDW_NOERASE는 WM_ERASEBKGND 메시지 보류 중인 억제합니다.

- RDW_NOFRAME는 WM_NCPAINT 메시지 보류 중인 억제합니다. 이 플래그 RDW_VALIDATE를 사용 하 여 사용 해야 하며 RDW_NOCHILDREN를 사용 하 여 일반적으로 사용 됩니다. 이 옵션으로 창의 각 부분에서 제대로 그리기 하지 못할 수 신중 하 게 사용 해야 합니다.

- RDW_NOINTERNALPAINT 내부 WM_PAINT 메시지 보류 중인 억제합니다. 이 플래그는 잘못 된 영역에서 결과 WM_PAINT 메시지를 적용 되지 않습니다.

- 유효성을 검사 RDW_VALIDATE *lpRectUpdate* 또는 *prgnUpdate* (하나 NULL 일 수 없습니다)만 합니다. 둘 다 NULL 인 경우 전체 창 유효성이 검사 됩니다. 이 플래그는 내부 WM_PAINT 메시지를 적용 되지 않습니다.

다음 플래그 컨트롤 다시 그리기를 수행 하는 경우 발생 합니다. 그리기에서 수행 되지 않는 `RedrawWindow` 이러한 비트 중 하나를 지정 하지 않으면 작동 합니다.

- RDW_ERASENOW는 영향을 받는 windows (지정 된 대로 RDW_ALLCHILDREN 및 RDW_NOCHILDREN 플래그를 통해)는 함수가 반환 하기 전에 필요한 경우 WM_NCPAINT 및 WM_ERASEBKGND 메시지를 수신 하는 것 되도록 합니다. WM_PAINT 메시지 지연 됩니다.

- RDW_UPDATENOW는 영향을 받는 windows (지정 된 대로 RDW_ALLCHILDREN 및 RDW_NOCHILDREN 플래그를 통해)는 함수가 반환 하기 전에 필요한 경우 WM_NCPAINT, WM_ERASEBKGND, 및 WM_PAINT 메시지를 수신 하는 것 되도록 합니다.

기본적으로 영향을 받는 windows는 `RedrawWindow` 함수 지정된 된 창 WS_CLIPCHILDREN 스타일에 있는지 여부에 따라 달라 집니다. WS_CLIPCHILDREN 창의 자식 창 영향을 받지 않습니다. 그러나 WS_CLIPCHILDREN windows가 아닌 해당 windows 재귀적으로 유효성을 검사 하거나 WS_CLIPCHILDREN 창이 나타날 때까지 무효화 됩니다. 다음 windows 영향을 받는 제어 플래그는 `RedrawWindow` 함수:

- RDW_ALLCHILDREN 포함 자식 창에 다시 그리기 작업에 있는 경우.

- RDW_NOCHILDREN 제외 자식 창에 있는 다시 그리기 작업의 경우.

### <a name="return-value"></a>반환 값

창이 성공적으로 다시 그려질 경우에 0이 아닌 그렇지 않으면 0입니다.

### <a name="remarks"></a>설명

경우는 `RedrawWindow` 멤버 함수는 데스크톱 창의 부분을 무효화 하는 해당 창에서 수신 하지는 [WM_PAINT](http://msdn.microsoft.com/library/windows/desktop/dd145213) 메시지입니다. 바탕 화면에 그릴 응용 프로그램을 사용할지 [CWnd::ValidateRgn](#validatergn)를 [CWnd::InvalidateRgn](#invalidatergn)합니다 [CWnd::UpdateWindow](#updatewindow), 또는 [RedrawWindow](http://msdn.microsoft.com/library/windows/desktop/dd162911)

##  <a name="reflectchildnotify"></a>  CWnd::ReflectChildNotify

이 메시지 함수에서 프레임 워크에서 호출 됩니다 [OnChildNotify](#onchildnotify)합니다.
...
메시지가 반영 되었으면 하는 경우 TRUE입니다. 그렇지 않으면 FALSE입니다.

### <a name="remarks"></a>설명

도우미 함수를 반영 하는 것 *메시지* 원본입니다.

메시지에 직접 보내집니다 반영 [CWnd::OnWndMsg](#onwndmsg) 하거나 [CCmdTarget::OnCmdMsg](../../mfc/reference/ccmdtarget-class.md#oncmdmsg)합니다.
...
이 멤버 함수 호출 [SendChildNotifyLastMsg](#sendchildnotifylastmsg) 창으로 식별할 *hWndChild* OLE 컨트롤 또는 영구 구조의 창입니다.

메시지 리플렉션에 대 한 자세한 내용은 참조 하세요. [리플렉션된 메시지 처리](../../mfc/handling-reflected-messages.md)합니다.

##  <a name="releasedc"></a>  CWnd::ReleaseDC

디바이스 컨텍스트, 다른 응용 프로그램에서 사용 하기 위해 해제를 해제 합니다.
...
효과 `ReleaseDC` 멤버 함수 장치 컨텍스트 형식에 따라 달라 집니다.

응용 프로그램을 호출 해야 합니다는 `ReleaseDC` 멤버 함수를 호출할 때마다 합니다 [GetWindowDC](#getwindowdc) 멤버 함수 및 각 호출에 [GetDC](#getdc) 멤버 함수입니다.
...
- `CWnd::reposQuery` 컨트롤 막대 레이아웃 수행 되지 않습니다. 대신 *lpRectParam* 레이아웃 실제로 수행한 것 처럼 클라이언트 영역의 크기를 사용 하 여 초기화 됩니다.
...
합니다 *nIDFirst* 하 고 *nIDLast* 클라이언트 영역의 위치가 변경 될 컨트롤 막대 Id의 범위를 정의 하는 매개 변수입니다. 합니다 *nIDLeftOver* 매개 변수 위치가 변경 되 면 이며 나머지 컨트롤 막대로 채워지지 클라이언트 영역에 맞게 크기를 조정 하는 자식 창 (일반적으로 뷰)의 ID를 지정 합니다.

##  <a name="runmodalloop"></a>  CWnd::RunModalLoop

이 멤버 함수를 검색, 변환 또는 될 때까지 메시지를 디스패치 호출 [ContinueModal](#continuemodal) FALSE를 반환 합니다.
...
- MLF_NOIDLEMSG 없는 송신 [WM_ENTERIDLE](http://msdn.microsoft.com/library/windows/desktop/ms645422) 부모에 대 한 메시지입니다.

- 창에 WM_KICKIDLE 메시지를 보내지 MLF_NOKICKIDLE 않습니다.

- MLF_SHOWONIDLE는 메시지 큐 유휴 상태가 되 면 창을 표시 합니다.

### <a name="return-value"></a>반환 값

값을 지정 합니다 *n 결과* 매개 변수가 전달 되는 [EndModalLoop](#endmodalloop) 다음 모달 루프를 종료 하는 데 사용 되는 멤버 함수입니다.

### <a name="remarks"></a>설명

기본적으로 `ContinueModal` 후 FALSE를 반환 합니다 `EndModalLoop` 라고 합니다. 제공 된 값을 반환 합니다 *n 결과* 에 `EndModalLoop`입니다.
...
합니다 `ScreenToClient` 멤버 함수에 지정 된 화면 좌표를 대체 *lpPoint* 하거나 *lpRect* 클라이언트 좌표를 사용 하 여 합니다. 왼쪽 위 모퉁이 기준으로 새 좌표는는 `CWnd` 클라이언트 영역입니다.

### <a name="example"></a>예

  예를 참조 하세요 [CListCtrl::GetItemRect](../../mfc/reference/clistctrl-class.md#getitemrect)합니다.
...
캐럿이 있는 경우는 `CWnd` 스크롤되는, `ScrollWindow` 스크롤 완료 된 후 캐럿을 다음 복원 하 고 자동으로를 방지 하기 위해 캐럿을 숨깁니다. 캐럿 위치에 따라 조정 됩니다.

영역으로 처리 합니다 `ScrollWindow` 멤버 함수는 다시 표시 되지 않습니다 하지만 현재로 결합 됩니다 `CWnd` 개체의 업데이트 영역입니다. 응용 프로그램이 결국 받을 [WM_PAINT](http://msdn.microsoft.com/library/windows/desktop/dd145213) 영역을 다시 그려야 할는 되었음을 알리는 메시지입니다. 동시에 이루어집니다 스크롤 미 검사 영역 repaint를 호출 합니다 [UpdateWindow](#updatewindow) 멤버 함수 호출 직후 `ScrollWindow`합니다.

경우 *lpRect* 가 null 인 경우 모든 자식 창에는 windows에서 지정 된 양만큼 오프셋 하는 위치를 *xAmount* 하 고 *yAmount*, 및 잘못 된 (만져) 영역에는 `CWnd` 또한 오프셋 됩니다. `ScrollWindow` 경우 빠릅니다 *lpRect* NULL입니다.

하는 경우 *lpRect* 가 NULL이 자식 창의 위치를 변경 하 고 잘못 된 영역에 없는 `CWnd` 오프셋 되지 됩니다. 방지 하기 위해 문제를 업데이트 하는 중 때 *lpRect* not NULL을 호출 합니다 `UpdateWindow` 멤버 함수를 다시 그릴 `CWnd` 호출 하기 전에 `ScrollWindow`입니다.
...
- SW_ERASE SW_INVALIDATE를 사용 하 여 지정 된 경우 전송 하 여 새로 무효화 된 영역을 지웁니다를 [WM_ERASEBKGND](http://msdn.microsoft.com/library/windows/desktop/ms648055) 창에는 메시지입니다.

- 로 식별 되는 지역 SW_INVALIDATE 무효화 *prgnUpdate* 스크롤한 후 합니다.

- 가리키는 사각형 교차 하는 모든 자식 창은 SW_SCROLLCHILDREN 스크롤합니다 *lpRectScroll* 에 지정 된 픽셀의 개수로 *dx* 하 고 *dy*합니다. Windows 보냅니다를 [WM_MOVE](http://msdn.microsoft.com/library/windows/desktop/ms632631) 교차 하는 모든 자식 창에 메시지 *lpRectScroll*이동 하지 않는 경우에 합니다. 자식 창을 스크롤할 및 커서 사각형이 스크롤 사각형 교차 하는 경우에 캐럿 위치가 변경 되 면 합니다.

### <a name="return-value"></a>반환 값

SIMPLEREGION (사각형 무효화 지역), COMPLEXREGION (사각형이 아닌 무효화 지역; 겹치는 사각형) 또는 NULLREGION (무효화 된 영역 없음), 반환 값은 함수가 성공할 경우 그렇지 않은 경우 반환 값은 오류입니다.

### <a name="remarks"></a>설명

이 함수는 비슷합니다는 [ScrollWindow](http://msdn.microsoft.com/library/windows/desktop/bb787591) 몇 가지 추가 기능을 사용 하 여 함수입니다.

하는 경우 [SW_INVALIDATE](http://msdn.microsoft.com/library/windows/desktop/bb787593) 하 고 [SW_ERASE](http://msdn.microsoft.com/library/windows/desktop/bb787593) 지정 되지 않은 경우는 `ScrollWindowEx` 멤버 함수에서 스크롤 된 영역을 무효화 하지 않습니다. 이러한 플래그 중으로 설정 된 경우 `ScrollWindowEx` 이 영역을 무효화 합니다. 영역 응용 프로그램이 호출 될 때까지 업데이트 되지 않습니다는 [UpdateWindow](http://msdn.microsoft.com/library/windows/desktop/dd145167) 멤버 함수를 호출 합니다 [RedrawWindow](http://msdn.microsoft.com/library/windows/desktop/dd162911) 멤버 함수 (지정 [RDW_UPDATENOW](http://msdn.microsoft.com/library/windows/desktop/dd162911) 또는[ RDW_ERASENOW](http://msdn.microsoft.com/library/windows/desktop/dd162911)), 검색 또는 합니다 [WM_PAINT](http://msdn.microsoft.com/library/windows/desktop/dd145213) 응용 프로그램 큐에서 메시지입니다.

창에 있으면 합니다 [WS_CLIPCHILDREN](http://msdn.microsoft.com/library/windows/desktop/ms632679) 스타일을 지정 하 여 반환 된 영역 *prgnUpdate* 하 고 *lpRectUpdate* 는 스크롤 창의 전체 영역을 나타내는 업데이트할 영역 업데이트 해야 하는 자식 창에 포함 합니다.

경우는 [SW_SCROLLCHILDREN](http://msdn.microsoft.com/library/windows/desktop/bb787593) 플래그가 지정 된, Windows 업데이트 되지 것입니다 제대로 화면 자식 창에 일부 스크롤됩니다. 소스 사각형 밖에 있는 특정 스크롤된 자식 창의 부분 지워지지 및 비활성 제대로 새 대상에서. 사용 된 [DeferWindowPos](http://msdn.microsoft.com/library/windows/desktop/ms632681) 없는 완전히 내 이동 자식 창에 대 한 Windows 함수를 *lpRectScroll* 사각형입니다. SW_SCROLLCHILDREN 플래그가 설정 되어 있고 캐럿 사각형이 스크롤 사각형 교차 하는 경우에 커서 위치가 변경 되 면 합니다.

모든 입력 및 출력 좌표 (에 대 한 *lpRectScroll*, *lpRectClip*를 *lpRectUpdate*, 및 *prgnUpdate*)에 대 한 것으로 간주 됩니다 창의 CS_OWNDC 또는 있는 클래스 스타일 있는지 여부에 관계 없이 클라이언트 좌표입니다. 사용 합니다 [LPtoDP](http://msdn.microsoft.com/library/windows/desktop/dd145042) 하 고 [DPtoLP](http://msdn.microsoft.com/library/windows/desktop/dd162474) 변환 하는 논리적 좌표에서 필요한 경우 Windows 함수입니다.

##  <a name="sendchildnotifylastmsg"></a>  CWnd::SendChildNotifyLastMsg

이 멤버 함수는 자식 창에는 작업을 처리할 수 있도록 자식 창 알림 메시지를 부모 창에서 제공 하기 위해 프레임 워크에서 호출 됩니다.
...
자식 창을 부모;에 전송 된 메시지를 처리 하는 경우 0이 아닌 값 그렇지 않으면 0입니다.

### <a name="remarks"></a>설명

`SendChildNotifyLastMsg` 메시지를 리플렉션하는 경우 원본에 현재 메시지를 보냅니다.
...
컨트롤이 없는 경우 0 또는 컨트롤의 창 프로시저에서 반환한 값을 지정 합니다.

### <a name="remarks"></a>설명

`SendDlgItemMessage` 메시지가 처리 될 때까지 멤버 함수가 반환 하지 않습니다.

사용 하 여 `SendDlgItemMessage` 가져오는 동일를 `CWnd`* 지정된 제어를 호출 합니다 [SendMessage](#sendmessage) 멤버 함수입니다.
...
메시지 처리;의 결과 보낸 메시지의 해당 값에 따라 달라 집니다.

### <a name="remarks"></a>설명

`SendMessage` 멤버 함수는 창 프로시저를 직접 호출 하 고 창 프로시저는 메시지를 처리할 때까지 반환 되지 않습니다. 이 달리는 합니다 [PostMessage](#postmessage) 창의 메시지 큐에 메시지를 배치 하 고 즉시 반환 하는 멤버 함수입니다.
...
모든 하위 창이 지정된 된 Windows 메시지를 보낼이 멤버 함수를 호출 합니다.
...
하는 경우 *bDeep* 은 FALSE 창 바로 아래 자식에만 전송 됩니다; 그렇지 메시지는 모든 하위 창을에 전송 됩니다.

하는 경우 *bDeep* 하 고 *bOnlyPerm* 를 임시 windows 아래 검색이 계속 됩니다. 이 경우 검색 하는 동안 발생 하는 영구 창만 메시지를 받습니다. 하는 경우 *bDeep* 은 FALSE 창 바로 아래 자식에만 전송 됩니다.
...
호출 스레드에서 창을 만든 경우 `SendNotifyMessage` 창의 창 프로시저를 호출 하 고 창 프로시저에서 메시지를 처리할 때까지 반환 되지 않습니다. 창을 다른 스레드에 의해 만들어진 경우 `SendNotifyMessage` 창 프로시저에 메시지를 전달 하 고 반환 즉시; 메시지 처리를 완료 하기 위해 창 프로시저에 대 한 대기 하지 않습니다.
...
`SetActiveWindow` 임의로 활성 창 및 입력된 포커스를 받는 응용 프로그램을 수 있으므로 주의 해 서 멤버 함수를 사용 해야 합니다. 일반적으로 Windows 정품 인증 모든 담당합니다.

##  <a name="setcapture"></a>  CWnd::SetCapture

현재 전송 수를 입력 하는 모든 후속 마우스 `CWnd` 커서의 위치와 관계 없이 개체입니다.
...
이전에 모든 마우스 입력을 수신 하는 창 개체에 대 한 포인터입니다. 이러한 창이 없는 경우 NULL입니다. 반환된 된 포인터 임시 수 있으며 나중에 사용할 저장 되어야 합니다.

### <a name="remarks"></a>설명

때 `CWnd` 모든 마우스 입력, 응용 프로그램을 더 이상 필요 합니다 [ReleaseCapture](http://msdn.microsoft.com/library/windows/desktop/ms646261) 다른 windows 마우스 입력을 받을 수 있도록 작동 합니다.

마우스 입력 캡처된 하는 동안 없습니다 있으면 WM_SETCURSOR 또는 WM_NCHITTEST 메시지를 활성 창으로 전송 됩니다.
...
`SetCaretPos` 멤버 함수는 현재 작업 창을 소유 하는 경우에 캐럿을 이동 합니다. `SetCaretPos` 캐럿을 숨길지 여부 캐럿을 이동 합니다.

캐럿이 공유 리소스입니다. 캐럿 소유 하지 않는 경우 창을 캐럿을 이동 하지 않아야 합니다.
...
이 창을 클립보드의 내용이 변경 될 때마다 (WM_DRAWCLIPBOARD 메시지)을 통해 알림을 받는 창 체인에 추가 합니다.
...
성공 하면 클립보드 뷰어 체인에서 다음 창 핸들입니다. 응용 프로그램 (저장할 수 있습니다 멤버 변수)이이 핸들을 저장 해야 하 고 클립보드 뷰어 체인 메시지에 응답할 때 사용 합니다.

### <a name="remarks"></a>설명

클립보드 뷰어 체인의 일부인 창에 응답 해야 합니다 [WM_DRAWCLIPBOARD](#ondrawclipboard)를 [WM_CHANGECBCHAIN](#onchangecbchain), 및 [WM_DESTROY](#ondestroy) 메시지 및 다음 메시지를 전달 합니다. 체인의 창입니다.

이 멤버 함수는 창에 WM_DRAWCLIPBOARD 메시지를 보냅니다. 응용 프로그램 호출 하는 동안 수신한 WM_DRAWCLIPBOARD 메시지에서 전달 해서는 안 되므로 클립보드 뷰어 체인에서 다음 창에 대 한 핸들 아직 반환 되지 않은 `SetClipboardViewer`합니다.

응용 프로그램 자체를 클립보드 뷰어 체인에서 제거 하려면 호출 해야 합니다 [ChangeClipboardChain](#changeclipboardchain) 멤버 함수입니다.
...
성공할 경우 창의 이전 식별자 그렇지 않으면 0입니다.

### <a name="remarks"></a>설명

창 대화 상자에서 컨트롤 뿐 아니라 모든 자식 창이 수 있습니다. 창의 최상위 창 일 수 없습니다.
...
`SetDlgItemInt` 보냅니다를 [WM_SETTEXT](http://msdn.microsoft.com/library/windows/desktop/ms632644) 메시지 지정된 된 컨트롤입니다.

### <a name="example"></a>예

  예를 참조 하세요 [CWnd::SetDlgItemText](#setdlgitemtext)합니다.

##  <a name="setdlgitemtext"></a>  CWnd::SetDlgItemText

창 또는 대화 상자를 소유 하는 컨트롤의 텍스트 또는 캡션을 설정 합니다.
...
`SetDlgItemText` 보냅니다를 [WM_SETTEXT](http://msdn.microsoft.com/library/windows/desktop/ms632644) 메시지 지정된 된 컨트롤입니다.
...
키보드 입력 창으로 보내지면 및 다양 한 시각 신호를 사용자에 대 한 변경 됩니다. 전경 창에는 현재 사용자가 작업 창이입니다. 전경 창이 최상위 창 (프레임 창 또는 대화 상자)에 적용 됩니다.
...
이전에 입력된 포커스가 있던 창 개체에 대 한 포인터입니다. 이러한 창이 없는 경우 NULL입니다. 반환된 된 포인터는 임시적 이며 저장 되어야 합니다.

### <a name="remarks"></a>설명

입력된 포커스를이 창에 대 한 모든 후속 키보드 입력을 전달합니다. 이전에 입력된 포커스가 있던 모든 창에는 손실 됩니다.

`SetFocus` 멤버 함수는 [WM_KILLFOCUS](http://msdn.microsoft.com/library/windows/desktop/ms646282) 입력 포커스가 있는 창으로 메시지 및 [WM_SETFOCUS](http://msdn.microsoft.com/library/windows/desktop/ms646283) 입력된 포커스를 받는 창에 메시지를 합니다. 창 또는 부모 활성화 됩니다.

현재 창 활성화 되어 있지만 포커스가 없는 경우 (즉, 창이 포커스가), 모든 키를 누를 메시지가 생성 됩니다 [WM_SYSCHAR](#onsyschar), [WM_SYSKEYDOWN](#onsyskeydown), 또는 [WM_SYSKEYUP](#onsyskeyup).

##  <a name="setfont"></a>  CWnd::SetFont

창에 지정 된 글꼴을 사용 하 여 WM_SETFONT 메시지를 보냅니다.
...
창 WM_SETFONT 메시지를 처리 하지 않는 한 효과가 없습니다. 파생 되는 여러 MFC 클래스 `CWnd` WM_SETFONT 메시지에 대 한 메시지 처리기를 포함 하는 미리 정의 된 창 클래스에 연결 하기 때문에이 메시지를 처리 합니다. 클래스에서 파생 되는이 방법을 사용 하려면 `CWnd` WM_SETFONT 메시지에 대 한 메서드 처리기를 정의 해야 합니다.

##  <a name="seticon"></a>  CWnd::SetIcon

식별 한 대로 특정 아이콘으로 핸들을 설정 하려면이 멤버 함수를 호출 *hIcon*합니다.
...
창 클래스 등록 되 면이 아이콘을 선택 합니다.

### <a name="example"></a>예

  예를 참조 하세요 [CWnd::GetSystemMenu](#getsystemmenu)합니다.
...
*bAlpha*  
계층화 된 창의 불투명도 설명 하는 데는 알파 값입니다. 자세한 내용은 참조 하세요. 합니다 `SourceConstantAlpha` 의 멤버는 [BLENDFUNCTION](http://msdn.microsoft.com/library/windows/desktop/dd183393) 구조입니다. 때 *bAlpha* 0 인 창이 완전히 투명 하 게 됩니다. 때 *bAlpha* 255 이면 창은 불투명 합니다.

*dwFlags*  
수행할 동작을 지정 합니다. 이 매개 변수는 다음 값 중 하나 이상의 수 있습니다. 가능한 값 목록을 참조 하세요 [SetLayeredWindowAttributes](http://msdn.microsoft.com/library/windows/desktop/ms633540)합니다.
...
이 멤버 함수는 함수의 기능을 에뮬레이트합니다 [SetLayeredWindowAttributes](http://msdn.microsoft.com/library/windows/desktop/ms633540)Windows SDK에 설명 된 대로 합니다.
...
메뉴 변경 되 면 0이 아닌 값 그렇지 않으면 0입니다.

### <a name="remarks"></a>설명

인해 창에서 메뉴 변경 내용을 적용 하려면 다시 그려져 야 합니다.

`SetMenu` 이전 메뉴를 삭제 하지 않습니다. 응용 프로그램에서 호출 해야 합니다 [CMenu::DestroyMenu](../../mfc/reference/cmenu-class.md#destroymenu) 멤버 함수를이 작업을 수행 합니다.

### <a name="example"></a>예

  예를 참조 하세요 [CMenu::LoadMenu](../../mfc/reference/cmenu-class.md#loadmenu)합니다.

##  <a name="setowner"></a>  CWnd::SetOwner

지정 된 창 개체에 현재 창의 소유자를 설정합니다.
...
그런 다음이 소유자 현재 창 개체에서 명령 메시지를 받을 수 있습니다. 기본적으로 현재 창의 부모에 해당 소유자입니다.

창 계층에 관련 되지 않은 창 개체 간의 연결을 설정 하는 것이 유용 합니다. 예를 들어 [CToolBar](../../mfc/reference/ctoolbar-class.md) 부모 대신 해당 소유자에 게 알림을 전송 합니다. 이 도구 모음을 (예: 내부 프레임 창) 다른 창으로 알림을 전송 하는 동안 (예: OLE 컨테이너 응용 프로그램 창) 하나의 창의 자식 될 수 있습니다. 또한 서버 창 비활성화 되었거나 진행 하는 동안 활성화 하는 경우 편집 프레임 창이 소유한 모든 창은 숨기 거 나 표시 합니다. 이 소유권에 대 한 호출을 사용 하 여 명시적으로 설정 되어 `SetOwner`입니다.
...
성공 하면 이전 부모 창 개체에 대 한 포인터입니다. 반환된 된 포인터 임시 수 있으며 나중에 사용할 저장 되어야 합니다.

### <a name="remarks"></a>설명

자식 창을 표시 하지 않은 경우 Windows는 적절 한 다시 그리기 및 다시 그리기를 수행 합니다.

##  <a name="setproperty"></a>  CWnd::SetProperty

지정 된 OLE 컨트롤 속성을 설정 하려면이 멤버 함수 호출 *dwDispID*합니다.
...
응용 프로그램이 호출 `SetRedraw` 다시 그려져 야 또는 다시 그려지는에서 변경을 방지 하기 위해 변경할 수 있도록 합니다.
...
이 멤버 함수를 설정 하거나 다시 그리기 플래그를 지웁니다. 다시 그리기 플래그가 해제 되는 동안 각 변경 후 업데이트 되지 않습니다 내용과 하지 그릴 다시 그리기 플래그를 설정할 때까지 합니다. 예를 들어, 목록 상자에 몇 가지 항목을 추가 해야 하는 응용 프로그램 다시 그리기 플래그를 지우려면, 항목을 추가한 다음 다시 그리기 플래그를 설정 합니다. 마지막으로 응용 프로그램 호출 수를 [Invalidate](#invalidate) 또는 [InvalidateRect](#invalidaterect) 멤버 함수 목록 상자 그려야 하 게 합니다.
...
이 멤버 함수를 호출 하는 정보를 설정 합니다 `SCROLLINFO` 스크롤 막대에 대 한 구조를 유지 관리 합니다.
...
MFC Windows 메시지 처리기 스크롤 막대 위치를 나타내는 [CWnd::OnHScroll](#onhscroll) 하 고 [CWnd::OnVScroll](#onvscroll)만 16 비트 위치 데이터를 제공 합니다. [GetScrollInfo](#getscrollinfo) 고 `SetScrollInfo` 스크롤 막대 위치 데이터의 32 비트를 제공 합니다. 응용 프로그램을 호출할 수 있습니다 따라서 `GetScrollInfo` 중 하나를 처리 하는 동안 `CWnd::OnHScroll` 또는 `CWnd::OnVScroll` 32 비트 스크롤 막대 위치 데이터를 가져오려고 합니다.

> [!NOTE]
> [CWnd::GetScrollInfo](#getscrollinfo) 응용 프로그램을 32 비트 스크롤 막대 위치를 사용 합니다.

##  <a name="setscrollpos"></a>  CWnd::SetScrollPos

스크롤 상자의 현재 위치를 설정 하 고 요청 된 경우 스크롤 상자의 새 위치를 반영 하도록 스크롤 막대를 다시 그립니다.
...
설정 *bRedraw* FALSE로 유용 때마다 다른 함수를 이후에 호출 하 여 스크롤 막대를 다시 그려집니다.
...
숨기 거 나 표준 스크롤 막대를 표시 하려면 데도 수 있습니다.

응용 프로그램에서 스크롤 막대 알림 메시지를 처리 하는 동안 스크롤 막대를 숨기려면이 함수를 호출 하지 않습니다.

경우에 대 한 호출 `SetScrollRange` 에 대 한 호출 바로 뒤에 오는 합니다 [SetScrollPos](#setscrollpos) 멤버 함수는 *bRedraw* 에서 매개 변수는 `SetScrollPos` 멤버 함수에서 스크롤 막대를 방지 하기 위해 0 이어야 합니다. 두 번 그리고 합니다.

표준 스크롤 막대의 기본 범위는 0 100 까지입니다. Scroll bar 컨트롤의 기본 범위는 빈 (모두를 *nMinPos* 하 고 *nMaxPos* 값은 0). 지정 된 값의 차이 *nMinPos* 하 고 *nMaxPos* INT_MAX 보다 크지 않아야 합니다.
...
*lpfnTimer*  
응용 프로그램에서 제공한 주소를 지정 `TimerProc` 콜백 함수를 처리 합니다 [WM_TIMER](http://msdn.microsoft.com/library/windows/desktop/ms644902) 메시지입니다. WM_TIMER 메시지 응용 프로그램의 메시지 큐에 배치 되며 처리 하 여이 매개 변수가 NULL 인 경우는 `CWnd` 개체입니다.

### <a name="return-value"></a>반환 값

함수에 성공 하는 경우 새로운 타이머의 타이머 식별자입니다. 이 값을 통해 전달 된 값과 같지 않을 수도 합니다 *nIDEvent* 매개 변수입니다. 응용 프로그램에는 반환 값을 항상 전달 해야 합니다 [KillTimer](#killtimer) 멤버 함수는 타이머를 중지 합니다. 성공 하면 0이 아닌 값 그렇지 않을 경우 0입니다.

### <a name="remarks"></a>설명

간격 값을 지정 하 고 시스템 설치 응용 프로그램의 설치 메시지 큐에 WM_TIMER 메시지를 게시 또는 응용 프로그램 정의 메시지 전달 된 간격이 경과할 때마다 `TimerProc` 콜백 함수입니다.

합니다 *lpfnTimer* 콜백 함수 이름을 지정 해야 `TimerProc`를 선언 해야 정적으로 정의 된 다음과 같은 있지만.
...
이 예제에서는 `CWnd::SetTimer`, `CWnd::OnTimer`, 및 `CWnd::KillTimer` WM_TIMER 메시지를 처리할 수 있습니다. 첫 번째 타이머에 2 초 마다 기본 프레임 창에 WM_TIMER 메시지를 보낼 수 있도록 설정 되어 `OnStartTimer`입니다. `OnTimer` 주 프레임 창에 대 한 WM_TIMER 메시지를 처리 하는 이벤트 처리기입니다. 이 메서드는 PC 스피커를에서 2 초 마다 경고음을 낼지 시킵니다. 두 번째 타이머 메시지를 보냅니다 콜백 함수에 3.75 초 마다. `OnStopTimer` 호출 하 여 두 타이머 모두 중지 됩니다 `CWnd::KillTimer` 각 타이머 id
...
도움말 컨텍스트 식별자를 지정된 된 기간을 사용 하 여 연결 하려면이 멤버 함수를 호출 합니다.
...
자식 창에 도움말 컨텍스트 식별자를 찾을 수 없는 경우 부모 창의 식별자를 상속 합니다. 마찬가지로, 소유 된 창을 도움말 컨텍스트 식별자에 없는 경우 해당 소유자 창의 식별자를 상속 합니다. 이 상속 도움말 컨텍스트 식별자의 대화 상자에 대 한 하나의 식별자와 해당 컨트롤의 모든 설정에 응용을 프로그램을 수 있습니다.
...
- `wndTop` Z-순서의 맨 위에 있는 창을 배치합니다.

- `wndTopMost` 창의 모든 최상위 아닌 windows를 배치합니다. 비활성화 된 경우에 창 맨 위에 있는 위치로 유지 관리 합니다.

- `wndNoTopMost` 모든 비 최상위 창의 위쪽 창의 위치를 변경 (즉, 모든 최상위 창 뒤). 이 플래그는 맨 위 창 이미입니다 하는 경우에 효과가 없습니다.
...
- 창을 활성화 되지 SWP_NOACTIVATE 않습니다. 이 플래그를 설정 하지 않으면, 창 활성화 되며 맨 위 또는 맨 위 그룹의 맨 위로 이동 (설정에 따라를 *pWndInsertAfter* 매개 변수).

- SWP_NOCOPYBITS는 클라이언트 영역의 전체 내용을 삭제합니다. 이 플래그를 지정 하지 않으면 클라이언트 영역의 올바른 콘텐츠 저장 되며 창이 크기 또는 위치가 변경 되 면 클라이언트 영역에 다시 복사 됩니다.

- 현재 위치 SWP_NOMOVE 유지 (무시 합니다 *x* 하 고 *y* 매개 변수).

- Z-순서에서 소유자 창의 위치를 변경할 SWP_NOOWNERZORDER 않습니다.

- 변경 내용을 다시 그려지지 SWP_NOREDRAW 않습니다. 이 플래그를 설정 하는 경우 모든 종류의 없습니다 다시 발생 합니다. 클라이언트 영역, 비클라이언트 영역 (제목 및 스크롤 막대가 포함) 및 부모 창 이동된 창 결과로 처리 되지 않는 부분이이 적용 됩니다. 이 플래그를 설정 하는 경우 응용 프로그램 무효화 하거나 다시 그려야 하는 부모 창과 창의 모든 부분이 다시 그리도록 명시적으로 해야 합니다.

- SWP_NOREPOSITION SWP_NOOWNERZORDER 동일 합니다.

- SWP_NOSENDCHANGING은 WM_WINDOWPOSCHANGING 메시지 수신을 창을 방지 합니다.

- 현재 크기 SWP_NOSIZE 유지 (무시 합니다 *cx* 하 고 *cy* 매개 변수).

- 현재 정렬 SWP_NOZORDER 유지 (무시 *pWndInsertAfter*).

- SWP_SHOWWINDOW 창을 표시합니다.

### <a name="return-value"></a>반환 값

함수에 성공 하면 0이 아닌 값 그렇지 않을 경우 0입니다.

### <a name="remarks"></a>설명

Windows가 Z-순서에 따라 화면에 따라 정렬 됩니다. Z-순서의 맨 위에 있는 창 순서 대로 다른 모든 창 위에 나타납니다.

모든 자식 창에 대 한 좌표가 클라이언트 좌표 (기준으로 부모 창의 클라이언트 영역의 왼쪽 위 모퉁이).

창이 Z-순서의 맨 이동할 수 있습니다 설정 합니다 *pWndInsertAfter* 매개 변수를 **& wndTopMost** 및 SWP_NOZORDER 플래그가 설정 되어 있지 않음을 확인 또는를 창의 Z 순서를 설정 하 모든 기존 맨 위 창 위에 것입니다. 맨 위 창에 최상위 수행 될 경우 소유 된 창 맨 위에 있는 이루어집니다. 해당 소유자가 변경 되지 않습니다.

맨 위 창 아래로 위치가 변경 되 면 맨 위에 있는 더 이상 ( **& wndBottom**) Z-순서 또는 모든 맨 위 창 후 합니다. 최상위 창 맨 위 수행 되 면 맨 위 windows 내용이 모든 해당 소유자 및 소유 된 창입니다.
...
맨 위 창에는 최상위 창 소유할 수 있지만 그 반대 방향은 불가능 합니다. 최상위 창이 소유한 모든 창 (예: 대화 상자)가 소유한 모든 windows 소유자 위에 유지 되도록 맨 위 창에 대 한 자체입니다.

Windows 버전 3.1 이상에서 windows Z 순서의 맨 위로 이동 하 고 있습니다 WS_EX_TOPMOST 스타일을 설정 하 여 잠금 수 수 있습니다. 이러한 최상위 창에 최상위 위치로 비활성화 하는 경우에 유지 관리 합니다. 예를 들어 WinHelp 항상 위 명령을 topmost 도움말 창 고를 사용 하면 선택한 응용 프로그램에 반환 하는 경우 계속 표시 됩니다.

최상위 창, 호출 `SetWindowPos` 사용 하 여 합니다 *pWndInsertAfter* 매개 변수를 **& wndTopMost**, 창을 만들 때 WS_EX_TOPMOST 스타일을 설정 또는 합니다.

WS_EX_TOPMOST 스타일을 사용 하 여 모든 창을 포함 하는 Z 순서를 사용 하 여 창을 이동 합니다 **& wndTopMost** 값의 모든 최상위 windows에서는 하지만 아래 모든 최상위 창 위쪽에 배치 됩니다. WS_EX_TOPMOST 비트 하지 않고 비활성화 된 창의 활성화 하는 응용 프로그램 창의 모든 맨 위 창 높지만 모든 최상위 창 이동 됩니다.

경우 `SetWindowPos` 될 때 호출 되는 *pWndInsertAfter* 매개 변수는 **& wndBottom** 및 `CWnd` 최상위 창에는 창 상태가 손실 됩니다 최상위 (WS_EX_TOPMOST 취소), 및 창이 Z-순서의 맨 아래에 배치 됩니다.
...
함수가 성공 하면 반환 값은 0이 아닌 값입니다. 함수가 실패하면 반환 값은 0입니다.

### <a name="remarks"></a>설명

창의 창 영역 좌표가 되지 않은 창의 클라이언트 영역 창의 왼쪽 위 모퉁이 기준으로 합니다.

호출에 성공한 후 `SetWindowRgn`, 운영 체제 영역 핸들에 의해 지정 된 지역 소유 *hRgn*합니다. 운영 체제에 영역의 복사 해도, 따라서 없도록 더 이상이 영역 핸들을 사용 하 여 함수 호출 및이 지역 핸들을 닫지 마세요.

##  <a name="setwindowtext"></a>  CWnd::SetWindowText

지정 된 텍스트 창의 제목을 설정합니다.
...
창 컨트롤이 면 텍스트 컨트롤에 설정 됩니다.

이 함수는 [WM_SETTEXT](http://msdn.microsoft.com/library/windows/desktop/ms632644) 이 창에 보낼 메시지입니다.
...
`ShowCaret` 멤버 함수는 현재 셰이프가 및 없습니다 숨겨진 두 번 이상 연속 하는 경우 캐럿만 보여 줍니다. 캐럿이이 창에서 소유 하지 않은, 경우에 캐럿 표시 되지 않습니다.

캐럿을 숨기는 누적 됩니다. 경우는 [HideCaret](#hidecaret) 멤버 함수를 호출한 5 회 연속으로 `ShowCaret` 캐럿을 표시 하려면 5 번을 호출 해야 합니다.

캐럿이 공유 리소스입니다. 창에 입력된 포커스가 또는 활성화 된 경우에 캐럿을 표시 됩니다.

### <a name="example"></a>예

  예를 참조 하세요 [CWnd::CreateCaret](#createcaret)합니다.
...
### <a name="example"></a>예
  ...
  예를 참조 하세요 [CWnd::SetWindowPos](#setwindowpos)합니다.
...
창을 이전에 표시 되었으면 0이 아닌 값 인 경우 0는 `CWnd` 이전에 숨겨져 있습니다.

### <a name="remarks"></a>설명

`ShowWindow` 사용 하 여 주 창에는 응용 프로그램당 한 번만 호출 해야 합니다 [CWinApp::m_nCmdShow](../../mfc/reference/cwinapp-class.md#m_ncmdshow)합니다. 에 대 한 후속 호출 `ShowWindow` 에 의해 지정 된 대신 위에 나열 된 값 중 하나를 사용 해야 `CWinApp::m_nCmdShow`합니다.

### <a name="example"></a>예

  예를 참조 하세요 [CWnd::CalcWindowRect](#calcwindowrect)합니다.

##  <a name="subclassdlgitem"></a>  CWnd::SubclassDlgItem

대화 상자 템플릿에서 만들어진 컨트롤 "동적으로 하위 클래스"를이 멤버 함수를 호출 하 고이 연결 `CWnd` 개체입니다.
...
Windows 메시지를 라우트해야 컨트롤을 동적으로 서브클래싱된 경우는 `CWnd`의 메시지 맵 및 메시지 처리기에서 호출 된 `CWnd`먼저 클래스의 합니다. 기본 클래스에 전달 되는 메시지는 컨트롤의 기본 메시지 처리기로 전달 됩니다.

이 멤버 함수를 Windows 컨트롤에 연결 된 `CWnd` 개체 및 컨트롤의 대체 `WndProc` 및 `AfxWndProc` 함수입니다. 함수 저장 이전 `WndProc` 반환한 위치에는 `GetSuperWndProcAddr` 멤버 함수입니다.
...
창 "동적으로 하위 클래스"를이 멤버 함수를 호출 하 고이 연결 `CWnd` 개체입니다.
...
Windows 메시지를 통해 라우팅되는 창을 동적으로 서브클래싱된 경우는 `CWnd`의 메시지 맵 및 메시지 처리기에서 호출을 `CWnd`먼저 클래스의 합니다. 기본 클래스에 전달 되는 메시지 창에서 기본 메시지 처리기로 전달 됩니다.

이 멤버 함수를 Windows 컨트롤에 연결을 `CWnd` 개체 및 창의 대체 `WndProc` 및 `AfxWndProc` 함수입니다. 이전에 대 한 포인터를 저장 합니다 `WndProc` 에 `CWnd` 개체입니다.

> [!NOTE]
> 창을 연결 해야 합니다 하지 이미 MFC 개체에이 함수가 호출 될 때입니다.
...
로 잠긴 창의 잠금을 해제 하려면이 멤버 함수 호출 `CWnd::LockWindowUpdate`합니다.
...
사용 하 여 한 번에 하나의 창만 잠글 수 `LockWindowUpdate`입니다. 참조 [CWnd::LockWindowUpdate](#lockwindowupdate) Win32 함수나 [LockWindowUpdate](http://msdn.microsoft.com/library/windows/desktop/dd145034) windows 잠금에 대 한 자세한 내용은 합니다.

##  <a name="unsubclasswindow"></a>  CWnd::UnsubclassWindow

설정 하려면이 멤버 함수 호출 `WndProc` 원래 값으로 돌아가서에서 HWND로 식별 되는 창 분리는 `CWnd` 개체입니다.
...
대화 상자에 데이터를 초기화 하거나 검색 하 고 대화 상자 데이터 유효성을 검사 하려면이 멤버 함수를 호출 합니다.
...
작업에 성공 하면 0이 아닌 값 그렇지 않으면 0입니다. 하는 경우 *bSaveAndValidat*e가 TRUE 이면 반환 값이 0이 아닌 값 이면 데이터의 유효성이 성공적으로 검사 됩니다.

### <a name="remarks"></a>설명

프레임 워크를 자동으로 호출 `UpdateData` 사용 하 여 *bSaveAndValidate* 의 기본 구현에서 모달 대화 상자를 만들 때 FALSE로 설정 [CDialog::OnInitDialog](../../mfc/reference/cdialog-class.md#oninitdialog)합니다. 호출 대화 상자 표시 되기 전에 발생 합니다. 기본 구현을 [CDialog::OnOK](../../mfc/reference/cdialog-class.md#onok) 사용 하 여이 멤버 함수 호출 *bSaveAndValidate* 데이터를 검색 하려면 TRUE로 설정 하 고 성공 하면 대화 상자를 기준으로 닫힙니다. (대화 상자에서 취소 단추를 클릭 하는 경우 대화 상자를 하지 않고 닫아 검색 중인 데이터입니다.)

##  <a name="updatedialogcontrols"></a>  CWnd::UpdateDialogControls

대화 상자 단추 및 대화 상자 또는 사용 하는 창에서 다른 컨트롤의 상태를 업데이트 하려면이 멤버 함수를 호출 합니다 [ON_UPDATE_COMMAND_UI](message-map-macros-mfc.md#on_update_command_ui) 콜백 메커니즘입니다.
...
자식 컨트롤에 처리기가 없는 경우와 *bDisableIfNoHndler* 가 TRUE 인 자식 컨트롤을 사용할 수 없습니다.

프레임 워크를이 멤버 함수를 호출 대화 상자 모음 또는 도구 모음 컨트롤에 대 한 응용 프로그램의 일부로 유휴 처리 합니다.
...
*pblend*  
에 대 한 포인터를 [BLENDFUNCTION](http://msdn.microsoft.com/library/windows/desktop/dd183393) 계층화 된 창을 작성할 때 사용할 투명도 값을 지정 하는 구조입니다.

*dwFlags*  
수행할 동작을 지정 합니다. 이 매개 변수는 다음 값 중 하나 이상의 수 있습니다. 가능한 값 목록을 참조 하세요 [UpdateLayeredWindow](http://msdn.microsoft.com/library/windows/desktop/ms633556)합니다.
...
이 멤버 함수는 함수의 기능을 에뮬레이트합니다 [UpdateLayeredWindow](http://msdn.microsoft.com/library/windows/desktop/ms633556)Windows SDK에 설명 된 대로 합니다.

##  <a name="updatewindow"></a>  CWnd::UpdateWindow

클라이언트 영역을 전송 하 여 업데이트를 [WM_PAINT](http://msdn.microsoft.com/library/windows/desktop/dd145213) 업데이트 영역에 비어 있지 않은 경우 메시지입니다.
...
`UpdateWindow` 멤버 함수에서 WM_PAINT 메시지를 직접 응용 프로그램 큐 무시 하 고 보냅니다. 업데이트 영역을 선택 하지 않으면 WM_PAINT 전송 되지 않습니다.
...
창의 업데이트 영역에서 사각형을 제거 하 여 지정된 된 사각형 내에서 클라이언트 영역을 확인 합니다.
...
합니다 [BeginPaint](#beginpaint) 멤버 함수는 전체 클라이언트 영역에 자동으로 유효성을 검사 합니다. 모두를 `ValidateRect` 또는 [ValidateRgn](#validatergn) 업데이트 영역의 부분 하기 전에 유효성을 검사 해야 하는 경우 멤버 함수를 호출 해야 [WM_PAINT](http://msdn.microsoft.com/library/windows/desktop/dd145213) 다음 생성 됩니다.

Windows 현재 업데이트 영역 유효성을 검사할 때까지 WM_PAINT 메시지를 생성 하는 계속 됩니다.
...
지정된 된 영역 만들어야만 이전 지역 함수에서. 지역 좌표를 클라이언트 좌표로 간주 됩니다.

합니다 [BeginPaint](#beginpaint) 멤버 함수는 전체 클라이언트 영역에 자동으로 유효성을 검사 합니다. 모두를 [ValidateRect](#validaterect) 또는 `ValidateRgn` 다음 부분 업데이트 영역의 유효성을 검사 해야 하는 경우 멤버 함수를 호출 해야 [WM_PAINT](http://msdn.microsoft.com/library/windows/desktop/dd145213) 메시지가 생성 됩니다.

##  <a name="windowfrompoint"></a>  CWnd::WindowFromPoint

지정된 된 지점이 포함 된 창을 검색 합니다. *지점* 화면에서 요소의 화면 좌표를 지정 해야 합니다.
...
점이 있는 창 개체에 대 한 포인터입니다. 창이 지정 된 지점에 존재 하는 경우 NULL입니다. 반환된 된 포인터 임시 수 있으며 나중에 사용할 저장 되어야 합니다.

### <a name="remarks"></a>설명

`WindowFromPoint` 경우에 지점 창 내에서 숨김 또는 비활성화 된 창의 검색 하지 않습니다. 응용 프로그램을 사용 해야 합니다 [ChildWindowFromPoint](#childwindowfrompoint) 제한적인 검색에 대 한 멤버 함수입니다.
...
*nCmd*  
요청한 도움말의 형식을 지정합니다. 가능한 값 목록과 미치는 영향에 대 한 합니다 *dwData* 매개 변수를 참조 합니다 [WinHelp](http://msdn.microsoft.com/library/windows/desktop/bb762267) Windows SDK에 대 한 Windows 함수입니다.

### <a name="remarks"></a>설명

참조 [CWinApp::WinHelp](../../mfc/reference/cwinapp-class.md#winhelp) 자세한 내용은 합니다.

##  <a name="registertouchwindow"></a>  CWnd::RegisterTouchWindow

등록 하거나 Windows touch 지원을 등록을 취소 합니다.
