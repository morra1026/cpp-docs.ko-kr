...
|[CMFCRibbonGallery::EnableMenuResize](#enablemenuresize)|사용 하거나 메뉴 패널의 크기 조정 사용 하지 않도록 설정 합니다.|
|[CMFCRibbonGallery::EnableMenuSideBar](#enablemenusidebar)|팝업 메뉴의 왼쪽 세로 막대를 사용 하지 않도록 설정 하거나 합니다.|
...
|[CMFCRibbonGallery::GetIconsInRow](#geticonsinrow)|리본 갤러리의 행의 항목 수를 반환합니다.|
|[CMFCRibbonGallery::GetItemToolTip](#getitemtooltip)|갤러리의 항목과 연결 된 도구 설명 텍스트를 반환 합니다.|
|[CMFCRibbonGallery::GetLastSelectedItem](#getlastselecteditem)|사용자가 선택한 갤러리에서 마지막 항목의 인덱스를 반환 합니다.|
...
|[CMFCRibbonGallery::IsMenuResizeEnabled](#ismenuresizeenabled)|메뉴 크기 조정의 사용 여부를 지정 합니다.|
...
|[CMFCRibbonGallery::OnAfterChangeRect](#onafterchangerect)|( `CMFCRibbonButton::OnAfterChangeRect`을 재정의합니다.)|
|[CMFCRibbonGallery::OnDraw](#ondraw)|(재정의 [cmfcribbonbutton:: Ondraw](../../mfc/reference/cmfcribbonbutton-class.md#ondraw).)|
|[CMFCRibbonGallery::OnEnable](#onenable)|( `CMFCRibbonBaseElement::OnEnable`을 재정의합니다.)|
...
|[CMFCRibbonGallery::RemoveItemToolTips](#removeitemtooltips)|갤러리에서 모든 항목에서 도구 설명을 제거합니다.|
...
|[CMFCRibbonGallery::SetButtonMode](#setbuttonmode)|드롭다운 단추 또는 리본에서 직접 색상표 리본 갤러리를 표시할지 여부를 지정 합니다.|
|[CMFCRibbonGallery::SetGroupName](#setgroupname)|그룹의 이름을 설정합니다.|
|[CMFCRibbonGallery::SetIconsInRow](#seticonsinrow)|갤러리에서 행당 항목 수를 정의합니다.|
|[CMFCRibbonGallery::SetItemToolTip](#setitemtooltip)|갤러리에서 항목에 대 한 도구 설명 텍스트를 설정합니다.|
|[CMFCRibbonGallery::SetPalette](#setpalette)|리본 갤러리 색상표에 연결합니다.|
|[CMFCRibbonGallery::SetPaletteID](#setpaletteid)|갤러리 항목을 선택 하는 경우 WM_COMMAND 메시지에 전송 되는 명령 ID를 정의 합니다.|
...
설정 전에 갤러리 항목을 삽입할 *bOnTop* TRUE로 합니다. 설정할 *bOnTop* 갤러리 아래의 항목을 삽입 하려면 FALSE로 합니다.

> [!NOTE]
>  매개 변수 *nIndex* 갤러리의 맨 위에 있는 및 갤러리의 맨 아래에 삽입 하는 인덱스를 지정 합니다. 갤러리 하기 전에 항목 한 위치를 삽입 해야 할 경우 설정 하는 예를 들어 *nIndex* 1 및 *bOnTop* TRUE로 합니다. 갤러리 아래의 항목 하나의 위치를 삽입 해야 하는 경우 마찬가지로 설정할 *nIndex* 1 및 *bOnTop* FALSE로 합니다.
...
리본 갤러리의 모든 콘텐츠를 제거 하려면이 메서드를 호출 합니다. 이 리본 갤러리에 새 리본 갤러리 또는 그룹의 집합을 연결 하기 전에 수행 되어야 합니다.

##  <a name="cmfcribbongallery"></a>  CMFCRibbonGallery::CMFCRibbonGallery

생성 하 고 초기화 된 [CMFCRibbonGallery](../../mfc/reference/cmfcribbongallery-class.md) 개체입니다.
...
*nLargeImageIndex*<br/>
단추에 표시할 큰 이미지의 0부터 시작 하는 인덱스입니다.

*imagesPalette*<br/>
에 대 한 참조를 [CMFCToolBarImages](../../mfc/reference/cmfctoolbarimages-class.md) 갤러리에 표시할 이미지를 포함 하는 개체입니다.
...
*sizeIcon*<br/>
갤러리 이미지의 픽셀에서 크기를 지정합니다.

*nIconsNum*<br/>
갤러리의 아이콘 수를 지정 합니다.

*bDefaultButtonStyle*<br/>
기본 또는 소유자가 그린 단추 스타일을 사용할지 여부를 지정 합니다.

### <a name="remarks"></a>설명

##  <a name="enablemenuresize"></a>  CMFCRibbonGallery::EnableMenuResize

사용 하거나 메뉴 패널의 크기 조정 사용 하지 않도록 설정 합니다.
...
팝업 메뉴의 왼쪽 세로 막대를 사용 하지 않도록 설정 하거나 합니다.
...
사용자가 선택한 리본 갤러리에서 마지막 항목의 인덱스를 반환 합니다.
...
색상표 드롭 다운 메뉴 단추;으로 표시 되 면 TRUE입니다. FALSE 이면 색상표를 리본에서 직접 표시 됩니다.
...
TRUE 이면 메뉴 크기 조정에 사용 하도록 설정 합니다. 그렇지 않으면 FALSE입니다.
...
Office XP 스타일 세로 막대의 팝업 메뉴의 왼쪽에 그려지는 경우 TRUE입니다. 그렇지 않으면 FALSE입니다.
...
갤러리를 다시 그리도록 하려면이 함수를 호출 합니다. 런타임 시 갤러리의 내용을 변경한 경우이 메서드를 호출 해야 합니다.

##  <a name="removeitemtooltips"></a>  CMFCRibbonGallery::RemoveItemToolTips

갤러리에서 모든 항목에서 도구 설명을 제거합니다.
...
드롭다운 단추 또는 리본에서 직접 색상표 리본 갤러리를 표시할지 여부를 결정 합니다.
...
사용자가 갤러리 항목을 선택할 때 WM_COMMAND 메시지에 전송 되는 명령 ID를 정의 합니다.
