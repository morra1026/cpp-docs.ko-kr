...
|[CMFCToolBar::CalcSize](#calcsize)|레이아웃 계산 프로세스의 일부로 프레임 워크에서 호출 됩니다. (재정의 [CPane::CalcSize](../../mfc/reference/cpane-class.md#calcsize).)|
...
|[CMFCToolBar::IsBasicCommand](#isbasiccommand)|기본 명령 목록에서 명령을 인지 확인 합니다.|
...
1. 주 프레임 창에 대 한 WM_CREATE 메시지를 처리할 때 호출 [CMFCToolBar::Create](#create) 하거나 [CMFCToolBar::CreateEx](#createex) 도구 모음을 만들고 해당 스타일을 지정 합니다.
...
일반적으로 이 메서드를 사용할 필요가 없습니다. 사용 하도록 설정 하려면 합니다 **사용자 지정** 호출 도구 모음에 표시 되는 단추를 [cmfctoolbar:: Enablecustomizebutton](#enablecustomizebutton) 메서드. 사용 하도록 설정 합니다 **표시 단추를 한 줄** 또는 **표시 단추를 두 줄** 단추를 호출 [cmfctoolbar:: Setsiblingtoolbar](#setsiblingtoolbar)합니다.
...
프레임 워크 도구 모음 레이아웃을 계산 하려면이 메서드를 호출 합니다. 도구 모음을 다른 열 너비를 지정 하는 파생된 클래스에서이 메서드를 재정의 합니다.

##  <a name="getcommandbuttons"></a>  CMFCToolBar::GetCommandButtons

모든 도구 모음에서 지정 된 명령 ID를 응용 프로그램에 있는 단추의 목록을 반환 합니다.
...
단추 및 도구 모음의 구분 기호 수입니다.

##  <a name="getcustomizebutton"></a>  CMFCToolBar::GetCustomizeButton

에 대 한 포인터를 검색 합니다 `CMFCCustomizeButton` 도구 모음을 사용 하 여 연결 된 개체입니다.
...
에 대 한 포인터를 `CMFCCustomizeButton` 도구 모음을 사용 하 여 연결 된 개체입니다.

### <a name="remarks"></a>설명

이 메서드를 검색 합니다 **사용자 지정** 끝 도구 모음에 나타나는 단추입니다. 사용 하 여는 [cmfctoolbar:: Enablecustomizebutton](#enablecustomizebutton) 메서드를 추가 합니다 **사용자 지정** 도구 모음에 단추.

호출할 수 있습니다 합니다 [CMFCToolBar::IsExistCustomizeButton](#isexistcustomizebutton) 도구 모음을 잘못 포함 되어 있는지 확인 하는 방법 `CMFCCustomizeButton` 개체입니다.

##  <a name="getdefaultimage"></a>  CMFCToolBar::GetDefaultImage

지정 된 명령 ID 사용 하 여 도구 모음 단추에 대 한 기본 이미지의 인덱스를 반환합니다.
...
이 공유 메서드를 사용 하 여 지정 된 명령 ID 사용 하 여 도구 모음 단추에 대 한 기본 이미지의 인덱스를 검색 합니다. 반환 값은 응용 프로그램의 모든 도구 모음에 대 한 도구 모음 단추 이미지의 공유 컬렉션의 인덱스입니다. 호출 된 [CMFCToolBar::GetImages](#getimages) 메서드를이 컬렉션에 대 한 포인터를 가져옵니다.
...
사용 하 여 비활성화 된 도구 모음 단추 이미지를 로드 합니다 [CMFCToolBarEditBoxButton 클래스](../../mfc/reference/cmfctoolbareditboxbutton-class.md) 하 고 [CMFCToolBar::LoadBitmap](#loadbitmap) 메서드.

##  <a name="getdisabledmenuimages"></a>  CMFCToolBar::GetDisabledMenuImages

응용 프로그램에서 비활성화 된 메뉴 단추에 사용 되는 이미지의 컬렉션에 대 한 포인터를 반환 합니다.
...
사용 하 여 비활성된 이미지를 로드 합니다 [CMFCToolBarEditBoxButton 클래스](../../mfc/reference/cmfctoolbareditboxbutton-class.md) 메서드.
...
이 메서드는 NULL이 아닌 값을 반환 하는 경우 및 *pIndex* 가 NULL이 가리키는 값 *pIndex* 메뉴 단추 도구 모음 단추 컬렉션의 인덱스에 설정 됩니다.

##  <a name="getgraydisabledbuttons"></a>  CMFCToolBar::GetGrayDisabledButtons

이미지의 비활성화 된 단추가 흐리게 표시 되는 일반 단추 이미지 버전 되는지 여부를 지정 또는 사용 안 함된 단추 이미지의 컬렉션에서 사용 합니다.
...
비활성화 된 단추가;의 이미지를 흐리게 표시. 비활성된 이미지의 컬렉션에서 이미지를 가져오려면 FALSE입니다.

### <a name="remarks"></a>설명

사용 하 여 [CMFCToolBar::SetGrayDisabledButtons](#setgraydisabledbuttons) 흐리게 이미지와 비활성된 이미지의 컬렉션에서 이미지 간에 전환할 수 있습니다.
...
도구 모음 단추 개체;에 대 한 포인터 또는 없습니다 단추가 강조 표시 하는 경우 NULL입니다.

### <a name="remarks"></a>설명

키보드 포커스가 있으면 도구 모음 단추를 강조 표시 됩니다. 도구 모음 단추는이 응용 프로그램에서 작동 중 추적 하는 경우 도구 모음 단추가 강조 표시도 (자세한 내용은 [CMFCToolBar::GetHotBorder](#gethotborder) 하 고 [CMFCToolBar::SetHotBorder](#sethotborder)) 및 마우스 가 없는 도구 모음 단추 또는 메뉴 항목에 키보드 포커스가 있을 때에서 가리키고 있습니다.

##  <a name="gethotborder"></a>  CMFCToolBar::GetHotBorder

도구 모음 단추가 있는지 여부를 결정 *핫 트래킹*합니다. 단추 핫 트래킹 이면 가로질러 마우스를 이동할 때 강조 표시 됩니다.
...
핫 트랙; 도구 모음 단추 경우 TRUE 그렇지 않으면 FALSE입니다.

### <a name="remarks"></a>설명

기본적으로 도구 모음 단추 핫 트랙 됩니다.
...
도구 모음 않았습니다 직전에 입력된 포커스가 있는 창에 대 한 핸들을 반환 합니다.
...
파생 되지 않은 창 핸들 [CMFCBaseToolBar 클래스](../../mfc/reference/cmfcbasetoolbar-class.md), 이전에 입력; 중점을 두거나 창인지 없는 경우 NULL이 있었습니다.

### <a name="remarks"></a>설명

경우는 `CMFCToolBar` 컨트롤이 입력된 포커스를 받을, 나중에 복원할 수 있도록 포커스를 손실 하는 창에 핸들을 저장 합니다.

##  <a name="getignoresettext"></a>  CMFCToolBar::GetIgnoreSetText

단추 레이블 설정에 대 한 호출을 무시할지 여부를 지정 합니다.
...
TRUE 이면 단추 레이블 설정에 대 한 호출에 대해서는; 그렇지 않으면 FALSE입니다.

### <a name="remarks"></a>설명

##  <a name="getimages"></a>  CMFCToolBar::GetImages

포인터를 반환 기본 컬렉션 단추 이미지 응용 프로그램에서입니다.
...
에 대 한 포인터를 [CMFCToolBarImages 클래스](../../mfc/reference/cmfctoolbarimages-class.md) 응용 프로그램의 모든 도구 모음에 대 한 기본 이미지의 컬렉션을 포함 하는 개체입니다.

### <a name="remarks"></a>설명

이 공유 메서드 응용 프로그램에 대 한 모든 기본 컬렉션에 대 한 도구 모음 이미지를 제공 합니다. 호출 된 [CMFCToolBar::LoadBitmap](#loadbitmap) 이미지 컬렉션에 추가 하는 방법입니다.
...
도구 모음 단추 이미지의 전역 목록에서이 도구 모음에 대 한 도구 모음 단추 이미지를 찾는 데 인덱스 오프셋을 반환 합니다.
...
모든 도구 모음 기본 이미지는 전역에 저장 됩니다 [CMFCToolBarImages 클래스](../../mfc/reference/cmfctoolbarimages-class.md) 목록입니다. 도구 모음의 각 단추에 대 한 이미지 목록에 연속적으로 저장 됩니다. 이미지의 인덱스에 계산 하려면 도구 모음에서 단추의 인덱스가 해당 도구 모음 단추에 대 한 이미지 목록 부분에 대 한 오프셋을 추가 합니다.

호출 [CMFCToolBar::ButtonToIndex](#buttontoindex) 단추에 대 한 포인터를 지정 하는 도구 모음 단추의 인덱스를 가져오려고 합니다.

호출 [CMFCToolBar::GetImages](#getimages) 도구 모음 이미지의 컬렉션에 대 한 포인터를 가져오려고 합니다.
...
지정된 된 인덱스에서 단추의 경계 사각형을 반환 합니다.
...
콜드 이미지는 사용자가 도구 모음 단추 상호 작용 하지 때 사용 되는 이미지입니다. 호출 [CMFCToolBar::LoadBitmapEx](#loadbitmapex) 콜드 큰 이미지를 로드 하려고 합니다.

##  <a name="getlargedisabledimages"></a>  CMFCToolBar::GetLargeDisabledImages

응용 프로그램에서 큰 비활성화 된 도구 모음 단추 이미지의 컬렉션에 대 한 포인터를 반환합니다.
...
큰 이미지는 일반 도구 모음 단추 이미지의 큰 버전입니다. 호출 [CMFCToolBar::LoadBitmapEx](#loadbitmapex) 하거나 [CMFCToolBar::LoadBitmap](#loadbitmap) 큰 이미지를 로드 하려고 합니다.
...
큰 이미지는 일반 도구 모음 단추 이미지의 큰 버전입니다. 호출 [CMFCToolBar::LoadBitmapEx](#loadbitmapex) 큰 이미지를 로드 하려고 합니다.
...
잠긴된 콜드 이미지 또는 도구 모음 잠겨 있지 않은 경우 NULL 컬렉션에 대 한 포인터입니다.

### <a name="remarks"></a>설명

잠긴된 이미지는 사용자 도구 모음을 사용자 지정할 수 없습니다 하는 경우 프레임 워크를 사용 하는 일반 도구 모음 단추 이미지의 버전입니다. 콜드 이미지는 사용자가 도구 모음 단추 상호 작용 하지 때 사용 되는 이미지입니다.

이 메서드는 도구 모음 잠겨 있지 않은 경우 NULL을 반환 합니다. 이 메서드 생성 어설션 실패가 디버그 빌드에서 도구 모음 잠겨 있지 않은 경우. 잠긴된 도구 모음에 대 한 자세한 내용은 참조 하세요. [CMFCToolBar::IsLocked](#islocked)합니다.

호출 된 [CMFCToolBar::LoadBitmapEx](#loadbitmapex) 잠긴된 콜드 이미지를 로드 하는 방법입니다.

##  <a name="getlockeddisabledimages"></a>  CMFCToolBar::GetLockedDisabledImages

도구 모음에서 잠긴된 비활성된 이미지의 컬렉션에 대 한 포인터를 반환합니다.
...
잠긴된 비활성된 이미지 또는 도구 모음 잠겨 있지 않은 경우 NULL 컬렉션에 대 한 포인터입니다.

### <a name="remarks"></a>설명

잠긴된 이미지는 사용자 도구 모음을 사용자 지정할 수 없습니다 하는 경우 프레임 워크를 사용 하는 일반 도구 모음 단추 이미지의 버전입니다. 비활성된 이미지는 단추 TBBS_DISABLED 스타일에는 프레임 워크에서 사용 되는 이미지입니다.

이 메서드는 도구 모음 잠겨 있지 않은 경우 NULL을 반환 합니다. 이 메서드 생성 어설션 실패가 디버그 빌드에서 도구 모음 잠겨 있지 않은 경우. 잠긴된 도구 모음에 대 한 자세한 내용은 참조 하세요. [CMFCToolBar::IsLocked](#islocked)합니다.

호출 된 [CMFCToolBar::LoadBitmapEx](#loadbitmapex) 개체는 잠금에 대 한 이미지를 사용 하지 않도록 설정 합니다.
...
잠긴된 도구 모음 단추 이미지의 도구 모음 잠겨 있지 않은 경우 NULL 컬렉션에 대 한 포인터입니다.

### <a name="remarks"></a>설명

잠긴된 이미지는 사용자 도구 모음을 사용자 지정할 수 없습니다 하는 경우 프레임 워크를 사용 하는 일반 도구 모음 단추 이미지의 버전입니다.
...
A `CSize` 빈 잠긴된 도구 모음 이미지의 크기를 지정 하는 구조 `CSize` 도구 모음 잠겨 있지 않은 경우 구조체입니다.

### <a name="remarks"></a>설명

잠긴된 이미지는 사용자 도구 모음을 사용자 지정할 수 없습니다 하는 경우 프레임 워크를 사용 하는 일반 도구 모음 단추 이미지의 버전입니다.

이 메서드는 반환 된 `CSize` 도구 모음 잠겨 있지 않은 경우 0 너비와 높이가 0 구조체입니다. 이 메서드 생성 어설션 실패가 디버그 빌드에서 도구 모음 잠겨 있지 않은 경우. 잠긴된 도구 모음에 대 한 자세한 내용은 참조 하세요. [CMFCToolBar::IsLocked](#islocked)합니다.

호출 된 [CMFCToolBar::SetLockedSizes](#setlockedsizes) 잠긴된 이미지 크기를 지정 하는 방법입니다.

##  <a name="getlockedmenuimages"></a>  CMFCToolBar::GetLockedMenuImages

포인터를 반환 잠긴 도구 모음의 컬렉션 메뉴 이미지의 도구 모음입니다.
...
잠긴된 도구 모음 메뉴 이미지 또는 도구 모음 잠겨 있지 않은 경우 NULL 컬렉션에 대 한 포인터입니다.

### <a name="remarks"></a>설명

잠긴된 이미지는 사용자 도구 모음을 사용자 지정할 수 없습니다 하는 경우 프레임 워크를 사용 하는 일반 도구 모음 메뉴 이미지의 버전입니다.

이 메서드는 도구 모음 잠겨 있지 않은 경우 NULL을 반환 합니다. 이 메서드 생성 어설션 실패가 디버그 빌드에서 도구 모음 잠겨 있지 않은 경우. 잠긴된 도구 모음에 대 한 자세한 내용은 참조 하세요. [CMFCToolBar::IsLocked](#islocked)합니다.

호출 된 [CMFCToolBar::LoadBitmapEx](#loadbitmapex) 잠긴된 메뉴 이미지를 로드 하는 방법입니다.
...
`CSize` 픽셀에서 메뉴 단추의 크기를 나타내는 개체입니다.

### <a name="remarks"></a>설명

도구 모음에 메뉴 단추의 크기 전역 변수로 유지 됩니다 하 고이 정적 메서드 여 검색할 수 있습니다.
...
호출 된 [CMFCToolBar::LoadBitmapEx](#loadbitmapex) 메뉴 이미지를 로드 하는 방법입니다.

호출 된 [CMFCToolBar::SetMenuSizes](#setmenusizes) 단추와 해당 이미지의 크기를 설정 하는 방법입니다.
...
사용자가 사용자 지정 되지 전에 프레임 워크 도구 모음 단추의 복사본을 만듭니다. 합니다 [CMFCToolBar::SetButtons](#setbuttons) 메서드 원래 단추의 목록에 제공 된 배열의 각 단추의 복사본을 추가 합니다. 합니다 [cmfctoolbar:: Restoreoriginalstate](#restoreoriginalstate) 메서드는 리소스 파일에서 로드 하 여 도구 모음의 원래 상태를 복원 합니다.

도구 모음에 대 한 원래 단추의 목록을 설정 하려면 호출을 [CMFCToolBar::SetOrigButtons](#setorigbuttons) 메서드.

##  <a name="getorigresetbuttons"></a>  CMFCToolBar::GetOrigResetButtons

도구 모음 사용자 지정 되지 않은 재설정 단추의 컬렉션을 검색 합니다.
...
사용자 지정 되지 않은 목록에 대 한 참조를 도구 모음의 단추를 다시 설정 합니다.

### <a name="remarks"></a>설명

클릭할 때 합니다 **재설정** 단추 사용자 지정 모드, 프레임 워크에 있는 동안이 메서드를 사용 하 여 복원 도구 모음에서 제거 된 단추입니다.

합니다 [CMFCToolBar::SetButtons](#setbuttons) 메서드 호출 후 원래 재설정 단추의 목록에 각 도구 모음 단추의 복사본을 추가 합니다 [CMFCToolBar::OnReset](#onreset) 메서드. 재정의할 수 있습니다는 [CMFCToolBar::OnReset](#onreset) 누르면 후 단추의 모양을 사용자 지정 하는 메서드는 **재설정** 단추.
...
호출 된 [cmfctoolbar:: Loadtoolbarex](#loadtoolbarex) 의 도구 모음 리소스 ID를 설정 하는 방법입니다.

##  <a name="getroutecommandsviaframe"></a>  CMFCToolBar::GetRouteCommandsViaFrame

개체, 상위 프레임 또는 소유자를 확인, 도구 모음에 명령을 보냅니다.
...
상위 프레임; 도구 모음에 명령을 보냅니다 하는 경우 0이 아닌 값 소유자 도구 모음에 명령을 보냅니다 하는 경우 0입니다.

### <a name="remarks"></a>설명

상위 프레임은 기본적으로 도구 모음에 명령을 보냅니다. 호출 [CMFCToolBar::SetRouteCommandsViaFrame](#setroutecommandsviaframe) 이 동작을 변경 합니다.

이 메서드가 아닌 값을 반환 하는 경우 사용 하 여 부모 프레임 개체에 대 한 포인터를 검색할 수 있습니다는 `CMFCToolBar::GetCommandTarget` 메서드. 이 메서드를 사용 하는 예제 VisualStudioDemo 예제를 참조 하세요.
...
높이 (픽셀 단위)의 도구 모음 단추입니다.

### <a name="remarks"></a>설명

프레임 워크 도구 모음 레이아웃을 계산 하려면이 메서드를 호출 합니다. 도구 모음을 다른 높이 지정 하는 파생된 클래스에서이 메서드를 재정의 합니다.
...
도구 모음 단추;에 대 한 도구 설명이 표시 되 면 TRUE입니다. 그렇지 않으면 FALSE입니다.

### <a name="remarks"></a>설명

기본적으로 도구 설명이 표시 됩니다. 호출 하 여 정적이 플래그를 변경할 수 있습니다 [CMFCToolBar::SetShowTooltips](#setshowtooltips)합니다.
...
사용 하도록 설정 하는 방법에 대 한 자세한 내용은 합니다 **표시 단추를 한 줄** 및 **표시 단추를 두 줄** 단추를 참조 하세요 [cmfctoolbar:: Setsiblingtoolbar](#setsiblingtoolbar)합니다.
...
호출 된 [CMFCToolBar::SetUserImages](#setuserimages) 응용 프로그램에서 사용자 정의 이미지의 컬렉션을 설정 하는 방법입니다.
...
하는 경우 *iInsertAt* 가-1 이면이 메서드는 단추를 도구 모음 단추는 목록 끝에 추가 합니다.

호출 된 [CMFCToolBar::InsertSeparator](#insertseparator) 도구 모음에 구분 기호를 삽입 하는 방법입니다.
...
두 개의 기존 단추 사이 구분 기호를 삽입 하려면이 메서드를 호출 합니다. 하는 경우 *iInsertAt* 가-1 이면이 메서드는 도구 모음 단추의 목록 끝에 구분 기호를 추가 합니다.

이 메서드를 사용 하 여 빈 프로그램 도구 모음에 구분 기호를 추가할 수 없습니다.

호출 된 [CMFCToolBar::InsertButton](#insertbutton) 도구 모음에 단추를 삽입 하는 방법입니다.

##  <a name="invalidatebutton"></a>  CMFCToolBar::InvalidateButton

제공된 된 인덱스에 있는 도구 모음 단추의 클라이언트 영역을 무효화 합니다.
...
프레임 워크 도구 모음 단추와 연결 된 클라이언트 영역을 업데이트 하는 경우이 메서드를 호출 합니다. 호출 된 [CWnd::InvalidateRect](../../mfc/reference/cwnd-class.md#invalidaterect) 의 클라이언트 사각형을 사용 하 여 메서드를 `CMFCToolBarButton` 제공된 된 인덱스에 있는 개체입니다.

##  <a name="isaddremovequickcustomize"></a>  CMFCToolBar::IsAddRemoveQuickCustomize

사용자가 추가 하거나 도구 모음 단추를 사용 하 여 제거할 수 있는지 여부를 결정 합니다 **사용자 지정** 메뉴 옵션입니다.
...
사용자가 사용할 수 있으면 TRUE를 **사용자 지정** ; 도구 모음을 수정 하는 메뉴 옵션, 그렇지 않으면 FALSE입니다.

### <a name="remarks"></a>설명

##  <a name="isaltcustomizemode"></a>  CMFCToolBar::IsAltCustomizeMode

지정 여부 *빠른 사용자 지정* 단추를 끌어 하는 데 사용 되는 합니다. 빠른 사용자 지정을 사용 하는 경우 사용자 수 Alt 키를 길게 누른 단추를 새 위치로 끕니다.
...
빠른 사용자 지정; 끌어 사용 중인 경우 TRUE입니다. 그렇지 않으면 FALSE입니다.

### <a name="remarks"></a>설명

##  <a name="isautograyinactiveimages"></a>  CMFCToolBar::IsAutoGrayInactiveImages

비활성 (비-강조 표시) 단추 이미지의 자동 생성 여부를 지정 합니다.
...
자동으로 비활성 이미지 흐리게 표시 하는 옵션이 활성화 되 면 TRUE입니다. 그렇지 않으면 FALSE입니다.
...
이 정적 메서드는 명령으로 지정 되었는지 여부를 결정 *uiCmd* 전역 기본 명령 목록에 속합니다. 호출 하 여 기본 명령의 목록을 변경할 수 있습니다 [CMFCToolBar::AddBasicCommand](#addbasiccommand) 하거나 [CMFCToolBar::SetBasicCommands](#setbasiccommands)합니다.
...
막대 추가 테두리 크기를 사용 하는 단추를 표시할 수 있으면 TRUE 그렇지 않으면 FALSE입니다.

### <a name="remarks"></a>설명

도구 모음 개체 테두리 확장 하는 단추를 표시할 수 있는 경우 TRUE를 반환 합니다. 도구 모음 단추를 처리 하는 경우이 메서드를 호출 합니다 [CMFCToolBarButton::OnChangeParentWnd](../../mfc/reference/cmfctoolbarbutton-class.md#onchangeparentwnd) 알림 해당 내부 추가 테두리 크기 플래그를 적절 하 게 설정 됩니다. 이 내부 플래그를 호출 하 여 나중에 검색할 수 있습니다 [CMFCToolBarButton::IsExtraSize](../../mfc/reference/cmfctoolbarbutton-class.md#isextrasize)합니다.

파생 된 클래스에서이 메서드를 재정의 `CMFCToolBar` 고 표시줄에 추가 테두리 크기를 사용 하 여 도구 모음 단추를 표시 하 고 그렇지 않으면 FALSE를 반환할 수 있으면 TRUE를 반환 합니다. 기본 구현에서는 TRUE를 반환합니다.
...
이 정적 메서드는 명령으로 지정 되었는지 여부를 결정 *uiCmd* 허용 되지 않는 명령 전역 목록에 속합니다.

호출 하 여 허용 되지 않는 명령의 목록을 변경할 수 있습니다 [CMFCToolBar::SetNonPermittedCommands](#setnonpermittedcommands)합니다.

##  <a name="iscommandrarelyused"></a>  CMFCToolBar::IsCommandRarelyUsed

명령을 거의 사용 되는지 여부를 결정 합니다.
...
`IsCommandRarelyUsed` 메서드 경우 FALSE를 반환 합니다. 또는 다음 상황에서 발생 합니다.

- 지정 된 명령이 속할 기본 명령 목록

- 지정된 된 명령의 표준 명령 중 하나는
...
도구 모음 프레임 워크를 사용자 지정 모드 인지 여부를 지정 합니다.
...
프레임 워크를 사용자 지정 모드에 있으면 TRUE입니다. 그렇지 않으면 FALSE입니다.

### <a name="remarks"></a>설명

호출 하 여 사용자 지정 모드를 전환할 수 있습니다 [CMFCToolBar::SetCustomizeMode](#setcustomizemode)합니다.

사용자는 사용자 지정 대화 상자를 호출 하는 경우 모드를 변경 하는 프레임 워크 ( [CMFCToolBarsCustomizeDialog 클래스](../../mfc/reference/cmfctoolbarscustomizedialog-class.md)).

##  <a name="isdragbutton"></a>  CMFCToolBar::IsDragButton

도구 모음 단추를 끄는 적이 있는지 여부를 결정 합니다.
...
도구 모음에 포함 되어 있는지 확인 합니다 **사용자 지정** 단추입니다.
...
도구 모음 포함 되어 있으면 TRUE를 **사용자 지정** 단추, 그렇지 않으면 FALSE입니다.

### <a name="remarks"></a>설명

이 메서드가 TRUE를 반환 합니다 [CMFCToolBar::GetCustomizeButton](#getcustomizebutton) 에 대 한 포인터를 반환 하는 메서드는 **사용자 지정** 끝 도구 모음에 나타나는 단추입니다.

사용 하 여는 [cmfctoolbar:: Enablecustomizebutton](#enablecustomizebutton) 메서드를 추가 합니다 **사용자 지정** 도구 모음에 단추.

##  <a name="isfloating"></a>  CMFCToolBar::IsFloating

도구 모음이 있는지 여부를 결정 합니다.
...
도구 모음; 부동 창인 경우 TRUE입니다. 그렇지 않으면 FALSE입니다.

##  <a name="islargeicons"></a>  CMFCToolBar::IsLargeIcons

응용 프로그램에서 도구 모음 현재 큰 아이콘을 표시 하는지 여부를 지정 합니다.
...
응용 프로그램에서 큰 아이콘;를 사용 하는 경우 TRUE입니다. 그렇지 않으면 FALSE입니다.

### <a name="remarks"></a>설명

호출 [CMFCToolBar::SetLargeIcons](#setlargeicons) 큰 아이콘 및 일반 아이콘 사이 전환 합니다.

프레임 워크를 자동으로 모드를 변경 하는 사용자가 토글할 때 합니다 **큰 아이콘** 확인란에는 **옵션** 페이지를 **사용자 지정** 대화 상자.

##  <a name="islastcommandfrombutton"></a>  CMFCToolBar::IsLastCommandFromButton

지정 된 도구 모음 단추에서 명령을 보냈는지 가장 최근에 실행 여부를 확인 합니다.
...
도구 모음 잠 겼으 면 TRUE 그렇지 않으면 FALSE입니다.

### <a name="remarks"></a>설명

이 메서드는 사용자 위치 도구 모음 단추와 같은 사용자 지정 작업을 수행할 수 없는 경우 TRUE를 반환 합니다.
...
도구 모음에서 사용자에 의해 생성 된 경우 TRUE입니다. 그렇지 않으면 FALSE입니다.
...
이 메서드는 메뉴 애니메이션 형식, 메뉴 섀도 스타일 및 Windows 레지스트리에서 큰 아이콘 표시 여부와 같은 전역 매개 변수를 로드 합니다.

합니다 [CWinAppEx::LoadState](../../mfc/reference/cwinappex-class.md#loadstate) 메서드는 응용 프로그램의 초기화 프로세스의 일부분으로이 메서드를 호출 합니다.
...
프레임 워크는 응용 프로그램의 초기화 프로세스의 일부분으로이 메서드를 호출합니다. 자세한 내용은 [CWinAppEx::LoadState](../../mfc/reference/cwinappex-class.md#loadstate)합니다.
...
큰 이미지의 크기 (높이 또는 너비) 및 일반 이미지 차원 간의 비율을 지정합니다.
...
기본 비율은 2입니다. 크게 또는 작게 큰 도구 모음 이미지를 확인 하려면이 값을 변경할 수 있습니다.

프레임 워크는 큰 이미지 집합을 지정 하지 않으면 하는 경우이 데이터 멤버를 사용 합니다. 예를 들어, 크기가 16 x 16 작은 이미지의 집합만 제공 하 고 크기 24 x 24 큰 이미지를 하는 경우이 데이터 멤버 1.5로 설정 합니다.
...
프레임 워크를 호출한 [CBasePane::DoPaint](../../mfc/reference/cbasepane-class.md#dopaint) 도구 모음 배경에 맞게 합니다.
...
파생된 클래스에서 사용자 지정 배경을 그리려면이 메서드를 재정의 합니다.
...
기본 구현은 아무 작업도 수행하지 않습니다. 재정의 `OnReset` 에서 파생 된 클래스에서 `CMFCToolBar` 도구 모음에서 도구 모음을 원래 상태로 반환 될 때 대체 되어야 하는 더미 단추를에 하는 경우.
...
프로세스 알림 기본값으로 도구 모음 단추의 텍스트를 변경 하려면이 메서드를 재정의 합니다.
...
도구 모음 단추에 대 한 도구 설명이 표시 되려고 할 때 프레임 워크에서이 메서드를 호출 합니다. 하는 경우 `OnUserToolTip` 프레임 워크에 의해 반환 된 텍스트를 포함 하는 도구 설명이 표시 TRUE를 반환 합니다 `OnUserToolTip` 에 *strTTText*합니다. 그렇지 않은 경우 도구 설명 단추 텍스트를 포함합니다.
...
도구 모음을 소유한 창에 WM_COMMAND 메시지를 게시 합니다.
...
이 메서드를 호출 하 여 도구 모음을 소유한 창에 WM_COMMAND 메시지를 게시 [CWnd::PostMessage](../../mfc/reference/cwnd-class.md#postmessage) 으로 지정 된 단추의 명령 ID를 전달 하는 *wParam* 매개 변수입니다.

사용 된 [ON_COMMAND](message-map-macros-mfc.md#on_command) 멤버 함수에 WM_COMMAND 메시지를 매핑하는 매크로입니다.
...
프레임 워크 다시 또는 도구 모음을 삭제 하는 경우이 메서드를 호출 합니다.
...
이 메서드는 단추의 제거의 영향을 받는 추가 도구 모음 특성을 업데이트 합니다. 예를 들어이 메서드는 도구 모음에서 불필요 한 구분 기호를 제거 하 고 바로 가기 키의 테이블을 다시 빌드합니다.

에 대 한 자세한 내용은 합니다 **사용자 지정** 단추를 참조 하십시오 [cmfctoolbar:: Enablecustomizebutton](#enablecustomizebutton)합니다.
...
도구 모음 단추를 사용 하 여 도구 모음 단추를 대체합니다.
...
이 메서드를 호출 합니다 [cmfctoolbar:: Restoreoriginalstate](#restoreoriginalstate) 복원할 수 있는 응용 프로그램에서 각 모음의 메서드. 사용 된 [CMFCToolBar::CanBeRestored](#canberestored) 도구 모음을 복원할 수 있는지 여부를 결정 하는 방법입니다.
...
이 메서드는을 초기화 하는 이미지 컬렉션을 지웁니다 합니다 [CMFCToolBar::LoadToolBar](#loadtoolbar) 하 고 [CMFCToolBar::LoadBitmap](#loadbitmap) 메서드.
...
TRUE 이면 메서드가 성공 하거나 사용자 정의 메서드가 실패 하면 FALSE 또는 도구 모음입니다.

### <a name="remarks"></a>설명

이 메서드를 사용 하 여 리소스 파일에서 도구 모음을 로드 합니다 [CMFCToolBar::LoadToolBar](#loadtoolbar) 메서드.

프레임 워크를 선택 하면이 메서드를 호출 합니다 **모두 다시** 단추를 **도구 모음** 사용자 지정 대화 상자의 페이지입니다.
...
Windows 레지스트리에 도구 모음에 대 한 상태 정보를 저장합니다.
...
프레임 워크는 응용 프로그램 상태를 레지스트리에 저장 하는 경우이 메서드를 호출 합니다. 자세한 내용은 [CWinAppEx::SaveState](../../mfc/reference/cwinappex-class.md#savestate)합니다.

##  <a name="setbasiccommands"></a>  CMFCToolBar::SetBasicCommands

사용자 메뉴를 열 때 항상 표시 되는 명령의 목록을 설정 합니다.
...
사용 된 [CMFCToolBar::AddBasicCommand](#addbasiccommand) 기본 명령 목록에 명령을 추가 하는 방법입니다. 사용 된 [CMFCToolBar::GetBasicCommands](#getbasiccommands) 응용 프로그램에서 사용 되는 기본 명령 목록을 검색 하는 방법입니다.

이 메서드를 사용 하는 예제에 대 한 탐색기 샘플을 참조 하세요.
...
디버그 빌드에서이 메서드를 어설션 실패로 경우 생성 하 여 지정 된 인덱스 *nIndex* 올바르지 않습니다.

호출 된 [CMFCToolBar::SetButtonStyle](#setbuttonstyle) 만 단추의 스타일을 설정 하는 방법입니다.
...
도구 모음에서 기존 단추를 제거 하 고 새 단추 컬렉션을 삽입 하려면이 메서드를 호출 합니다.

이 메서드를 추가 합니다 **사용자 지정** 단추 도구 모음 및 도구 모음의 부모 창에 메시지를 AFX_WM_RESETTOOLBAR 보냅니다. 에 대 한 자세한 내용은 합니다 **사용자 지정** 단추를 참조 하십시오 [cmfctoolbar:: Enablecustomizebutton](#enablecustomizebutton)합니다.

##  <a name="setbuttonstyle"></a>  CMFCToolBar::SetButtonStyle

지정된 된 인덱스에서 도구 모음 단추의 스타일을 설정합니다.
...
이 메서드는 유효한 도구 모음 단추에 지정 된 인덱스를 참조 하지 않는 경우 FALSE를 반환 합니다.

##  <a name="setcommandusageoptions"></a>  CMFCToolBar::SetCommandUsageOptions

드물게 사용 되는 명령을 응용 프로그램의 메뉴에 나타나지 않음 지정 합니다.
...
기본 방법 및 최근에 사용한 메뉴 항목을 확인 하려면 프레임 워크에서는 표시 되는 알고리즘을 사용자 지정 하려면이 메서드를 호출 합니다. 기본 명령에 대 한 자세한 내용은 참조 하세요. [CMFCToolBar::AddBasicCommand](#addbasiccommand)합니다.

이 클래스가 사용 하 여 `CMFCCmdUsageCount` 명령의 사용 횟수를 추적 하는 클래스입니다. 이 클래스에 대 한 자세한 내용은 참조 하세요. [CMFCCmdUsageCount 클래스](../../mfc/reference/cmfccmdusagecount-class.md)합니다.

##  <a name="setcustomizemode"></a>  CMFCToolBar::SetCustomizeMode

사용 하거나 응용 프로그램의 모든 도구 모음에 대 한 사용자 지정 모드를 사용 하지 않도록 설정 합니다.
...
이 메서드는 레이아웃을 조정 하 고 응용 프로그램에서 각 도구 모음을 다시 그립니다. 호출 된 [CMFCToolBar::IsCustomizeMode](#iscustomizemode) 응용 프로그램 사용자 지정 모드에 있는지 확인 하는 방법

##  <a name="setgraydisabledbuttons"></a>  CMFCToolBar::SetGrayDisabledButtons

도구 모음에서 사용할 수 없는 단추는 흐리게 표시 여부 또는 단추를 사용할 수 없는 이미지 사용 되는지 여부를 지정 합니다.
...
호출 된 [CMFCToolBar::GetHotBorder](#gethotborder) 결정 하는 방법 여부를 도구 모음 핫 트랙 해당 단추입니다.
...
도구 모음에서 잠긴된 단추 및 잠긴된 이미지의 크기를 설정합니다.
...
잠긴 단추의 기본 크기는 23 x 22 픽셀입니다. 잠긴된 이미지의 기본 크기는 16x15 픽셀입니다.

호출 된 [CMFCToolBar::GetLockedImageSize](#getlockedimagesize) 의 크기를 검색 하는 방법 잠긴 이미지입니다. 호출 된 [CMFCToolBar::GetButtonSize](#getbuttonsize) 의 크기를 검색 하는 방법 잠긴 도구 모음 단추입니다.
...
호출 된 [CMFCToolBar::GetMenuButtonSize](#getmenubuttonsize) 메뉴 단추의 크기를 결정 하는 방법 및 [CMFCToolBar::GetMenuImageSize](#getmenuimagesize) 메뉴 단추 이미지의 크기를 결정 하는 방법입니다.

이 메서드를 사용 하는 예제에 대 한 IEDemo 및 MSMoneyDemo 샘플을 참조 하세요.
...
프레임 워크를 클릭할 때이 메서드를 호출 합니다 **표시 단추를 한 줄** 단추입니다.

호출 합니다 [cmfctoolbar:: Setsiblingtoolbar](#setsiblingtoolbar) 메서드를 사용 하도록 설정 합니다 **표시 단추를 한 줄** 또는 **표시 단추를 두 줄** 단추. 호출 하는 경우 [cmfctoolbar:: Setsiblingtoolbar](#setsiblingtoolbar) 이 도구 모음에 대 한 형제 도구 모음에서이 도구 모음의 행으로 이동 됩니다. 그렇지 않은 경우이 도구 모음 형제 행 이동 합니다.

프레임 워크 호출을 [cmfctoolbar:: Settworowswithsibling](#settworowswithsibling) 메서드를 클릭할 때를 **표시 단추를 두 줄** 단추입니다.
...
호출 된 [CMFCToolBar::CanBeClosed](#canbeclosed) 사용자 도구 모음을 닫을 수 있는지 여부를 결정 하는 방법입니다.

##  <a name="setroutecommandsviaframe"></a>  CMFCToolBar::SetRouteCommandsViaFrame

상위 프레임 또는 소유자 도구 모음에 명령을 전송 하는지 여부를 지정 합니다.
...
사용 하도록 설정한 후이 메서드를 호출 합니다 **사용자 지정** 도구 모음에 나타나는 단추입니다. 사용 하도록 설정 합니다 **사용자 지정** 단추를 호출 합니다 [cmfctoolbar:: Enablecustomizebutton](#enablecustomizebutton) 메서드.

도구 모음의 형제를 검색 하려면 호출 [CMFCToolBar::GetSiblingToolBar](#getsiblingtoolbar)합니다.

##  <a name="setsizes"></a>  CMFCToolBar::SetSizes

모든 도구 모음에 단추와 이미지의 크기를 지정 합니다.
...
호출 된 [CMFCToolBar::GetImageSize](#getimagesize) 도구 모음 단추 이미지의 크기를 검색 하는 방법입니다. 호출 된 [CMFCToolBar::GetButtonSize](#getbuttonsize) 도구 모음 단추의 크기를 검색 하는 방법입니다.
...
프레임 워크를 클릭할 때이 메서드를 호출 합니다 **표시 단추를 두 줄** 단추입니다.

호출 합니다 [cmfctoolbar:: Setsiblingtoolbar](#setsiblingtoolbar) 메서드를 사용 하도록 설정 합니다 **표시 단추를 한 줄** 또는 **표시 단추를 두 줄** 단추. 호출 하는 경우 [cmfctoolbar:: Setsiblingtoolbar](#setsiblingtoolbar) 이 도구 모음에 대 한 형제 도구 모음을 별도 행으로 이동 됩니다. 그렇지 않은 경우이 도구 모음 별도 행으로 이동 됩니다.

프레임 워크 호출을 [cmfctoolbar:: Setonerowwithsibling](#setonerowwithsibling) 메서드를 클릭할 때 합니다 **표시 단추를 한 줄** 단추.
...
사용자 정의 이미지를 사용 하 여 사용자가 사용자 지정 된 도구 모음 단추에 그릴는 프레임 워크입니다. 지정 된 이미지 목록 *pUserImages* 응용 프로그램의 모든 도구 모음 간에 공유 됩니다.

이 메서드 경우 어설션 실패가 디버그 빌드에서 생성 된 `CMFCToolBarImages` 개체 올바르지 않거나 이미지 크기가 다른 도구 모음의 기본 이미지 크기입니다.

OutlookDemo, ToolTipDemo, 및 VisualStudioDemo 샘플 이미지 사용자 정의 전역 컬렉션을 설정 하려면이 메서드를 사용 합니다. 응용 프로그램의 작업 디렉터리에 있는 UserImages.bmp 이라고 하는 파일을 로드 합니다.

호출 된 [CMFCToolBar::GetUserImages](#getuserimages) 응용 프로그램에서 사용자 정의 이미지의 컬렉션을 검색 하는 방법입니다.
...
이 메서드를 호출 [CMFCToolBar::WrapToolBar](#wraptoolbar) 확대 도구 모음 내의 단추 위치를 변경 합니다.

반환 값이 호출 하 여 결정 [CMFCToolBar::CalcSize](#calcsize)합니다.

##  <a name="translatechar"></a>  CMFCToolBar::TranslateChar

지정 된 키 코드를 올바른 바로 가기 키에 해당 하는 경우 단추 명령을 실행 합니다.
...
이 메서드 내 단추가 필요한 경우 추가 행으로 래핑 도구 모음에서 단추를 다시 설정 합니다.

##  <a name="m_bdontscaleimages"></a>  CMFCToolBar::m_bDontScaleImages

높은 DPI 모드에서 도구 모음 이미지를 크기 조정 여부를 지정 합니다.
