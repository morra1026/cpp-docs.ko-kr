...
3.  호출 된 [만들기](#create) (또는 [CreateEx](#createex)) Windows 도구 모음을 만들고 연결 하는 함수는 `CToolBar` 개체입니다.  
...
2.  호출 된 [만들기](#create) (또는 [CreateEx](#createex)) Windows 도구 모음을 만들고 연결 하는 함수는 `CToolBar` 개체입니다.  
  
3.  호출 [LoadBitmap](#loadbitmap) 도구 모음 단추 이미지를 포함 하는 비트맵을 로드 합니다.  
  
4.  호출 [SetButtons](#setbuttons) 단추 스타일을 설정 하는 비트맵의 이미지를 사용 하 여 각 단추를 연결 합니다.  
