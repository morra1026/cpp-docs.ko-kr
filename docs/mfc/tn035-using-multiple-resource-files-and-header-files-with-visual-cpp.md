...
- 수 경우 여러 리소스 파일 및/또는 헤더 파일에 프로젝트와 그 방법은 분할 하 시겠습니까

- 일반적인 헤더를 공유 하려면 어떻게 해야 있습니다. 두 H 파일입니다. RC 파일

- 여러 프로젝트 리소스를 어떻게 나눌 수 있습니다. RC 파일
...
- **어떻게 Visual c + + 관리 리소스 파일과 헤더 파일에 대 한 개요** Visual c + +의 Resource Set Includes 명령을 여러 리소스 파일과 같은 프로젝트에서 헤더 파일을 사용할 수 있습니다 하는 방법의 개요를 제공 합니다.
...
- **추가 헤더 파일을 포함 하 여** 여러 헤더 파일을 포함 해야 할 하 고 자세하게 설명 하는 방법에 설명 합니다.

- **두 헤더 파일을 공유 합니다. RC 파일** 여러 간에 헤더 파일 하나를 공유 하는 방법을 보여 줍니다. 다른 프로젝트에 또는 동일한 프로젝트에서 RC 파일입니다.

- **동일한 프로젝트에서 여러 리소스 파일을 사용 하 여** 여러 프로젝트가 중단 하려는 위치에 대해 설명 합니다. RC 파일, 및 수행 하는 방법의 세부 정보를 제공 합니다.

- **Visual c + + 파일을 편집할 수 없는 적용** 어떻게 되었는지 확인할 수 있습니다 Visual c + + 편집 하지 않으며 사용자 지정 리소스를 실수로 서식을 다시 지정에 대해 설명 합니다.

- **여러 Visual c + + 편집 하 여 공유 하는 기호를 관리 합니다. RC 파일** 여러 간에 동일한 기호를 공유 하는 방법에 설명 합니다. RC 파일 하는 방법과 중복 ID 숫자 값을 할당 하지 마세요.

- **간의 종속성을 관리 합니다. RC입니다. CPP, 및입니다. H 파일** 불필요 한 다시 컴파일하지 Visual c + +를 방지 하는 방법에 대해 설명 합니다. 리소스 기호 파일에 의존 하는 CPP 파일입니다.

- **Visual c + + 관리 집합 포함 정보** 어떻게 Visual c + +는 추적 여러 (중첩 됨)에 대 한 기술 정보를 제공 합니다. RC 파일 및 여러 헤더 파일을 # 하 여 include로 지정 된는 합니다. RC 파일입니다.

**리소스 파일 및 헤더 파일을 관리 하는 Visual c + +의 개요**
...
기본적으로 Visual C++에서는 리소스 파일의 이름(예: MYAPP.RC)과 관계없이 해당하는 헤더 파일의 이름을 RESOURCE.H로 지정합니다. 사용 하 여는 **리소스 내용** 명령을 합니다 **뷰** Visual c + +의 메뉴에서 기호 헤더 파일을 업데이트 하 여이 헤더 파일의 이름을 변경할 수는 **SetIncludes**대화 상자.

**읽기 전용 기호 지시문**

Visual C++에서는 지정된 .RC 파일에 대해 헤더 파일 하나만 편집하지만, 추가 읽기 전용 헤더 파일에 정의된 기호에 대한 참조를 지원합니다. 사용 하 여는 **리소스 내용** 에서 명령을 합니다 **보기** 메뉴 Visual c + +에서는 추가 읽기 전용 헤더 파일은 개수에 관계 없이 읽기 전용 기호 지시문으로 지정할 수 있습니다. "읽기 전용" 제한은 .RC 파일에 새 리소스를 추가할 경우 읽기 전용 헤더 파일에 정의된 기호를 사용할 수 있지만 리소스를 삭제할 경우 기호가 읽기 전용 헤더 파일에 여전히 정의되어 있음을 의미합니다. 읽기 전용 기호에 할당된 숫자 값은 변경할 수 없습니다.

**컴파일 시간 지시문**

Visual C++에서는 .RC 파일이 다른 .RC 파일 안에 #include로 지정되는 리소스 파일 중첩 기능도 지원됩니다. Visual C++를 사용하여 지정된 .RC 파일을 편집할 경우 #include로 지정된 파일의 리소스를 볼 수 없습니다. 하지만 .RC 파일을 컴파일할 때 #include로 지정된 파일도 컴파일됩니다. 사용 하 여는 **리소스 내용** 명령을 **보기** Visual c + +에서 메뉴를 개수에 관계 없이 지정할 수 있습니다 # include로 지정 합니다. 컴파일 시간 지시문으로 RC 파일입니다.

될까요 Visual c + +로 읽어를 합니다. RC 파일을 #include 다른 합니다. RC 파일 *되지* 컴파일 시간 지시문으로 지정 합니다. 이 상황은 이전에 텍스트 편집기를 사용하여 수동으로 관리해 왔던 .RC 파일을 Visual C++로 가져올 때 발생할 수 있습니다. Visual C++에서 #include로 지정된 .RC 파일을 읽을 때 #include로 지정된 리소스가 부모 .RC 파일에 병합됩니다. 부모 .RC 파일을 저장할 경우 #include 문이 사실상 #include로 지정된 리소스로 대체됩니다. 이 병합이 발생 하지 않으려면 제거 해야 합니다 #include 문을 부모에서입니다. RC 파일 *이전* Visual c + +로 읽어 다음 Visual c + +를 사용 하 여 다시 추가 동일한 #include 문을 컴파일 시간 지시문입니다.

Visual c + +에서 저장을 합니다. RC 파일은 위의 세 가지 Set Includes 정보 (기호 헤더 파일, 읽기 전용 기호 지시문 및 컴파일 시간 지시문)에서 #include 지시문 *고* TEXTINCLUDE 리소스에서입니다. TEXTINCLUDE 리소스는 구현 세부 정보를 처리 하기 위해 일반적으로 필요 하지 않은 설명 [어떻게 Visual c + +에서 Set Includes 정보가](#_mfcnotes_tn035_set_includes)합니다.
...
MYAPP 합니다. RC Visual c + +를 사용 하 여 편집할 수 있는 응용 프로그램 리소스 파일입니다.
...
RES\MYAPP 합니다. RC2를 포함 하지만 리소스를 Visual c + +에서 편집할 수는 컴파일된 최종에 포함 됩니다. EXE 파일입니다. Visual C++에서 버전 리소스(이 릴리스의 새로운 기능)를 비롯하여 모든 표준 리소스를 편집할 수 있으므로 응용 프로그램 마법사에서는 기본적으로 이러한 리소스를 만들지 않습니다. 고유의 사용자 지정 서식 리소스를 이 파일에 추가하려는 경우 응용 프로그램 마법사에 의해 빈 파일이 생성됩니다.

사용자 지정 서식 리소스를 사용하는 경우 이러한 리소스를 RES\MYAPP.RC2에 추가하고 Visual C++ 텍스트 편집기를 사용하여 편집할 수 있습니다.

AFXRES.RC 및 AFXPRINT.RC에는 프레임워크의 특정 기능에 필요한 표준 리소스가 포함되어 있습니다. RES\MYAPP.RC2와 마찬가지로, 프레임워크에서 제공한 이러한 두 리소스 파일은 MYAPP.RC 끝에 #include로 지정되며 Set Includes 대화 상자의 컴파일 시간 지시문에 지정됩니다. 따라서 Visual C++에서 MYAPP.RC를 편집하는 동안 이러한 프레임워크 리소스를 직접 보거나 편집하지는 않지만, 이러한 리소스는 응용 프로그램의 이진 .RES 파일 및 최종 .EXE 파일로 컴파일됩니다. 하면 수정 절차 등 표준 프레임 워크 리소스에 대 한 자세한 내용은 참조 하세요 [Technical Note 23](../mfc/tn023-standard-mfc-resources.md)합니다.
...
사용 하 여 추가 읽기 전용 헤더 파일을 포함할 수 있습니다는 **리소스 내용** 지정 하는 명령을 #include에서 같이 두 번째 읽기 전용 기호 지시문 문을:
...
마법사가 만든 RES\MYAPP 합니다. RC2 파일은 리소스를 포함 하는 파일의 예로 *되지* 실수로 Visual c + +에 읽기 및 쓰기를 서식 정보가 손실 된 상태로 다시 합니다. 이러한 경우가 발생하지 않도록 하기 위해 RES\MYAPP.RC2 파일의 시작 부분에 다음 줄을 추가합니다.
...
Visual c + + 컴파일 경우 합니다. 또한 RC 파일인 `APSTUDIO_INVOKED` 뿐만 `RC_INVOKED`합니다. 응용 프로그램 마법사가 만든 파일 구조가 손상되고 Visual C++에서 위의 #error 줄을 읽고 심각한 오류로 보고한 후 .RC 파일의 읽기를 중단합니다.

**여러 Visual c + + 편집 하 여 공유 하는 기호를 관리 합니다. RC 파일**
...
ID_TOOLS_SPELL 기호는 공유 헤더 파일이 MYSHARED.H에서 유지됩니다. 이 공유 헤더 파일은 개발자가 텍스트 편집기를 사용하여 직접 관리하며, Visual C++에 의해 직접 편집되지 않습니다. 두 리소스에서 MYSTRS 파일입니다. RC 및 MYMENUS 합니다. 지정할 RC를 #include MYSHARED 합니다. MYAPP에 대 한 읽기 전용 지시문에 대 한 H입니다. RC를 사용 하는 **리소스 내용** 명령을 앞에서 설명한 대로 합니다.
...
`_APS_NEXT_RESOURCE_VALUE` 대화 상자 리소스, 메뉴 리소스 등에 대 한 사용 될 다음 기호 값이입니다. 리소스 기호 값의 유효 범위는 1 ~ 0x6FFF입니다.
...
`_APS_NEXT_SYMED_VALUE` 기호 값을 수동으로 할당할 때 발생 하는 다음 기호 값은 기호 브라우저에서 새 명령을 사용 합니다.

Visual C++는 새 .RC 파일을 만들 때 최저 유효 값보다 조금 높은 값부터 시작합니다. 또한 응용 프로그램 마법사는 이러한 값을 MFC 응용 프로그램에 보다 적합한 형태로 초기화합니다. ID 값 범위에 대 한 자세한 내용은 참조 하세요. [Technical Note 20](../mfc/tn020-id-naming-and-numbering-conventions.md)합니다.

Visual c + + 정의 같은 동일한 프로젝트에도 새 리소스 파일을 만들 때마다 이제 `_APS_NEXT_` 값입니다. 다시 말해서 두 .RC 파일에 여러 대화 상자를 추가하는 경우 동일한 #define 값이 서로 다른 대화 상자에 할당될 가능성이 높습니다. 예를 들면 첫 번째 .RC 파일의 IDD_MY_DLG1에는 두 번째 .RC 파일의 IDD_MY_DLG2와 동일한 101이라는 번호가 할당될 수 있습니다.

이를 방지하려면 해당 .RC 파일에서 네 개의 ID 도메인 각각에 대한 별도 수치 영역을 예약해야 합니다. 수동으로 업데이트 하 여이 작업을 수행 합니다 `_APS_NEXT` 의 각 값을 합니다. RC 파일 **하기 전에** 리소스 추가 시작 합니다. 예를 들어 경우 첫 번째입니다. 기본값을 사용 하 여 RC 파일 `_APS_NEXT` 값을 다음 할당 하려는 `_APS_NEXT` 두 번째 값입니다. RC 파일:
...
Visual C++에서는 파일을 저장할 때 항상 //{{NO_DEPENDENCIES}} 주석 줄이 .RC 파일에 추가됩니다. 경우에 따라 RESOURCE.H에 대한 빌드 종속성 회피로 인해 링크 타임에는 발견되지 않은 런타임 오류가 발생할 수도 있습니다. 예를 들어, 기호 브라우저를 사용하여 리소스에 대한 기호에 할당된 숫자 값을 변경하는 경우 해당 리소스를 참조하는 .CPP 파일이 다시 컴파일되지 않을 경우 응용 프로그램 런타임 시 해당 리소스를 올바르게 찾아서 로드할 수 없습니다. 이러한 경우에 명시적으로 다시 컴파일해야 하나. 사용자가 알고 있는 CPP 파일 리소스의 기호 변경 영향을 받습니다. H 또는 select **모두 다시 빌드**합니다. 특정 리소스 그룹에 대 한 기호 값을 자주 변경 해야 하는 경우 아마도 것이 더 편리 하 고 별도 읽기 전용 헤더 파일에 이러한 기호를 안전 하 게 위의 섹션에 설명 된 대로 [포함 추가 헤더 파일](#_mfcnotes_tn035_including)합니다.

## <a name="_mfcnotes_tn035_set_includes"></a> Visual c + +를 관리 하는 방법 집합에 포함 됩니다 정보 * *
