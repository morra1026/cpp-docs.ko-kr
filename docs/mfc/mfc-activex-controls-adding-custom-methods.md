...
4.  바로 가기 메뉴에서 클릭 **추가** 을 클릭 한 다음 **메서드 추가**합니다.  
  
     이 메서드 추가 마법사를 엽니다.  
  
5.  에 **메서드 이름** 상자에 입력 *PtInCircle*합니다.  
  
6.  에 **내부 이름** 상자에, 메서드의 내부 함수의 이름을 입력 하거나 기본값을 사용 (이 예제의 경우 *PtInCircle*).  
  
7.  에 **반환 형식** 상자를 클릭 합니다 **VARIANT_BOOL** 메서드의 반환 형식에 대 한 합니다.  
  
8.  사용 하 여는 **매개 변수 형식이** 및 **매개 변수 이름** 이라는 매개 변수를 추가 하는 컨트롤 *xCoord* (형식 *OLE_XPOS_PIXELS*).  
