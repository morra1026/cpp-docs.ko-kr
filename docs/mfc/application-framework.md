...
Microsoft Foundation 클래스 (MFC) 라이브러리의 핵심은 c + + 형태로 Windows API의 큰 부분을 캡슐화 합니다. 라이브러리 클래스 브러시, 펜, 컨트롤을 다른 표준 Windows 항목 등 일반적인 GDI 개체는 windows 대화 상자, 장치 컨텍스트를 나타냅니다. 이러한 클래스는 제공을 편리 하 게 캡슐화 하는 Windows의 구조에 c + + 멤버 함수 인터페이스입니다. 이러한 클래스를 사용 하는 방법에 대 한 자세한 내용은 참조 하세요 [창의 개체 항목](../mfc/window-objects.md)합니다.
